...
\contact{Department of Mathematics and Statistics, University of Calgary, street, zip code, city, country}{cunning@math.ucalgary.ca}
\contact{Pacific Institute for the Mathematical Sciences, University of Calgary, street, zip code, city, country}{roed.math@gmail.com}
...
\researchsupported{We gratefully acknowledge the financial support of the Pacific Institute for the Mathematical Sciences
...
If $G$ is a commutative algebraic group over $\FF_q$ which is not connected, however, then one must keep track of the choice of \eqref{introbox}, leading to the category of pairs $(\qcs{E},\mu_\qcs{E})$ where $\qcs{E}$ is a rank-one local system on $G$ and $\mu_\qcs{E} : m^*\qcs{E} \to \qcs{E}\boxtimes\qcs{E}$ is an isomorphism of local systems on $G\times G$. 
If the component group scheme $\pi_0(G)$ is not cyclic, then the trace of Frobenius provides an epimorphism from isomorphism classes of pairs $(\qcs{E},\mu_\qcs{E})$ to characters of $G(\FF_q)$, but that group homomorphism need not be injective. 
Every character of $G(\Fq)$ may be geometrized as a pair $(\qcs{E},\mu_\qcs{E})$ but perhaps not uniquely. 
...
Here we also see the importance of working with Weil sheaves; without them, we would not be able to geometrize the character of the \'etale group scheme $\ZZ$ over $\FF_q$ given by $1\mapsto \ell$, for example.
...
such as $A^*\ceq \Hom(A,\EEx)$ for any group $A$, 
...
determines a point geometric point $\bg$ fixed by $\Frob{G}$. 
...
Let $G$ be a smooth commutative group scheme over $\Fq$.
%For all quasicharacter sheaves $\qcs{L}$ and $\qcs{L}'$ on $G$, every $\rho \in \Hom(\qcs{L},\qcs{L}')$ is either trivial or an isomorphism.
Moreover, 
\[
\Aut(\qcs{L}) \iso \Hom(\pi_0(\bG)_{\Weil{}},\EEx),
\]
...
%For the second, f
Fix $\qcs{L} = (\gqcs{L},\mu,\phi)$ and consider the homomorphism $\Aut(\qcs{L}) \to \Hom(\bG_{\Weil{}},\EEx)$ defined in Proposition~\ref{prop:autornaught_etale}.
...
For every non-negative integer $n$, $\Gr^R_n(X)$ is {\it smooth} commutative group scheme over $S_0$ with component group scheme
%$\pi_0(\Gr^R_n(X))= 
$\pi_0(X) \times_S S_0$
...
canonically.
%
The Greenberg transform $\Gr_R(X)$ of $X$ is a commutative group scheme over $\Fq$ with component group scheme
$
%\pi_0(\Gr_R(X))= 
\pi_0(X) \times_S S_0
$
...
X_K(K),
$
canonically, but $\Gr_R(X)$ is not locally of finite type, therefore not locally of finite presentation, and therefore not smooth.
%
...
\begin{definition}
We construct an evil subcategory $\QC(X_K)$ of $\QC(\Gr_R(X))$ as follows:
...
The evil category of quasicharacter sheaves for $X_K$ is rigid monoidal in the obvious way.
\end{definition}
...
\]
where $X$ is a N\'eron model for $X_K$, as functors from $\mathcal{N}_K$ to the category of abelian groups. 
...
We thank Alessandra Bertrapelle and Cristian Gonz\'ales-Avil\'es for disabusing us of a misapprehension concerning the Greenberg realization functor and for their help in the proof of Proposition~\ref{prop:wrK}.
}
