...
In this way we are led to the category $\QC(G)$ of {\it quasicharacter sheaves} on $G$ (\S\ref{sec:category}).
...
Morphisms in $\QC(G)$ are then homomorphisms of Weil sheaves which are compatible with the extra structure.
...
\]
arising from the spectral sequence, the isomorphism $S_G$ allows us to show (\S\ref{ssec:SandT}) that $\TrFrob{G}$ is surjective with kernel $\Hh^2(\bG,\EEx)^{\Weil{}}$.
...
connected algebraic groups and \'etale commutative group schemes -- we turn to the general case.
...
\end{equation}
Using pullbacks of quasicharacter sheaves (\S\ref{sec:Frob}), we obtain a diagram
\begin{equation} \label{eq:intro-snake}
  ...
  \end{tikzcd}
  \end{equation}
The rows of this diagram are exact (\S\S\ref{ssec:restriction},\ref{ssec:component}), so we may apply the snake lemma to prove the main theorem of the paper%, Theorem~\ref{thm:snake} $\QCiso{G}$ into a short exact sequence
...
%We conclude in Section~\ref{sec:basechange} by
