...
The isomorphism $\mu : \bm^* \gqcs{L} \to \gqcs{L} \boxtimes\gqcs{L} $ together with
\ref{QC.3} guarantee
...
canonically.
%
The Greenberg transform $\Gr_R(X)$ of $X$ is still a commutative group scheme over $\Fq$ with component group scheme
$
\pi_0(\Gr_R(X))= \pi_0(X) \times_S S_0
$
and its $\Fq$-rational points are given by
$
\Gr_R(X)(\Fq)\iso X_K(K),
$
canonically, but $\Gr_R(X)$ is not locally of finite type, and therefore not smooth.

%
Moreover, the morphism of $\Fq$-schemes $\varrho^R_n(X) : \Gr_R(X) \to \Gr^R_n(N)$ induces a functor
\[
...
Although $\Gr_R(X)$ is not locally of finite type, and thus not smooth, we begin by considering the rigid monoidal category $\QC(\Gr_R(X))$ defined in Section~\ref{sec:category}, without insisting that the commutative group scheme is smooth.
