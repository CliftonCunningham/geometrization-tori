...
\newcommand{\Sch}[1]{{\left(\operatorname{Sch}_{/#1}\right)}}
...
As Deligne explained in \cite{deligne:SGA4.5}*{Sommes trig.}, if $G$ is a connected commutative algebraic group over $\FF_q$, then the trace of Frobenius provides a bijection between characters of $G(\FF_q)$ and isomorphism classes of rank-one $\ell$-adic local systems $\mathcal{E}$ on $G$ for which 
...
{\it A priori}, these sheaf homomorphisms do not form a category, but since $G$ is connected, the existence of the isomorphism \eqref{introbox} for $\qcs{E}$ makes it unique; as a consequence, there is no impediment to categorifying the dictionary in this case. \todo{clarify}
...
In order to extend the dictionary from commutative algebraic groups over finite fields to smooth commutative group schemes $G$ over $\FF_q$, we simply replace the local system $\qcs{E}$ on $G$ with a Weil local system while retaining the extra structure $\mu_\qcs{E}$. 
Set ${\bar G} := G\times_{\Spec{\FF_q}} \Spec{{\bar \FF}_q}$ and let $\Frob{G} : \bG \to \bG$ be the Frobenius automorphism for $\bG$.
In this way we are led to the category $\QC(G)$ of {\it quasicharacter sheaves} on $G$. Objects in $\QC(G)$ are triples $(\gqcs{L}, \mu,\phi)$, where $\gqcs{L}$ is a rank-one local system on ${\bar G}$ and $\phi : \Frob{G}^* \gqcs{L}\to \gqcs{L}$ and $\mu : {\bar m}^* \gqcs{L} \iso \gqcs{L} \boxtimes \gqcs{L}$ are isomorphisms of sheaves satisfying a compatibility condition. Morphisms in $\QC(G)$ are then homomorphisms of Weil sheaves which are compatible with the extra structure.


We first test the category of quasicharacter sheaves by examining its objects on \'etale commutative group schemes $G$ over $\FF_q$; \'etale group schemes form a counterpoint to connected algebraic groups, since the component group of a smooth commutative group scheme is an \'etale group scheme. 
We find that the group homomorphism 
\begin{equation}
T_G: \QCiso{G} \to \Hom(G(\FF_q),\EEx)
\end{equation}
provided by the trace of Frobenius is surjective with kernel $\Hh^2(\bG,\EEx)^{\Weil{}}$, where $\Weil{} \subset \Gal({\bar\FF}_q/\FF_q)$ is the Weil group for $\FF_q$.
One can show, using the Kunneth formula, that $\Hh^2(\bG,\EEx)$ is non-trivial unless $\bG$ is cyclic.
The fact that $T_G$ may not be an isomorphism should not be too troubling: if quasicharacter sheaves $(\gqcs{L}, \mu,\phi)$ and $(\gqcs{L}', \mu',\phi')$ determine the same character of $G(\FF_q)$ then $(\gqcs{L},\phi)$ and $(\gqcs{L}',\phi')$ are isomorphic as Weil sheaves.
Here we also see the importance of working with Weil sheaves; without them, we would not be able to geometrize the character of the \'etale group scheme $\ZZ$ 
with image $\ell^{\ZZ}$, for example. %given by $1\mapsto \ell$, for example.
...
%
As we show in the paper, while characters of connected commutative algebraic groups $G$ can be geometrized using  local systems trivialized by discrete isogenies to $G$, this is not the right perspective if one wishes to geometrize characters of smooth commutative group schemes, such as \'etale commutative group schemes. Indeed, quasicharacter sheaves on \'etale commutative group schemes that are trivialized by discrete isogenies are invisible to the trace of Frobenius.


Having understood quasicharacter sheaves on commutative smooth group schemes over $\FF_q$ in two extreme cases -- connected algebraic groups and \'etale commutative group schemes -- we turn to the general case. Remarkably, we find that the general case can be understood from these two extreme cases, using the component group sequence
\begin{equation}
0 \to G^0 \to G \to \pi_0(G) \to 0.
\end{equation}
The main result of the paper uses this sequence to find $\QCiso{G}$ by situating it in a short exact sequence
\begin{equation}
...
This allows us to adapt our work on quasicharacter sheaves to construct a category $\QC(X_K)$ of quasicharacter sheaves for $X_K$, which are certain sheaves on $\Gr_R(X)\times_{\Spec{\Fq}} \Spec{\bFq}$, with extra structure.
The ability to generalize the function-sheaf dictionary to non-connected group schemes, of which the component group of the special fibre of the N\'eron model $X$ is an important example, plays a crucial role in this application.
%since these group schemes are not connected in general.

Specializing to the case when $X_K$ is an algebraic torus $T$ over $K$, and writing $\underline{T}$ for the Neron model $X$ in this case, this paper gives a canonical short exact sequence 
\begin{equation}
...
We further show that automorphism groups in the $\QC(T)$ are given, for every quasicharacter sheaf $\qcs{F}$ for $T$, by
\begin{equation}
...
%\todo{Maybe move this from the introduction into the body of the paper somewhere.}
In this regard, we are reminded of the work of David Vogan \cite{vogan:93a}, in which he finds a geometrization of complete Langlands parameters for $p$-adic groups as equivariant perverse sheaves on Vogan varieties and, in the process, is led to study the representations of all the pure rational forms of the $p$-adic group simultaneously.
...
Indeed, Bernstein has suggested to us that our category of quasicharacters for $T$ may be tied to characters of all the pure rational forms of $T$.
...
such as $A^*\ceq \Hom(A,\EEx)$ for any group $A$ and using $G$ for a commutative group scheme over $\FF_q$,
in Section~\ref{sec:category} we define the category of quasicharacter sheaves on $G$.
Section~\ref{sec:Frob} introduces the trace of Frobenius homomorphism $\TrFrob{G} : \QCiso{G} \to G(\FF_q)^*$.
...
we show in Section~\ref{ssec:connected} that every quasicharacter sheaf can be described in both of these ways.
We also show that $\TrFrob{G}$ is an isomorphism in this case.

In Section~\ref{sec:main} we prove that the rows of \eqref{eq:intro-snake} are exact and
use the snake lemma to determine the kernel and cokernel of $\TrFrob{G}$ for general $G$.
We show that $\QCiso{G^0} \to \QCiso{G}$ is surjective in Section~\ref{ssec:restriction} and
that the remainder of the top row of \eqref{eq:intro-snake} is exact in Section~\ref{ssec:component}.
We put it together in Theorem~\ref{thm:snake} of Section~\ref{ssec:snake}, showing that $\TrFrob{G}$
is surjective with kernel $\Hh^2(\pi_0(\bG), \EEx)^{\Weil{}}$.  Having described the isomorphism classes of objects,
in Theorem~\ref{thm:autornaught} of Section~\ref{ssec:qcmor} we show that
\[
\Aut(\qcs{L}) \cong \Hom(\pi_0(\bG)_{\Weil{}}, \EEx)
\]
and that every morphism is either trivial or an isomorphism.  We conclude in Section~\ref{sec:basechange} by
giving functorial versions of the restriction and norm homomorphisms 
of character groups
$G(k')^* \to G(k)^*$ and $G(k)^* \to G(k')^*$
and describing how quasicharacter sheaves behave under Weil restriction.

Section~\ref{sec:applications} gives applications of quasicharacter sheaves to commutative algebraic groups over local fields.
We begin with a review of N\'eron models and of the Greenberg transform in Section~\ref{ssec:rev_Greenberg}.  We then
define, in Section~\ref{ssec:qc_on_GN}, a quasicharacter sheaf for an algebraic group over a local field to be a quasicharacter
sheaf on the Greenberg transform of the N\'eron model.  In Theorem~\ref{thm:QCXK}, we translate our results from earlier sections
to give a description of the isomorphism classes and automorphism groups
in the category of quasicharacter sheaves on such an algebraic group.
We specialize to $p$-adic tori in Section~\ref{ssec:qc_tori}, and discuss
Weil restriction and base change over local fields in Section~\ref{ssec:wrK}.
Finally, we give a categorical version of a result of Chai and Yu \cite{chai-yu:01a},
...
(This idea is expanded upon in Section~\ref{sec:stalks}.)
...
\subsection{Stalks of quasicharacter sheaves}\label{sec:stalks}
...
From here, Deligne's function-sheaf dictionary for connected commutative algebraic groups over finite fields \citelist{\cite{deligne:SGA4.5}*{Sommes trig.},\cite{laumon:87a}*{1.1.3}} \todo{Fix citelist} gives us all we need, as we briefly recall.
...
%For the second, f
Fix $\qcs{L} = (\gqcs{L},\mu,\phi)$ and consider the group homomorphism $\Aut(\qcs{L}) \to \Hom(\bG_{\Weil{}},\EEx)$ defined in Proposition~\ref{prop:autornaught_etale}.
...
Note that the image of $\Aut(\qcs{L}) \to \Hom(\bG_{\Weil{}},\EEx)$ is contained in $\Hom_\text{cts}(\bG_{\Weil{}},\EEx)$, where $\bG_{\Weil{}}$ is given the topology of the base of the espace \'etal\'e attached to $\gqcs{L}$.
...
given by \cite{bosch-lutkebohmert-reynaud:NeronModels}*{\S 7.6}.
...
\end{proposition}
\begin{proof}
This result follows immediately from Lemma~\ref{lem:pullback} together with the identifications
\[
...
In this section we consider connected commutative algebraic groups over $K$ that admit a N\'eron model, by which we mean an lft N\'eron model.
...
These algebraic groups over $K$ form an additive category, denoted by $\mathcal{N}_K$ below, in a natural way, as a full subcategory of connected commutative algebraic groups over $K$. This category includes algebraic tori over $K$ and abelian varieties over $K$ unipotent K-wound groups.
...
\]
 ...
 \Hom_n(X_K(K),\EEx).
\]
In this section we will see how to geometrize and categorify quasicharacters of $X_K(K)$ using quasicharacter sheaves.
...
over $R_n$, for each $n\in \NN$, to the category of schemes over $\Fq$; \todo{font for categories, subscripts on homsets}
...
This observation is key to the proof that, for any $X$ lft over $R$, the projective limit 
\[
...
\[
\varrho^R_n(X) : \Gr^R(X) \to \Gr^R_n(X),\qquad \forall n\in \NN_0.
\]
...
We now define a category which will geometrize quasicharacters of $X_K(K)$.
We begin by considering the rigid monoidal category $\QC(\Gr_R(X))$ defined in Section~\ref{sec:category} without insisting that the commutative group scheme is smooth.

\begin{definition}
Let $\QC(X_K)$ be the following evil subcategory of $\QC(\Gr_R(X))$:
objects in $\QC(X_K)$ are the $\ell$-adic sheaves $\varrho^R_n(X)^*\qcs{L}$, for $n\in \NN$ and $\qcs{L} \in \QC(\Gr^R_n(X))$; morphism from $\varrho^R_n(X)^*\qcs{L}$ to $\varrho^R_m(X)^*\qcs{L}'$ are $\varrho^R_m(X)^*\rho$ for $\rho \in \Hom(\varrho^R_{n\leq m}(X)^*\qcs{L},\qcs{L}')$ when $n\leq m$, and $\varrho^R_n(X)^*\rho$ for $\rho \in \Hom(\qcs{L},\varrho^R_{m\leq n}(X)^*\qcs{L}')$ when $m\leq n$.
 We refer to $\QC(X_K)$ as the {\it evil category of quasicharacter sheaves for $X_K$}; objects will simply be called {\it 
quasicharacter sheaves for $X_K$}.
We put a rigid monoidal structure on $\QC(X_K)$ in the same way as for $\QC(G)$ in Section \ref{sec:category}.
\end{definition}


\begin{remark}
The evil category of quasicharacter sheaves for $X_K$ is evil in the categorical sense.
If $\varrho^R_n(X)^* : \QC(\Gr^R_n(X)) \to \QC(\Gr_R(X))$ is full then the construction above is easily modified to be non-evil; however, we do not know if $\varrho^R_n(X)^*$ is full.
In any case, a non-evil alternative construction of $\QC(X_K)$ is available, but it is quite ugly.
For objects, one considers triples $\qcs{F} \ceq (n, \{\qcs{F}_i\}_{i \ge n}, \{\alpha_{i \le j}\}_{j \ge i \ge n})$, where $n$
is a positive integer, each $\qcs{F}_i$ is a quasicharacter sheaf on $\Gr^R_i(X)$ and each $\alpha_{i \le j} : \qcs{L}_j \to f^*_{i \le j} \qcs{L}_i$ is an isomorphism; of course, $\alpha_{i \le i}$ is the identity and the $\alpha_{i \le j}$ are compatible with each other.  
If $\qcs{F} \ceq (n, \{\qcs{F}_i\}, \{\alpha_{i \le j}\})$
and $\qcs{F}' \ceq (m, \{\qcs{F}'_i\}, \{\alpha'_{i \le j}\})$ are objects then $\Hom(\qcs{F}, \qcs{F}')$ is the set
of equivalence classes of pairs $(k, \{\beta_i\}_{i \ge k})$, where $k \ge m, n$ and the $\beta_i : \qcs{F}_i \to \qcs{F}'_i$ are
morphisms of quasicharacter sheaves so that
\[
\begin{tikzcd}
\qcs{F}_j \rar{\alpha_j} \dar{\beta_i} & f^*_{i \le j} \qcs{F}_i \dar{f_{i \le j}^*\beta_j} \\
\qcs{F}'_j \rar{\alpha_j} & f^*_{i \le j} \qcs{L}'_i
\end{tikzcd}
\]
...
The following statements follow directly from Lemma~\ref{lem:autornaught}, Proposition~\ref{prop:functorialG}, Theorems~\ref{thm:snake} and \ref{thm:autornaught}, properties of N\'eron models and our observations about the component group of $\Gr^R_n(X)$.
\begin{enumerate}
\item Trace of Frobenius gives a natural transformation from
\[
...
\] 
\item For all quasicharacter sheaves $\qcs{L}$, $\qcs{L}'$ on $\Gr^R_n(X)$ and for every $\rho \in \Hom(\qcs{L},\qcs{L}')$, either $\rho$ is trivial or $\rho$ is an isomorphism.
\item For all quasicharacter sheaves $\qcs{L}$ on $\Gr^R_n(X)$ there is a canonical isomorphism
\[
\Aut(\qcs{L}) \iso \Hom((\pi_0(X)_{\bFq})_{\Weil{}},\EEx).
\]
\end{enumerate} \todo{move to a lemma?}
So Theorem~\ref{thm:QCXK} now follows from the fact that each $\QC(\Gr^R_n(X))$ is a full subcategory of $\QC(X_K)$ and the observation that $\QC(X_K)$ a `union' of the subcategories $\QC(\Gr^R_n(X))$ in $\QC(\Gr_R(X))$.
\end{proof}
...
\begin{equation}
0 \to \Hh^2(X_*(T)_{\mathcal{I}_K},\EEx)^{\Frob{}} \to \QCiso{T} \to \Hom_\text{qc}(T(K),\EEx) \to 0.
\end{equation}
...
\] 
such that the restriction of the functor
\[
f^* : \QC(\Res_{k'/k} \Gr_{R'}(X')) \to  \QC(\Gr_R(X))
\]
induces
\[
\begin{tikzcd}[column sep=60]
...
\]
obtained by applying Lemma~\ref{lem:pullback} to \eqref{men} gives
\[
...
\]
When restricted to the appropriate evil subcategories, this induces
\[
...
Suppose $X_K\in \mathcal{N}_K$ with N\'eron model $X$ and $X'_{K'}\in \mathcal{N}_K'$ with N\'eron model $X'$.
Suppose $m$ and $m'$ are non-negative integers such that 
$
R_m \iso R'_{m'},
$
in which case $K$ and $K'$ are said to be $m$-close, and
...
The isomorphism \eqref{eq:schematic_transfer} can indeed exist between quasicharacters of algebraic tori over local fields, even when the characteristics of $K$ and $K'$ differ.
...
$\underline{T} \times_S S_n$ and $\underline{T}' \times_{S'} S'_n$, where $\underline{T}$ is a N\'eron model of $T$ and $\underline{T}'$ is a N\'eron model of $T'$, assuming that $T$ and $T'$ are sufficiently congruent.
...
We thank Takashi Suzuki for some very helpful observations and suggestions, including those related to our use of the Hochschild-Serre spectral sequence.
We thank Joseph Bernstein for suggesting a connection between our category of quasicharacter sheaves on $G$ and his sheaves on the stacky classifying space of $G$.
