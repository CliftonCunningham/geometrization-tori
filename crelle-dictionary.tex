...
{\it A priori}, these sheaf homomorphisms do not form a category, but since $G$ is connected, the existence of the isomorphism \eqref{introbox} for $\qcs{E}$ makes it unique; as a consequence, there is no impediment to categorifying the dictionary in this case.

If $G$ is a commutative algebraic group over $\FF_q$ which is not connected, however, then the isomorphism \eqref{introbox} need not be unique, so one must keep track of the choice of \eqref{introbox}. This leads to the category of pairs $(\qcs{E},\mu_\qcs{E})$ where $\qcs{E}$ is a rank-one local system on $G$ and $\mu_\qcs{E} : m^*\qcs{E} \to \qcs{E}\boxtimes\qcs{E}$ is a chosen isomorphism of local systems on $G\times G$. 
...
In this way we are lead to the category $\QC(G)$ of {\it quasicharacter sheaves} on $G$ in which objects are triples $(\gqcs{L}, \mu,\phi)$ where $\gqcs{L}$ is a rank-one local system on ${\bar G} := G\times_{\Spec{\FF_q}} \Spec{{\bar \FF}_q}$ and $\phi : \Frob{G}^* \gqcs{L}\to \gqcs{L}$ and $\mu : {\bar m}^* \gqcs{L} \iso \gqcs{L} \boxtimes \gqcs{L}$ are isomorphisms of sheaves; the isomorphisms $\mu$ and $\phi$ must of course be compatible in the obvious way. Morphism in $\QC(G)$ are then homomorphisms of Weil sheaves which are compatible with the extra structure.
...
\end{equation}
provided by the trace of Frobenius is surjective with kernel $\Hh^2(\bG,\EEx)^{\Weil{}}$, canonically, where $\Weil{} \subset \Gal({\bar\FF}_q/\FF_q)$ is the Weil group for $\FF_q$,
...
and, in the process, is lead to study the representations of all the pure rational forms of the $p$-adic group simultaneously.
...
such as $A^*\ceq \Hom(A,\EEx)$ for any group $A$ and using $G$ for a smooth commutative group scheme over $\FF_q$,
