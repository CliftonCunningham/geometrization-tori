...
\begin{abstract}
We introduce the category of quasicharacter sheaves on smooth commutative group schemes $G$ over finite fields $k$ and, in doing so, expand the scope of the function-sheaf dictionary from connected commutative algebraic groups to this setting.
...
As an application, we find a category of sheaves for which isomorphism classes determine quasi\-characters in the case of algebraic tori, abelian varieties and unipotent wound groups over local fields. 
...
\end{align*}
we use the first as the derivative $d_0$ and the second to induce $d_1$.
...
Since condition~\ref{qc.4} is satisfied for $\{ \brho_x \tq x\in G(\bFq)\}$, it defines a morphism $\rho : \qcs{L}\to \qcs{L}'$, which is clearly and isomorphism. 
%
...
\end{proposition}
\begin{proof}
This result follows easily from Propositions~\ref{prop:ses_desc}, \ref{prop:SGiso} and \ref{prop:sur_etale}.
\end{proof}

Combining Propositions~\ref{prop:sur_etale} and \ref{prop:etale-iso} gives the following
...
Conversely, if $\rho : G(\bFq)_{\Weil{}} \to \EEx$ is any homomorphism then defining $\brho_x$ as multiplication by $\rho(x)$ will define $\gqcs{L} \to \gqcs{L}'$ satisfy the two diagrams in \ref{qc.4}.  
...
To do this we use \'etale descent to see that pullback along $f$ gives an equivalence between local systems on $G$ and $A$-equivariant local systems on $H$; {\it cf.\ } \cite{bernstein-luntz:equivariant_sheaves}*{Prop 8.1.1}. 
