...
specifically to quasicharacters of $p$-adic tori.
...
\[
\Aut(\qcs{F}) \iso \Hh^1(X_*(T_K)_{\mathcal{I}_K},\EEx)^{\Weil{}} = (\check{T}_\ell)^{\Weil{K}},
\]
...
\todo{Can we use this sort or argument to prove $(X_{\Weil{}})^* \iso (X^*)^{\Weil{}}$? If so, let's re-state the $\Aut(\qcs{L})$ result as $\Hh^1(\pi_0(\bG),\EEx)^{\Weil{}}$ since the iso classes are $\Hh^2(\pi_0(\bG),\EEx)^{\Weil{}}$.}
\end{proof}
...
As in the proof of Proposition~\ref{prop:finite}, use \'etale descent to see that pullback by the Lang isogeny $\Lang : G\to G$ defines an equivalence of categories from local systems on $G$ and $G(\Fq)$-equivariant local systems on $G$. 
...
In this way we see that $\bQC(G)$ is equivalent to the category of characters of $G(\Fq)$.
A simple calculation on stalks at geometric points ${\bar g}$ above $\Fq$-rational points $g \in G(\Fq)$ reveals that, if $\qcs{E}$ matches $\psi : G(\Fq)\to \EEx$ under this equivalence, then the action of Frobenius on $\qcs{E}{\bar g}$ is multiplication by $\psi(g)^{-1}$.
In other words, for every $\qcs{E}$ in $\bQC(G)$, the trace of $\Lang^*\qcs{E}$, as a representation of $G(\Fq)$, is $\trFrob{\qcs{E}}^{-1}$.
This proves parts (i) and (ii) of Proposition~\ref{prop:connected}.

To finish the proof of part (iii), suppose $\Lang^*\qcs{E} = V$ with isomorphism $V \to V\otimes V$ and observe that the equivalence above establishes a bijection between $\Aut(\qcs{E})$ and the group of automorphisms of $\rho : V\to V$ for which 
\[
\begin{tikzcd}
...
\begin{equation}
\Aut(\qcs{F}) \iso \Hom(X_*(T_K)_{\Weil{K}},\EEx)  = (\check{T}_\ell)^{\Weil{K}}
\end{equation}
