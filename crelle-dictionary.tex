...
As an application, we find a category of sheaves whose isomorphism classes correspond to quasicharacters in the case of algebraic tori, abelian varieties and unipotent wound groups over local fields. 
...
determine the same character of $G(\Fq)$ then $(\gqcs{L},\phi)$ and $(\gqcs{L}',\phi')$ are isomorphic as Weil sheaves.
...
\]
see \cite{Vakil:spectral_sequences}, which follows \cite{Weibel:Homological_Algebra}*{Ch 5 and \S 7.5}.
The standard derivative on cochains yields two derivatives,
\begin{align*}
d_G &: E^{i,j} \to E^{i,j+1} \mbox{and} \\
...
The second diagram in \ref{qc.4} shows that the association $x \mapsto \brho_x$ is a homomorphism from $G(\bFq)$ to $\EEx$ and the first diagram in \ref{qc.4} shows that it factors through $G(\bFq)_{\Weil{}}$.  Conversely, if $\rho : G(\bFq)_{\Weil{}} \to \EEx$ is any homomorphism then defining $\brho_x$ as multiplication by $\rho(x)$ will satisfy the two diagrams.  Composition of morphisms corresponds to pointwise multiplication in this correspondence, showing that the resulting bijection is actually a group isomorphism.
\end{proof}
