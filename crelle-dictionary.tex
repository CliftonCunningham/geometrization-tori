...
If the component group scheme $\pi_0(G)$ (which is an \'etale commutative group scheme over $\FF_q$) is not cyclic, then while the trace of Frobenius provides a surjection from isomorphism classes in the category of pairs $(\qcs{E},\mu_\qcs{E})$ to characters of $G(\FF_q)$, that group homomorphism need not be injective. 
...
Here we also see the importance of working with Weil sheaves; for example, without them, we would not be able to geometrize the character of the \'etale group scheme $\ZZ$ over $\FF_q$ given by $1\mapsto \ell$, for example.
...
Indeed, he has suggested to us that our category of quasicharacters for $T$ may be tied to characters of all the pure rational forms of $T$.
...
sequence for $G$ yields the diagram
 \begin{equation} \label{eq:intro-snake}
  \begin{tikzcd}[row sep=20]
    ...
    0 \rar & \Hom(\pi_0(G)(\FF_q),\EEx) \rar & \Hom(G(\Fq),\EEx) \rar & \Hom(G^0(\FF_q),\EEx) \rar & 0,
  \end{tikzcd}
  \end{equation}
...
is surjective with kernel $\Hh^2(\pi_0(\bG), \EEx)^{\Frob{}}$.  Having described the isomorphism classes of objects,
...
giving functorial versions of the restriction and norm homomorphisms $\Hom(G(k'),\EEx) \to \Hom(G(k),\EEx)$ and $\Hom(G(k),\EEx) \to \Hom(G(k'),\EEx)$
