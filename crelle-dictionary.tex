...
\[ 
\Hom_{en-1}(X_{K'}(K'),\EEx) \to \Hom_{n-1}(X_K(K)),\EEx)
\]
Note that $X_{K'}(K') = X_{K}(K')$. Passing to limits now defines
\[ 
\Hom_\text{qc}(X_{K}(K'),\EEx) \to \Hom_\text{qc}(X_K(K)),\EEx)
\]
...
Suppose $m$ is a non-negative integers such that 
\[
...
\QC(\Gr^{\OK}_m(X)) \cong \QC(\Gr^{\OL}_{m}(Y))
\end{equation}
which induces an isomorphism
\begin{equation}\label{eq:transfer}
\Hom_m(X_K(K),\EEx)  \iso  \Hom_{m}(Y_{L}(L),\EEx).
\end{equation}
The isomorphism \eqref{eq:transfer} is an instance of {\it transfer} of (certain) quasicharacters between $X_K(K)$ and $Y_{L}(L)$. 
...
$T \times_{\Spec{\OK}} \Spec{\OK/\varpi_n\OK}$ and $M \times_{\Spec{\OL}} \Spec{\OL/\varpi^n\OL}$, assuming that $T_K$ and $M_L$ are sufficiently congruent.
They define a quantity $h$ (the smallest integer so that $\varpi^h$ lies in the
Jacobian ideal associated to a natural embedding of $T$ into an induced torus \cite{chai-yu:01a}*{\S 8.1}) and show  that if $n > 3h$ and $T_K$ and $M_L$ are $n$-congruent then there is a canonical isomorphism of smooth group schemes
 $
\Gr_{n-3h}(T) \to \Gr_{n-3h}(M)
 $
...
\begin{proposition}\label{prop:transfer}
 ...
 \[
 \QC(\Gr^{\OK}_m(T)) \iso \QC(\Gr^{\OL}_m(M))
 \]
 determined by $\alpha, \beta$ and $\phi$ inducing an isomorphism
 \[
\Hom_m(T_K(K), \EEx) \iso  \Hom_m(M_L(L), \EEx)
 \]
