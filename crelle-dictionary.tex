...
specifically to quasicharacters of $p$-adic tori (\S\ref{sec:quasicharacters}).
...
\subsection{Quasicharacters}\label{sec:quasicharacters}
...
This observation is key to the proof that, \todo{add reference to the Stacks project} for any scheme $X$ over $R$, the projective limit 
\[
...
As we explained in the Introduction, our original motivation for this paper was to find a geometrization and categorification of quasicharacters of $p$-adic tori. 
 \todo{Is this the right place to recall/define the term quasicharacter?}
...
Accordingly, by Theorem~\ref{thm:QCXK}, isomorphism classes in $\QC(T)$   becomes
\begin{equation}
0 \to \Hh^2(X_*(T_K)_{\mathcal{I}_K},\EEx)^{\Weil{}} \to \QCiso{T_K} \to \Hom_\text{qc}(T(K),\EEx) \to 0.
\end{equation}
...
\Aut(\qcs{F}) \iso \Hom(X_*(T_K)_{\Weil{K}},\EEx).
\end{equation}
\todo{This is dual to Lemma 2.6, and isn't true in general.  Is the fact that $X_*(T_K)$ is torsion-free sufficient?} where $\Weil{K}$ is the Weil group for $K$.

% and $\check{T}_\ell$ is the $\ell$-adic dual torus to $T_K$.
