...
\subsection{Geometrization of characters}\label{ssec:CCSflat}

\begin{definition}\label{def:CCS}
The category $\CCS^\flat(G)$ of commutative character sheaves on $G$ is the category of triples $(\cs{L},\mu,\xi)$, where $\xi  : m^*\cs{L} \to \theta^* m^*\cs{L}$ such that the following diagram of Weil sheaves on $G \times G$ commutes:
  \[
  \begin{tikzcd}[row sep=30]
  ...
  \end{tikzcd}
  \]
A morphism $(\cs{L},\mu,\xi) \to (\cs{L},\mu',\xi')$ in $\CCS^\flat(G)$ is a morphism $\alpha : (\cs{L},\mu) \to  (\cs{L}',\mu')$ in $\CS(G)$ such that the following diagram commutes:
  \[
  \begin{tikzcd}[row sep=30, column sep=50]
   m^*\cs{L} \arrow{d}[swap]{\xi} \arrow{r}{m^*\alpha} & m^*  \cs{L}' \arrow{d}{\xi'}\\
   \theta^*m^*\cs{L} \arrow{r}{\theta^* m^*\alpha} &  \theta^*m^*\cs{L}'.
  \end{tikzcd}
  \]
\end{definition}

\begin{theorem}
If $\cs{L}\in  \CCS^\flat(G)$ then $\trFrob{\cs{L}} \in  G(k)^*$.
For every $\chi \in G(k)^*$ there is some $\cs{L}\in  \CCS^\flat(G)$ such that $\trFrob{\cs{L}} =\chi$.
\end{theorem}
...
character sheaves whose restriction to the commutator subgroup of $G(k)$ is nontrivial.
...
We define the following category to track the trivialization on the commutator  subgroup.
