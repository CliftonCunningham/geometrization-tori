...
Let $\Loc(G\ab)$ be the category of local systems on $G\ab$ and let $\Loc_{G\der}(G)$
...
Now consider the category $\CSab'(G)$ of triples $(\cs{L},\nu,\mu)$ where $(\cs{L},\mu)\in \CS(G)$ and $(\cs{L},\nu)\in \Loc_{G\der}(G)$; thus, $\mu : m^*\cs{L} \to \cs{L}\boxtimes\cs{L}$ and $\nu :  n^*\cs{L} \to p^*\cs{L}$ are isomorphisms where $n : G\der\times G\to G$ is the restriction of $m$ to $G\der\times G$ and where $p_n : G\der\times G\to G$ is projection to the second component. \todo{How is $\nu_2$ the same as $\nu$ above?}
Morphisms are morphisms in $\Loc(G)$ commuting with $\mu$ and $\nu$. \todo{Is this definition of morphisms sufficiently clear?} 
Consider the functor
\[
\begin{array}{rcl}
\CS(G\ab) &\to& \CSab'(G) \\
...
Let $i : G\der \to G$ be the kernel of $q:G \to G\ab$ and define $j : G\der \to G\times G$ by $j(h) = (i(h),1_G)$. \todo{There's some kind of typo here....}
