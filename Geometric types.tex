...
In order to use these tools we must restrict our attention to Yu type data that satisfy two technical conditions, given as Hyptheses~\ref{H0} and \ref{H1}. 
...
With this theorem, we provide all of the ingredients needed to parametrize certain supercuspidal representations of arbitrary depth as rational virtual conjugation equivariant Weil sheaf complexes on group schemes over finite fields. 
...
\subsection{Conjugation equivariant sheaves}\label{ssec:equivariant}

A Weil local system $\cs{L}$ is said to be \emph{conjugation equivariant} if $a^*\cs{L} \iso p_2^*\cs{L}$, where  $a : G\times G\to G$ is conjugation $a(x,y)=xyx^{-1}$ and $p_2: G\times G\to G$ is projection $p_2(x,y)=y$. 
If $(\cs{L},\mu)$ is a linear character sheaf on $G$, then $\cs{L}$ is conjugation equivariant. Indeed,
the isomorphism $\mu : m^*\cs{L} \to \cs{L}\boxtimes\cs{L}$ determines an isomorphism $a^*\cs{L} \to p_2^*\cs{L}$ of Weil local systems on $G\times G$ by the following diagram.
\[
...
\subsection{Lusztig's character sheaves}
...
When specialized to the case of characters $\chi$ of $G(k)$, Lusztig's theory of character sheaves geometrizes $\chi$ as a rational virtual equivariant Weil sheaf complex on $G$.  Of course, the resulting objects are generally not local systems.
...
\end{enumerate}
The representation $\orho^0$ and the quasicharacters $(\varphi^0, \ldots , \varphi^d)$ enjoy certain properties which allow Yu to construct the representations $\orho^i$ of $\oK^i$ that form the sequence of types $(\oK^i,\orho_i)$, for $i=1, \ldots, d$.
%
...
\begin{enumerate}
\labitem{H0}{H0} the geometric component group of the reductive quotient of the special fibre of the group scheme $\underline{G}^0$ is cyclic;
...
Hypothesis~\ref{H1a} allows us to use Proposition~\ref{prop:quasicharacters} to geometrize each quasicharacter $\varphi_i$ using local systems on $G_i$, or more precisely, using commutative character sheaves on $G_i$;
alternatively, Hypothesis~\ref{H1b} may be used to geometrize each quasicharacter $\varphi_i$ using an element in the Grothendieck group of perverse sheaves on $G_i$, tensored with ${\bar\QQ}$.
...
Let $\oK^i$, $\orho^0$ and $\varphi^i$, be a Yu type datum, with $i=0,\ldots, d$, satisfying Hypotheses~\ref{H0} and \ref{H1}.
There is an element $\cs{F}_i$ in the Grothendieck group of conjugation equivariant Weil sheaf complexes on $G^i$, tensored with ${\bar\QQ}$, such that $\trFrob{\cs{F}_i} = \trace(\orho_i)$, where $\orho_i$ is the representation of $\oK^i$ constructed from the Yu type datum in Section~\ref{ssec:review}, for $i=0,\ldots, d$.
\end{theorem}
...
We now define rational virtual conjugation equivariant Weil sheaves $\cs{A}^i_r$ on $G_r^i \ceq \Gr^{R}_r(\underline{G}^{i})$, for $i=0,\ldots ,d$, recursively, following the construction of the representations $\orho_i$, as reviewed in Section~\ref{ssec:review}.
First, set $\cs{A}_r^0 = A_r^0$ and note that \eqref{eqn:tracerho} commutes with $\trace(\orho^0)_r$ replaced by $\trFrob{\cs{A}_r^0}$.
Using Hypothesis~\ref{H1}, let $\cs{L}_i$ be the geometrization of the quasicharacter $\varphi_i \ceq \varphi^i_{r_i}$ appearing in \ref{M1}.
If \ref{H1a} applies, then $\cs{L}_i$ is a rational virtual character sheaf on $G_i \ceq G^i_{r_i}$, using \cite{lusztig:disconnected1};
if \ref{H1b} applies, then $\cs{L}_i$ is the commutative character sheaf on $G_i \ceq G^i_{r_i}$ given by Proposition~\ref{prop:quasicharacters} which, by Section~\ref{ssec:equivariant}, is conjugation equivariant.
In either case, $\cs{L}_i$ is a rational virtual conjugation equivariant Weil sheaf complex and
\[
...
Consider the rational virtual conjugation equivariant Weil sheaf 
\[
\cs{B}_r^{i+1} \ceq \cs{K}_r^{i+1} \otimes (p_r^{i})^*(\cs{A}_r^{i}\otimes \cs{L}_r^{i})
\]
on $G_r^{i}\ltimes J_r^{i+1}$.
...
\]
Then $\cs{C}_r^{i+1}$ is an equivariant Weil sheaf and
\begin{eqnarray*}
\trFrob{\cs{C}^{i+1}_r}(x)
...
Let $\cs{A}_r^{i+1}$ be the rational virtual conjugation equivariant Weil sheaf on $G_r^i$ given by $\cs{A}_r^{i+1} = \frac{1}{n} \cs{C}_r^{i+1}$. 
%
This completes the inductive definition of $\cs{A}_r^i$ so that the following diagram commutes.
\[
\begin{tikzcd}
G^{i+1}(\Fq) \arrow{rr}{\trace(\orho_{i+1})} \arrow{rd} && \EE\\
& G_r^{i+1}(\Fq)\arrow{ru}[swap]{{\trFrob{\cs{A}_r^{i+1}}} } & 
\end{tikzcd}
\]

Now set $\cs{F}^i_r = \cs{A}_r^{i} \otimes \cs{L}_r^i$, for $i=0, \ldots ,d$.
Then $\cs{F}^i_r$ is a rational virtual conjugation equivariant Weil sheaf on $G^i_r = \Gr^{R}_r(\underline{G}^i)$ such that
\[
\begin{tikzcd}
G^i(\Fq) \arrow{rr}{\trace(\orho_i)} \arrow{rd} && \EE\\
...
\]
commutes.
Let $\cs{F}^i$ be the rational virtual equivariant Weil sheaf on the group scheme $G^i= \Gr_{R}(\underline{G}^i)$ obtained by pulling back $\cs{F}_r^i$ along $G^i \to G^i_r$.
Then 
\[
