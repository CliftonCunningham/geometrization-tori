...
not necessarily commutative.  In order to define commutative character sheaves, we need to define
some auxiliary categories.
\begin{itemize}
\item Let $\CSab(G)$ denote the category of triples $(\cs{L},\mu,\beta)$ where $(\cs{L},\mu) \in \CS(G)$ and
...
Since the quotient $q : G \to G\ab$ is a $G\der$-torsor in the fppf topology \cite{demazure:SGA3-VIA}*{Thm. 3.2}, it is also an $G\der$-torsor in the fpqc topology.  
