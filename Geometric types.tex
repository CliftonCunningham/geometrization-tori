...
If $x \in G(k)$, write $x = L(y)$ and define $\tilde{\chi}(x) = \chi(\tilde{L}(y))$.
Note that $\tilde{\chi}$ is well defined since changing $y$ to $gy$ for $g \in G(k)$ has the
effect of conjugating $\tilde{L}(y)$ by $g$, which has no effect on the value $\chi(\tilde{L}(y))$.
