...
%
Let $\Loc_{G\der}(G)$ be the category of $G\der$-equivariant Weil local systems on $G$, whose definition we now recall.
...
\end{equation}
commutes.
This defines $\Loc_{G\der}(G)$, the category of $G\der$-equivariant Weil local systems on $G$.
...
It now follows that $\CS\der(G) \to \CSab(G)$ is an equivalence. \todo{Add more detail to this proof?}
\end{proof}


Set $G^2 = G\times G$. Then $G^2\der = G\der\times G\der$ and $G^2\ab = G\ab\times G\ab$.
...
q^* : \CS(G\ab) \to \CS(G)
\]
given on objects by $(\cs{L}\ab,\mu\ab) \mapsto (q^*\cs{L}\ab, (q^2)^* \mu\ab)$ where $q^2= q\times q$; this is an instance of \cite{cunningham-roe:13a}*{Lemma 1.4}.
...
We may now begin to relate $\CSab(G)$ to $\CS(G\ab)$ via the quotient map $q : G \to G\ab$.
...
Let $L_2 : \Loc(G^2\ab) \to \Loc_{G^2\der}(G^2)$ be the comparison functor for the quotient $q^2  : G^2 \to G^2\ab$. \todo{Add reference to Lemma 3.3}
...
\end{array}
\]
Since both $L$ and $L_2$ are equivalences, this functor $\CS(G\ab) \to \CSab'(G)$ is an equivalence of categories. \todo{I'm a bit wary of constructing an equivalence componentwise in this way; how does it show that the result is full and faithful since it's just talking about objects?}
...
We start with a method that will allow us to situate the diagram in Definition \ref{def:CCS} within $\CSab(G \times G)$.
...
By \cite{cunningham-roe:13a}*{Thm 3.9},  $\Aut_{\CCS(G\ab)}(\cs{L}\ab,\mu\ab) = \Hom(\pi_0(\bar{G}\ab)_{\Frob{}},\EEx)$.
\end{proof}
