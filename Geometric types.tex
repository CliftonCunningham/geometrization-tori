...
\title[Commutative character sheaves and the geometrization of types]{Commutative character sheaves and the geometrization of types for supercuspidal representations}
...
\begin{abstract}
We begin by describing a function-sheaf dictionary for one-dimen\-sion\-al characters of arbitrary smooth group schemes over finite fields.  
...
Then, as an application, we use these \emph{commutative character sheaves} to geometrize the types for supercuspidal representations of $p$-adic groups appearing in the work of Jiu-Kang Yu.
This application also uses Lusztig's character sheaves on disconnected reductive algebraic groups over finite fields.
\end{abstract}
...
\end{quotation}
We now explain the first sentence; in Section~\ref{ssec:geotypes} we follow the suggestion in the second sentence.
...
Finally, as an application, we use the category $\CCS(G)$ to give a geometric description of types for supercuspidal representations of connected, reductive groups over $p$-adic fields as they appear in the work of Jiu-Kang Yu.
...
Let $(C,\rho)$ be a Yu type and let $G^0 = \Gr_{R}(\underline{G}^0)$ be the Greenberg transform of the smooth group scheme $\underline{G}^0$ appearing in Section~\ref{ssec:review}.
