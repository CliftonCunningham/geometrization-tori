...
We then combine this with Lusztig's character sheaves on reductive algebraic groups over finite fields, and the geometrization of the Weil representation found by Gurevich and Hadani, to find \emph{geometric types} for supercuspidal representations of tamely ramified $p$-adic groups.
\end{abstract}
...
As proved by Ju-Lee Kim in \cite{kim:07a}, all irreducible supercuspidal representations of tamely ramified $p$-adic groups can be built from the datum introduced by Jiu-Kang Yu in \cite{yu:01a}*{\S 15}.
While the type, in the sense of Bushnell \& Kutzko \cite{bushnell-kutzko:98a}, of a supercuspidal representation built from Yu datum can be constructed directly from the datum, it is convenient to consider and intermediate object, introduced in \cite{yu:01a}*{Remark 15.4}, which we call a \emph{Yu type datum}. 
Yu type data are studied in \cite{Yu:models}, which concludes with the following observation.
%
\begin{quotation}
{\it Therefore, up to some linear characters, all the ingredient representations [appearing in Yu type data] are on groups of
...
A Yu type datum determines a sequence $(\oK^i,\orho_i)$, for $i=0, \ldots, d$, of types for supercuspidal representations of a sequence of $p$-adic groups.
The main result of \cite{Yu:models} explains how to find a smooth group scheme over $\O$ to with $ \underline{G}^i(\O)=\,^\circ K^i $, where $\O$ is the ring of integers of the $p$-adic field.
In this paper we show in Theorem~\ref{thm:geotypes} that if the geometric component group of the reductive quotient of the special fibre of the group scheme $\underline{G}^0$ is cyclic, then each type $(\oK^i,\orho_i)$ in this sequence can be replaced by a pair $(\underline{G}^i, \cs{F}^i)$, 
where $\cs{F}^i$ is a virtual Weil sheaf on the Greenberg transform $G^i$ of $\underline{G}^i$, such that 
\begin{equation}\label{eqn:intro1}
\trFrob{\cs{F}^i} = \trace(\,^\circ\rho_i),
\end{equation}
where $\trFrob{\cs{F}^i}$ refers to the function on $G^i(k) = \underline{G}^i(\OK) = \,^\circ K^i$ obtained by evaluating the trace of the action of Frobenius on the virtual complex $\cs{F}^i$.
\[
\begin{tikzcd}
...
We refer to the pairs $(\underline{G}^i, \cs{F}^i)$ as \emph{geometric types} for supercuspidal representations.

In order to prove Theorem~\ref{thm:geotypes}, we must address the lacuna highlighted in the quote from \cite{Yu:models}, above; namely, we must find a way to geometrize linear characters of groups of the form $\underline{H}(\O)$, where $\underline{H}$ is a smooth group scheme over $\O$. 
In order to do this is a systematic manner, we begin this paper by finding the function-sheaf dictionary for characters of arbitrary smooth group schemes over finite fields. 
When coupled with the Greenberg transform, this will allow for the geometrization of linear characters of $\underline{H}(\O)$.
...
This category clarifies several questions about $\CS(G)$. 
...
Armed with the function-sheaf dictionary for smooth group schemes over finite fields, we may return to the task of geometrizing Yu type data. 
...
These pieces are assembled in Section~\ref{ssec:geotypes}, to prove Theorem~\ref{thm:geotypes}.
...
For the base case $i=0$, set $\orho_0 \ceq \orho^i\otimes \varphi^i$; see \ref{Y1} above.
Now fix $i$.
...
Comparing with \eqref{eq:tensor}, we see that $\trFrob{\cs{B}_m^{i+1}}$ is precisely the function obtained by factoring the character of $\orho^{i+1}$ through $(\underline{G}^{i}\ltimes \underline{J}^{i+1})(R) \to (\underline{G}^{i}\ltimes \underline{J}^{i+1})(R/\mathfrak{p}_K^{m+1})$ using the canonical identification $(G_m^{i}\times_{G_m} J_m^{i+1})(\Fq) =  (\underline{G}^{i}\ltimes \underline{J}^{i+1})(R/\mathfrak{p}_K^{m+1})$. 
...
booktitle={{Autour des sch\'emas en groupes - \'Ecole d'\'et\'e franco-asiatique de g\'eom\'etrie alg\'ebrique et de th\'eorie des nombres. Volume III}},
    ...
    publisher={Soci\'et\'e Math\'ematique de France},
     issn={1272-3835},
     note={\href{http://smf4.emath.fr/Publications/PanoramasSyntheses/2016/47/html/smf_pano-synth_47_227-258.php}{Panoramas et synth\`eses \textbf{47} (2016), 579--622}}
