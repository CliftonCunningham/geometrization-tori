...
Since the quotient $q : G \to G\ab$ is a $G\der$-torsor in the fppf topology \cite{demazure:SGA3-VIA}*{Thm. 3.2}, it is also an $H$-torsor in the fpqc topology.  
Let $\Loc(G\ab)$ be the category of local systems on $G\ab$.
Let $m : G\times G\to G$ be in $G$ and let $n : G\der\times G\to G$ be the restriction of multiplication in $G$ to $G\der\times  G$ and $p : G\der\times G \to G$ projection to the second component.  
Let $\Loc(G\ab)$ be the category of local systems on $G\ab$ and let $\Loc_{G\der}(G)$ be the category of $G\der$-equivariant local systems on $G$.
Thus, objects in $\Loc_{G\der}(G)$ are pairs $(\cs{L},\nu)$ where $\cs{L}$ is a local system on $G$ and $\nu : n^*\cs{L} \to p^*\cs{L}$ is an isomorphism such that 
\[
\begin{tikzcd}
s^*n^*\cs{L} \arrow{r}{s^*(\nu)} & s^*p^*\cs{L}\\
...
\end{tikzcd}
\]
commutes, where $s: G \to G\der\times G$ is $s(g) = (1,g)$ and such that the following diagram commutes, where $n_1(h_1,h_2,g) = (h_1h_2,g)$, $n_2(h_1,h_2,g) = (h_1,n(h_2,g))$ and $p_2(h_1,h_2,g) = (h_2,g)$.
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
A morphism $(\cs{L},\nu)\to (\cs{L}',\nu')$ in $\Loc_{G\der}(G)$ is a morphism $\alpha :\cs{L} \to \cs{L}'$ such that...
...
the restriction of $\cs{L}$ to $G\der$ is trivial if and only if $(\cs{L},\mu) \cong q^*(\cs{L}\ab,\mu\ab)$ in $\CS(G)$, for some $(\cs{L}\ab,\mu\ab) \in \CS(G\ab)$. \todo{What is the ``kernel'' of the forgetful functor?  Ie, is there a natural parameterization of the possible choices of $\beta$?}
\end{corollary}
