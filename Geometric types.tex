...
If $(\cs{L},\mu) \in \CS(G)$ then $m^*\cs{L}$ equipped with the isomorphism $\mu_m^2: (m^2)^* (m^*\cs{L}) \to m^*\cs{L} \boxtimes m^*\cs{L}$ defined by the diagram below.
\[
\begin{tikzcd}
...
By Lemma~\ref{lemma:morinCSab}, the diagram on the right, below, is in $\CS(G\ab)$ and the diagram on the left, below, is in $\CS\ab(G)$.
\[
...
&  \theta^*(\cs{L}\boxtimes\cs{L}) 
 ...
 \end{tikzcd}
\]
The diagram on the left, above, is precisely the result of applying the functor $(q^2)^*\ab$ to the diagram on the right, above; in particular $\gamma = (q^2)^*\ab \xi$.
