...
Under these assumptions we show how each representation $\orho_i$ can be replaced by a pair $(\underline{G}^i, \cs{F}^i)$, where $\cs{F}^i$ is a {\it rational virtual} sheaf complex on the Greenberg transform $G^i$ of $\underline{G}^i$, by which we mean $\cs{F}^i$ is an element in the Grothendieck group of such sheaves, tensored with $\QQ$. 
...
In order to do so in a systematic manner, we begin this paper by describing the function-sheaf dictionary for certain \todo{Certain?} characters of arbitrary smooth group schemes over finite fields. 
...
In previous work \cite{cunningham-roe:13a}, we studied the function-sheaf dictionary to characters
...
It follows that every object in $\CCS(G)$ determines a Frobenius-stable character sheaf on $G$, in the sense of \cite{lusztig:85a}*{Def.~2.10}. \todo{Must revisit this}
...
In this section, we give methods for geometrizing characters that do not vanish on $G\der(k)$, though the results may not be local systems or conjugation equivariant.
...
the isomorphism $\mu : m^*\cs{L} \to \cs{L}\boxtimes\cs{L}$ determines an isomorphism $a^*\cs{L} \to p_2^*\cs{L}$ of Weil local systems on $G\times G$ by the following diagram. \todo{problem with bottom left of diagram}
\[
...
(m\times (i\circ p_1))^* ( \cs{L}\boxtimes\cs{L}) \arrow[equal]{d}{m= p_1\circ(m\times(i\circ p_1))}[swap]{i\circ p_1 = p_1\circ(m\times(i\circ p_1))} & p_1^*\cs{L} \otimes p_2^*\cs{L} \otimes p_1^*\cs{L}^\vee \arrow[equal]{u} \\
...
When specialized to the case of characters $\chi$ of $G(k)$, Lusztig's theory of character sheaves geometrizes $\chi$ as a rational virtual equivariant Weil sheaf complex on $G$.  Of course, the resulting objects are generally not local systems, in general.

Here is an example when this method can be used to geometrize a character of $G(k)$ which is not trivial on $G\der(k)$.
Let $G = \SL_2$ over $\FFF$.
...
We remark that for $p > 3$ and $G = \SL_2$, we have $G\der(k) = G(k)\der$ so, in this case, any character of $G(k)$ may be geometrized by a local system by Theorem~\ref{thm:geo}, so Lusztig's character sheaves are not required.
...
Let $G = \SL_2$ over $\FFF$ and let $\chi : G(k) \to \mu_3 \subset \EEx$ be the character non-trivial on $G\der(k)$ considered above.
In this case, $\tilde{\chi}= \chi^{-1}$. 
Since $G$ is connected and $\tilde{\chi}$ is a character Proposition~\ref{prop:chitilde} produces a local system $\cs{L}$ on $G$ which geometrizes $\chi$; however, this $\cs{L}$ is not conjugation equivariant 

The same method of geometrization applies to $G = \PGL_2$ when $k$ has odd characteristic.
In this case, $G\der = G$, but $G(k)\der = \PSL_2(k)$ has index $2$ in $G(k)$, so any non-trivial character $\chi$ of $G(k)$ ...\todo{Need help finishing this}
