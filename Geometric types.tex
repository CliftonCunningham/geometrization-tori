...
If $(\cs{L},\mu) \in \CS(G)$ then $m^*\cs{L}$ equipped with the isomorphism $\mu^2: (m^2)^* (m^*\cs{L}) \to m^*\cs{L} \boxtimes m^*\cs{L}$ defined by the diagram below.
\[
\begin{tikzcd}
(m^2)^* (m^*\cs{L}) \arrow{d}{(m^2)^*\mu} \arrow{rr}{\mu^2} && m^*\cs{L} \boxtimes m^*\cs{L}\\
