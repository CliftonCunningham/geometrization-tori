...
Let $q : G\to G\ab$ be the quotient
...
%\begin{itemize}
%\item 
Let $\Loc(G)$ and $\Loc(G\ab)$ be the categories of local systems on $G$ and $G\ab$, respectively.
%\item 
Let $\Loc_{G\der}(G)$ be the category of $G\der$-equivariant local systems on $G$, whose definition we now recall.
...
Then $q : G \to G\ab$ is a regular epimorphism of smooth group schemes with kernel pair $(n,p)$.
\[
\begin{tikzcd}
G\der\times G
...
\end{tikzcd}
\]
Objects in the category $\Loc_{G\der}(G)$ of $G\der$-equivariant local systems on $G$ are pairs $(\cs{L},\nu)$ where $\cs{L}$ is a local system on $G$ and $\nu : n^*\cs{L} \to p^*\cs{L}$ is an isomorphism such that the diagram
\[
\begin{tikzcd}
s^*n^*\cs{L} \arrow{r}{s^*(\nu)} & s^*p^*\cs{L}\\
\cs{L} \arrow[equal]{u}{n\circ s = \id_G}  \arrow{r}{\id_{\cs{L}}} & \cs{L} \arrow[equal]{u}[swap]{p\circ s = \id_G},
\end{tikzcd}
\]
commutes, and also such that the following diagram commutes, where $n_1(h_1,h_2,g) = (h_1h_2,g)$, $n_2(h_1,h_2,g) = (h_1,n(h_2,g))$ and $p_2(h_1,h_2,g) = (h_2,g)$.
\[
\begin{tikzcd}
& \arrow[equal]{dl}{n\circ n_2=n\circ n_1} n_2^*n^*\cs{L} \arrow{rr}{n_2^*(\nu)} && n_2^*p^*\cs{L} \arrow[equal]{dr}[swap]{p \circ n_2 = n\circ p_2}& \\
n_1^*n^*\cs{L} \arrow{dr}[swap]{n_1^*(\nu)} &&&& \arrow{dl}{p_2^*(\nu)} p_2^*n^*\cs{L}\\
& n_1^*p^*\cs{L} \arrow[equal]{rr}{p\circ n_1 = p\circ p_2} && p_2^*p^*\cs{L} .
\end{tikzcd}
\]
Morphisms in $\Loc_{G\der}(G)$ are morphisms in $\Loc(G)$ commuting with $\nu$.

Suppose $\cs{L}\ab\in \Loc(G\ab)$.
The local system $q^*\cs{L}\ab \in \Loc(G)$ is equivariant, for $\nu : n^*q^*\cs{L}\ab\to p^*q^*\cs{L}\ab$ defined by
...
%\item 
Let $\CS_{\text{eq}}(G)$ be the category of triples $(\cs{L},\nu,\mu)$ where $(\cs{L},\mu)\in \CS(G)$ and $(\cs{L},\nu)\in \Loc_{G\der}(G)$.
%Thus, $\mu : m^*\cs{L} \to \cs{L}\boxtimes\cs{L}$ and $\nu :  n^*\cs{L} \to p^*\cs{L}$ are isomorphisms.
Morphisms in $\CS_{G\der}(G)$ are morphisms in $\CS(G)$ commuting with $\nu$. 
%\end{itemize}
...
q^* : \CS(G\ab) \to \CS(G)
\]
given on objects by $(\cs{L}\ab,\mu\ab) \mapsto (q^*\cs{L}\ab, (q^2)^* \mu\ab)$ where $q^2= q\times q$; see \cite{cunningham-roe:13a}*{Lem 1.4}.
...
For $(\cs{L},\mu,\nu)\in \CSab'(G)$, $j^*(\nu) : \cs{L}\vert_{G\der} \to (\EE)_{G\der}$ is an isomorphism. \todo{For $j^*(\nu)$ to make sense, need $j : G\der \to G\der \times G$}
...
%\todo{I assume this should be $p^*\cs{L}$?} by
\[
\begin{tikzcd}
& n^* \cs{L} \arrow{dl}{m\circ k = n} \arrow{r} & p^*\cs{L} &  \\ 
...
It now follows that $\CSab'(G) \to \CSab(G)$ is an equivalence. \todo{Is it easy to see why the morphisms that commute with $\mu$ and $\beta$ are the same as those commuting with $\mu$ and $\nu$?  Is this straight from the definition of the functor?}
\end{proof}
