...
Set $m' = i \circ m \circ \theta$; we have $j\circ c = m \circ (m \times m')$. \todo{$j$ is not defined, and the needed definition conflicts with the definition of $i$ and $j$ given later.}
...
\end{tikzcd}
\]
This shows that $(m^*\cs{L},\mu_m^2,\beta_m^2) \in \CS\ab(G)$. 
Similar work defines $(\cs{L}\boxtimes\cs{L},\mu_\boxtimes^2,\beta_\boxtimes^2) \in \CS\ab(G)$.
...
Let $m : G\times G\to G$ be multiplication in $G$, $n : G\der\times G\to G$ be its restriction to $G\der\times G$,
...
Then 
\[
i^*(\cs{L},\mu) \iso i^*q^*(\cs{L}\ab,\mu\ab)
\]
 in $\CS(G\der)$.
Since $q\circ i = 1_{G\ab}$, it follows that $\cs{L}\vert_{G\der} \iso (\EE)_{G\der}$ in $\CS(G\der)$.
\end{proof}
