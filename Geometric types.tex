...
let $p_n : G\der\times G \to G$ be projection to the second component, and let $s: G \to G\der\times G$ be given by $s(g) = (1,g)$.
Objects in $\Loc_{G\der}(G)$ are pairs $(\cs{L},\nu)$ where $\cs{L}$ is a local system on $G$ and
...
Morphisms in $\Loc_{G\der}(G)$ are morphisms in $\Loc(G)$ commuting with $\mu$ and $\nu$. 
\end{itemize}
...
Let $i\der : G\der \to G$ be the kernel of $q:G \to G\ab$ and define $j : G\der \to G\der\times G$ by $j(h) = (i\der(h),1_G)$. 
...
Then for $(\cs{L},\mu,\beta) \in \CSab(G)$, define $\nu : n^*\cs{L} \to p_n^*\cs{L}$ \todo{I assume this should be $p^*\cs{L}$?} by
\[
\begin{tikzcd}
