...
where $\cs{F}^i$ is a {\it rational virtual Weil local system} on the Greenberg transform $G^i$ of $\underline{G}^i$, by which we mean an element in the group obtained  by tensoring the Grothendieck group of such local systems with $\QQ$.
Writing $\trFrob{\cs{F}^i}$ for the function on $G^i(k) = \underline{G}^i(\OO{}) = \,^\circ K^i$ obtained by evaluating the trace of the action of Frobenius on the rational virtual complex $\cs{F}^i$, we show in Theorem~\ref{thm:geotypes} that 
