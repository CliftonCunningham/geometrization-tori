...
Note that the resulting sheaf is a local system, but is not necessarily conjugation equivariant in the sense of Section \ref{ssec:equivariant1} because
the Lang map is not a homomorphism when $G$ is nonabelian.

\subsection{Equivariant Weil sheaf complexes}\label{ssec:equivariant}

Suppose $(\cs{L},\mu)$ is a linear character sheaf on $G$. 
As the diagram below explains, the isomorphism $\mu : m^*\cs{L} \to \cs{L}\boxtimes\cs{L}$ determines an isomorphism $a^*\cs{L} \to p_2^*\cs{L}$ of Weil local systems on $G\times G$, where  $a : G\times G\to G$ is conjugation $a(x,y)=xyx^{-1}$ and $p_2: G\times G\to G$ is projection $p_2(x,y)=y$. \todo{problem with bottom left of diagram}
\[
...
We will see an example of this phenomenon in the next section.

\subsection{Examples}\label{ssec:badderived_examples}

We now consider examples of both of these methods.  Suppose first that $G = \SL_2$ over $\FFF$.
Then $G\der = G$, but $G(k)\der$ is the subgroup of semisimple elements,
isomorphic to the quaternion group $Q_8$ and of index $3$.
There is thus a nontrivial character $\chi : G(k) \to \mu_3 \subset \EEx$.  The method of Section \ref{ssec:Lang_pushforward} applies
since $G$ is connected and $\tilde{\chi} = \chi^{-1}$ is also a character.

Alternatively, we use the method discussed in Section~\ref{ssec:equivariant}. 
...
For $p > 3$ and $G = \SL_2$, we have $G\der(k) = G(k)\der$ so the methods described above are not required.
However, there are other examples for large $p$ where $G\der(k) \ne G(k)\der$.  For example, when $G = \PGL_2$ and $k$ has odd characteristic, then $G\der = G$, but $G(k)\der = \PSL_2(k)$ has index $2$ in $G(k)$.  As in the case of $\SL_2(\FFF)$, both of the methods described in this section apply to $\PGL_2$.

\todo{I don't know what you mean here; should this still be included?}
