...
Suppose that
\begin{enumerate}
\labitem{(L.1)}{L.1} $G(k) \subset \image(L)$ (which will hold if $G$ is connected).
\end{enumerate}
Then we may associate to each $\chi$ a function $\tilde{\chi}$ as follows.
Define $\tilde{L} : G \to G$ by $\tilde{L}(y) = \Frob{G}(y)y^{-1}$.
If $x \in G(k)$, write $x = \tilde{L}(y)$ and define $\tilde{\chi}(x) = \chi(L(y))$.
...
Frobenius acts by
\[
(^{\Frob{}}s)(y) = s(\Frob{}^{-1}(y)) = s(yx^{-1}) = s(yx^{-1}y^{-1}y) = \tilde{\chi}(yxy^{-1})s(y) = \tilde{\chi}(\tilde{L}(y))s(y) = \chi(x)s(y),
\]
thus proving the claim. Need $z$ with $L(z) = \tilde{L}(y)$.  Note that $z = \Frob{G}(y^{-1})$ does not do the job.
\end{proof}
