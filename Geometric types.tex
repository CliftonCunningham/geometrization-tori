...
\subsubsection{Rigidification} \todo{Not sure that's the right subsection title. Other  suggestions?}
...
\end{tikzcd}
\]
Then $\beta = r\der \circ j^*(\nu)$, where $r\der : (\cs{L}_1)_{G\der} \to (\EE)_{G\der}$ is the isomorphism of constant local systems  determined by the rigidification $r: \cs{L}_1 \to \EE$ of $\cs{L}$ determined by $\mu$  (see \cite{cunningham-roe:13a}*{Remark~1.11}).
To show that $(\cs{L},\mu,\beta) \in \CS\ab(G)$, we must see that $\beta$ is a morphism in $\CS(G\der)$.
By Proposition~\ref{prop:CSGabab}, $\cs{L} = q^*\cs{L}\ab$.
...
and define $j : G\der \to G\der\times G$ by $j(h) = (h,1)$. 
