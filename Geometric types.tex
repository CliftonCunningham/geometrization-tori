...
\author{David Roe}
\address{Department of Mathematics, Massachusetts Institute of Technology, 77 Massachusetts Ave, Cambridge, MA, United States, 02139.}
\email{roed@mit.edu}
...
\end{abstract}
\todo{Since Yu's article ``Construction of tame supercuspidal representation'' [20] is a key ingredient, I would like to inform the authors that, as recently noticed by Loren Spice, Proposition 14.1 and Theorem 14.2 in [20] are not always true (they rely on a misprinted statement in a reference). An alternative proof of Yu's main results which avoids these steps was provided by Jessica Fintzen in ``On the construction of tame supercuspidal representations'', arXiv:1908.09819. I think that it should be mentioned, even if does not affect the results of manuscript.}
...
As proved by Ju-Lee Kim in \cite{kim:07a} and Jessica Fintzen in \cite{fintzen:18a}, \todo{check this fix} all irreducible supercuspidal representations of a tamely ramified $p$-adic group $G$ can be built from ``data'' introduced by Jiu-Kang Yu in \cite{yu:01a}*{\S 15} as long as $p$ does not divide the order of the Weyl group of $G$.
...
Under certain assumptions on the Yu type datum, we show how each representation $\orho_i$ can be replaced by a pair $(\underline{G}^i, \cs{F}^i)$, where $\cs{F}^i$ is a formal ${\bar\QQ}$-linear \todo{${\bar\QQ}$ is not defined} combination of conjugation-equivariant sheaf complexes on the Greenberg transform $G^i$ of $\underline{G}^i$.
%
...
% but again there are more character sheaves than there are characters. 
% as pointed out by Kamgarpour \cite{kamgarpour:09a}*{(1.1)}.
...
Let $T$ be a maximal torus in the base change $\bG$ of $G$ to an algebraic closure $\bar{k}$ of $k$, and let $\gcs{L}_T$ be the restriction of $\gcs{L}$ to $T$.
...
In this paper we use a common formalism for Weil sheaves, writing $\cs{L}$ for the pair $(\gcs{L},\phi)$, where $\gcs{L}$ is an $\ell$-adic sheaf on $\bG \ceq G\otimes_{\Fq} \bFq$ \todo{the notation $\bFq$ isn't defined} and where $\phi : \Frob{}^*\gcs{L} \to \gcs{L}$ is an isomorphism of $\ell$-adic sheaves. 
...
Let us use the notation $\mathsf{D}^b_{c,G}(G;\EE)$ for the category of conjugation-equivariant objects in $\mathsf{D}^b_{c}(G;\EE)$, as defined in \cite{bernstein-lunts:equivariant} for example, and $\mathsf{KD}^b_{c,G}(G;\EE)$ for the Grothendieck group of $\mathsf{D}^b_{c,G}(G;\EE)$; we set $\mathsf{K}_{\bar\QQ}\textsc{D}^b_{c,G}(G;\EE)\ceq \mathsf{KD}^b_{c,G}(G;\EE)\otimes_{\ZZ} {\bar\QQ}$.
Thus, Lusztig's result shows that for every representation $\rho$ of $G(k)$ there is some $\cs{F}\in \mathsf{K}_{\bar\QQ}\textsc{D}^b_{c,G}(G;\EE)$ such that $\trace\rho = \trFrob{\cs{F}}$.
This result is extended in the papers beginning with \cite{lusztig:disconnected1} \todo{it is necessary to provide the precise reference in ``the papers beginning with [18]'' where the existence of F is proved.} to reductive groups $G$ over $k$ for which the geometric component group for $G$ is cyclic.

When specialized to the case of characters $\chi$ of $G(k)$, Lusztig's theory of character sheaves geometrizes $\chi$ using $\mathsf{K}_{\bar\QQ}\textsc{D}^b_{c,G}(G;\EE)$; of course, the resulting geometrization is generally not a local system.
%
...
%the rational virtual equivariant Weil sheaf complex
\[
...
%let $\cs{L}_r$ be a geometrization of the character $\varphi_r: G_r(\Fq) \to \EEx$.
\end{proof}
...
For the rest of Section~\ref{sec:types}, $R$ is the ring of integers of a $p$-adic field. \todo{I think that that ``p-adic field'' can be replaced by the more general notion of ``non-archimedean local field with finite residue field'' (that is, fields with equal characteristic should be be allowed) thanks to the results of arXiv:1810.04198 already mentioned.}
...
\end{table}%
\todo{the quasicharacter $\varphi^i$ is defined to be the restriction to $^\circ K_i$ of the quasicharacter $\phi_i$ which appears in [20].  Yu assumes that $\phi^i$ is $G^{i+1}$ generic.  It would be interesting to see if $G^{i+1}$-genericity property could be translated into a condition of $\varphi^i$, viewed as a quasicharacter of $\underline{G}^i(R)$ as in M2.}
...
\] 
for each $i=0,\ldots d$, such that $\underline{J}^i(R) = J_i$ as a subgroup of $C$ \todo{define $C$} and such that the image of the $R$-points under the multiplication map $\underline{J}^0 \times \cdots \times \underline{J}^i \to \underline{G}$ is $\oK^i$, for $i=0, \ldots , d$.
...
In fact, $\varrho_0$ is itself obtained by pulling back a representation $\varrho_0^\reductive$ \todo{why can $\varrho_0$ be obtained by pulling back a representation $\varrho_0^\reductive$} along the $\Fq$-points of the quotient $\underline{G}^0_\Fq \to (\underline{G}^0)_\Fq^\reductive$.
...
\labitem{H1(b)}{H1b} the quasicharacter $\varphi_i$ of $G_i$ is trivial on  $G_{i,\operatorname{der}}(k)$. \todo{It would be worth to provide an example of supercuspidal representation (with positive depth) of a non-commutative p-adic group for which the hypothesis H1(b) is satsified}
\end{enumerate}
\end{enumerate}
...
Let $(\oK^i, \orho^0, \varphi^i,d)$ be a Yu type datum that satisfies Hypotheses~\ref{H0} and \ref{H1}, and let $\orho_i$ be the representation of $\oK^i$ constructed from it in Section~\ref{ssec:review}.  Then for each $i=0,\dots, d$ there is an element $\cs{F}_i \in \mathsf{K}_{\bar\QQ}\mathsf{D}^b_{c,G^i}(G^i;\EE)$ such that
\[
...
Let $r_i$ be the level of $\varphi^i$ as defined in Section~\ref{ssec:quasicharacters}. \todo{the precise reference in the series of Lusztig articles on character sheaves on disconnected groups must be provided.}
Set $r = \max\{ r_i \tq i=0, \ldots ,d\}$.

By \cite{lusztig:disconnected1}, there is $A \in \mathsf{K}_{\bar\QQ}\mathsf{D}^b_{c,(\underline{G}^0)_\Fq\red}((\underline{G}^0)_\Fq\red;\EE)$ such that 
%${\bar A}$ is a rational virtual character sheaf on $(\underline{G}^0)_{\bFq}^\reductive$ and
\[
...
if \ref{H1b} applies, then $\cs{L}_i\in \CCS(G_i)$, using Proposition~\ref{prop:quasicharacters}. \todo{provide exact reference}
...
\todo{It would be worth to quote also the recent article by Jeffrey Hakim ``Constructing tame
supercuspidal representations'' (Represent. Theory 22, 2018), which revisits Yu's construction in an
interesting manner.}
...
\bib{bertapelle-gonzales:Greenberg}{article}{
   author={Bertapelle, Alessandra},
   author={Gonz\'{a}lez-Avil\'{e}s, Cristian D.},
...
\bib{cunningham-roe:13a}{article}{
   ...
   author={Roe, David},
   title={From the function-sheaf dictionary to quasicharacters of $p$-adic
   tori},
...
\bib{demazure:SGA3-VIB}{inproceedings}{
      author={Bertin, J.E.},
       title={{Expos\'e VI$_\mathrm{B}$: G\'en\'eralit\'es sur les pr\'esch\'emas en groupes}},
        date={1970},
   booktitle={S\'eminaire de G\'eom\'etrie Alg\'ebrique du Bois Marie, Sch\'emas en groupes I (SGA3) {Proprietes generales des schemas en
  groupes}},
      editor={Demazure, Michel},
...
\bib{fintzen:18a}{unpublished}{
   author={Fintzen, Jessica},
   title={Types for tame $p$-adic groups},
   date={2018},
   note={\href{http://arxiv.org/abs/1810.04198}{arXiv:1810.04198 [math.RT]}}
}
...
\bib{lusztig:86a}{article}{
	Author = {George Lusztig},
%	Date-Added = {2017-06-28 19:26:19 +0000},
%	Date-Modified = {2017-06-28 19:28:23 +0000},
	Journal = {Advances in Math.},
%	Keywords = {qcsheaves},
	Number = {2},
	Pages = {{103--155}},
