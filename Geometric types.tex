...
%[appearing in Yu type data]
 are on groups of the form $\underline{H}(\mathcal{O})$, where $\underline{H}$ is a smooth group scheme over $\mathcal{O}$ [a henselian discrete valuation with finite residue field $\kappa$], and the representations are inflated from $\underline{H}(\kappa)$. These results suggest that algebraic geometry and group schemes should play an
...
In this paper we show that if the geometric component group of the reductive quotient of the special fibre of the group scheme $\underline{G}^0$ is cyclic, then each representation $\orho_i$ in this sequence can be replaced by a pair $(\underline{G}^i, \cs{F}^i)$, 
where $\cs{F}^i$ is a {\it rational virtual} sheaf complex on the Greenberg transform $G^i$ of $\underline{G}^i$, by which we mean an element in the group obtained by tensoring the Grothendieck group of such local systems with $\QQ$.
...
\end{tikzcd}
\]
%Theorem~\ref{thm:geotypes} shows how to construct the rational virtual Weil sheaf $\cs{F}^i$ from the Yu type datum, so that \eqref{eqn:intro1} holds.
...
%Then in Section \ref{ssec:proofs}, we give proofs of the results in Section \ref{sec:noncom}, which require a development of equivariant linear character sheaves. \todo{Fix this.}
% 
...
% is given in Section~\ref{ssec:geotypes}. 
 %Specifically, we show that if $C$ is a compact $p$-adic group and $\rho$ is a representation of $C$ and $(C,\rho)$ is a type for a tamely ramified $p$-adic group then there is a  group scheme $G$ over $\Fq$ such that $G(\Fq) = C$ and a rational virtual Weil sheaf $\mathcal{A}$ on $G$ such that $\trFrob{\cs{A}} = \trace(\rho)$.
...
We also thank Masoud Kamgarpour for helpful conversations, and the anonymous referee of this paper, would pointed out an error in an earlier version of the paper and helped us clarify several points.
...
% \todo{$j$ is not defined, and the needed definition conflicts with the definition of $i$ and $j$ given later.}
...
the pullback functor $q^* : \CS(G\ab) \to \CS(G)$ in terms of the forgetful functor $\CSab(G) \to \CS(G)$.
...
Let $\Loc(G)$ and $\Loc(G\ab)$ be the categories of Weil local systems on $G$ and $G\ab$, respectively.
%
...
\end{align*}
Define $s : G\to G\der\times G$ by $s(g) = (1,g)$.
A $G\der$-equivariant Weil local sytem on $G$ is a Weil local system $\cs{L}$ on $G$ together with an isomorphism  
\[
\nu : n^*\cs{L} \to p^*\cs{L}
\] 
...
\begin{tikzcd}
\ &  \arrow{dl}{n\circ b_1 = n\circ b_2}  b_2^*  a^*\, \mathcal{L} \arrow{rr}{b_2^*(\nu)} && b_2^* p^*\, \mathcal{L} \arrow{dr}[swap]{p\circ b_2 = a\circ b_3} & \\
b_1^* n^*\, \mathcal{L} \arrow{dr}{b_1^*(\nu)} &&&&   \arrow{dl}[swap]{b_3^*(\nu)} b_3^* n^*\, \mathcal{L} \\
& b_1^* p^*\, \mathcal{L}  && \arrow{ll}[swap]{p\circ b_3 = p\circ b_1} b_3^* p^*\, \mathcal{L} & 
\end{tikzcd}
\end{equation}
%In the diagram above, the arrows labeled with equations come from canonical isomorphisms of functors on Weil sheaves derived from equations \eqref{eqn:bap}; so, for example, the top left isomorphism comes from $b_1^* n^* \iso b_2^* n^*$ since $n\circ b_1 = n\circ b_2$.
...
\begin{proof}
Let $i\der : G\der \to G$ be the kernel of $q:G \to G\ab$ and define $j : G\der \to G\der\times G$ by $j(h) = (h,1_G)$. 
If $(\cs{L},\mu,\nu)\in \CS\der(G)$ then $j^*(\nu) : \cs{L}\vert_{G\der} \to (\EE)_{G\der}$ is an isomorphism. 
This defines a functor on objects by
\[
\begin{array}{rcl}
\CS\der(G) &\to& \CSab(G) \\
(\cs{L},\mu,\nu) &\mapsto& (\cs{L},\mu,j^*(\nu))
...
\[
\begin{tikzcd}
& n^* \cs{L} \arrow{dl}{m\circ k = n} \arrow{r}{\nu} & p^*\cs{L} &  \\ 
...
%and $\vartheta : \cs{L}\boxtimes \cs{L} \to \theta^*(\cs{L}\boxtimes \cs{L})$ 
is a morphism of $G^2\der$-equivariant Weil local systems on $G^2$. 
\end{lemma}
...
%d^* (n\times n)^* (\cs{L}\boxtimes\cs{L}) & d^* (p\times p)^* (\cs{L}\boxtimes\cs{L}) \\
...
\[
\begin{tikzcd}[column sep=40]
n^*\cs{L} \arrow[equal]{d} \arrow{r}{\nu(\cs{L}\ab)} &  \arrow[equal]{d} p^*\cs{L} \\
n^* ( q^*\cs{L}\ab) \arrow{r}{q\circ n = q\circ p}
& p^* (q^*\cs{L}\ab)
\end{tikzcd}
\]
Then $(q^*\cs{L}\ab,\nu(\cs{L}\ab))$ satisfies \eqref{E1} and \eqref{E2}, so $(q^*\cs{L}\ab,\nu(\cs{L}\ab))$ is an object in $\Loc\der(G)$.
...
\end{lemma}
\begin{proof}
The quotient $q : G \to G\ab$ is an $G\der$-torsor in the fppf topology by \cite{demazure:SGA3-VIA}*{Thm. 3.2}, and thus an $G\der$-torsor in the fpqc topology.
...
q^* : \CS(G\ab) \to \CS(G)
\]
given on objects by $(\cs{L}\ab,\mu\ab) \mapsto (q^*\cs{L}\ab, (q^2)^* \mu\ab)$; this is an instance of \cite{cunningham-roe:13a}*{Lemma 1.4}.
...
If $\alpha : (\cs{L}\ab,\mu\ab) \to (\cs{L}\ab',\mu\ab')$ is a morphism in $\CS(G\ab)$, 
then $q^*(\alpha\ab) : (\cs{L},\mu) \to (\cs{L}',\mu')$ satisfies \cite{cunningham-roe:13a}*{CS4}, so $\alpha$ is a morphism in $\CS(G)$.
...
\end{tikzcd}
\end{equation}
The definition of $q\ab^* : \CS(G\ab)\to \CS\der(G)$ will be revisited in the proof of the following result.
We may now relate $\CSab(G)$ to $\CS(G\ab)$ via the quotient map $q : G \to G\ab$.


\begin{proposition} \label{prop:CSGabab}
Suppose $G$ is a smooth group scheme.  
Then pullback along $q : G \to G\ab$ defines an equivalence $\CS(G\ab) \to \CSab(G)$.
\end{proposition}
\begin{proof}
...
Using Lemma~\ref{lem:HH}, we may rewrite the comparison functor $q\ab^*$ on objects by
\[
\begin{array}{rcl}
\CS(G\ab) &\to& \CS\der(G) \\
...
and $(\cs{L},\mu,\beta) \in \CSab(G)$ then $(\cs{L},\mu,\varphi \circ \beta) \in \CSab(G)$.  By \cite{cunningham-roe:13a}*{Theorem 3.9},
...
If $(\cs{L},\mu,\beta)\in \CS\ab(G)$ then $\mu  : m^*\cs{L} \to \cs{L}\boxtimes\cs{L}$, $\gamma : m^*\cs{L} \to  \theta^*(m^*\cs{L})$ and $\vartheta: \cs{L}\boxtimes\cs{L} \to \theta^*(\cs{L}\boxtimes \cs{L})$ are morphisms in $\CS\ab(G\times G)$.
\end{lemma}
\begin{proof}
%Set $G^2 = G\times G$. 
...
\[
\begin{tikzcd}
(m^2)^* (m^*\cs{L}) \arrow{d}{(m^2)^*\mu} \arrow{rr}{\mu_m^2} && m^*\cs{L} \boxtimes m^*\cs{L}\\
...
\[
\begin{tikzcd}
(m^*\cs{L})\vert_{G^2\der} \arrow{d}{\mu\vert_{G^2\der}} \arrow{r}{\beta^2_m} & (\EE)_{G^2\der}\\
...
By Proposition~\ref{prop:CSGabab}, pullback along the abelianization $q : G \to G\ab$ induces an equivalence $q^*\ab : \CS\ab(G)\to CS\ab(G)$.
...
%\begin{remark}\label{rem:dictionary}
Observe that $\CCS(G)$ is a subcategory of $\CS\ab(G)$, by Definition~\ref{def:CCS}, and that Definition~\ref{def:CSab} makes it clear that there is a forgetful functor $\CS\ab(G) \to \CS(G)$, and also a forgetful functor $\CS(G)\to \Loc(G)$.
Consequently, while the category $\CCS(G)$ geometrizes characters of $G(\Fq)$ in the sense explained in Theorem~\ref{thm:geo}, it is also true that $\CS(G)$, and even $\Loc(G)$ as introduces in Section~\ref{ssec:equivariant1}, geometrizes characters of $G(\Fq)$ in the sense that for every group homomorphism $\chi : G(\Fq) \to \EEx$ there is an object $\cs{L}$ in $\Loc(G)$ such that $\trFrob{\cs{L}} = \chi$.
The point of category $\CCS(G)$ is to make this translation from characters $\chi$ to Weil local systems $\cs{L}$ as precise as possible. Theorem~\ref{thm:geo} shows that benefit of equipping Weil local systems $\cs{L}$ with the extra structure $\mu$ and $\beta$ makes the defect in this dictionary as small as it could possibly be, namely, the image $\Delta_G$ of the connecting homomorphism $G\ab(\Fq) \to \Hh^1(\Fq, G\der)$.
%\end{remark}
...
By \cite{lusztig:disconnected1}, there is a rational virtual Weil sheaf $A = ({\bar A},\phi)$ on $(\underline{G}^0)_\Fq^\reductive$ such that ${\bar A}$ is a rational virtual character sheaf on $(\underline{G}^0)_{\bFq}^\reductive$ and
\[
