...
By \cite{lusztig:disconnected4}*{Thm. 21.21}, this result extends to all reductive groups $G$ over $k$ without the hypothesis that $G$ is connected.% for which the geometric component group for $G$ is cyclic.
...
%\todo{why can $\varrho_0$ be obtained by pulling back a representation $\varrho_0^\reductive$} along the $\Fq$-points of the quotient $\underline{G}^0_\Fq \to (\underline{G}^0)_\Fq^\reductive$ 
...
\begin{enumerate}
\labitem{H1(a)}{H1a} $G_i$ is reductive, or % with cyclic geometric component group, or
...
Taking the trivial representation for $\orho^0$ gives a Yu datum not satisfying \ref{H1b}. \todo{Does Yu require tameness?  Can we say that this is an example if so?}

In contrast, there is a broad class of representations that automatically satisfy \ref{H1b}.
Recall that a representation is \emph{toral} \citelist{\cite{adler:98a} \cite{kaletha:19a}*{\S 6}}
...
Let $(\oK^i, \orho^0, \varphi^i,d)$ be a Yu type datum that satisfies Hypotheses~%\ref{H0} and 
...
%\todo{the precise reference in the series of Lusztig articles on character sheaves on disconnected groups must be provided.}
...
\bib{adler:98a}{article}{
	author={Adler, Jeffrey},
...
\bib{lusztig:disconnected4}{article}{
   author={Lusztig, G.},
