...
A \emph{Yu-type} is a pair $(J, \rho)$, where $J$ is a compact group and $\rho$ is a representation of $J$ built in the following way:
\begin{enumerate}
\item $\underline{G}$
\item $\rho = \tau_0 \otimes \cdots \tau_d$ 
for certain representations $\tau_i$ of $J$.
\item $\tau_0$ is the pull-back of a representation $\sigma_0$ along $J \to J/J_0$, where $J_0$ is the pro-$p$ unipotent radical of $J$.
\item $J = J^0 \cdots J^d$ for certain compact groups $J^i$.
