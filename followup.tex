...
We introduce and establish the basic properties of \emph{commutative character sheaves} on smooth group schemes $G$ over finite fields $\Fq$. 
For instance, show that show that the category of commutative character sheaves on $G$ is equivalent to the category of commutative character sheaves on $G\ab$. 
We give three applications.
First, we use commutative character sheaves to produce a functorial geometrization of characters of $G(\Fq)$ in all cases, thus repairing a defect in the function-sheaf dictionary for algebraic groups $G$ which appears when $\Hh^2(\pi_0(\bG),\EEx)^{\Frob{}}$ is non-trivial. 
Second, we revisit the motivations for the stacky abelianization construction, and show that, after replacing the character sheaves on $G$ with commutative character sheaves on $G$, the schematic abelianization $G \to G\ab$ is correct.
...
\todo{I think we should hold the term `character sheaves for G', when $G$ is not commutative, in reserve, and not use it here for non-commutative $G$. So I might advocate going back to the full definition here.\\ -CC}
...
The quotient $G \to G/C$ exists in the category of smooth group schemes over $\Fq$.
The category of commutative character sheaves on $G$ is equivalent to the category of commutative character sheaves on $G/C$: 
 \[
 \CCS(G) \iso \CCS(G/C).
 \]
 \end{theorem}
...
\begin{proof}
Set $G\ab \ceq G/C$ and let $q : G \to G\ab$ be the quotient in the category of smooth group schemes over $\Fq$.
Recall the functor $q^* : \CCS(G\ab)\to \CCS(G)$ from  . . . 
The strategy of the proof is indicated by the diagram below. 
\[
\begin{tikzcd}
\CCS(G)& \\
& \CCS_C(G) \arrow{ul}[swap]{\text{forget}} \\
\CCS(G\ab) \arrow{uu}{g^*} \arrow{ur}[swap]{\text{comparison}} & 
\end{tikzcd}
\]
We will first use smooth descent to show that the comparison functor is an equivalence from $\CCS(G\ab)$ to the category $\CCS_C(G)$ of $C$-equivariant commutative character sheaves on $G\ab$, the latter defined below.
Then we will show that the forgetful functor from $\CCS_C(G)$ to $\CCS(G)$ is also an equivalence.
...
and let $p : C \times G \to G$ be projection to the second component.
