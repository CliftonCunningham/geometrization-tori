...
Here we shall use  \cite{BGA} for the definition and fundamental properties of the Greenberg transform.
...
We refer the reader to \cite{BS} for more detail.
...
$
where $H^{i+1}$ is a Heisenberg $p$-group in the sense of \cite{yu:construction}*{}.
%\labitem{Q3}{Q3} 
%\item
...
$
such that the pair $(f_{i+1}, j_{i+1})$ is a symplectic action of $C^i$ on $H^{i+1}$ in the sense of \cite{yu:construction}.
...
As a step toward geometrizing the type $(C,\rho)$, we now recall some results from \cite{yu:models}.
...
Regarding the construction of the type $(C,\rho)$, the paper \cite{yu:models} concludes with the following provocative observation, which we have just revisited.
\begin{quotation}
...
By \cite{lusztig:CSDG}, there is a virtual Weil sheaf $A = ({\bar A},\phi)$ on $(\underline{G}^0)_\Fq^\reductive$ such that ${\bar A}$ is a virtual character sheaf on $(\underline{G}^0)_{\bFq}^\reductive$ and
\[
...
\]
The special fibre $(\underline{G}^0)_\Fq$ of the smooth group scheme $\underline{G}^0$ is itself a smooth group scheme, and may be identified with the Greenberg transform $G_0^0 = \Gr^{\OK}_0(\underline{G}^0)$ \cite{CR}*{\S 4.3}. 
...
%Then $\cs{A}^0$ is a quasicharacter sheaf, in the sense of \cite{CR}*{Def 4.3}, on $\Gr^{\OK}(\underline{G}^0)$ from which the character of $\rho^0$ may be recovered.
