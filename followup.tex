...
\end{enumerate}
The pair $(f_i, j_{i+1})$ is a symplectic action of $J^0\cdots J^i$ on $H^{i+1}$, so it defines a group homomorphism 
\begin{equation}\label{eq:symaction}
\begin{tikzcd}
h_{i+1} : (J^0\cdots J^i)\ltimes J^{i+1} \arrow{r} & (J^0\cdots J^i)\ltimes H^{i+1} \arrow{r} & \Sp(V_{i+1})\ltimes V_{i+1}^\sharp.
\end{tikzcd}
\end{equation}
%
...
\end{tikzcd}
\]
Then $h_1^*(\omega_1)$ is a representation of $J^0\ltimes J^1$, determined by the quasicharacter $\varphi_1$ of $J^1$. 
...
The character of $\,^\circ \rho_d$ on $J^0\cdots J^d$ satisfies
\[
...
By \cite{yu:03a}*{Prop 10.2} there is a canonical affine smooth group scheme $\underline{G}$ of finite type over $\OK$ such that \[\underline{G}(\OK)= \,^\circ K\] 
For every positive integer $n$, let $G_n$ be the Greenberg transform of $\underline{G} \times_{\Spec{\OK}} \Spec{\OK/\mathfrak{m}_K^n}$, where $\mathfrak{m}_K$ is the maximal ideal in $\OK$.
Then $G_n$ is a smooth group scheme over $\Fq$ and $G_n(\Fq) = \underline{G}(\OK/\mathfrak{m}_K^n)$, canonically.
Let $G$ be the Greenberg transform of $\underline{G}$.
Then $G$ is a group scheme over $\Fq$ and $G(\Fq) = \underline{G}(\OK) = \,^\circ K$, canonically.
...
Regarding this construction of the type $(\,^\circ  K,\,^\circ \rho)$, the paper \cite{yu:03a} concludes with the following provocative observation.
\begin{quotation}
