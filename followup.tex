...
By a simple adaptation of what in \cite{gurevich-hadani:07a}*{Appendix B.1} are called the non-normalized kernels and the normalization coefficients, it is easy to geometrize the function in \eqref{Cayley} with a $\Sp(V)\ltimes V^\sharp$-equivariant irreducible Weil sheaf $\cs{K}$ (in fact, a Weil local system) on $S_0 \times V^\sharp$ such that 
\[
\trFrob{\cs{K}(\frac{\dim V}{2})} = \trace(W_\psi)\vert_{S_0\times V^\sharp},
\]
where $\cs{K}(\frac{\dim V}{2})$ is the Tate twist of $\cs{K}$.
%
Next, one forms $\cs{K}_\psi \ceq j_{!*} \cs{K}$, the perverse extension of $\cs{K}$ along $j : S_0 \times V^\sharp \hookrightarrow \Sp(V) \ltimes V^\sharp$; recall that $S_0$ is Zariski dense in $\Sp(V)$. 
...
A Yu type is a pair $(C,\rho)$, where $C$ is a compact group and $\rho$ is a representation of $C$,  olnstructed from the following information, derived from a Yu datum:
\begin{enumerate}
