...
That would put the justification(s) for this definition in closer proximity with this definition.}
...
The following proposition will be proven in Section \ref{ssec:proofs}.
...
Thus, $\beta : \cs{L}\vert_{G\der} \to (\EE)_{G\der}$ determines the isomorphism $m^*\cs{L} \otimes \theta^* m^* i^*\cs{L} \to (\EE)_G\otimes (\EE)_G$ by the diagram
\[
\begin{tikzcd}
m^*\cs{L} \otimes \theta^* m^* i^*\cs{L} \arrow{d}{c = m \circ (m \times (i \circ m \circ \theta))}  \arrow[dashed]{r} & (\EE)_G\otimes (\EE)_G\\
c^* \cs{L}\vert_{G\der} \arrow{r}{c^*(\beta)} &  c^*(\EE)_{G\der}, \arrow{u}
\end{tikzcd}
\]
where the arrow labeled with the equation is the canonical isomorphism coming from the isomorphism of functors defined by the equation.
Using the monoidal structure of the category of Weil local systems on $G\times G$, this isomorphism, in turn, defines an isomorphism
$
...
\todo{Should remark that it reduces to the other definition in the commutative case}
...
The precise definition of the functor $\CCS(G\ab) \to \CCS(G)$ will be given in Section~\ref{ssec:proofs}, with the proof of Theorem~\ref{thm:Gab}.
...
\subsection{Equivariant Weil local systems}\label{ssec:equivariant1}
...
\subsection{Equivariant linear character sheaves}\label{ssec:equivariant2}

An $H$-equivariant linear character sheaf on $G$ is a triple $(\cs{L},\mu, \nu)$ where $(\cs{L},\mu)$ is a linear character sheaf and $\nu$ is compatible with $\mu$ in the sense \todo{of...}. 
...
A morphism of $H$-equivariant linear character sheaves $(\cs{L},\mu,\nu) \to (\cs{L}',\mu',\nu')$ is a morphism of $H$-equivariant Weil sheaves $\alpha : \cs{L}\to \cs{L}'$ \todo{How does this $\nu$ give you the $\nu$ of Section 3.2?} which is also a morphism of linear character sheaves.
%so
...
If $(\cs{L},\mu,\nu)$ is an $H$-equivariant linear character sheaf on $G$ then $\mu : m^*\cs{L} \to \cs{L}\boxtimes \cs{L}$ is a morphism of $H\times H$-equivariant linear character sheaves on $G\times G$; likewise $\vartheta : \cs{L}\boxtimes \cs{L} \to \theta^*(\cs{L}\boxtimes \cs{L})$. 
\end{lemma}

\begin{proof} 
Pulling back $\cs{L}$ along $m : G\times G \to G$ produces the linear character sheaf $(m^*\cs{L}, m_2^*(\mu))$, where $m_2 : G\times G\times G\times G\to G\times G$ is defined by $m_2(x_1,y_1,x_2,y_2) = (m(x_1,x_2), m(y_1,y_2)$;
this can be seen by following the argument of \cite{cunningham-roe:13a} \todo{which part of [2]?}.
Set $\cs{L}_2\ceq m_2^*\cs{L}$ and $\mu_2 \ceq m_2^*(\mu)$.
The linear character sheaf $(\cs{L}_2,\mu_2)$ comes equipped with the isomorphism $\nu_2 : a_2^*\cs{L}_2\to p_2^*\cs{L}_2$ where $a_2 : H\times H\times G\times G\to G\times G$ is given by $a_2(h_1,g_1,h_2,g_2) = (a(h_1,g_1), a(h_2,g_2)$ and $p_2 : H\times H\times G\times G\to G\times G$ is given by $p_2(h_1,g_1,h_2,g_2)= ( g_1,g_2)$. 
The  isomorphism $\nu_2 : a_2^*\cs{L}_2\to p_2^*\cs{L}_2$ satisfies the appropriate version of \eqref{ECS1}, so  $(\cs{L}_2,\mu_2,\nu_2)$ is indeed an $H\times H$-equivariant linear character sheaf on $G\times G$.
Likewise $\cs{L}\boxtimes \cs{L}$ is an $H\times H$-equivariant linear character sheaf with isomorphism $\nu \boxtimes\nu$, as indicated in the diagram below.
\[
\begin{tikzcd}
\arrow{d}[swap]{\nu_2} a_2^*(m^*\cs{L}) \arrow{r}{a_2^*(\mu)} & a_2^*(\cs{L}\boxtimes\cs{L})  \arrow{r} & (a^*\cs{L})\boxtimes (a^*\cs{L}) \arrow{d}{\nu\boxtimes\nu} \\
p_2^*(m^*\cs{L}) \arrow{r}{p_2^*(\mu)} & p_2^*(\cs{L}\boxtimes\cs{L}) \arrow{r} & (p^*\cs{L})\boxtimes (p^*\cs{L}) 
\end{tikzcd}
\]
Since this diagram commutes, $(\cs{L}\boxtimes \cs{L},\mu\boxtimes \mu,\nu\boxtimes\nu)$ is also an $H\times H$-equivariant linear character sheaf on $G\times G$.
The appropriate version of \eqref{E3} also commutes, so $\mu : m^*\cs{L} \to \cs{L}\boxtimes\cs{L}$ is a morphism of equivariant linear character sheaves.
The argument that $\vartheta : \cs{L}\boxtimes\cs{L} \to \theta^*(\cs{L}\boxtimes\cs{L})$ is morphism of $H\times H$-equivariant linear character sheaves on $G\times G$ is also straightforward.
\end{proof}
...
Now suppose $q : G \to G_0$ is a regular epimorphism with kernel pair $(a,p)$ in the category of schemes
\[
\begin{tikzcd}
H\times G
 \arrow[shift left=2]{r}{a}
...
q^* : \CS(G_0) \to \CS(G)
\]
given on objects by $(\cs{L}_0,\mu_0) \mapsto (q^*\cs{L}_0, (q\times q)^* \mu_0)$; see \cite{cunningham-roe:13a}*{}.
...
\[
\begin{tikzcd}
a^*\cs{L} \arrow[equal]{d} \arrow[dashed]{rrr}{\nu} & & &  \arrow[equal]{d} p^*\cs{L} \\
a^* ( q^*\cs{L}_0) \arrow{r} 
& (q\circ a)^* \cs{L}_0 \arrow[equal]{r} 
& (q\circ p)^*\cs{L}_0 \arrow{r} 
& p^* (q^*\cs{L}_0)
\end{tikzcd}
\]
...
then $q^*(\alpha_0) : (\cs{L},\mu) \to (\cs{L}',\mu')$ satisfies \cite{cunningham-roe:13a}*{CS4}, arguing as in \cite{cunningham-roe:13a} so $\alpha$ is a morphism in $\CS(G)$.
...
We may now revisit the definition of the functor $q_H^*: \CS(Q)\to \CS_H(G)$.  \todo{Change of notation $G_0$ to $Q$}
By (a small part of) Lemma~\ref{lem:HH}, $\mu_0$ is a morphism of linear character sheaves on $G_0\times G_0$.
Thus, on objects, $q_H^*: \CS(Q)\to \CS_H(G)$ is given by $q_H^*(\cs{L}_0,\mu_0) = (L_1(\cs{L}_0), L_2(\mu_0))$. 
To verify that $(L_1(\cs{L}_0), L_2(\mu_0))$ is an object of $\CS_H(G)$, one must confirm \cite{cunningham-roe:13a}*{CS1, CS2, CS3}, \eqref{E1}, \eqref{E2} and \eqref{ECS1}. 
Conditions \cite{cunningham-roe:13a}*{CS1, CS2, CS3} follow from an adaptation of \cite{cunningham-roe:13a}*{} while conditions \eqref{E1}, \eqref{E2} are a consequence of the work above. 
Condition \eqref{ECS1} is confirmed by brute force.
On maps, $q_H^*: \CS(Q)\to \CS_H(G)$ is given by $q_H^*(\alpha_0) = L_1(\alpha_0)$.
To verify that $L_1(\alpha_0)$ is a map in $\CS_H(G)$ we must confirm conditions \cite{cunningham-roe:13a}*{CS4} and \eqref{E3}.
Condition \cite{cunningham-roe:13a}*{CS4} follows from an adaptation of \cite{cunningham-roe:13a}*{} while condition \eqref{E3} is a consequence of the work above.

With this description of $q_H^*: \CS(Q)\to \CS_H(G)$ is is easy to see that it is an equivalence.
Its adjoint is given on objects by $(\cs{L},\mu,\nu) \mapsto (L_1(\cs{L},\nu), L_2(\nu))$, \todo{Shouldn't these be Rs?} again making use of Lemma~\ref{lem:HH}, and on morphisms by $\alpha \mapsto L_1(\alpha)$ \todo{Again, R?}.
The adjunction is built from the adjunctions for $(L_1,R_1)$ and $(L_2, R_2)$.
\end{proof}


\subsection{Closed subgroups}\label{ssec:closed}
...
%\end{tikzcd}
%\]
Define $f : H\times G \to G\times G$ by $f(h,g) = (i(h),g)$ and note that $a = m\circ f$ and $p = p_2\circ f$.
% (a(h,g) g^{-1}, g) = (h,g)$.
...
\end{equation}
or more precisely, by the following diagram of isomorphisms. \todo{What is $p_1$? And $p_\circ f$?}
\[
\begin{tikzcd}
...
$\CS_H(G)$ is equivalent to the category $\CS^H(G)$ of triples $(\cs{L},\mu,\beta)$ where $(\cs{L},\mu)\in \CS(G)$ and $\beta : \cs{L}\vert_H \to (\EE)_H$ is an isomorphism in $\CS(H)$, with morphisms $\alpha : (\cs{L},\mu,\beta) \to (\cs{L}',\mu',\beta')$ defined by the condition
...
This isomorphism is constrained conditions from Sections~\ref{ssec:equivariant1} and \ref{ssec:equivariant2} as they apply to $\nu : a^*\cs{L}\to p^*\cs{L}$.  \todo{We seem to be missing a preposition}
It is straightforward, tedious exercise to show that those conditions are exactly equivalent to the condition
...
\CS(G\ab) \arrow{dl}{q_{G\der}^*} \\
\CS^{G\der}(G) & \arrow[equal]{l} \CS'(G) \arrow{u}{\text{forget}}  \arrow{r}{\beta \mapsto \nu} &  \CS_{G\der}(G) \arrow{u}{\text{forget}}  & 
\end{tikzcd}
\]
%
Here we use the notation $\CS'(G)$ from Section~\ref{ssec:noncomdef} and the observation that it is precisely the category $\CS^{G\der}(G)$, taking the case $H=G\der$ in Section~\ref{ssec:closed}.
Since the quotient $q : G \to G\ab$ is a trivial $G\der$-torsor in the fppf topology \cite{SGA3}*{Expos\'e {VI}$_A$, Thm. 3.2}, 
it now follows from Lemma~\ref{lem:torsor} that the comparison functor $\CS(G\ab)\to \CS_{G\der}(G)$ is an equivalence.
On the other hand, by Lemma~\ref{lem:beta}, $\CS'(G) \to \CS_{G\der}(G)$, defined by $(\cs{L},\mu,\beta)\mapsto (\cs{L},\mu, \nu)$, is an equivalence.
...
&  \CS_{G\der}(G) \arrow{u}{\text{forget}}  
&  \arrow[bend left=35pt, dashed]{dll} \CCS(G\ab) \arrow{u} \arrow{dl} \\
\CCS(G) \arrow{u} \arrow{r}{\beta \mapsto \nu} 
& \arrow{u}  \CCS_{G\der}(G) 
& 
\end{tikzcd}
\]
...
Together with Lemma~\ref{lem:HH}, we may now interpret the diagram in Definition~\ref{def:CCS} as a diagram in $\CS_{G\der\times G\der}(G\times G)$.
By Lemma~\ref{lem:beta}, this corresponds uniquely to a diagram in $\CS(G\ab\times G\ab)$, necessarily the diagram in Definition~\ref{def:CCScom}.
...
if and only if $(\cs{L}\ab,\mu\ab)\in \CCS(G)$, completing the proof of Theorem~\ref{thm:Gab}.
...
Let $\QCS(G)$ be the category of commutative quasicharacter sheaves on $G$. . . .  as in \cite{cunningham-roe:13a}.

\begin{proposition}\label{prop:QCS}
$\Hom_\text{cts}(\underline{G}(\OK),\EEx) \iso \QCSiso{G}$.
\end{proposition}
