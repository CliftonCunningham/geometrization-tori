...
In the remainder of this section we use Theorem~\ref{thm:geo} to geometrize the representation $\rho$ of $C$: subject to a hypothesis on the component group of the depth-zero part of the datum and also on a conjecture on the geometrization of the character of Heisenberg-Weil representations, we find a group scheme $G$ over $\Fq$ such that $G(\Fq) = C$ and a Weil sheaf $\mathcal{F}$ on $G$ such that $\trFrob{\cs{F}} = \trace(\rho)$.
The proof of this statement is quite technical, so it must be delayed to Section~\ref{ssec:geotypes}, after reviewing types in Section~\ref{ssec:types} and models Section~\ref{ssec:models} developed by Yu, for which we must also review some facts about the Heisenberg-Weil representation in Section~\ref{ssec:Jacobi}, and also make a conjecture about the geometrization of that representation, Conjecture~\ref{conj}.
...
For every quasicharacter/continuous linear character $\chi : \underline{G}(S) \to \EEx$ there is a Weil sheaf $\cs{L}$ on $G$ such that \[\trFrob{\cs{L}} = \trace \chi.\] \end{proposition}

\begin{proof}
Set $S = \Spec{R}$ and, for every non-negative integer, set $S_n = \Spec{R/\mathfrak{m}^{n+1}}$.
 Then $G_n \ceq \Gr_n^{R}(\underline{G})$, the Greenberg transform of $\underline{G} \times_{S} S_n$, is a smooth group scheme over $\Fq$ and 
\[
G_n(\Fq) = \underline{G}(S_n),
\]
canonically.
\end{proof}
...
For use below, we recall some basic facts about the Heisenberg-Weil representation and make a conjecture about the geometrization of these representations.
We refer the reader to \cite{berndt-schmidt:98a} for more details.
<<<<<<< Updated upstream
...
Then  is the Jacobi group. 
From the construction above, it is clear that $\Sp(V)\ltimes V^\sharp$ may be viewed as the $\Fq$-points of an algebraic group over $\Fq$; we will refer to that algebraic group as the Jacobi group.

Let $\psi : Z \to \EEx$ be an additive character and let $\omega_\psi$ be the Heisenberg representation on $V^\sharp$ with central character $\psi : Z \to \EEx$. 
...
% through the Egorov identity \cite{Gurevich-Hadani:geometric_Weil}*{(1.3.1)}.\todo{Find better reference.}
...
%Notation as above.
There is an equivariant Weil sheaf $\cs{W}_\psi$ on the Jacobi group such that 
\[
...
>>>>>>> Stashed changes
\iffalse
Pick a Lagrangian splitting $V = U \oplus U'$. 
%such that $\langle v,v' \rangle = (u,w') - (u',w)$ for $v=(u,w)$ and $v'=(u',w')$.
Let $B \to V^\sharp$ be the subgroup defined by the diagram below.
\[
\begin{tikzcd}
\ &&& U' \arrow{d} &\\
1 \arrow{r} & Z \arrow{r} & V^\sharp \arrow{r} & \arrow[bend right]{l} V \arrow[bend right]{u} \arrow[bend left]{d} \arrow{r} & 1\\
1 \arrow{r} & \arrow[equals]{u} Z \arrow{r} & \arrow[bend right]{l}  B \arrow{u} \arrow{r} & \arrow[bend right]{l}  U \arrow{u} \arrow{r} & 1 
\end{tikzcd}
\]
Extend $\psi$ to $\psi_B : B \to \EEx$ in the obvious way. 
...
=======
A Yu type is then a pair $(C,\rho)$, where $C$ is a compact group and $\rho$ is a representation of $C$, constructed directly from the following information, derived from a Yu datum:
\begin{enumerate}
\labitem{Y0}{Y0} a sequence of compact groups $C^0 \subseteq C^1 \subseteq \cdots \subseteq C^d= C$;
\labitem{Y1}{Y1} a continuous representation $\rho_0$ of $C^0$;
\labitem{Y2}{Y2} a quasicharacter $\varphi^i : C^i \to \CC^\times$, for $i=0, \ldots d$.
\end{enumerate}
The representations $(\rho_0, \varphi^0, \ldots , \varphi^d)$ enjoy certain properties which allow Yu to construct representations $\rho^i$ of $C^i$, for $i=0, \ldots, d$, such that $\rho = \rho^d \otimes \varphi^d$.
%
Since we will need some facts about this construction, we review some further detail here.
...
<<<<<<< Updated upstream
We can now recall how Yu uses all this to construct representations $\rho^i$ of $C^i$, for $i=1, \ldots, d$ and thus the type $(C,\rho) = (C^d,\rho^d)$.
The representations $\rho^i$ are defined recursively. 
For the base case $i=0$, the representation $\rho^0$ is part of the datum; see \ref{Y1} above.
%The key to defining the representation $\rho^{i+1}$ from $\rho^i$ is \ref{Y3}, above.
=======
...
<<<<<<< Updated upstream
This defines the representation $\rho^{i+1}$ of $C^{i+1}$. \todo{I didn't use $\varphi^d$, so this can't be quite right.}
This completes the recursive definition and this determines $\rho^d = \rho$, which is the representation of $C^d = C$ determining the Yu type $(C,\rho)$.
=======
...
\]
<<<<<<< Updated upstream
of morphisms of affine smooth group schemes of finite type over $\OK$ such that, on $\OK$-points it gives the sequence $C^0 \subseteq C^1 \subseteq \cdots \subseteq C^d= C$ above.
Indeed, this is the main result of \cite{yu:03a}.

%\labitem{M2}{M2}
As explained in \cite{yu:03a}*{\S 10.4}, there is morphism of affine smooth group schemes of finite type over $\OK$ 
\[
\underline{J}^i \to \underline{G},
\] 
for each $i=0,\ldots d$, such that $\underline{J}^i(\OK) = J^i$ as a subgroup of $C$ and such that the image of the $\OK$-points under the multiplication map $\underline{J}^0 \times \cdots \times \underline{J}^i \to \underline{G}$ is $C^i$, for $i=0, \ldots , d$.
%\labitem{M3}{M3}
There is a natural action of $\underline{G}^i$ on $\underline{J}^{i+1}$ in the category of smooth affine group schemes over $\OK$ so that the group scheme
\[
\underline{G}^i \ltimes \underline{J}^{i+1}
\]
gives $(\underline{G}^i \ltimes \underline{J}^{i+1})(\OK) = C^i \ltimes J^{i+1}$
=======
...
\[
<<<<<<< Updated upstream
\underline{J}^{i+1}(\OK) \to \underline{J}^{i+1}(\Fq) = \underline{J}^{i+1}_\Fq(\Fq) \to V^{i+1}_\Fq(\Fq) = V_{i+1}.
=======
...
We may now revisit the ingredients in the construction of the representation $\rho$ of $\underline{G}(\OK)$ along the lines indicated by Yu and recalled in Section~\ref{ssec:review}.
=======
...
\labitem{M1}{M1}
<<<<<<< Updated upstream
The continuous representation $\rho^0$ of $C^0$ is a representation of $\underline{G}^0(\OK)$ obtained by inflation along $\underline{G}^0(\OK) \to \underline{G}^0(\Fq)$ from a representation $\rho_0$ of $\underline{G}^0(\Fq) = \underline{G}^0_\Fq(\Fq)$.
In fact, $\rho_0$ is itself obtained by pulling back a representation $\rho_0^\reductive$ along the $\Fq$-points of the quotient $\underline{G}^0_\Fq \to (\underline{G}^0)_\Fq^\reductive$.
\labitem{M2}{M2} The quasicharacters $\varphi^i$ are quasicharacters of $\underline{G}^i(\OK)$, for $i=0, \ldots d$.
\labitem{M3}{M3}
Diagram \eqref{eq:semiprod} is now replaced by the following diagram of smooth group schemes over $\OK$.
=======
...
\begin{proposition}\label{prop:geotypes}
Let $(C,\rho)$ be a Yu type.
With reference to Section~\ref{ssec:review}, for $i=0, \ldots d$, let $G^i = \Gr_{R}(\underline{G}^i)$ be the Greenberg transform of the smooth group scheme $\underline{G}^i$ appearing in Section~\ref{ssec:models}.
Assume $\pi_0(\underline{G}^0)_\Fq^\reductive)$ is cyclic.
Assume Conjecture~\ref{conj:Jacobi}.
Then there is a virtual Weil sheaf $\cs{A}$ on $G= G^d$ such that $\trFrob{\cs{A}} = \trace(\rho)$.
\end{proposition}
...
& \underline{G}^i(\OK/\mathfrak{p}^{m+1}) \arrow{ru}[swap]{\varphi^i_m} 
\end{tikzcd}
\] 
...
Using Proposition~\ref{prop:quasicharacters}, let $\cs{L}^i_m$ be a geometrization of the linear character $\varphi^i_m: G_m^i \to \CC^\times$; so
\[
...
%Then $\cs{L}^i$ is a quasicharacter sheaf, in the sense of \cite{cunningham-roe:13a}*{Def 4.3}, on $\Gr^{\OK}(\underline{G}^i)$ from which the quasicharacter $\varphi^i$ may be recovered.
...
Then 
\[
\trFrob{A^0} = \trace \rho_0.
\]
The special fibre $(\underline{G}^0)_\Fq$ of the smooth group scheme $\underline{G}^0$ is itself a smooth group scheme, and may be identified with the Greenberg transform $G_0^0 = \Gr^{\OK}_0(\underline{G}^0)$ \cite{cunningham-roe:13a}*{\S 4.3}. 
With $m\in \NN$ as above, let $\cs{A}_m^0$ be the Weil sheaf on the algebraic group$G_m^i$ obtained by pull-back from $A^0$ along the affine morphism $G_m^i \to G_0^0$.
Factor
\[
\begin{tikzcd}
G^0(\Fq) \arrow{rr}{\trace(\rho^0)} \arrow{rd} && \EE \\
& G_m^0(\Fq) \arrow{ru}[swap]{\trace(\rho^0)_m} 
\end{tikzcd}
\]
Observe that $\trace(\rho^0)_m$ may be recovered from $\cs{A}_m^0$:
\[
\trFrob{\cs{A}^0_m} = \trace(\rho^0)_m
\]
%Let $\cs{A}^0$ be the pull-back of $\cs{A}^0_m$ along  $\Gr^{\OK}(\underline{G}^0) \to \Gr^{\OK}_{m}(\underline{G}^0)$.
%Then $\cs{A}^0$ is a quasicharacter sheaf, in the sense of \cite{cunningham-roe:13a}*{Def 4.3}, on $\Gr^{\OK}(\underline{G}^0)$ from which the character of $\rho^0$ may be recovered.
...
Let $\cs{W}_{i+1}$ be the Weil sheaf promised by Conjecture~\ref{conj:Jacobi} on the Jacobi group such that
\[
...
We now define Weil sheaves $\cs{A}^i_m$ on $G_m^i \ceq \Gr^{R}_m(\underline{G}^{i})$, for $i=1,\ldots ,d$, recursively, following the construction of the representations $\rho^i$, as reviewed in Section~\ref{ssec:review}.
We have already defined $\cs{A}^0_m$.
\iffalse
We have already defined $\cs{A}^0_m$ so that 
\[
\trFrob{\cs{A}^0_m} = \trace(\rho_0)_m
\]
where $\trace(\rho_0)_m : G_m^0(\Fq) \to \EE$ is defined by 
\[
\begin{tikzcd}
G^0(\Fq) \arrow{rr}{\trace(\rho_0)} \arrow{rd} && \EE\\
& G^0_m(\Fq). \arrow{ru}[swap]{\trace(\rho_0)_m} & 
\end{tikzcd}
\]
and the canonical identification $G_m^0(\Fq) = \underline{G}^0(\OK/\mathfrak{p}_K^{m+1})$.
\fi
Suppose $\cs{A}^i_m$ on $G_m^i$ is defined such that
\[
\begin{tikzcd}
...
\end{eqnarray*}
Comparing with \eqref{eq:tensor}, we see that $\trFrob{\cs{B}_m^{i+1}}$ is precisely the function obtained by factoring the character of $\rho^{i+1}$ through $(\underline{G}^{i}\ltimes \underline{J}^{i+1})(\OK) \to (\underline{G}^{i}\ltimes \underline{J}^{i+1})(\OK/\mathfrak{p}_K^{m+1})$, using the canonical identification $(G_m^{i}\times_{G_m} J_m^{i+1})(\Fq) =  (\underline{G}^{i}\ltimes \underline{J}^{i+1})(\OK/\mathfrak{p}_K^{m+1})$. 
In particular, $\trFrob{\cs{B}^{i+1}_m}$ is trivial on $(G_m^{i}\times_{G_m} J_m^{i+1})(\Fq)$.
It follows that 
\[
\trFrob{\cs{\tilde A}^{i+1}_m} = n \trace(\rho^{i+1})
\]
...
Then $\tau$ is the representation of the $\Fq$-points of the special fibre of the smooth model $\underline{G}^0$ over $\OK$ and the representation $\rho^0$ of $\underline{G}(\OK)$ is produced from $\tau$ by inflation.
...
\[
\trace \rho^0(k) = \trFrob{\bar B}(k_\Fq)
\]
