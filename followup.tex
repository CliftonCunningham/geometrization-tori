...
In Section~\ref{sec:types} we give an application of Theorem~\ref{thm:geo} by geometrizing types for supercuspidal representations of $p$-adic groups. Specifically, we show that if $C$ is a compact $p$-adic group and $\rho$ is a representation of $C$ and $(C,\rho)$ is a type for a tamely ramified $p$-adic group then there is a  group scheme $G$ over $\Fq$ such that $G(\Fq) = C$ and a virtual Weil sheaf $\mathcal{A}$ on $G$ such that $\trFrob{\cs{A}} = \trace(\rho)$.
As preparation for the proof of this statement, we review types (Section~\ref{ssec:review}) and smooth integral models (Section~\ref{ssec:models}), both as developed by Yu.  
We also review some facts about the Heisenberg-Weil representation and its geometrization (Section~\ref{ssec:Jacobi}).
These elements are pulled together in Section~\ref{ssec:geotypes}, where the proof Theorem~\ref{thm:geotypes}.
...
sheaf on $G$ arises through the Lang isogeny; since the covering group is $G(k)$, which is commutative,
every character sheaf on $G$ is commutative. \todo{why does this imply that the sheave is commutative in our sense?}
\end{proof}
...
\]
Since $(\theta^* m^* i^*\cs{L})^\vee \iso \theta^* m^* i^* (\cs{L}^\vee)$ canonically, and since $i^*(\cs{L}^\vee) \cong \cs{L}$ canonically, this defines the promised isomorphism
\[
\begin{tikzcd}
...
\begin{remark}
Note that when $\Hh^1(k,G\der) = 0$, as is the case when $G\der(\bFq)$ ``is simply connected,'' \todo{Can we make this more precise?} then $\CCSiso{G} \iso G(\Fq)^*$, so
...
s^*(\nu) = \id_{\cs{L}}
\end{equation}
and the following diagram of isomorphisms of local systems on $H\times H\times G$ commutes. \todo{Maybe we should give a reference for this definition?}
...
%
Next, one forms $\cs{K}_\psi \ceq j_{!*} \cs{K}_0[\dim V]$, the perverse extension of $\cs{K}_0 [\dim V]$ along $j : S_0 \times V^\sharp \hookrightarrow \Sp(V) \ltimes V^\sharp$; recall that $S_0$ is Zariski dense in $\Sp(V)$. 
It follows from \cite{gurevich-hadani:07a} that 
\[
\trFrob{\cs{K}_\psi} = \trace(W_\psi).
\]
Then $\cs{K}_\psi$ is the promised Weil sheaf complex.
