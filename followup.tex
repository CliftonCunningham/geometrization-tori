...
\title[Geometrization of characters of smooth group schemes over finite fields]{Geometrization of characters of smooth group schemes over finite fields with an application to types for supercuspidal representations}
...
\keywords{commutative character sheaves, abelianization,quasicharacters, types,supercuspidal representations}
...
As an application, we use these \emph{commutative character sheaves} to geometrize types for supercuspidal representations of $p$-adic groups, assuming the existence of the geometrization of the Heisenberg-Weil representation over finite fields.
\end{abstract}
...
assuming the existence of a geomtrization of the Heisenberg-Weil representations over finite fields.
...
%\todo{Update based on any changes in Section 4}
...
In Section~\ref{sec:types} we use Theorem~\ref{thm:geo} to geometrize the representations of compact $p$-adic groups appearing in the types for : subject to a hypothesis on the component group of the depth-zero part of the datum and also on a conjecture on the geometrization of the character of Heisenberg-Weil representations, we find a group scheme $G$ over $\Fq$ such that $G(\Fq) = C$ and a Weil sheaf $\mathcal{F}$ on $G$ such that $\trFrob{\cs{F}} = \trace(\rho)$.
