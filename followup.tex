...
\title[Commutative Character Sheaves]{The function-sheaf dictionary for characters of smooth group schemes}
...
& \underline{G}(R/\mathfrak{p}^{m+1}) \arrow{ru}[swap]{\varphi^m} 
\end{tikzcd}
\] 
...
Using Theorem~\ref{thm:geo}, let $\cs{L}_m$ be a geometrization of the character $\varphi^m: G_m \to \EEx$; so
\[
\trFrob{\cs{L}_m} = \varphi^m
\]
on $G_m(\Fq)$.
%
...
\end{enumerate}
%
The representation $\rho_0$ and the quasicharacters $(\varphi^0, \ldots , \varphi^d)$ enjoy certain properties which allow Yu to construct representations $\rho_i$ of $C^i$, for $i=1, \ldots, d$ such that $\rho_d = \rho$ as representations of $C^d = C$.
%
...
In fact, the quotient $J_{i+1} \to V_{i+1}^\sharp$ factors through a quotient $J_{i+1} \to H_{i+1}$ and an isomorphism 
$
j_{i+1} : H_{i+1} \to V_{i+1}^\sharp,
$
where $H_{i+1}$ is a Heisenberg $p$-group in the sense of \cite{yu:01a}*{}.
%\labitem{Q3}{Q3} 
%\item
...
$
such that the pair $(f_{i+1}, j_{i+1})$ is a symplectic action of $C^i$ on $H_{i+1}$ in the sense of \cite{yu:01a}.
%
...
This completes the recursive definition and thus determines $\rho_d$. 
...
\begin{center}
\begin{tabular}{c|l}
$C^0$ & $C^0 = \,^\circ K^0 = G^0(F)_y$, \cite{yu:01a}*{\S 15} \\
$\rho_0$ & $\,^\circ \rho^0$ \cite{yu:01a}*{\S 15} \\
$C^{i+1}$ & $C^{i+1} =  \,^\circ K^i = (\,^\circ K^0) \vec{G}^{(i+1)}(F)_{y,(0, s_0, \ldots, s_{i})}$, \cite{yu:01a}*{\S 15} \\
