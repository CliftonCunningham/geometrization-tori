...
We now suppose that $H$ is a subgroup of $G$ and that the action $a : H\times G\to G$ is obtained by restricting the action $m : G \times G\to G$ to $H\times G$.
...
by 
\[
\nu = (\beta \boxtimes \id_{\cs{L}}) \circ  f^*\mu,
\] 
...
f^*(\cs{L}\boxtimes \cs{L}) \arrow{r} & 
\cs{L}\vert_{H} \boxtimes \cs{L} \arrow{r}{\beta\boxtimes \id_{\cs{L}}} & 
(\EE)_{H} \boxtimes \cs{L} \arrow{u} 
\end{tikzcd}
\]
In this way we see we may replace $(\cs{L},\mu,\nu)$ with a triple, $(\cs{L},\mu,\beta)$, from which $(\cs{L},\mu,\nu)$ may be recovered.
The isomorphism $\beta : \cs{L}\vert_{H} \to (\EE)_{H}$ is constrained by two properties, corresponding to the diagrams in Sections [] and [].


\begin{lemma}
$\CS_H(G)$ is equivalent to the category of triples $(\cs{L},\mu,\beta)$ where $(\cs{L},\mu)\in \CS(G)$ and $\beta : \cs{L}\vert_H \to (\EE)_H$ is an isomorphism in $\CS(H)$, with morphisms $\alpha : (\cs{L},\mu,\beta) \to (\cs{L}',\mu',\beta')$ defined by the condition
...
The abelianization $G\ab\ceq G/G\der$ exists in schemes over $\Fq$
and the quotient $q : G \to G\ab$ is faithfully flat and a $G\der$-torsor in the fppf topology \cite{SGA3}*{Expos\'e {VI}$_A$, Thm. 3.2}.
Then
\[
\begin{tikzcd}
%{} & G\times G \arrow{d}{c} &&&\\
1 \arrow{r}{i} & G\der \arrow{r}{} & G \arrow{r}{q} & G\ab \arrow{r} & 1
\end{tikzcd}
\]
gives
\[
...
\arrow{u}{\text{forget}} \CS'(G) \arrow{r}{\beta \mapsto \nu_\beta} & \CS_{H}(G) \arrow{u}{\text{forget}}  & 
\end{tikzcd}
\]
%
It now follows from Lemma~[] that the comparison functor $\CS(G\ab)\to \CS_{G\der}(G)$ is an equivalence.
On the other hand, by Lemma~[], $\CS_{G\der}(G)\to \CS'(G)$, defined by $(\cs{L},\mu,\nu)\mapsto (\cs{L},\mu, \beta_\nu)$, is an equivalence.
Since the essential image of the forgetful functor $\CS'(G)\to \CS(G)$ is precisely the category of $(\cs{L},\mu)\in \CS(G)$ such that $\cs{L}\vert_H \iso (\EE)_H$, this completes the proof of 
Proposition~\ref{prop:Gder_triv}.
\end{proof}
...
\begin{tikzcd}
\CS'(G) &\arrow{l}[swap]{q^*} 
\CS(G\ab) \arrow{l}{\text{`comparison'}} \\
\CCS(G) \arrow{u}{\text{forget}}  & \CCS(G\ab) \arrow{u} 
\end{tikzcd}
\]
