...
In this paper, we do not consider Lusztig's geometrization of arbitrary characters of the non-abelian group $G(k)$
using character sheaves \cite{lusztig:85a}*{Def.~2.10}, but focus only on the case of $1$-dimensional characters
$\Hom(G(k), \EEx)$.  The category $\CS(G)$ has a straightforward generalization to this case, but again
...
\todo{We should give the relationship between $\CCSiso{G}$ and $G(k)^*$ here as a corollary.  Also, include the automorphism group of a commutative character sheaf.}
...
\subsection{Quotient by a closed subgroup}\label{ssec:closed}
...
$\CS_H(G)$ is equivalent to the category $\CS^H(G)$ of triples $(\cs{L},\mu,\beta)$ where $(\cs{L},\mu)\in \CS(G)$ and $\beta : \cs{L}\vert_H \to (\EE)_H$ is an isomorphism in $\CS(H)$; morphisms $(\cs{L},\mu,\beta) \to (\cs{L}',\mu',\beta')$ in $\CS^H(G)$ are morphisms $\alpha: \cs{L} \cs{L}'$ in $\CS(G)$ such that $\beta = \beta' \circ \alpha\vert_{H}$.
\end{lemma}
