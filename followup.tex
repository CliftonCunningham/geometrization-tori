...
q^* : \CS(Q) \to \CS(G)
\]
given on objects by $(\cs{L},\mu) \mapsto (q^*\cs{L}, (q\times q)^* \mu)$; cf [Dictionary].
Then $q^*\cs{L}$ is an $H$-equivariant linear character sheaf on $G$ when equipped with the canonical isomorphism
\[
\nu : a^* q^* \cs{L} \mathop{\longrightarrow}\limits^{\iso} (q\circ a)^* \cs{L} = (q\circ p)^*\cs{L} \mathop{\longrightarrow}\limits^{\iso} p^* q^*\cs{L}.
\]
This simple observation defines the comparison functor
\[
q_H^* : \CS(Q) \to CS_H(G)
\]
and shows that the functor $q^* : \CS(Q) \to \CS(G)$ factors according to the following commuting diagram of functors
\[
\begin{tikzcd}
\CS(G) &\arrow{l}[swap]{q^*} 
\CS(Q) \arrow{dl}{q_H^*}\\
