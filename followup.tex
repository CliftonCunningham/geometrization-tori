...
If $(\cs{L},\mu,\nu)$ is an $H$-equivariant linear character sheaf on $G$ then $\mu : m^*\cs{L} \to \cs{L}\boxtimes \cs{L}$ is a morphism of $H\times H$-equivariant Weil local systems on $G\times G$; likewise $\vartheta : \cs{L}\boxtimes \cs{L} \to \theta^*(\cs{L}\boxtimes \cs{L})$. 
\end{lemma}
...
The Weil local system $\cs{L}_2$ on $G \times G$ comes equipped with the isomorphism $\nu_2 : a_2^*\cs{L}_2 \to p_2^*\cs{L}_2$ defined by ...\todo{Working here.}
This isomorphism satisfies \eqref{E1} and \eqref{E2} as they apply here.
%
...
The argument that $\vartheta : \cs{L}\boxtimes\cs{L} \to \theta^*(\cs{L}\boxtimes\cs{L})$ is morphism of $H\times H$-equivariant linear character sheaves on $G\times G$ is also straightforward.\todo{Add detail here.}
\end{proof}
...
On objects, $q_H^*: \CS(Q)\to \CS_H(G)$ is given by $q_H^*(\cs{L}_0,\mu_0) = (L_1(\cs{L}_0), L_2(\mu_0))$. 
...
Together with Lemma~\ref{lem:HH}, we may now interpret the diagram in Definition~\ref{def:CCS} as a diagram in $\Loc_{H\times H}(G\times G)$.
By Lemma~\ref{lem:torsor}, this corresponds uniquely to a diagram in $\Loc(G\ab\times G\ab)$, necessarily the diagram in Definition~\ref{def:CCScom}.
