...
%
Regarding the construction of the type $(\,^\circ  K,\,^\circ \rho)$, the paper \cite{yu:models} concludes with the following provocative observation.
...
In Section~\ref{sec:types} we use Theorem~\ref{thm:geo} to results push this observation further by geometrizing the representation $\,^\circ \rho$ of $\,^\circ K$: we find a group scheme $G$ over $\Fq$ such that $G(\Fq) = \,^\circ K$ and a virtual Weil sheaf $\mathcal{F}$ on $G$ such that $\trFrob{\cs{F}} = \trace(\,^\circ \rho)$.
%from which the character of $\,^\circ \rho$ can be recovered using the trace of Frobenius. 

\subsection{Review of Jacobi theory over finite fields}

For use below, we recall some basic facts about the Heisenberg-Weil representation.
...
\begin{enumerate}
\item[(Q.3)] A quotient $\,^\circ f_i : \,^\circ K^i \to \Sp(V_{i+1})$, such that the pair $(f_i, j_{i+1})$ is a symplectic action of $\,^\circ K^i$ on $H^{i+1}$ in the sense of \cite{yu:construction}.
\end{enumerate}
...
This determines a Weil representation on the same representation space as $\omega_{i+1}$.
Let $W_{i+1}$ be the Heisenberg-Weil representation of the Jacobi group $\Sp(V_{i+1})\ltimes V_{i+1}^\sharp$.
Then $h_{i+1}^*(W_{i+1})$ is a representation of $\,^\circ K^i \ltimes J^{i+1}$.
...
\subsection{Geometrization of the character of the Heisenberg-Weil representation}
