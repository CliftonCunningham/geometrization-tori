...
%
In this section we use Theorem~\ref{thm:geo} to geometrize the representation $\,^\circ \rho$ of $\,^\circ K$: we find a group scheme $G$ over $\Fq$ such that $G(\Fq) = \,^\circ K$ and a virtual Weil sheaf $\mathcal{F}$ on $G$ such that $\trFrob{\cs{F}} = \trace(\,^\circ \rho)$.
...
The there is a Weil sheaf $\cs{F}$ on $\Gr(\underline{G})$ such that $\trFrob{\cs{F}} = \trace \chi$. \todo{Missing proof}
\end{proposition}
...
\subsection{Geometrization of characters of types}\label{ssec:geometrization}

\todo{Everything below here is under construction}

We begin explain how to geometrize the characters of these ingredient representations, using the discussion from Section~\ref{ssec:models}.
\begin{enumerate}
\item A representation $\sigma$ of the $\Fq$-points of the reductive group $(\underline{G}^0)_\Fq^\reductive$ over $\Fq$. 
...
Let $A$ be a Weil sheaf on $(\underline{G}^0)_\Fq^\reductive$ such that, as a sheaf ${\bar A}$ on $(\underline{G}^0)_\Fq^\reductive \otimes \bFq$, it is a character sheaf, and
\[
\trFrob{\bar A} = \trace \sigma.
\]
...
\item For $i=0, \ldots, d$, a quasicharacter (continuous linear character) $\varphi_i$ of $\underline{G}^i(\OK)$. 
\item For $i=0, \ldots, d-1$, a representation $W_{i+1}$ of the the Jacobi group $\Sp(V_{i+1}) \ltimes V_{i+1}^\sharp$, viewed as the $\Fq$-points of an algebraic group over $\Fq$.
...
With notation as above, suppose $\pi_0((\underline{G}^0)_\Fq^\reductive)$ is cyclic.
Then there is a virtual admissible Weil sheaf $\mathcal{F}$ on $G$ such that $\trFrob{\mathcal{F}} = \trace \rho$ under the canonical isomorphism $G(\Fq) = \underline{G}(\OK)$.
\end{proposition}
