...
Then $c$ is a smooth morphism of $\Fq$ schemes and $j\circ c = m \circ (m \times (i \circ m \circ \theta))$.
Thus, $\beta : \cs{L}\vert_{G\der} \to (\EE)_{G\der}$ determines the isomorphism $m^*\cs{L} \otimes \theta^* m^* i^*\cs{L} \to (\EE)_{G\times G}$ by the commuting diagram of isomorphisms, below.
\[
\begin{tikzcd}
\arrow{d} c^* \cs{L}\vert_{G\der} \arrow{r}{c^*(\beta)} 
	&  c^*(\EE)_{G\der}\\
\arrow{d}{j\circ c = m \circ (m \times (i \circ m \circ \theta))} (j\circ c)^*\cs{L} 
	&  m^*\cs{L} \otimes (i \circ m \circ \theta)^*\cs{L} \arrow[dashed]{u}  \\
\arrow{d} (m \circ (m \times (i \circ m \circ \theta)))^*\cs{L} 
	& (m \times (i \circ m \circ \theta))^* (p_1^*\cs{L} \otimes p_2^*\cs{L}) \arrow{u} \\
(m \times (i \circ m \circ \theta))^* m^* \cs{L} \arrow{r}{(m \times (i \circ m \circ \theta))^*(\mu)} 
	& (m \times (i \circ m \circ \theta))^* (\cs{L} \boxtimes \cs{L}) \arrow{u}
\end{tikzcd}
\]
...
We now suppose that $j : H \hookrightarrow G$ is a closed subgroup and that the action $a : H\times G\to G$ is obtained by restricting the action $m : G \times G\to G$ to $H\times G$.
