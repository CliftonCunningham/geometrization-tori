...
\]
Since $(\theta^* m^* i^*\cs{L})^\vee \iso \theta^* m^* i^* (\cs{L}^\vee)$ canonically, and since $\cs{L}^\vee \iso i^*\cs{L}$, \todo{should we write $i^*(\cs{L}^\vee) \cong \cs{L}$ instead?} canonically, this defines the promised isomorphism
\[
\begin{tikzcd}
