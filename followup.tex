...
\subsection{Geometrization of quasicharacters of smooth group schemes over $p$-adic fields}
...
Let $\underline{G}$ be a smooth group scheme over $\OK$.
Let $\chi : \underline{G}(\OK) \to \CC^\times$ be a quasicharacter/continuous linear character. 
The there is a Weil sheaf $\cs{L}$ on $\Gr(\underline{G})$ such that $\trFrob{\cs{L}} = \trace \chi$. \todo{Missing proof}
\end{proposition}
...
1 \arrow{r} & J^{i+1} \arrow{r} & \,^\circ K^i \ltimes J^{i+1} \arrow{d} \arrow{r} & \,^\circ K^i \arrow{r} & 1\\
&& \,^\circ K^{i+1} \arrow{d} && \\
&& 1 &&
\end{tikzcd}
\end{equation}
...
h_{i+1}^*(W_{i+1}) \otimes \inf(\,^\circ\rho_i \otimes \varphi_i)
\end{equation}
of $\,^\circ K^i \ltimes J^{i+1}$ is trivial on $\,^\circ K^{i}\cap J^{i+1}$, so $h_{i+1}^*(W_{i+1}) \otimes \inf(\,^\circ\rho_i \otimes \varphi_i)$ descends to $\,^\circ K^{i+1}$. This defines $\,^\circ \rho_{i+1}$.\todo{I didn't use $\varphi_d$, so this can't be quite right.}

%The character of $\,^\circ \rho_d$ on $\,^\circ K^d = \,^\circ K = J^0 \cdots J^d$ satisfies
%\[
%\trace(\,^\circ \rho)(k_0 \cdots k_d) = \varrho(k_0)  \varphi_0(k_0) \cdots \varphi_d(k_d),
%\]
%for $k_i\in J^i$.
...
Then there is a Weil sheaf $\mathcal{F}$ on $G$ such that $\trFrob{\mathcal{F}} = \trace \rho$ under the canonical isomorphism $G(\Fq) = \underline{G}(\OK)$.
\end{proposition}
...
\[
\begin{tikzcd}
\underline{G}^i(\OK) \arrow{rr}{\varphi^i} \arrow{rd} && \CC^\times\\
...
Let $A^+$ be the Weil sheaf on $(\underline{G}^0)_\Fq$ obtained by pullback along the quotient $(\underline{G}^0)_\Fq \to (\underline{G}^0)_\Fq^\reductive$.
Then 
\[
\trFrob{A^+} = \trace \tau.
\]
...
\[
\begin{tikzcd}
\underline{G}^0(\OK) \arrow{rr}{\trace(\varrho)} \arrow{rd} && \CC\\
...
Suppose $\cs{A}^i_m$ on $G_m^i$ is defined; we shall construct $\cs{A}_m^{i+1}$ on $G_m^{i+1}$.
...
Before continuing, let us calculate: 
\begin{eqnarray*}
...
%\item 
For $i=0, \ldots, d$, a quasicharacter (continuous linear character) $\varphi_i$ of $\underline{G}^i(\OK)$. 
...
For $i=0,\ldots, d-1$, let $\cs{F}_{i+1}$ be the Weil sheaf on the $\Fq$-scheme $\Gr(\underline{G}^i)$ obtained by applying Proposition~\ref{prop:quasicharacters} to $\varphi_i : \underline{G}^i(\OK) \to \CC^\times$.
