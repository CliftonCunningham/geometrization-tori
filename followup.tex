...
&= c^* ((\EE)_C).
\end{align*}
Since $c : G \times G \to C$ is smooth with geometrically connected fibres, \todo{Must insert proof of this! \\ -CC} the functor $c^*$ is full and faithful \cite{BBD}*{Prop 4.2.5} \todo{I'll find a better reference. Big stone. Small bird. \\ -CC} so $c^* (\cs{L}\vert_C) \iso c^* ((\EE)_C)$ implies $\cs{L}\vert_C \iso (\EE)_C$. 
\end{proof}
...
\end{align*}
 ...
 \begin{proof}
 By \'etale descent, pullback induces an equivalence of categories between $\CCS(G)$ and $\CCS(G/C)$. \todo{I need to expand on this trick. \\ -CC}
...
\todo{I think I got this wrong. I think our sheaves are not extra information in the commutative case. Masound's definition is designed to match the definition coming from central extensions, while we have one that matches Ext1 in Ab, in the connected case. So, I think the modification is easy: add mop to diagram 2.4 in Masoud's paper. \\ -CC}
 \end{remark}
