...
\subsection{Geometrization of generic data for supercuspidal representations}

As proved in \cite{kim:exhaustion}, many (complex) supercuspidal representations of $p$-adic groups can be built from the generic data of \cite{yu:construction}*{\S 15} and, moreover, the type (in the sense of \cite{bushnell-kutzko:98a}) of the resulting supercuspidal representations can be constructed directly from the generic data. 
...
A generic datum is a triple $(\vec{\G}, \pi_0, \vec{\phi})$ where: $\vec{\G}$ is a tower of $p$-adic groups $\G^0 \subsetneq \G^1 \subsetneq \cdots  \subsetneq \G^d = \G$; $\pi_0$ is a depth-zero representation of $\G^0(F)$, and $\vec{\phi} = (\phi_0, \ldots , \phi_{d-1})$ where $\phi_i : \G^i(K) \to \EEx$ is a quasicharacter (continuous group homomorphism).  
...
In \cite{yu:smooth} it is shown that, for $0 \leq i < d$, the $p$-adic group $\G^i(K)$ admits a smooth integral model, in the sense that there is a smooth group scheme $\underline{G}^i$ over $\OK$ such that $\underline{G}^i(\OK) = \G^i(K)$.
Set $G^i_n \ceq \Gr^R_n(\underline{G}^i)$ and $G^i \ceq \Gr_R(\underline{G}^i)$.
Then $G^i(\Fq) = \G^i(K)$ (canonical isomorphism of groups).
It now follows from Corollary~\ref{cor:QCS} that the quasicharacter $\phi_i : G^i(\Fq) \to \EEx$ determines a quasicharacter sheaf $\cs{L}_i$, unique up to isomorphism, from which the quasicharacter $\phi_i$ can be recovered using the trace of Frobenius. 
For each $0\leq i < d$, let $n_i$ be the smallest non-negative integer $n$ such that $\phi_i : G^i(\Fq) \to \EEx$ factors through $G^i(\Fq) \to G^i_n(\Fq)$.
