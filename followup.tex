...
We now suppose that $H$ is normal in $G$, that the fppf-quotient $G \to G'\ceq H\backslash G$ exists.
...
\begin{proof}
The quotient $G \to G\ab$ is a $G\der$-torsor in the etale topology, \todo{Either true or it becomes a condition on $G$.} so the result follows from Lemma~\ref{lem:descent}.
\end{proof}
