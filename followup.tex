...
\end{enumerate}
The groups $C^i$ here are the groups $\,^\circ K^i$ in \cite{yu:01a}; the representation $\rho^0$ here is the representation $\pi_0\vert_{\,^\circ K^0}$ in \cite{yu:01a}; the quasicharacters $\varphi^i$ here are the quasicharacters $\phi_i\vert_{\,^\circ K^i}$ in \cite{yu:01a}.
%
The representation $\rho^0$ and the quasicharacters $(\varphi^0, \ldots , \varphi^d)$ enjoy certain properties which allow Yu to construct representations $\rho^i$ of $C^i$, for $i=1, \ldots, d$ such that $\rho^d = \rho$.
%

Since we will need some facts about this construction, we review some further detail here.
In Table~\ref{table:notation} we explain how to convert the rest of the constructions appearing in this section into the notation of \cite{yu:01a}.
%
First, Yu introduces 
\begin{enumerate}
\labitem{Y3}{Y3}
...
The vector space $V_{i+1}$ is then equipped with a symplectic pairing $\langle\ ,\ \rangle_{i+1} : V_{i+1}\times V_{i+1} \to Z_{i+1}$, where $Z_{i+1}$ is a one-dimensional vector space over $\Fq$, itself equipped with an additive character $\psi_{i+1} : Z_{i+1} \to \EEx$.
%\labitem{Q2}{Q2} 
%\item 
...
$
where $V_{i+1}^\sharp$ is the Heisenberg group determined by $V_{i+1}$, $Z_{i+1}$, $\langle\ ,\ \rangle_{i+1}$ and $\psi_{i+1}$.
In fact, the quotient $J_{i+1} \to V_{i+1}^\sharp$ factors through a quotient $J_{i+1} \to H^{i+1}$ and an isomorphism 
$
...
Write $\inf(\rho^i \otimes \varphi^i)$ for the representation of $C^i \ltimes J_{i+1}$ obtained by pulling back $\rho^i$ along $C^i \ltimes J_{i+1} \to C^i$. 
...
Now set $\rho \ceq \rho^d\otimes \varphi^d$. 
This is the representation of $C^d = C$ determining the Yu type $(C,\rho)$.
%The character of $\rho^d$ on $C^d = C = J_0 \cdots J_d$ satisfies
%\[
...
\begin{center}
\begin{tabular}{c|l}
$C^0$ & $\,^\circ K^0 = G^i(F)_y$, \cite{yu:01a}*{\S 15} \\
$\rho^0$ & $\,^\circ \rho_0$ \cite{yu:01a}*{\S 15} \\
$C^{i+1}$ & $\,^\circ K^i = (\,^\circ K^0) \vec{G}^{(i+1)}(F)_{y,(0, s_0, l\dots, s_{i})}$, \cite{yu:01a}*{\S 15} \\
$\rho^{i+1}$ & $\,^\circ \rho_{i+1}$, \cite{yu:01a}*{\S 15} \\
...
The vector space $V_{i+1}$ may realized as the $\Fq$-points on a variety $V^{i+1}_\Fq$, where $V^{i+1}_\Fq$, appears as a quotient $J_{i+1} \to V^{i+1}$ of algebraic groups over $\Fq$. Then the quotient $J_{i+1} \to V_{i+1}$ is realized as the composition
\[
\underline{J}^{i+1}(R) \to \underline{J}^{i+1}(\Fq) = \underline{J}^{i+1}_\Fq(\Fq) \to V^{i+1}_\Fq(\Fq) = V_{i+1}.
\]
Likewise, the Heisenberg $p$-group $H^{i+1}$, appearing in \ref{ssec:review}, may be realized as a quotient of algebraic groups, and $J_{i+1}\to H^{i+1}$ as the composition 
\[
