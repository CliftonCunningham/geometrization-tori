...
As an application, we use the category $\CCS(G)$ to give a geometric description of types for supercuspidal representations of connected, reductive groups over $p$-adic fields.
...
Then $\gcs{L}$ appears in the semisimple complex $\operatorname{ind}_{B,T}^{\bG}(\gcs{L}_T)$.
It follows that every object in $\CCS(G)$ determines Frobenius-stable character sheaf on $G$, in the sense of \cite{lusztig:85a}*{Def.~2.10}.
...
but they are precisely the part needed to describe one dimensional characters of such groups.
\bigskip
