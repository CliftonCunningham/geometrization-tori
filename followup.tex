...
sheaves on $\bGxG$ such that the following diagrams commute,
  ...
  \bm_3^*\gcs{L} \arrow{rr}{(\bm \tight{1}{\times}{2} \id)^*\mu} \arrow[swap]{d}{(\id \tight{2}{\times}{1} \bm)^*\mu} 
    &&  \bm^*\gcs{L} \boxtimes \gcs{L} \dar{\mu \tight{0}{\boxtimes}{1} \id}  && \bm^*\gcs{L} \arrow{r}{\mu} \arrow{dr}[swap]{\mu}& \gcs{L} \boxtimes \gcs{L} \arrow{d}{\iso}\\
...
With reference to \cite{cunningham-roe:13a}*{Def. 1.1}, a we see that a character sheaf $\cs{L} = (\gcs{L},\mu, \phi)$ on $G$ is commutative if it satisfies only one additional condition: that
\[
\begin{tikzcd}
