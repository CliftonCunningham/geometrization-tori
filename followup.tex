...
\begin{tikzcd}
%{} & G\times G \arrow{d}{c} &&&\\
1 \arrow{r} & H \arrow{r}{i} & G \arrow{r}{q} & G\ab \arrow{r} & 1
\end{tikzcd}
\]
gives
\[
...
\CS(G\ab) \arrow{dl}{q_{H}^*}[swap]{\iso} \\
 \CS'(G) \arrow{u}{\text{forget}}  \arrow{r}{\beta \mapsto \nu}[swap]{\iso} &  \CS_{H}(G) \arrow{u}{\text{forget}}  & 
\end{tikzcd}
\]
%
Here we use the notation $\CS'(G)$ from Section~\ref{ssec:noncomdef} and the observation that it is precisely the category $\CS^{H}(G)$ from Section~\ref{ssec:closed} for $H=G\der$.
...
&  \arrow[bend left=35pt, dashed]{dll} \CCS(G\ab) \arrow{u} \arrow{dl}[swap]{\iso} \\
...
Together with Lemma~\ref{lem:HH}, we may now interpret the diagram in Definition~\ref{def:CCS} as a diagram in $\CS_{H\times H}(G\times G)$.
