...
\section{Application: geometric types for supercuspidal representations}
...
{Application to certain supercuspidal representations of $p$-adic groups}
...
Let $G$ be a group scheme acting on $X$ by $m : G \times X \to X$; let $p : G\times X \to X$ be projection.

Recall that a morphism $f : X\to S$ is a regular epimorphism if . . . . coequalizer for the pair of morphisms $(m,p)$ if ....

A morphism $f : X\to S$ is a $G$-torsor if it is equivariant for the trivial action of $G$ on $S$ and if there is a covering $\{ S_i \tq i \in I\}$ of $S$ such that, for each $i\in I$, there is an isomorphism $X\times_S S_i \to G\times S_i$ such that composition with projection $G\times S_i \to S_i$ is $X\times_S S_i \to S_i$.


We recall the definition of the category $\Loc_G(X)$ of $G$-equivariant local systems on $X$.
Objects in $\Loc_G(X)$ are pairs $(\cs{L}, \nu)$ where $\cs{L} \in \Loc(X)$ and $\nu : m^*\cs{L} \to p^*\cs{L}$ is an isomorphism in $\Loc(G\times X)$
such that the following diagram commutes, where $m_0(a,b,x) = (a,m(b,x))$ and $m_1(a,b,x) = (m(ab),x)$ and $p_2(a,b,x) = (b,x)$.
\[
\begin{tikzcd}
...
A morphism $(\cs{L},\nu) \to (\cs{L}',\nu')$ in $\Loc_G(X)$ is a morphism $\alpha: \cs{L} \to \cs{L}'$ in $\Loc(X)$ such that . . . .

Let $f : X\to S$ be a regular epimorphism with kernel pair $(m,p)$. 
If $\cs{S} \in \Loc(S)$ then $f^*\cs{S} \in \Loc(X)$ comes equipped with a canonical isomorphism $m^* (f^* \cs{S}) \to p^* (f^*\cs{S})$, since $f\circ m = f\circ p$.
This defines $f_G^* : \Loc(S) \to \Loc_G(X)$ such that the functor $f^* : \Loc(S) \to \Loc(X)$ factors as below.
\[
\begin{tikzcd}
\Loc(X) 
&&\arrow{ll}[swap]{f^*} 
\Loc(S) \arrow{dl}{f_G^*}\\
& \arrow{ul}{\text{forget}} \Loc_G(X) & 
\end{tikzcd}
\]
...
If $f: X\to S$ is a $G$-torsor and a regular epimorphism for $(m,p)$, then $f_G^* : \Loc(S) \to \Loc_G(X)$ admits a left-adjoint.
% $f^G_! : \Loc_G(X) \to \Loc(S)$
\end{lemma}



\begin{remark}
Comparison with the comparison functor
\end{remark}


\begin{lemma}
In this case, the functor $f^* : \Loc(S)\to \Loc(X)$ admits a left-adjoint 
\end{lemma}
\begin{proof}
The left-adjoint $f_! : \Loc(X) \to \Loc(S)$ is defined as follows.
Now, suppose $\cs{L}\in \Loc(X)$.
For each $i\in I$, define $\cs{S}_i \in \Loc(S_i)$ by $\cs{S}_i\ceq t_i^* \cs{L}$.
The sheaves $\cs{S}_i$ glue to define a sheaf $\cs{S}$ on $S$ by definition above.
Morphisms of these sheaves are likewise defined locally . . . 
...
is a coequalizer of $m$ and projection $G\times X \to X$  
...
\]  
From \cite[Prop~4.2.5]{BBD}  it follows that $\mathcal{G}\in \obj{\mathcal M}{Y}$. This completes the proof of Proposition~\ref{lem:descent}.
\end{proof}

\begin{proposition}
Let $f : X \to S$ be a morphism of smooth group schemes.
Suppose $G$ is a smooth group scheme acting on $X$.
Suppose $f$ is a $G$-torsor.
Then the comparison functor, pictured below,
\[
\begin{tikzcd}
\CCS(X) 
&&\arrow{ll}[swap]{f^*} 
\CCS(S) \arrow{dl}{\text{comparison}}\\
& \arrow{ul}{\text{forget}} \CCS_G(X) & 
\end{tikzcd}
\]
is an equivalence.
\end{proposition}
