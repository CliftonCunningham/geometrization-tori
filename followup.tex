...
\begin{lemma}\label{lem:GderGder}
If $(\cs{L},\mu,\beta)$ is a commutative character sheaf on $G$ then $\gamma : m^*\cs{L} \to \theta^* m^* \cs{L}$ is a morphism of $G\der\times G\der$-equivariant linear character sheaves on $G\times G$.
\end{lemma}
...
The isomorphism $\beta : \cs{L}\vert_{H} \to (\EE)_{H}$ is constrained conditions from Sections~\ref{ssec:equivariant1} and \ref{ssec:equivariant2}.
...
Using Lemmas~\ref{lem:HH} and \ref{lem:GderGder}, we may interpret the diagram in Definition~\ref{def:CCS} as a diagram in $\CS_{G\der\times G\der}(G\times G)$.
