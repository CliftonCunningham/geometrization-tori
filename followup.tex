...
Let $G$ be a smooth group scheme over a finite field $\Fq$; that is, let $G$ be a group scheme over $\Fq$ for which the structure morphism $G \to \Spec{\Fq}$ is smooth in the sense of [include reference for this definition].
This implies $G \to \Spec{\Fq}$ is locally of finite type; it does not imply the map is of finite type.
We remark that the identity component $G^0$ of $G$ is of finite type over $\Fq$, while the component group scheme $\pi_0(G)$ of $G$ is an \'etale group scheme over $\Fq$, and both are smooth over $\Fq$.
...
For any local system $\gcs{L}$ on $\bG \ceq G \times_{\Spec{\Fq}} \Spec{\bFq}$ there is an isomorphism $\gcs{L} \boxtimes \gcs{L} \to {\bar \theta}^*(\gcs{L} \boxtimes \gcs{L})$ given on stalks by $\gcs{L}_{g} \otimes \gcs{L}_{h} \to \gcs{L}_{h} \otimes \gcs{L}_{g}$.
...
\labitem{(CCS.2)}{CS.2} $\mu: \bm^* \gcs{L} \to \gcs{L} \boxtimes \gcs{L}$ is an isomorphism of
sheaves on $\bG \times \bG$ and there is an isomorphism ${\bar m}^* \gcs{L} \to {\bar m}\op^* \gcs{L}$ such that the following diagrams commute,
  \[
  \begin{tikzcd}[row sep=30]
  \bm_3^*\gcs{L} \arrow{rr}{(\bm \tight{1}{\times}{2} \id)^*\mu} \arrow[swap]{d}{(\id \tight{2}{\times}{1} \bm)^*\mu} 
    &&  \bm^*\gcs{L} \boxtimes \gcs{L} \dar{\mu \tight{0}{\boxtimes}{1} \id}  && \bm^*\gcs{L} \arrow{d} \arrow{r}{\mu} & \gcs{L} \boxtimes \gcs{L} \arrow{d}\\
    \gcs{L} \boxtimes \bm^* \gcs{L} \arrow{rr}{\id \boxtimes \mu}
    &  &  \gcs{L} \boxtimes \gcs{L} \boxtimes \gcs{L} && {\bar m}\op^*\gcs{L} \arrow{r}{{\bar \theta}^*\mu} &  {\bar \theta}^*(\gcs{L} \boxtimes \gcs{L})
  \end{tikzcd}
  \]
where $m_3 \ceq m\circ (m\tight{1}{\times}{2}\id) = m\circ (\id\tight{2}{\times}{1} m)$,
 and where 
$\gcs{L} \boxtimes \gcs{L} \to {\bar \theta}^*(\gcs{L} \boxtimes \gcs{L})$ is the isomorphism of sheaves  above;
%given by $x\otimes y \mapsto y \otimes x$, by which we meant the case of the sheaf isomorphism $\cs{L}_1 \boxtimes \cs{L}_2 \to \cs{L}_2\boxtimes \cs{L}_1$ when $\cs{L}_1 = \cs{L}_2 = \cs{L}$.
...
\begin{lemma}
If $G$ is also commutative, then $\CCS(G)$ is a full subcategory of $\CS(G)$, the 
later defined in \cite{cunningham-roe:13a}.
\end{lemma}

\begin{proof}
 a we see that a character sheaf $\cs{L} = (\gcs{L},\mu, \phi)$ on $G$ is commutative if it satisfies only one additional condition:
Consequently, comparing with \cite{cunningham-roe:13a}*{Def. 1.1} we find conditions (\textbf{CS}.1), (\textbf{CS}.3) and (\textbf{CS}.4) appearing in the definition of the category $\CS(G)$, coincide exactly with conditions (\textbf{CCS}.1), (\textbf{CCS}.3) and (\textbf{CCS}.4), above.
\end{proof}

\begin{remark}
In fact, \cite{cunningham-roe:13a}*{Def. 1.1} makes sense even if $G$ is not commutative. Extending that definition to include smooth group schemes over $\Fq$, it is again true that $\CCS(G)$ is a full subcategory of $\CS(G)$.
\end{remark}
...
\begin{tikzcd}
 ...
 \end{tikzcd}
 \]
from \cite{cunningham-roe:13a}*{Prop. 2.7}.  For a character sheaf $\cs{L}$ on $G$, recall that $S_G$ maps
$[\cs{L}]$ to $[\alpha \oplus \beta]$, where $\alpha \in C^0(\Weil{}, C^2(\bG, \EEx))$ is obtained from $\mu$ and
$\beta \in C^1(\Weil{}, C^1(\bG, \EEx))$ is obtained from $\phi$.  Suppose that $\cs{L}$ is a commutative character sheaf
and $\TrFrob(\cs{L}) = 1$.  Then $S_G([\cs{L}])$ is in the image of $\Hh^2(\bG, \EEx)^\Weil{}$, so is cohomologous to
$[\alpha', 0]$.  Since $\alpha$ is symmetric and coboundaries are symmetric, $\alpha'$ is symmetric as well.
So by Lemma \ref{lem:symtriv}
\end{proof}

\section{Applications}
 
\subsection{Geometrization of generic types supercuspidal representations}

As proved in \cite{kim:exhaustion}, many (complex) supercuspidal representations of $p$-adic groups can be built from the generic data of \cite{yu:construction}*{\S 15} and, moreover, that the type of the resulting supercuspidal representations can be constructed directly from the generic data. Let us refer to the types arising in this manner as {\it generic types}.
In this section we show that, after fixing an isomorphism $\CC \approx \EE$, every generic type is geometrizable, in the sense that every part of generic type admits a description as a sheaf, from which the type can be recovered.

Let $K$ be a $p$-adic field. Let $\G$ be a connected reductive linear algebraic group over $K$. A generic datum is a triple $(\vec{\G}, \pi_0, \vec{\phi})$ where: $\vec{\G}$ is a tower of $p$-adic groups $\G^0 \subsetneq \G^1 \subsetneq \cdots \G^d = \G$; $\pi_0$ is a depth-zero representation of $\G^0(F)$, and $\vec{\phi} = (\phi_0, \ldots , \phi_{d-1})$ where $\phi_i : \G^i(K) \to \EEx$ is a quasicharacter (continuous group homomorphism). 
In \cite{yu:smooth}*{\S 10.5}, it is shown that 

\subsection{Stacky abelianization}
