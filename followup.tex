...
In this section we consider a different perspective on $\CS_H(G)$ of $\CS(G)$ which will help us understand how the forgetful functor $\CS_H(G) \to \CS(G)$ induces an equivalence $\CS_H(G) \to \CS'(G)$, in the case required to prove Proposition~1.2.
...
Every $H$-equivariant linear character sheaf $(\cs{L},\mu, \nu)$ defines an isomorphism $\beta : f_1^*\cs{L} \to (\EE)_{H\times G}$ by $\beta \boxtimes \id = \nu \circ f^*(\mu)^{-1}$ from which $\nu$ may be recovered. 
In this way we see we may replace $(\cs{L},\mu,\nu)$ with another triple, $(\cs{L},\mu,\beta)$.
What are the defining properties of $\beta$?
