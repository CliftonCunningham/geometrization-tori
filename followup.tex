...
Let $G\der \hookrightarrow G$ denote the closed subgroup generated by the commutator morphism $c: G\times G \to G$ given by $c(x,y) = xyx^{-1}y^{-1}$; see \cite{SGA3}*{{VI}$_B$, 7.2}.

\begin{lemma}
The quotient $q : G \to G\ab$ is a smooth $G\der$-torsor, and the coequalizer of $m, p : G\der \times G \to G$.
\end{lemma}

The quotient $q : G \to G\ab$ is the coequalizer of left $G\der$-multiplication $n : G\der\times G \to G$ and projection $p : G\der\times G \to G$.
\[
\begin{tikzcd}
G\der\times G
...
\end{tikzcd}
\]
The morphism $q$ is surjective and smooth of relative dimension $\dim G\der$.
...
\begin{proof}
It follows from the lemma that descent theory that $q$ is an 
...
Consider the morphism $c:  G \times G \to G\der$ given by $c(g,h)= ghg^{-1}h^{-1}$.
This morphism is smooth (it is a composition of smooth maps, since $m : G \times G \to G$ is smooth) but
...
%(In other language, $q^*$ is a monadic functor.)
