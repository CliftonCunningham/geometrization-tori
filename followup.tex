...
$c_1 : H \times G\times G \to G\times G$ defined by $c_1(h, g_1, g_2) \ceq (a(h,g_1),g_2)$;  $c_2 : H \times G\times G \to H\times G$ defined by $c_2(h,g_1,g_2) \ceq (h, m(g_1,g_2))$; and $c_3 : H \times G\times G \to G\times G$ defined by $c_3(h, g_1, g_2) = (g_1, g_2)$. 
...
Let us know specialize Section [] to the following case: $H = G\der$ and the action $a$ is the restriction of $m$ to $G\der\times G$.
Then $f : G\der \times G\to G\times G$ is simply the inclusion and $f_1: G\der\times G \to G$ is projection to the first coordinate followed by inclusion $G\der \hookrightarrow G$, so $f_1^*\cs{L} = \cs{L}\vert_{G\der} \boxtimes (\EE)_G$. 
Then $\beta : f_1^*\cs{L} \to (\EE)_{H\times G}$ determines an isomorphism $\beta : \cs{L}\vert_{G\der} \to (\EE)_{G\der}$, and vice versa.
