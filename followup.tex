...
These elements are pulled together in Section~\ref{ssec:geotypes}, where the proof Theorem~\ref{thm:geotypes} is given.
 ...
 \todo{There are some grammatical errors in this paragraph, and I'm not 100\% sure how they should be corrected}
...
Since the covering group of the Lang isogeny is $G(k)$, which is commutative, then the class of this extension satisfies $a(x,y) = a(y,x)$ for all $x,y \in \bG$. 
...
The category $\CCS(G)$ of commutative character sheaves on $G$ is the full subcategory of $\CS'(G)$ consisting of triples $(\cs{L},\mu,\beta)$ such that the following diagram of Weil sheaves on $G \times G$ commutes:
  ...
  \end{tikzcd}
  \]
Here $\gamma : m^*\cs{L} \to \theta^* m^* \cs{L}$ is the isomorphism built from $\beta : \cs{L}\vert_{G\der} \to (\EE)_{G\der}$ above. \todo{Why not use $\gamma$ directly instead of $\beta$?}
\end{definition}
...
For every quasicharacter character $\varphi : \underline{G}(S) \to \EEx$ \todo{What is $S$?} there is a Weil sheaf $\cs{L}$ on $G$ such that \[\trFrob{\cs{L}} =  \varphi.\] \end{proposition}
...
Let $V^\sharp$ be the Heisenberg group determined by $(Z, \langle\ ,\ \rangle)$. \todo{We should include a reference defining the Heisenberg group}
Then $V^\sharp$ is a split extension of $V$ by $Z$. \todo{split in what category?}
...
Let $\psi : Z \to \EEx$ be an additive character and let $\omega_\psi$ be the Heisenberg representation on $V^\sharp$ with central character $\psi$. 
...
The character $\trace(W_\psi) : \Sp(V)\ltimes V^\sharp \to \EE^\times$ of $W_\psi$ is studied in \cite{howe:73a}, where is it shown that $\trace(W_\psi)$ is determined by its restriction to $\Sp(V)$ and by $\psi$.  Moreover, $\trace(W_\psi)(g,v,z) =0$ unless $v\in \ker(g-1)$, in which case $(g,v,z)$ lies in a conjugacy class intersecting $\Sp(V) \ltimes Z$. 
%
...
%the boundary of $S_0$ is $\cup_{0< r\leq \dim V} S_r$. 
 From \cite{howe:73a}*{Prop. 2} we learn that if $g\in S_r$ then $q^{-r/2} \trace(W_\psi)(g)$ has complex norm $1$. \todo{We should say something about $\CC$ vs $\EE$}

It is one of the main results of \cite{gurevich-hadani:07a} \todo{Can we give a specific reference here?} (see also \cite{gurevich-hadani:11a}*{Thm. 4.5}) that there is a Weil sheaf complex $\cs{K}_\psi$ on $\Sp(V)\ltimes V^\sharp$ such that 
\begin{equation}
\trFrob{\cs{K}_\psi} = \trace(W_\psi).
\end{equation}
%
Since $\cs{K}_\psi$ is an object in Deligne's category $D^b_c(\Sp(V)\ltimes V^\sharp,\EE)$, the left hand side of this equality must be interpreted accordingly.  Although only the existence of $\cs{K}_\psi$ will be needed below, we recall its construction briefly.

Fix an isomorphism $\CC \approx \EE$ so that we may mix classical and geometric facts. 
Observe that $S_0$ is the domain of the Cayley transform $\cay : g \mapsto (g-1)^{-1}(g+1)$ on $\Sp(V)$.
If $(g,v,z) \in S_0\times V\times Z$ then
...
An adaptation of the non-normalized kernels and the normalization coefficients \cite{gurevich-hadani:07a}*{Appendix B.1} allows for the geometrization of \eqref{Cayley} with a $\Sp(V)\ltimes V^\sharp$-equivariant irreducible Weil sheaf $\cs{K}_0$ (in fact, a Weil local system) on $S_0 \times V^\sharp$ such that 
\[
\trFrob{\cs{K}_0(\frac{\dim V}{2})} = \trace(W_\psi)\vert_{S_0\times V^\sharp},
\]
where $\cs{K}_0(\frac{\dim V}{2})$ is the Tate twist of $\cs{K}$.
%
We set $\cs{K}_\psi \ceq j_{!*} \cs{K}_0[\dim V]$, the perverse extension of $\cs{K}_0 [\dim V]$ along $j : S_0 \times V^\sharp \hookrightarrow \Sp(V) \ltimes V^\sharp$.
...
\end{equation}
\end{enumerate}
%He then introduces the following quotients of $J_{i+1}$ and $C^i$, for $i=0, \ldots d-1$.
%
...
%\trace({\tilde\rho}_{i+1})(y)= \trace(h_{i+1}^*(W_{i+1})(y)\ \trace(\rho_i)(p_i(y))\ \varphi^i(p_i(y))
%\end{equation}
%for $y \in C^i \ltimes J_{i+1}$.
...
%The character of $\rho_d$ on $C^d = C = J_0 \cdots J_d$ satisfies
%\[
%\trace(\rho)(k_0 \cdots k_d) = \rho_0(k_0)  \varphi^0(k_0) \cdots \varphi^d(k_d),
%\]
%for $k_i\in J_i$.
...
$C^{i+1}$ & $\,^\circ K^{i+1} = (\,^\circ K^0) \vec{G}^{(i+1)}(F)_{y,(0, s_0, \ldots, s_{i})}$\\% \cite{yu:01a}*{\S 15} \\
...
%Then $\cs{A}^0$ is a quasicharacter sheaf, in the sense of \cite{cunningham-roe:13a}*{Def 4.3}, on $\Gr^{R}(\underline{G}^0)$ from which the character of $\rho_0$ may be recovered.
...
\bib{yu:03a}{unpublished}{
      author={Yu, Jiu-Kang},
       title={{Smooth models associated to concave functions in Bruhat-Tits
  theory}},
        date={2003},
        note={Preprint},
}
