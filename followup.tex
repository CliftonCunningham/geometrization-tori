...
\begin{remark}
Lemma~\ref{lem:Lder} provides another indication of the correctness of Definition~\ref{def:CCS} 
if one is looking for the geometrization of characters of smooth group schemes over finite fields: commutative character sheaves are trivial on $G\der$, rank-$1$ character sheaves are not.
In the next section we provide examples to show that $\CCS(G)$ and $\CS(G)$ are not isomorphic categories, even when $G$ is a connected reductive algebraic group. 
\end{remark}
...
An $H$-equivariant rank-$1$ character sheaf on $G$ is a rank-$1$ character sheaf $\cs{L} = (\gcs{L},\phi,\mu)$ on $G$ together with an isomorphism  $\nu : {\bar m}_H^*\gcs{L} \to {\bar p}_H^*\gcs{L}$ such that the following diagrams commute, expressing compatibility with $\phi$ and $\mu$:
\begin{enumerate}
\item
\[
\begin{tikzcd}
...
This exists in schemes and is a $G\der$-torsor in the etale topology since  . . . \todo{Either true or it becomes a condition on $G$. I hope smoothness does the trick. I've written to Masoud for help on this.}
%\todo{Proof needed}
...
\begin{proof}
The quotient $G \to G\ab$ is a $G\der$-torsor in the etale topology,  so the result follows from Lemma~\ref{lem:descent}.
\end{proof}
...
\begin{remark} 
It is not true that the categories $\CS(G\ab)$ and $\CS(G)$ are equivalent, even when $G$ is a connected reductive algebraic group, as we will see in the next subsection.
\end{remark}
