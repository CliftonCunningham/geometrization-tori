...
\section{Main Definition and Result}
...
\subsection{Commutative group schemes}

\begin{lemma}\label{lem:commutative}
If $G$ is commutative then the categories $\CCS(G)$ and $\CS(G)$ are equivalent.
\todo{I'm confused. There are extra isomorphisms in the former category, but I think they're equivalent for indirect reasons.}
\end{lemma}
...
We suppose $G\ab$ exists in group schemes over $\Fq$. \todo{This is a condition on $G$. Does it follow from smoothness? I've written to Masoud for help on this.}
...
\[
\begin{tikzcd}
 \CCS(G)  &&& \arrow{lll}[swap]{q^*}  \CCS(G\ab) \arrow{d}{\text{Equivariance}} \\
\CS_G(G)  \arrow{u}{\text{Equivariance}} &&& \CS_{G\ab}(G\ab) \arrow{d}{\text{Commutative group}}  \\
\CS_{G\der}(G) \arrow{u}{\text{Change of Action}} &&& \arrow{lll}{\text{Descent along torsor}}  \CS(G\ab)  \\
\end{tikzcd}
\]
...
We wish to consider the category $\CS_G(G)$ of $G$-equivariant linear character sheaves on $G$. To that end, in this section we introduce that notion in its general context.
...
$\CCS(G)$ and $\CS_G(G)$ are equivalent.
\end{lemma}
%\begin{proposition}
...
\[
\begin{tikzcd}
 \CCS(G)  &&& \arrow{lll}[swap]{q^*}  \CCS(G\ab) \arrow{d}{\text{Lem~\ref{lem:equivariant}}} \\
\CS_G(G)  \arrow{u}{\text{Lem~\ref{lem:equivariant}}} &&& \CS_{G\ab}(G\ab) \arrow{d}{\text{Lem~\ref{lem:commutative}}}  \\
\CS_{G\der}(G) \arrow{u}{\text{Lem~\ref{lem:action}}} &&& \arrow{lll}{\text{Lem~\ref{lem:descent}}}  \CS(G\ab)  \\
\end{tikzcd}
\]
\end{proof}
