...
Define
\begin{align*}
m_2 : G\times G\times G\times G &\to G\times G \\
...
\end{align*}
and set $\cs{L}_2\ceq m_2^*\cs{L}$ and $\mu_2 \ceq m_2^*(\mu)$.
Pulling back $\cs{L}$ along $m : G\times G \to G$ produces the linear character sheaf $(\cs{L}_2, \mu_2)$ on $G \times G$ (see \cite{cunningham-roe:13a}*{Lemma 1.4}),
which comes equipped with the isomorphism $\nu_2 : a_2^*\cs{L}_2 \to p_2^*\cs{L}_2$. 
This isomorphism satisfies the appropriate version of \eqref{ECS1}, so  $(\cs{L}_2,\mu_2,\nu_2)$ is indeed an $H\times H$-equivariant linear character sheaf on $G\times G$.
...
\end{tikzcd}
\]
By abuse of notation, we will also refer to the missing vertical map down the center of this diagram as $\nu \boxtimes \nu$.  Since this diagram commutes, $(\cs{L}\boxtimes \cs{L},\mu\boxtimes \mu,\nu\boxtimes\nu)$ is also an $H\times H$-equivariant linear character sheaf on $G\times G$.
The appropriate version of \eqref{E3} also commutes, so we can reinterpret
\[
\mu : (\cs{L}_2, \mu_2, \nu_2) \to (\cs{L}\boxtimes\cs{L}, \mu \boxtimes \mu, \nu \boxtimes \nu)
\]
as a morphism of equivariant linear character sheaves.
