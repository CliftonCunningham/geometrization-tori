...
%\end{tikzcd}
%\end{equation}
%commutes; see also [Dictionary, CS.4].
%
...
this can be seen by following the argument of [Dictionary].
Set $\cs{L}_2\ceq m^*\cs{L}$ and $\mu_2 \ceq m_2^*(\mu)$.
...
The  isomorphism $\nu_2 : a_2^*\cs{L}_2\to p_2^*\cs{L}_2$ satisfies the appropriate version of \cite{ECS1}, so  $(\cs{L}_2,\mu_2,nu_2)$ is indeed an $H\times H$-equivariant linear character sheaf on $G\times G$.
...
q^* : \CS(G_0) \to \CS(G)
\]
given on objects by $(\cs{L}_0,\mu_0) \mapsto (q^*\cs{L}_0, (q\times q)^* \mu_0)$; see [Dictionary, ].
%To see that $(q^*\cs{L}_0, (q\times q)^* \mu_0)$ is indeed a linear character sheaf on $G$, verify [Dictionary, CS.3], arguing as in [Dictionary].
...
then $q^*(\alpha_0) : (\cs{L},\mu) \to (\cs{L}',\mu')$ satisfies [Dictionary, CS.4], arguing as in [Dictionary], so $\alpha$ is a morphism in $\CS(G)$.
%\todo{I find myself having to make many references to [Dictionary] and promising that the results there adapt to the noncommutative group case. It makes me uneasy, but the alternative is to actually repeat the proofs here, with the weaker hypothesis on $G$, which would be ugly. I'm just agonizing out loud, I guess.} 
...
For use below, we write $L_2 : \Loc_{H\times H}(G\times G) \to \Loc(G_0\times G_0)$ for its adjoint and $(\epsilon_2, \eta_2)$ for the counit and unit of the adjunction.
...
To verify that $(L_1(\cs{L}_0), L_2(\mu_0))$ is an object of $\CS_H(G)$, one must confirm [Definition, CS1, CS2, CS3], \eqref{E1}, \eqref{E2} and \eqref{ECS1}. 
Conditions [Definition, CS1, CS2, CS3] follow from an adaptation of [Dictionary, ...] while conditions \eqref{E1}, \eqref{E2} are a consequence of the work above. 
Condition \eqref{ECS1} is confirmed by brute force.
On maps, $q_H^*: \CS(Q)\to \CS_H(G)$ is given by $q_H^*(\alpha_0) = L_1(\alpha_0)$.
To verify that $L_1(\alpha_0)$ is a map in $\CS_H(G)$ we must confirm conditions [Definition, CS4] and \eqref{E3}.
Condition [Definition, CS4] follows from an adaptation of [Dictionary, ...] while condition \eqref{E3} is a consequence of the work above.

With this description of $q_H^*: \CS(Q)\to \CS_H(G)$ is is easy to see that it is an equivalence.
Its adjoint is given on objects by $(\cs{L},\mu,\nu) \mapsto (L_1(\cs{L},\nu), L_2(\nu))$, again making use of Lemma~\ref{lem:HH}, and on objects by $\alpha \mapsto L_1(\alpha)$.
...
It is straightforward but quite unpleasant matter to show that those conditions are exactly equivalent to the condition that the isomorphism Weil local systems $\beta : \cs{L}\vert_{H} \to (\EE)_{H}$ is an isomorphism in the category of of linear character sheaves on $H$.
\end{proof}


\subsection{Proofs for results from Section \ref{sec:defs}}\label{ssec:proofs}
...
\]
%
Here we use the notation $\CS'(G)$ from Section~\ref{ssec:noncom} and the observation that it is precisely the category $\CS^{G\der}(G)$, taking the case $H=G\der$ in Section~\ref{ssec:closed}.
