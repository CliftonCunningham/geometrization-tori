...
These are exactly the non-trivial character sheaves in the kernel of the surjective group homomorphism $\TrFrob{G} : \CSiso{G} \to G(\Fq)^*$ given by the trace of Frobenius.
\labitem{(2)}{b2}
...
\labitem{(3)}{b3}
Like the category $\CS(G)$ of character sheaves on $G$, $\CCS(G)$ is a rigid monoidal category, described completely in \cite{Cunningham-Roe:QCS}.
Since $\CCS(G)$ is a full subcategory of $\CS(G)$, and since Items \ref{b1} and \ref{b2} above show $\CCSiso{G} \iso G(\Fq)^*$, canonically, the category $\CCS(G)$ is now completely described by the following consequence of \cite{Cunningham-Roe:QCS}*{Thm. 3.9}:
\[
\Aut(\cs{L}) \iso  \Hh^1(\pi_0(\bG), \EEx)^{\Weil{}}
\]
in $\CCS(G)$, for every commutative character sheaf $\cs{L}$ on $G$.
\end{enumerate}
