...
\begin{enumerate}
\item[(0)] a compact group $\,^\circ K$;
\item[(1)] compact open subgroups $J^i\subset \,^\circ K$, for $i=0, \ldots d$, such that \[\,^\circ K = J^0\cdots J^d;\]
\item[(2)] a continuous representation $\varpi_0$ of $J^0$, inflated from ...
\item[(3)] a quasicharacter $\varphi_i : J^0\cdots J^i \to \CC^\times$, for $i=0, \ldots d$.
\end{enumerate}
...
\begin{enumerate}
\item[(4)] a quotient $J^{i+1} \to V_{i+1}$ to a finite abelian group, and the structure of a $\Fq$-vector space on $V_{i+1}$, compatible with the abelian group structure above;
\item[(5)] a symplectic pairing $\langle\ ,\ \rangle_{i+1} : V_{i+1}\times V_{i+1} \to C_{i+1}$, where $C_{i+1}$ is a one-dimensional vector space over $\Fq$;
\item[(6)] an additive character $\psi_{i+1} : C_{i+1} \to \EEx$;
\item[(7)] a group homomorphism $\,^\circ f_i : J^0 \cdots J^i \to \Sp(V_{i+1})$;
\item[(8)] a quotient $J^{i+1} \to H^{i+1}$, where $H^{i+1}$ is a Heisenberg $p$-group;
\item[(9)] an isomorphism $j_{i+1} : H^{i+1} \to V_{i+1}^\sharp$, where $V_{i+1}^\sharp$ is the Heisenberg group determined by $(V_{i+1}, \langle\ ,\ \rangle_{i+1}, C_{i+1}, \psi_{i+1})$.
%\end{enumerate}
\item[(10)]The pair $(f_i, j_{i+1})$ is a symplectic action of $J^0\cdots J^i$ on $H^{i+1}$ in the sense of \cite{yu:construction}, so, together with the quotient $J^{i+1} \to H^{i+1}$, it defines a group homomorphism 
...
From this information, Yu constructs 
a representation $\,^\circ \rho_i$ of $J^0\cdots J^i$, for each $i=0, \ldots, d$, such that $\,^\circ \rho_d = \,^\circ \rho$,
constructed as follows.
%\end{enumerate}
%We will need some more detail about the construction of the representations $\,^\circ \rho^i$, so we review it below.
%
We will need some detail about this construction,   so we review it here.
...
Let $\omega_{i+1}$ be the Heisenberg representation of the Heisenberg group $V_{i+1}^\sharp$.
This determines the Weil representation $W_{i+1}$ on the same representation space as the Heisenberg representation of $V_{i+1}^\sharp$.
Let $W_{i+1} \ltimes \omega_{i+1}$ be the Heisenberg-Weil representation of the Jacobi group $\Sp(V_{i+1})\ltimes V_{i+1}^\sharp$.
\[
\begin{tikzcd}
\  & J^{i+1} \arrow{d} & \\
\Sp(V_{i+1}) \ltimes V_{i+1}^\sharp & \arrow{l}[swap]{h_{i+1}} (J^0 \cdots J^i) \ltimes J^{i+1} \arrow{r} \arrow{d} & J^0 \cdots J^i J^{i+1}\\
& J^0 \cdots J^i & 
\end{tikzcd}
\]
...
Now $h_1^*(W_1\ltimes \omega_1)$ is a representation of $J^0\ltimes J^1$, determined by the quasicharacter $\varphi_1$ of $J^1$. 
Write $\inf(\sigma_0)$ for the representation of $J^0 \ltimes J^1$ obtained by pulling back $\sigma_0$ along $J^0 \ltimes J^1 \to J^0$. 
By \cite{yu:01a}*{}, the representation 
$h_1^*(\omega_1) \otimes \varphi_1 \otimes \inf(\sigma_0)$ of $J^0 \ltimes J^1$ is trivial on $(J^0 \ltimes 1) \cap (1 \ltimes J^1)$, so $h_1^*(\omega_1) \otimes \varphi_2 \otimes\inf(\sigma_0\otimes)$ descends to a representation $\,^\circ \rho_1$ of $J^0 J^1$.  
%
...
\] 
%
As explained in \cite{yu:03a}*{\S 10.4}, the group scheme $\underline{G}$ comes with the following auxilliary data:
\begin{enumerate}
\item[(Ji)]
...
With these group schemes, the other groups and group homomorphisms appearing in Section~\ref{ssec:review} may be put into the framework needed for geometrization. 
...
\]
Finally, the group homomorphism $\,^\circ f_i : J^0\cdots J^i \to \Sp(V_{i+1})$ may be made geometric in much the same way. Writing $\underline{G}^{i}_\Fq$ for the special fibre $\underline{G}^{i}\times_S S_1$ of $\underline{G}^{i}$, and writing $\underline{G}^{i,\reductive}_\Fq$ for the reductive quotient of $\underline{G}^{i}_\Fq$, there is a quotient of algebraic groups $\underline{G}^{i,\reductive}_\Fq \to W^{i+1}_\Fq$ so that $\,^\circ f_i : J^0\cdots J^i \to \Sp(V_{i+1})$ is realized as the composition
\[
\underline{G}^{i}(\OK) \to \underline{G}^{i}(\Fq) = \underline{G}^{i}_\Fq(\Fq) \to  \underline{G}^{i,\reductive}_\Fq(\Fq) \to W^{i+1}_{\Fq}(\Fq) = \Sp(V_{i+1}).
\]

Let us return to the list of Section~\ref{ssec:review} and consider how we have recast each part.
\begin{enumerate}
\item[(0)] The compact group $\,^\circ K$ is replaced by the smooth group scheme $\underline{G}$ over $\OK$.
\item[(1)] The compact open subgroups $J^i \hookrightarrow \,^\circ K$ are replaced by the morphisms $\underline{J}^i \to \underline{G}$ of smooth group schemes over $\OK$.
\item[(2)] The representation $\varpi_0$ is obtained by pullback from a representation $\sigma_0$ of $\underline{J}^{0,\reductive}_\Fq$, which makes use of the following morphisms of group schemes.
\[
\underline{J}^{0} \longleftarrow \underline{J}^{0}_\Fq \to \underline{J}^{0,\reductive}_\Fq.
\]
\item[(3)] The quasicharacters $\varphi_i : \underline{J}^i(\OK) \to \CC^\times$ have not yet been geometrized.
\item[(4)] The quotient $J^{i+1}\to V_{i+1}$ has been replaced with the following morphisms of group schemes.
\[
\underline{J}^{i+1} \longleftarrow \underline{J}^{i+1}_\Fq \to \underline{J}^{i+1,\reductive}_\Fq \to V_{i+1,\Fq}.
\]
\item[(5)] The symplectic pairing $\langle\ ,\ \rangle_{i+1} : V_{i+1}\times V_{i+1} \to C_{i+1}$ is given by 
\item[(6)]
\item[(7)] The group homomorphism $\,^\circ f_i : J^0 \cdots J^i \to \Sp(V_{i+1})$ has been replaced by the following morphisms of group schemes.
\[
\underline{G}^{i} \longleftarrow \underline{G}^{i}_\Fq \to \underline{G}^{i,\reductive}_\Fq \to \Sp(V_{i,\Fq}),
\]
and $\Sp(V_{i,\Fq})$ is itself a reductive algebraic group over $\Fq$.
\item[(8)] The quotient $J^{i+1} \to H^{i+1}$ has been replaced by the following morphisms of group schemes.
\[
\underline{J}^{i+1} \longleftarrow \underline{J}^{i+1}_\Fq \to H^{i+1}_\Fq.
\] 
\item[(9)]
The Heisenberg group $V_{i+1}^\sharp$ is replaced by a unipotent algebraic group $V_{i+1,\Fq}^\sharp$ over $\Fq$, and the group isomorphism $H^{i+1} \iso V_{i+1}^\sharp$ is replaced by an isomorphism $H^{i+1}_\Fq \iso V_{i+1,\Fq}^\sharp$ of algebraic groups over $\Fq$.
\item[(10)]
...
\begin{proposition}
Let $(V, \langle\ , \rangle, C, \psi)$ be a finite-dimensional vector space over $\Fq$ and . .  .
Let $V^\sharp$ be the geometric Heisenberg group determined by this and let $\omega$ be the Heisenberg representation of $V^\sharp$.
Let $W$ be the Weil representation of $\Sp(V)$ determined by $(\omega,V^\sharp)$. 
Let $W\ltimes \omega$ be the resulting representation of the Jacobi group $\Sp(V)\ltimes V^\sharp$.
Then there is a virtual $W\ltimes \omega$-equivariant constructible sheaf $\mathcal{W}$ on $\Sp(V)\ltimes V^\sharp$ such that $\trFrob{\mathcal{W}} = \trace(W\ltimes \omega)$.
\end{proposition}

%\begin{proof}
%We begin by geometrizing the Heisenberg representation $(\omega, V^\sharp)$ to $\mathcal{H}$.
%\end{proof}
