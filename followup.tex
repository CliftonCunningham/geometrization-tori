...
In order to further justify referring to objects in $\CCS(G)$ as commutative character sheaves,
suppose for the moment that $G$ is a connected, reductive algebraic group over $\Fq$.
In this case, every object in $\CCS(G)$ is a Frobenius-stable character sheaf on $G$ in the sense of \cite{lusztig:85a}*{Def.~2.10}.
Moreover, if $A$ is a character sheaf on $G$ in this sense, and if the characteristic function of $A$ is a character of $G(k)$, then the dictionary determines an object in $\CCS(G)$ from $A$.
These two rules determine an injection from $\CCSiso{G}$ to isomorphism classes of Frobenius-stable character sheaves on $G$.
Accordingly, we may refer to character sheaves on $G$ that arise in this manner as commutative character sheaves.
Of course, such sheaves represent a small part of Lusztig's geometrization of characters of representations of connected, reductive groups. 
...
Then $a$ corresponds to the $\alpha \in C^0(\Weil{}, C^2(\bG, \EEx))$, above. \todo{Check this.}
