...
\end{tikzcd}
\end{equation}
and $s^*(\nu) = \id_{\cs{L}}$, where $s : G\to H\times G$ is given by $s(g) = (1,g)$.
...
\begin{equation}\label{eq:alphanu}
\begin{tikzcd}
...
\begin{equation}\label{eq:alphamu}
\begin{tikzcd}
...
This isomorphism satisfies \eqref{eq:nu} and \eqref{eq:munu}.
...
\end{tikzcd}
\]
Since this diagram commutes, and since the appropriate version of \eqref{eq:alphamu} also commutes, it follows that $\mu : m^*\cs{L} \to \cs{L}\boxtimes\cs{L}$ is a morphism of equivariant linear character sheaves.
The argument that $\vartheta : \cs{L}\boxtimes\cs{L} \to \theta^*(\cs{L}\boxtimes\cs{L})$ is also straightforward.
\end{proof}
...
q^* : \CS(G_0) \to \CS(G)
\]
given on objects by $(\cs{L}_0,\mu_0) \mapsto (q^*\cs{L}_0, (q\times q)^* \mu_0)$.
...
\end{tikzcd}
\]
Since this isomorphism satisfies \eqref{eq:munu}, it follows that $(\cs{L}, \mu,\nu)$ is an $H$-equivariant linear character sheaf on $G$.
If $\alpha_0 : (\cs{L}_0,\mu_0) \to (\cs{L}_0',\mu_0')$ is a morphism in $\CS(G_0)$, 
then $q^*(\alpha_0) : (\cs{L},\mu) \to (\cs{L}',\mu')$ satisfies \eqref{eq:alphamu} [Dictionary, CS.4], arguing as in [Dictionary], so $\alpha$ is a morphism in $\CS(G)$.
\todo{
I find myself having to make many references to [Dictionary] and promising that the results there adapt to the noncommutative group case. 
It makes me uneasy, but the alternative is to actually repeat the proofs here, with the weaker hypothesis on $G$, which would be ugly. 
I'm just agonizing out loud, I guess.
} 
...
Then, observe that the comparison functor for Weil local systems is an equivalence for flat surjective morphisms $Y \to X$ of affine schemes over $\Fq$.
It follows that $\Loc$ is a stack over schemes over $\Fq$ in the fppf topology [Vistoli]
Since $q : G\to Q$ is an $H$-torsor in the fppf topology, is now follows from descent theory that the comparison functor $\Loc(Q) \to \Loc_H(G)$ is an equivalence [Vistoli]
...
Since a linear character sheaf on $Q$ is a triple $(\cs{L}_0,\mu_0)$, where $\cs{L}_0$ is an object in $\Loc(Q)$ and $\mu_0$ is a map in $\Loc(Q\times Q)$, satisfying the condition in [CS.2 from Dictionary].
