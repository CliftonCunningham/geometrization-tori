...
Let $\varphi : \underline{G}(\OK) \to \CC^\times$ be a quasicharacter/continuous linear character. 
The there is a Weil sheaf $\cs{F}$ on $\Gr(\underline{G})$ such that $\trFrob{\cs{F}} = \trace \varphi$.
\end{proposition}
...
Let $\sigma$ be that representation of $\underline{G}^0(\Fq) = \underline{G}^0_\Fq(\Fq)$.
In fact, $\sigma$ is itself obtained by pulling back a representation $\sigma^0$ along the $\Fq$-points of the quotient $\underline{G}^0_\Fq \to (\underline{G}^0)_\Fq^\reductive$.
...
\subsection{Geometrization of characters of the ingredient representations}

\todo{Everything below here is under construction}
Recall that $\sigma^0$ is a representation of $(\underline{G}^0)_\Fq^\reductive$.
Suppose $\pi_0((\underline{G}^0)_\Fq^\reductive)$ is cyclic.
Let $A^0$ be a Weil sheaf on $(\underline{G}^0)_\Fq^\reductive$ such that, as a sheaf on $(\underline{G}^0)_\Fq^\reductive \otimes \bFq$, it is a character sheaf.
The existence of $A^0$ follows from \cite{Lusztig:CSDG}*{}.
Let $A$ be the Weil sheaf on $\underline{G}^0_\Fq$ obtained by pullback along $\underline{G}^0_\Fq \to (\underline{G}^0)_\Fq^\reductive$.

Let $n$ be an integer greater than the supremum of the depths of $\varphi_i : \underline{G}^i(\OK)\to \CC^\times$, for $i=0,\ldots, d$.
Then, for $i=0,\ldots, d$ we have the commuting diagram 
\[
\begin{tikzcd}
\end{tikzcd}
\]
