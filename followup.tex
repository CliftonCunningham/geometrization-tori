...
we succeed in geometrizing characters of $G(k)$ on the nose.  This condition will be satisfied
when $G\der(\bFq) = \underline{G}(K)$ for a simply connected group $\underline{G}$ over a local field $K$.
\end{remark}
...
\todo{Maybe we should give a reference for this definition?}
...
The notation of an $H$-equivariant Weil sheaf on $G$, as described in Section~\ref{ssec:equivariant1}, does not use that fact that the action $a: H\times G \to G$ is compatible with the group structure on $G$.
Here we a introduce a notion which makes use of that condition.
