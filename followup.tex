...
\subsection{Descent along a torsor}\label{ssec:descent}
...
Combining Theorems \ref{thm:trfrobiso} and \ref{thm:Gab} gives an isomorphism $\CCSiso{G} \to G\ab(k)^*$. \todo{Could we turn this into a declared corollary/theorem? It's a nice result.}
...
\begin{proposition}
Let $(\underline{G}(\OK),\rho)$ be a type such that  . . . 
Set $G = \Gr(\underline{G})$.
There is a virtual admissible $\ell$-adic sheaf $\mathcal{F}$ on $G$ such that $\trFrob{\mathcal{F}} = \trace \rho$ under the canonical isomorphism $G(\Fq) \iso \underline{G}(\OK)$.
\end{proposition}
