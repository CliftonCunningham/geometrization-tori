...
\todo{Is this what you had in mind?}
...
\begin{quotation}
Therefore, up to some linear characters, all the ingredient representations are on groups of
 the form $H(\mathcal{O})$, where $H$ is a smooth group scheme over $\mathcal{O}$, and the representations are inflated $H(\kappa)$. These results suggest that algebraic geometry and group schemes should play an
  from $H$
important role in the representation theory of p-adic groups.
\end{quotation}

\subsection{Smooth group schemes for types}

Let $\underline{G}$ be an affine smooth group scheme of finite type over $\OK$ arising in the work of Yu on types for supercuspidal groups.
We suppose the component group $\pi_0(\underline{G}_s\red)$ of the reductive quotient of the special fibre $\underline{G}_s$ of $\underline{G}$ is cyclic.

The group scheme $\underline{G}$ comes equipped with affine smooth group schemes of finite type over $\OK$
\[
\underline{J}^i \to \underline{G},
\]
for $j=0, 1, \ldots , d-1,d$, such that
\[
\underline{G}(\OK) = \prod_{i=0}^{d} \underline{J}^i(\OK).
\]
Let $\underline{G}^{j-1}$ be the canonical smooth model . . 
We suppose the component group $\pi_0((\underline{G}^{j-1}_s)\red)$ of the reductive quotient of the special fibre $\underline{G}^{j-1}_s$ of $\underline{G}^{j-1}$ is cyclic, for every $j=1, \ldots, d-1$. 

\subsection{Geometrization of types}

\begin{proposition}
Let $(\underline{G}(\OK),\tau)$ be a type such that  . . . 
Set $G = \Gr(\underline{G})$.
There is a virtual admissible $\ell$-adic sheaf $\mathcal{F}$ on $G$ such that $\trFrob{\mathcal{F}} = \trace \tau$ under the canonical isomorphism $G(\Fq) \iso \underline{G}(\OK)$.
\end{proposition}
