...
%\end{quotation}
%\noindent
In this paper we present a dictionary between quasicharacters of algebraic tori over local fields and quasicharacter sheaves.
...
We close the paper with Section~\ref{sec:reciprocity}, in which we suggest that quasicharacter sheaves may play a key role in a geometric and categorical form of the local Langlands correspondence for algebraic tori over local fields.
...
Let us say that $f : H\to G$ is a \emph{discrete isogeny} if it is a finite, surjective
...
The surjective ring homomorphism $\OK/\pK^{n+1} \to \OK/\pK^n$ determines a natural transformation $P_{n} : \Gr_{n+1} \to \Gr_n$ between additive functors.
...
\begin{remark}
If $K = \mathbb{Q}_q$ and 
$T = \Gm{\mathbb{Q}_q}$ then $\GN{T}^0 = \mathbb{W}^*_{\FF_q}$, 
...
We refer to objects in the rigid symmetric monoidal category 
...
\end{tikzcd}
\]
defined by $F_n(\qcs{L}) = [\qcs{L}_n]$ and $F_n(\alpha_n) = [\alpha_n]$, 
...
Note also that 
\Clifton{I haven't checked this! Have you?}
...
induces a map of group pro-schemes $\GN{T} \to \Res_{k'/k} \GN{T}'$ which, through the pull-back functor and the trace of Frobenius, induces the homomorphism
\[
\begin{tikzcd}[column sep=60]
...
By the Néron mapping property, the canonical closed immersion $T\hookrightarrow \Res_{K'/K}(T')$ extends, uniquely, to a morphism $\underline{T}\to \Res_{\mathcal{O}_{K'}/\OK}(\underline{T'})$ of smooth $\OK$-group schemes.
...
\]
Since $\left(\Res_{k'/k} \GN{T}'_{en} \right)(\Fq) = \left(\GN{T}'_{en}\right)(k')$, the pull-back functor $\QC(\Res_{k'/k} \GN{T}'_{en})\to \QC(\GN{T}_n)$ obtained by applying Proposition~\ref{prop:pullback} to \eqref{men} gives
\[
