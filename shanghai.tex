...
$\mathswab{T}_d$ and the group of characters $T(K) \to \EEx$ of
...
In order to define $\CS_b(G)$ as must recall a fundamental fact about finite etale covers of schemes. Let $X$ be a scheme, locally of finite type over $k$, and let $\pi : Y\to X$ and $\pi_1: Y_1\to X$ be finite etale covers and let $f_1 : Y_1\to Y$ be a morphism such that $\pi_1 = \pi \circ f_1$. Then $f_1$ determines a group homomorphism $\Aut(\pi_1)\to \Aut(\pi)$, as follows  . . .

Now we may define $\CS_b(G)$, where $G$ is a smooth group scheme locally of finite type over $\Fq$.
\begin{enumerate}
\labitem{(CSb.0)}{CSb.0}
An object in $\CS_b(G)$ is a pair $(\pi,\psi)$ where $\pi : H\to G$ is a finite etale cover in the category of group schemes over $\Fq$ such that $\Gal(\bFq/\Fq)$ acts trivially on $\ker {\bar \pi}$ (so $\ker {\bar \pi} = \ker \pi$) and $\psi : \ker \pi \to \Aut(V)$ is an irreducible representation on a $\EE$-vector space $V$.
\[
\begin{tikzcd}
V & \Aut(V) & 1 \rar & \arrow[bend right,swap]{ll}{\psi} \ker \pi \rar & H \rar{\pi} & G \rar &1
\end{tikzcd}
\]
\labitem{(CSb.1)}{CSb.1}
A morphism $T : (\pi,\psi) \to (\pi',\psi')$ is a linear transformation $T : V \to V'$ which, if non-trivial, has the property that there is a finite etale cover $\pi_1 : H_1 \to G$ in the category of group schemes over $\Fq$ and morphisms $H_1\to H$, $H_1\to H'$ inducing homomorphisms $\ker \pi_1 \to \ker\pi$ and $\ker\pi_1\to \ker\pi$ such that the following diagram commutes.
\[
\begin{tikzcd}
V \arrow{dd} & \Aut(V) \ar[equal]{dd} & 1 \rar & \arrow[swap,bend left]{ll}{\psi} \ker \pi \rar & H \rar{\pi} & G \rar &1\\
 &  & 1 \rar & \ker \pi_1 \rar \uar \dar & H_1 \uar \dar \rar{\pi_1} & \ar[equal]{u} \arrow[equal]{d} G \rar &1\\
V' & \Aut(V') & 1 \rar & \arrow[bend right]{ll}{\psi'} \ker \pi' \rar & H' \rar{\pi'} & G \rar &1
\end{tikzcd}
\]
\end{enumerate}
We refer to $\CS_b(G)$ as the category of \cdef{bounded character sheaves on $G$}.
\Clifton{I'd love to find a way to define $\CS_b(G)$ without insisting that $\psi$ is an \emph{irreducible} representation, and then prove that the full subcategory of simple objects in $\CS_b(G)$ is a full subcategory of $\CS(G)$. I'm studying my notes from BIRS, letters from Masoud, and SGA7, to see how to do this.}
\begin{proposition}
$\CS_b(G)$ is a full subcategory of $\CS(G)$.
\end{proposition}
