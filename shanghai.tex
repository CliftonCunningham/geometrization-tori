...
\gcs{L} \to \LxL$, where $m : G\times G \to G$ is
...
that $\Aut_{\EE}(\gcs{L}_{\bg}) \iso \EE^\times$, canonically (this
...
\begin{proposition}\label{prop:etale}
  ...
  isomorphism
  \[
  \CSiso{G} \longrightarrow \Hh^1(\Weil(\bFq/\Fq), \Hom_\text{grp}(G(\bFq),\EE^\times)),
...
\begin{proof}
  ...
  $\psi : \ker \bf \to \EE^\times$, defines a character sheaf on
  ...
  \[
  \xymatrix{
    \EE^\times && \ar[ll]_{\hskip-20pt \psi_x\otimes \psi_y} \Aut(\bf^x)\times \Aut(\bf^y)
  ...
  \xymatrix{
    ...
    \EE^\times & \ar@{.>}[l] \Aut(\bf^{x+y})  \ar@{=}[u] &  \ar@{.>}@(ul,dl)
  ...
  \xymatrix{
    ...
    \EE^\times & & \ar@{.>}[ll] \Aut(\bf^{\Frob(x)})  \ar@{=}[u] & \ar@{.>}@(ul,dl)
