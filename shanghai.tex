...
\section{The category of geometrized characters}\label{section:category}
...
We say that $(\alpha,T)$ in $C(G)$ is a \cdef{weak isomorphism} if $T$ is an isomorphism of vector spaces. 
Let \cdef{$C(G)[W^{-1}]$} be the localization of $C(G)$ at weak isomorphisms.
Now, let \cdef{$C_1(G)$} be the subcategory of $C(G)$ such that $\psi$ is a one-dimensional representation
and let \cdef{$\GCf(G)$} be the localisation of $C_1(G)$ at weak isomorphisms. 
This is the rigid symmetric monoidal category of \cdef{finite geometrized characters} of $G$.
...
% this morphism is commonly denoted by $(\alpha_1,T_1)^{-1}\circ (\alpha'_1,T'_1)$. 
 ...
 Since $f' = f \circ \alpha$ and $f$ and $f'$ are etale covers, 
 it follows that $\alpha$ is also an etale cover. 
 \Clifton{Is that right? If $f' = f \circ \alpha$ and $f$ and $f'$ are etale covers, 
 does it follow that $\alpha$ is also an etale cover? (Etale: Yes. Finite? Surjective?) If not, then I must modify the definition of weak isomorphism accordingly.} 
 ...
 Since $W$ is saturated, the functor $L_{W} : C(G)[W^{-1}] \to \Loc(G)$ is full. 
  ...
  Every morphism of group schemes $G'\to G$ 
  induces a monoidal functor $C(G) \to C(G')$ by base-change along $G'\to G$ 
  ({\it cf.}, Proposition~\ref{prop:pullback}) as follows. 
  Let $m : G'\to G$ be a morphism of group schemes. 
  Let $(f,\psi)$ be an object in $C(G)$. 
...
V\otimes V & \Aut(V\otimes V) & \arrow[swap]{l}{\psi\otimes \psi} \ker f \times \ker f \arrow{r}  & H\times H \arrow{r}{f\times f}  & G\times G \arrow[equal]{u} \\
 ...
 It follows immediately that the functor $L_G  \ceq  L_{W}\vert_{\GCf(G)} : \GCf(G) \to \GCb(G)$, 
...
A morphism $\varphi : \cs{L} \to \cs{L'}$ is given by 
\begin{enumerate}
 \labitem{(cs.3)}{cs.3} an indexed set $\varphi_x : \gcs{L}_x \to \gcs{L'}_x$
...
is easily seen to be an equivalence.  We may now define a \cdef{global section}
...
\begin{proof}
  ...
  on $G$ to $G^0$ is precisely the geometrized character of $G^0$ defined by
...
\end{proof}
\Clifton{I can flesh out this remark once I've promoted the definition of $\GCb(G)$ (category of bounded geometrized characters of $G$).}
\begin{remark}
  ...
  geometrized character of $G$. However, it is not true that every character
