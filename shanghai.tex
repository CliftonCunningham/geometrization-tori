...
\Clifton{There is another section at the end of the paper, now.}
...
\Clifton{I think we should seriously consider replacing $d$ with $n$ in Part~2. Actually, I've just done that.}
...
The surjective ring homomorphism $\OK/\pK^{n+1} \to \OK/\pK^n$ determines a natural transformation $P_{n} : \Gr_{d+1} \to \Gr_d$ 
...
Some fundamental properties of $f_n : \GN{T}_{n+1} \to \GN{T}_n$ are:
 \begin{enumerate}
  ...
  The morphism $f_n : \GN{T}_{n+1} \to \GN{T}_n$ is a surjective morphism of group schemes.
  \end{enumerate}
%
...
For any $n\leq m$ (positive integers) define $f_{n\leq m} : \GN{T}_{m} \to \GN{T}_{n}$ by $f_{n\leq m}\ceq f_n \circ f_{n+1} \circ \cdots \circ f_{m-1}$.
Let $f_{n\leq m}^* : \QC(\GN{T}_n) \to \QC(\GN{T}_{m})$ be the functor promised by Proposition~\ref{prop:pullback}.

\begin{proposition}
The functor $f_{n\leq m}^* : \QC(\GN{T}_n) \to \QC(\GN{T}_{m})$ is full and faithful.
\end{proposition}

\begin{proof}
It suffices to find a morphism of schemes $i_n : \GN{T}_{n}\hookrightarrow \GN{T}_{n+1}$ such that $f_n\circ i_n = \id_{\GN{T}_n}$. Base change along $\OK/\pK^{n+1} \to \OK/\pK^n$ determines a closed immersion of schemes $\TT_{n+1} \hookrightarrow \TT_n$ (not a morphism of group schemes). Applying the Greenberg functor produces a closed immersion of schemes $i_n : \GN{T}_{n} \hookrightarrow \GN{T}_{n+1}$. 
...
We now define a category, denoted by $\QC(\GN{T})$, which allows us to extend the notion of quasicharacter sheaf from $\GN{T}_n$ to $\GN{T}$. An object in $\QC(\GN{T})$ is an equivalence class in $\cup_{n} \obj \QC(\GN{T}_n)$ under the relation: $\qcs{L}_n\in \QC(\GN{T}_n)$ is equivalent to $\qcs{L}_{m}\in \QC(\GN{T}_m)$ if $f_{n\leq k}^* \qcs{L}_{n} = f_{m\leq k}^* \qcs{L}_m$ for some $n,m\leq k$. Objects in $\QC(\GN{T})$ all take the form
\[
...
\qquad k = \max\{ n,m\}.
\]
Duals in $\QC(\GN{T})$ are defined by
\[
[\qcs{L}_n]^\vee \ceq [\qcs{L}_n^\vee].
\]
Category $\QC(\GN{T})$ is rigid symmetric monoidal and comes equipped with monoidal subcategories
\[
\begin{tikzcd}
\ & & & \QC(\GN{T})\\
...
We refer to objects of $\QC(\GN{T})$ as \emph{quasicharacter sheaves on $\GN{T}$} or \emph{quasicharacter sheaves for $T$}.
