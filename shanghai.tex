...
We will work with constructible $\ell$-adic sheaves \cite{deligne:80a}*{\S 1.1}, \cite{SGA5}*{Expos\'es V, VI} 
...
\item and if the cokernel \David{Is $C(G)$ abelian?} \Clifton{Yes. In fact, that's stated, implicitly, in the paragraph above: a tensor category is an abelian category.} of a morphism $\phi \in \mor C(G)$ is in
...
facts on \'etale covers and pullbacks. \David{Is this sufficient?}\Clifton{Yes, I think so.}
\end{proof}
Let $W$ be the family of weak isomorphisms and \cdef{$C(G)[W^{-1}]$} be the localization of $C(G)$ at $W$.
...
To prove the next proposition, we will need a certain perspective on local systems.
 If $\qcs{E}$ is any local system on a connected scheme $G$ and $\bg$ is a geometric point on $G$,
 we may associate to $\qcs{E}$ a character of the \'etale fundamental group $\pi_1(G, \bg)$ \David{add citation}.\Clifton{I'll find one.}
 Since $\pi_1(G, \bg)$ is defined as a projective limit of automorphism groups of finite \'etale covers, a general
 local system will correspond to a projective limit of characters of such groups.  But some local systems can be
 described in terms of a single cover $f: H \to G$ and a representation $\psi: \ker f \to \Aut(V)$;
 ...
 not generalize well to non-connected $G$, the description in terms of $C(G)$ does.  \David{Look okay?}
...
and the fact that  $\Hom_{\text{qc}}(T(K),\EEx) = \varinjlim_{n\in \ZZ_{>0}} \GN{T}_n(k)^*$, where this last direct limit it taken
