...
\Clifton{I took the reference to BLR for $P_n$ out, because it didn't actually give any information. This must be in DG, but I don't have a copy.}
...
adapting the argument in \cite{bosch-lutkebohmert-reynaud:NeronModels}*{\S 7.6, Prop 6} 
gives a canonical isomorphism 
\Clifton{I need to check this.}
\begin{equation}
...
\Clifton{I'm not so sure about this . . . In fact, I don't believe it!}
\begin{equation}
...
And the Greenberg transform respects products:
\[
\Gr_n(\underline{T'}) = \Gr_n(\underline{T}\times_{k} {k'}) \iso \Gr_n(\underline{T})\times_{k} {k'} = \Gr_n(\underline{T})_{k'}.
\]
Thus, the 
