...
Let $\Frob{G} := \id_{G} \times \Frob{\Fq}$ be the Frobenius automorphism of $\bG$. 
...
\end{tikzcd}
\end{equation}
It is easy to see that $\QCiso{0}$ is trivial. In this paper we will find $\QCiso{G^0}$
...
$\iota : G \hookrightarrow \Res_{k'/k}(G_{k'})$ \cite{bosch-lutkebohmert-reynaud:NeronModels}*{\S 7.6}. 
...
\begin{proof}
 ...
 it follows that $\tau_\qcs{L}$ and $\tau_{\qcs{L'}}$ are cohomologous, 
  ...
  If $s$ is a global section for $\qcs{L}$ and $s'$ for $\qcs{L}'$ then $s \otimes_{\EE}s'$
  is a global section for $\qcs{L} \otimes_{\EE}\qcs{L}'$.  The three versions of \eqref{t}
  for $\qcs{L}, \qcs{L'}$ and $\qcs{L} \otimes_{\EE}\qcs{L}'$ imply that
  $\tau_{\qcs{L} \otimes_{\EE}\qcs{L}'}(\Frob{k}) = \tau_\qcs{L}(\Frob{k}) \tau_{\qcs{L}'}(\Frob{k})$,
...
\begin{proof}
  ...
  sum of cyclic $\ZZ[\Gamma_m]$-modules \cite{brandal:CommRingsFGModDecompose},
...
For each positive integer $d$, set $\TT_{d} \ceq \TT \times_\Spec{\OK} \Spec{\OK/\pK^d}$.
...
See \cite{greenberg:63a} and \cite{bosch-lutkebohmert-reynaud:NeronModels}*{Ch. 9, \S 6}
and the first few sections of \cite{stasinski:12a} for background on the Greenberg functor. 

For each positive integer $d$, consider the canonical surjective ring homomorphisms
$\OK/\pK^{d+1} \to \OK/\pK^d$ and the associated cartesian diagram of schemes 
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
The map $\GN{T}_{d} \hookrightarrow \GN{T}_{d+1}$ is not a morphism of group schemes.
There is, however, an epimorphism of group schemes $\GN{T}_{d+1} \to \GN{T}_{d}$ for
which $\GN{T}_{d} \hookrightarrow \GN{T}_{d+1}$ is a section.
...
The fundamental properties of $\GN{T}_d$ that we need are:
 \begin{enumerate}
  ...
  and the latter is finitely generated by \cite{xarles:93a}.)
  \labitem{(GN.2)}{d+1} 
  \end{enumerate}
...
can then be be viewed as pairing characters and parameters coming
 ...
 especially as revisited in \cite{suzuki-yoshida:12a}. 
