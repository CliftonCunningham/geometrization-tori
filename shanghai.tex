...
\begin{proof}
  Suppose $\cs{L}$ is a character sheaf on $G$.  Using Lemma~\ref{lemma:section}, we may choose a global section $s_{\cs{L}}$
  of $\cs{L}$.  We will construct a group isomorphism between $\CSiso{G(\bFq)}$ and
  ...
  of one-dimensional $\EE$-vector spaces; set
  ...
  isomorphism.  Suppose that $\cs{L}$ and $\cs{L}'$ are two character sheaves.  
