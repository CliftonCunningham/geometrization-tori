...
For any positive integer $d$ set $\GN{T}_d = \Gr_d(\TT)$ and $f_d = P_d(\TT)$.
...
\begin{proof}
\Clifton{Under construction}
Base change along $\OK/\pK^{d+1} \to \OK/\pK^d$ determines a closed immersion $\TT_{d+1} \hookrightarrow \TT_d$, though not a morphism of group schemes. Applying the Greenberg functor produces a closed immersion $i_d : \GN{T}_{d} \hookrightarrow \GN{T}_{d+1}$. Although $i_d$ is not a morphism of group schemes, it is a section of $f_d : \GN{T}_{d+1}\to \GN{T}_d$.
\end{proof}


Let $\QC(\GN{T})$ be the category of ind-objects from the full subcategories $\QC(\GN{T}_d)$. 
Objects in $\QC(\GN{T})$ are equivalence classes of objects from $\QC(\GN{T}_d)$ under the equivalence relation that identifies 
$\qcs{L}_1\in \QC(\GN{T}_{d_1})$ with $\qcs{L}_2\in \QC(\GN{T}_{d_2})$ 
if $f_1^*\qcs{L}_1 = f_2^*\qcs{L}_2$  in $\QC(\GN{T}_{\max\{d_1,d_2\}})$,
where $f_1 : \GN{T}_{\max\{d_1,d_2\}} \to \GN{T}_{d_1}$ 
(resp. $f_2 : \GN{T}_{\max\{d_1,d_2\}} \to \GN{T}_{d_2}$) 
is obtained by composing the maps $f_{d_1}, \cdots , f_{\max\{d_1,d_2\}}$ 
(resp. $f_{d_2}, \cdots , f_{\max\{d_1,d_2\}}$).
A map $\alpha : \qcs{F} \to \qcs{F}'$
