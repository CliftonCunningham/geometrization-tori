...
and $\Hom_{<\infty}(T(K),\EEx)$ for the group of characters of $T(K)$ of arbitrary finite depth; one might call these quasicharacters of $T(K)$, expanding on \cite{Tate:thesis}*{2.3}.
...
Then it follows from \cite{CY}*{Thm. 8.5} that if $T$ and $T'$ are $N$-congruent and $h=h'$ and $N > 3h$, 
...
can then be be viewed as pairing characters and parameters coming
 ...
 by the class field theory of Serre-Hasewinkel, \Clifton{Need references for S-H CFT.} especially as revisited in \cite{Suzuki&Yoshida:refinement}. 
