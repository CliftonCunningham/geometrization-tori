...
\GCiso{G} \rar{\text{ext'n of scalars}} \dar{\trFrob{G}} & \GCiso{G_{k'}} \dar{\trFrob{G_{k'}}} \\
G(k)^*  \rar{\Nm} & G(k')^*
...
$f : H\to G$ an isogeny (a finite, surjective etale morphism and a morphism of commutative group $\Fq$-schemes)
...
\begin{proposition}\label{prop:GCf-pull-back}
If $g : G'\to G$ is a morphism of group schemes then $(f,\psi) \mapsto (f_g,\psi_g)$ defines a monomial functor $g^* : \GCf(G) \to \GCf(G')$ where $f_g : H\times_G G'\to G'$ and $\psi_g : \ker f_g \to \Aut(V)$ are defined by the diagram below.
\[
\begin{tikzcd}
...
\begin{proof}
Before compare $m^*(f,\psi)$ and 
\end{proof}




Every morphism of group schemes $g: G'\to G$ 
defines a function $g^*: \obj C(G) \to \obj C(G')$ 
by $g^*(f,\psi) = (f_g,\psi_g)$ where
$f_g : H\times_G G'\to G'$ and $\psi_g : \ker f_g \to \Aut(V)$ are defined by the diagram below.
\[
\begin{tikzcd}
V & \Aut(V) & \arrow[swap]{l}{\psi} \ker f \arrow[hook]{r} & H \arrow{r}{f} &  G\\
  &  & \arrow[dashed]{ul}{\psi_g} \arrow[swap]{u}{(g_f)\vert_{\ker g_f}} \ker f_g \arrow[hook]{r} & \arrow[swap]{u}{g_f} H\times_G G' \arrow{r}{f_g} & \arrow{u}{g} G'.
...
\begin{proof}
 ...
 \Clifton{Reveal the commented-out details . . . I think I need them!}
% this morphism is commonly denoted by $(\alpha_1,T_1)^{-1}\circ (\alpha'_1,T'_1)$. 
 ...
 $m^*L(f,\psi) \to L(f,\psi) \boxtimes L(f,\psi)$ by a weak isomorphism in $C_1(G\times G)$.
 This morphism in $C_1(G\times G)$ is not unique, but its image in $\GCf(G\times G)$ is . . .!
