...
$\bG$ will be denoted by $\bm$. 
Let $\Frob{G}$ be the Frobenius automorphism of $\bG$.
\Clifton{I was a bit glib about Frobenius, previously.
Now I've made things more precise here; I'm in the process of tracking this improvement through the paper.}
Recall that $\Frob{G} = \id_{G}\times \Frob{\Spec{\bFq}}$ where
$\Frob{\Spec{\bFq}}$ is the automorphism of $\Spec{\bFq}$ 
opposite to the geometric Frobenius element $\Frob{\Fq}$ in $\Weil{\Fq} \subset \Gal(\bFq/\Fq)$.
...
%
\labitem{(CS.2)}{CS.2} $F : \Frob{G}^* \gcs{L} \to \gcs{L}$ is an
...
%
  ...
  \Frob{\GxxG}^* \bm^* \gcs{L} \arrow{rr}{\Frob{\GxxG}^*\mu}
    && \Frob{\GxxG}^*(\LxL)\\
    \arrow[equal]{u} \bm^*  \Frob{G}^* \gcs{L} \arrow[swap]{d}{\bm^* F}
    && \Frob{G}^*\gcs{L}\boxtimes \Frob{G}^*\gcs{L} \dar{F\boxtimes F} \arrow[equal]{u} \\
...
guarantees that the function $t^\Frob{\Fq}\cs{L} : G(\Fq)\to \EEx$,
defined by $g \mapsto F_{\bg}$, is a group homomorphism;
we refer to $t^\Frob{\Fq}\cs{L}$ as the \cdef{trace of Frobenius of $\cs{L}$}, 
even though no trace is involved in the definition, because the notion extends 
to any constructible $\ell$-adic Weil sheaf $(\gcs{L},F)$ on $G$, in which context,
$t^\Frob{(\gcs{L},F)} : G(\Fq)\to \EE$ is defined by $t^\Frob{(\gcs{L},F)}(g) \ceq \trace F_{\bg}$.

It is clear that $t^\Frob{\cs{L}\otimes \cs{L'}} = t^\Frob{\Fq}\cs{L} \cdot t^\Frob{\Fq}\cs{L'}$
(pointwise multiplication of functions) and that $t^\Frob{\Fq}\cs{L} = t^\Frob{\Fq}\cs{L'}$
...
\qquad \text{defined by}\qquad \cs{L} \mapsto t^\Frob{\Fq}\cs{L},
\]
...
\[
F_{k'} = F \circ \Frob{G}^*(F) \circ \cdots \circ (\Frob{G}^{n-1})^*(F).
\]
The commutativity of the diagram (CS.2) for $F_{k'}$ follows from the fact that $\Frob{G_{k'}} = \Frob{G}^n$.
...
\begin{proof}
 ...
 $F : \Frob{G}^* b_G^*\cs{L}_0 \to b_G^* \cs{L}_0$; 
 ...
 It follows that functor $(\cs{L}_0,\mu_0) \mapsto (b_G^*\cs{L}_0,b_{G\times G}^* \mu_0, F)$, denoted by  $B_G^*: \CSb(G) \hookrightarrow \CS(G)$ below, is full and faithful.
\end{proof}
...
\begin{proof}
 ...
 such that $L(\varphi,T)$ is an isomorphism precisely when $(\varphi,T)$ is a weak isomorphism. 
...
\begin{proof}
 %Let $G$ be a geometrically connected algebraic group over $\Fq$.
  ...
  \[
  \rho_g(\Frob) =  t^\Frob{\Fq}\cs{L}(g).
  \]
  
  On the other hand, we have already seen that $t^\Frob{\Fq}\cs{L} : G(\Fq) \to \EEx$
  ...
  it follows that $t^\Frob{\Fq}\cs{L}(g) = \rho_g(\Frob)$ is a root of unity,
  ...
  $\psi(g)(v) = t_\cs{L}^\Frob{\Fq}(g) v$ for $v\in V$. 
  ...
  By  \cite{Laumon}*{1.1.3.3}, $t_{L(f,\psi)}^\Frob{k'} = t_{\cs{L}}^\Frob{k}\circ N_{k'/k}$, for each finite extension $k'/k$.
  On the other hand, $t_{\cs{L}}^\Frob{k} \circ N_{k'/k} = t_{\cs{L}}^\Frob{k'}$, for each for each finite extension $k'/k$, 
...
\begin{proposition}\label{prop:etale}
  ...
  \[
  \forall g\in G(\Fq), \qquad t_\cs{L}(\Frob)(g) = t^\Frob{\Fq}\cs{L}(g).
  \]
\end{proposition}
...
\begin{proof}
  ...
  \labitem{(cs.2)}{cs.2} an indexed set of isomorphisms $F_{x} \ceq x^*F: \gcs{L}_{\Frob{\Fq}(x)} \to \gcs{L}_x$
    \Clifton{\textdbend There's hidden subtlety here too: maybe I
      should explain which Frobenius this is and how it acts. }
    such that
    \[
    \begin{tikzcd}[row sep=40]
      \gcs{L}_{\Frob{\Fq}(x)+\Frob(y)} \arrow[swap]{d}{F_{x+y}} \arrow{rr}{\mu_{\Frob{\Fq}(x),\Frob(y)}}
      && \gcs{L}_{\Frob{\Fq}(x)}\otimes \gcs{L}_{\Frob(y)} \dar{F_x \tight{0}{\otimes}{0} F_y} \\
  ...
  \labitem{(cs.3)}{cs.3} an indexed set $\varphi_x \ceq x^*\varphi: \gcs{L}_x \to \gcs{L'}_x$
    ...
    \[
    \begin{tikzcd}[column sep=40]
      \arrow[swap]{d}{F_x} \gcs{L}_{\Frob{\Fq}(x)} \rar{\varphi_{\Frob{\Fq}(x)}} & \gcs{L'}_{\Frob{\Fq}(x)} \dar{F_x'}
...
%
  ...
  $F_x : \gcs{L}_{\Frob{\Fq}(x)} \to \gcs{L}_x$ of one-dimensional $\EE$-vector
  spaces, so $F_x(s(\Frob{\Fq}(x)))$ is a scalar multiply of $s(x)$, for
  each $x\in G(\bFq)$; let $t_\cs{L}(\Frob)(x)$ be that scalar, so
  \begin{equation}\label{t}
    \forall x\in G(\bFq),\qquad F_{x}(s(\Frob{\Fq}(x)) = t_\cs{L}(\Frob)(x) s(x).
  \end{equation}
  ...
  $t_\cs{L}(\Frob{\Fq}^n) \ceq t_\cs{L}(\Frob{\Fq})\cdot \,^\Frob{\Fq} t_\cs{L}(\Frob{\Fq}^{n-1})$,
  ...
  condition~\ref{cs.3}, $t'(\Frob{\Fq}) = t(\Frob{\Fq}) \cdot (\,^{\Frob{\Fq}^{-1}}\phi \cdot \phi^{-1})$,
  ...
  that $t_\cs{L}(\Frob{\Fq}) = t^\Frob{\Fq}_\cs{L}$ on $G(\Fq)$, which is to
  say, when evaluated at $\Frob{\Fq}$, the cocycle $t_\cs{L}$ is an
...
\begin{proof}
  ...
  $F^0 : \Frob{G^0}^* \gcs{E} \to \gcs{E}$ and
  ...
  $F : \Frob{G}^* \gcs{L} \to \gcs{L}$ satisfying conditions \ref{CS.1}
  and \ref{CS.2}.
  ...
  Now we turn to the definition of $F : \Frob{G}^* \gcs{L} \to \gcs{L}$.
  ...
  $F_x : (\Frob{G}^x)^* \gcs{L}^{\Frob{\Fq}(x)} \to \gcs{L}^x$ by the canonical morphism
  ...
  $F_x : (\Frob{G}^* \gcs{L})\vert_{\bG^x} \iso (\Frob{G}^x)^* \gcs{L}^{\Frob{x}} \to \gcs{L}^x$
  now defines the isomorphism $F : \Frob{G}^* \gcs{L} \to \gcs{L}$.
...
\begin{proof}
  ...
  $G^0$. Then $t^\Frob{\Fq}\cs{L}\vert_{G^0(\Fq)} = t^\Frob{\cs{L}^0} =0$,
  ...
  $t^\Frob{\Fq}\cs{L}$. By Propositions~\ref{prop:etale} and \ref{prop:X},
  there is a a character sheaf $\cs{L}_\chi$ on $\pi_0(G)$, unique up
  to isomorphism, such that $t^\Frob{\cs{L}_\chi} = \chi$. By
