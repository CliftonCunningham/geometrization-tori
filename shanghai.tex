...
\labitem{(CS.1)}{CS.1} $\mu: \bm^* \gcs{L} \to \LxL$
  ...
  \end{tikzcd}
  \]
  commutes, where $m_3 \ceq m\circ (m\tight{1}{\times}{2}\id) = m\circ (\id\tight{2}{\times}{1} m)$;
...
\labitem{(CS.3)}{CS.3} if $\cs{L} = (\gcs{L},\mu,F)$ and
  ...
  of constructible $\ell$-adic sheaves on $\bG$ such that
  \[
...
with duals formed by applying the functor $\sheafHom(\ - \ ,(\EE)_{\bG})$ to
...
sheaves). The isomorphism $\mu : \bm^* \gcs{L} \to \LxL$
...
\begin{proof}
  ...
  statements about pullbacks of sheaves and maps of sheaves.
...
\end{tikzcd}
\end{equation}
It is easy to see that $\CSiso{0}$ is trivial. In this paper we will find $\CSiso{G^0}$ and $\CSiso{\pi_0(G)}$ under the hypothesis that $\pi_0(G)$ is geometrically finitely generated; the snake lemma then allows us to show
\[
\CSiso{G} \iso \Hom(G(\Fq),\EE^\times).
\]

\section{Bounded character sheaves and finite character sheaves}\label{sec:bounded}

 In this section we describe two full subcategories of $\CS(G)$
 which will illuminate some features of $\CS(G)$ and also play a role in some arguments below.
...
If $(\cs{L}_0,\mu_0)$ is bounded character sheaf then $b_G^* \cs{L}_0$ comes equipped with an isomorphism $F : \Frob_G^* b_G^*\cs{L}_0 \to b_G^*\cs{L}_0$ which is compatible with $b_G^* \mu_0$, in the sense of Condition~\ref{CS.3}.
Thus, base change gives us a functor $b_G^* : \CS_f(G) \to \CS(G)$.
This functor is fully faithful; see \cite{SGA7.2}*{Expos\'e XIII} and \cite{BBD}*{Prop. 5.2.1}.

To define the category of finite character sheaves we must work a bit harder.
Consider the following category $C(G)$:
an object in $C(G)$ is a pair $(f,\psi)$ where: $f : H\to G$ is an isogeny (a finite, surjective, etale, morphism of commutative group $\Fq$-schemes) for which the action of $\Gal(\bFq/\Fq)$ on the etale group scheme $\ker f$ is trivial, and $\psi : \ker f\to \Aut(V)$ is a representation on a finite-dimensional $\EE$-vector space $V$;
...
%
We say that $(\varphi,T)$ in $C(G)$ is a weak isomorphism if $T$ is an isomorphism of vector spaces. Let $C(G)[W^{-1}]$ be the localization of $C(G)$ at weak isomorphisms.
Now, let $C_1(G)$ be the subcategory of $C(G)$ such that $\psi$ is a one-dimensional representation
and let $\CS_f(G)$ be the localisation of $C_1(G)$ at weak isomorphisms. This is the category of \cdef{finite character sheaves} on $G$.
...
\begin{proof}
 To prove the proposition we exhibit a fully faithful monoidal functor $L_G : \CS_f(G) \to \CS_0(G)$.
 %
 ...
 Observe that $L$ takes weak isomorphisms in $C(G)$ to isomorphisms in $\Loc(G)$. To see this, let $(\varphi,T) : (f,\psi)\to (f',\psi')$ be a weak isomorphism, where $f:H\to G$ and $f':H'\to G$ as above. Since $T$ is an isomorphism, we may replace $V'$ with $V$ and $T$ with $\id_V$, without loss of generality. Then the equivariance condition on $T$ becomes $\psi' = \psi\circ \varphi$ on the finite etale group scheme $\ker f'$. Since $f' = f \circ \varphi$ and $f$ and $f'$ are etale covers, it follows that $\varphi$ is also an etale cover. It follows that $\varphi : H'\to H$ admits a section $\varphi' : H \to H'$, which provides the inverse to $L(\varphi,T) : L(f,\psi) \to L(f',\psi')$, above. It is also clear that if $L(\varphi,T)$ is an isomorphism of local systems, then $T$ must be an isomorphism of vector spaces, whence $(\varphi,T)$ is a weak isomorphism: $W$ is saturated.
 ...
 %
 Finally, we are ready to exhibit the full functor $L_G: \CS_b(G) \to \CS_0(G)$ promised above. 
 ...
 It follows immediately that 
  \[
  L_G  \ceq  L_{W}\vert_{\CS_b(G)} : \CS_b(G) \to \CS(G)
  \]
 is a fully faithfull monoidal functor, completing the proof.
\end{proof}

\Clifton{I'll address this point in the next section: "Because $\Gal(\bFq/\Fq)$ acts trivially on $\Aut({\bar f})$, the local system
 ${\bar L}_G(f,\psi)\ceq b_G^* L(f,\psi)$ comes equipped with a canonical isomorphism 
 $F : \Frob_G^* {\bar L}_G(f,\psi) \to {\bar L}_G(f,\psi)$ of constructible sheaves on ${\bar G}$."}
...
\begin{proposition}\label{prop:connected-1}
  ...
  the full subcategories $\CS_b(G) \hookrightarrow \CS_0(G) \hookrightarrow \CS(G)$ 
...
\begin{proof}
 %Let $G$ be a geometrically connected algebraic group over $\Fq$.
  ...
  mean a Weil sheaf on $G$ which is a local system on $\bG$).
  While is it not true that all Weil local systems on $G$
  descend to local systems on $G$, we will see that those that appear in the image of the
  forgetful functor from character sheaves do. 
  Then we will see that these local systems further appear in the image of 
  ...
  $\ell$-adic representations of $\W(G,\bg)$
  ...
  Weil group $\W(G, \bg)$. The $\Fq$-rational point $g$ under
  the geometric point $\bg$ determines a splitting
  $\Weil{\Fq}\to \W(G,\bg)$ of the exact sequence
  \[
  \begin{tikzcd}
  1 \rar & \pi_1(\bG, \bg) \rar &  \W(G,\bg) \rar
  & \ar[swap,bend right, dotted]{l}{g} \Weil{\Fq} \rar & 1
  \end{tikzcd}
  \]
  In this way the Weil local system $(\gcs{L},F)$ determines an $\ell$-adic character
  \[
  t_{(\gcs{L},F)}(g) : \Weil{\Fq} \to \EEx \qquad \text{defined by}\qquad \Frob \mapsto t^\Frob_{(\gcs{L},F)}(g)
  \]
  of the Weil group $\Weil{\Fq}$; the image of $\Frob$ under this
  ...
  $\Frob$ and $t_{(\gcs{L},F)}(g) : \Weil{\Fq} \to \EEx$ is
  ...
  \[
  \begin{tikzcd}
  1 \rar & \ar[equal]{d} \pi_1(\bG, \bg) \rar & \W(G,\bg) \rar \dar & \Weil{\Fq} \rar \dar & 1\\
...
If $\cs{L} = (\gcs{L}, \mu, F)$ is a character sheaf on $G$ we may define
a character sheaf $\nu(\cs{L}) = (\gcs{L}, \mu, F_{k'})$ on $G_{k'}$ by setting
\[
F_{k'} = F \circ \Frob_G^*(F) \circ \cdots \circ (\Frob_G^{n-1})^*(F).
\]
The commutativity of the diagram (CS.2) for $F_{k'}$ follows from the fact that $\Frob_{G_{k'}} = \Frob_G^n$.
Note that we may also think about the construction of $F_{k'}$ as taking the action of $\Weil{k}$ defined in Remark
...
\[
\begin{tikzcd}
\CSiso{G} \rar{\nu} \dar{t_G} & \CSiso{G_{k'}} \dar{t_{G_{k'}}} \\
...
\section{Functorial properties of the main isomorphism}
...
\]
and thus a map on character sheaves.
