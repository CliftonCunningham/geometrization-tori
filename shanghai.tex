...
We say that a section is \emph{trivial} if $s(x) = 0$ for all $x \in G(\bFq)$ 
\Clifton{Since `nontrivial section' is not the same as not a `trivial section', and since the term `trivial section' is never used below, perhaps there is no need to introduce it.}
and \emph{nontrivial} if $s(x) \ne 0$ for all $x \in G(\bFq)$.
...
We will now show that trace of Frobenius is an isomorphism.

%\input{snake}
\begin{theorem}\label{thm:snake}
  If $G$ is a smooth, commutative group scheme, locally of finite type over
  \Clifton{For every $\QC$-scheme $G$, the trace of Frobenius is an isomorphism.}
...
\begin{proof}
  ...
  the surjectivity of $\QCiso{G} \to \QCiso{G^0}$ follows from Proposition~\ref{prop:restriction} while exactness at $\QCiso{G}$ is given by Proposition~\ref{prop:middleexact}.
%  
...
\Clifton{Could you add a few words about the proof of this corollary?}   
