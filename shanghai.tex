...
on the Picard group of character sheaves on $G$, for all smooth
commutative group schemes $G$, locally of finite type over $\Fq$, 
...
$m : \GxG\to G$ for the multiplication morphism. We fix an
algebraic closure $\bFq$ of $\Fq$ and write $\bG $ for the
...
$\bG$ will be denoted by $\bm$. Let $\Frob_{G}$
(resp. $\Frob_{\GxxG}$) be the Frobenius automorphism of
$\bG$ (resp. $\bGxG$).
...
\begin{enumerate}
\labitem{(CS.0)}{CS.0} $\gcs{L}$ is a rank-$1$ $\ell$-adic
  local system on (the \'etale site of) $\bG$;
...
\labitem{(CS.2)}{CS.2} $F : \Frob_{G}^* \gcs{L} \to \gcs{L}$ is an
  isomorphism of sheaves on $\bG$ compatible with $\mu$ in the sense that the following diagram commutes.
%
  \[
  \begin{tikzcd}[row sep=20]
  \Frob_{\GxxG}^* \bm^* \gcs{L} \arrow{rr}{\Frob_{\GxxG}^*\mu}
    && \Frob_{\GxxG}^*(\LxL)\\
    \arrow[equal]{u} \bm^*  \Frob_{G}^* \gcs{L} \arrow[swap]{d}{\bm^* F}
    && \Frob_{G}^*\gcs{L}\boxtimes \Frob_{G}^*\gcs{L} \dar{F\boxtimes F} \arrow[equal]{u} \\
...
\labitem{(CS.3)}{CS.3} if $\cs{L} = (\gcs{L},\mu,F)$ and
  ...
  \Frob_G^* \gcs{L} \rar{\Frob_G^* \varphi} \arrow[swap]{d}{F} & \Frob_G^* \gcs{L'} \dar{F'}
...
with duals given by applying the functor $\sheafHom(\ - \ ,(\EE)_{\bG})$ (sheaf hom) to
...
If $W$ is a group equipped with a homomorphism $\alpha : W \to \Gal(\bFq/\Fq)$
then an \emph{action} of $W$ on $\gcs{L}$ is a collection of isomorphisms
\[
\gamma_w : \alpha(w)^* \gcs{L} \to \gcs{L}
\]
so that $\gamma_{uw} = \gamma_w \circ \alpha(w)^*\gamma_u$. \David{make sure I have the order right and add a reference}
Specifying an $F$ as in \ref{CS.2} is the same as giving an action of the Weil group $\Weil{k}$
on $\gcs{L}$ compatible with $\mu$.
\end{remark}

\section{Trace of Frobenius}\label{sec:Frob}

Let $\cs{L}$ be a character sheaf on $G$. Every $g\in G(\Fq)$
determines a point $\bg$ on $\bG$ fixed by $\Frob_G$ and
therefore an automorphism $F_{\bg}$ of $\gcs{L}_{\bg}$. Note
that $\Aut_{\EE}(\gcs{L}_{\bg}) \iso \EEx$, canonically (this
follows from the Condition~\ref{CS.1}). The isomorphism $\mu : \bm^* \gcs{L} \to \LxL$
guarantees that the function $t^\Frob_\cs{L} : G(\Fq)\to \EEx$,
defined by $g \mapsto F_{\bg}$, is a group homomorphism;
...
\[
\cdef{t_{G}} : \CSiso{G} \longrightarrow \Hom_\text{grp}(G(\Fq),\EEx),
...
\begin{proposition}\label{prop:pullback}
  ...
  \begin{tikzcd}[row sep=20, column sep=30]
   \CSiso{G} \rar{f^*} \arrow[swap]{d}{t_{G}} & \CSiso{H} \dar{t_H} \\
...
\begin{proof}
  ...
  $(\bfxf)^*\Frob_{\GxxG}^* \iso \Frob_{\GxxG} (\bfxf)^*$,
...
Let $\CS_0(G)$ be the category of pairs $(\cs{L}_0,\mu_0)$ 
...
morphisms in $\CS_f(G)$ are defined as in (the second part of) Condition~\ref{CS.2}. 
...
\begin{proposition}
$\CS_0(G)$ is (equivalent to) a full subcategory of $\CS(G)$.
\end{proposition}
...
\begin{proof}
 ...
 $F : \Frob_G^* b_G^*\cs{L}_0 \to b_G^* \cs{L}_0$; 
 moreover, the functor $\cs{L}_0 \mapsto (b_G^* \cs{L}_0,F)$  
 ...
 Moreover, $F$ is compatible with $\mu$ in the sense of Condition~\ref{CS.3}.
 It follows that functor $(\cs{L}_0,\mu_0) \mapsto (b_G^*\cs{L}_0,b_{G\times G}^* \mu_0, F)$, denoted by  $B_G^*: \CS_0(G) \hookrightarrow \CS(G)$ below, is full and faithful.
\end{proof}

\Clifton{Address the elephant in the room: Is $B_G^*: \CS_0(G) \hookrightarrow \CS(G)$ an equivalence? I'm pretty sure I can show that the answer is `no', using trace of Frobenius, as you have suggested.}

\section{Finite character sheaves}\label{sec:finite}

 %In this section we describe a full subcategory of $\CS_0(G)$
 %which will illuminate some features of $\CS_0(G)$ and also play a role in some arguments below.
...
$f : H\to G$ is an isogeny (a finite, surjective, etale, morphism of commutative group $\Fq$-schemes) 
...
a map $(f,\psi) \to (f',\psi')$ in $C(G)$ is a pair $(\varphi,T)$ 
where $\varphi$ is a morphism of schemes such that $f' = f\circ \varphi$
and $T : V\to V'$ is a linear transformation equivariant 
for the action of $\ker f'$ on $V'$ by $\psi'$ 
and the action of $\ker f'$ on $V$ by $\psi \circ \varphi$.
%
We say that $(\varphi,T)$ in $C(G)$ is a weak isomorphism if $T$ is an isomorphism of vector spaces. 
...
and let $\CS_f(G)$ be the localisation of $C_1(G)$ at weak isomorphisms. 
This is the category of \cdef{finite character sheaves} on $G$.

\Clifton{Add the monoidal category structure here.}

\begin{proposition}\label{prop:bounded}
 $\CS_f(G)$ is (equivalent to) a full subcategory of $\CS_0(G)$.
\end{proposition}

\begin{proof}
 To prove the proposition we exhibit a full and faithful monoidal functor $L_G : \CS_f(G) \to \CS_0(G)$.
 %
 We will do this by constructing a faithful monoidal functor $L : C_1(G) \to \CS_0(G)$ 
 such that ${\bar L}(\varphi,T)$ is an isomorphism precisely when $(\varphi,T)$ is a weak isomorphism. 
 Factoring along $C_1(G) \to \CS_0(G)$ will produce the full and faithful monoidal functor we seek.

 We start by putting a tensor category structure on $C(G)$. 
 ...
 Note that we may identify the stalk $(f_* V_H)_{\bar g}$ of $f_* V_H$ at a geometric point ${\bar g}$ on $G$ 
 ...
 For any map $(\varphi,T)$ in $C(G)$, let $L(\varphi,T) : L(f,\psi) \to L(f',\psi')$ be the homomorphism of local systems defined 
 %at geometric points $L(\varphi,T)_{\bar g} : L(f,\psi)_{\bar g} \to L(f',\psi')_{\bar g}$ 
 by $s \mapsto s' \ceq T\circ s\circ \varphi$. 
 ...
 It follows that a morphism in $C(G)[W^{-1}]$ is the equivalence class of 
  \[
   \begin{tikzcd}
   (f,\psi) \arrow{r}{(\varphi_1,T_1)} & (f_1,\psi_1) & \arrow[swap]{l}{(\varphi'_1,T'_1)} (f',\psi')
   \end{tikzcd}
  \]
 where $(\varphi',T') : (f,\psi) \to (f',T')$ is any map in $C(G)$ 
 and $(\varphi'_1,T'_1) : (f_1,\psi_1) \to (f',\psi')$ is a weak isomorphism; 
 this morphism is commonly denoted by $(\varphi_1,T_1)^{-1}\circ (\varphi'_1,T'_1)$. 
 %The equivalence relation identifies $(\varphi_1,T_1)^{-1}\circ (\varphi'_1,T'_1)$ 
 % with $(\varphi_2,T_2)^{-1}\circ (\varphi'_2,T'_2)$ if there are maps 
 %$(f_1,\psi_1)\to (f_0,\psi_0)$ and $(f_2,\psi_2)\to (f_0,\psi_0)$ making the following diagram commute
 %\[
 %  \begin{tikzcd}
 %  & (f_1,\psi_1) \arrow{d} & \\
 %  (f,\psi) \arrow{ur}{(\varphi_1,T_1)} \arrow{dr}{(\varphi_2,T_2)} & (f_0,\psi_0)  & \arrow[swap]{ld}{(\varphi'_2,T'_2)} \arrow[swap]{lu}{(\varphi'_1,T'_1)} (f',\psi')\\
 %  & (f_2,\psi_2) \arrow{u} & \\
 %  \end{tikzcd}
 %\]
 ...
 To see this, let $(\varphi,T) : (f,\psi)\to (f',\psi')$ be a weak isomorphism, 
 ...
 Then the equivariance condition on $T$ becomes $\psi' = \psi\circ \varphi$ on the finite etale group scheme $\ker f'$. 
 Since $f' = f \circ \varphi$ and $f$ and $f'$ are etale covers, it follows that $\varphi$ is also an etale cover. 
 It follows that $\varphi : H'\to H$ admits a section $\varphi' : H \to H'$, 
 which provides the inverse to $L(\varphi,T) : L(f,\psi) \to L(f',\psi')$, above. 
 It is also clear that if $L(\varphi,T)$ is an isomorphism of local systems, 
 then $T$ must be an isomorphism of vector spaces, 
 whence $(\varphi,T)$ is a weak isomorphism: $W$ is saturated.
 ...
 One more ingredient is needed before we can exhibit the full functor $L_G: \CS_f(G) \to \CS_0(G)$ promised above: 
 we must put rigid monoidal category structures on $\CS_f(G)$ and $\CS_0(G)$.
...
V\otimes V & \Aut(V\otimes V) & \arrow[swap]{l}{\psi\otimes \psi} \ker f \times \ker f \arrow{r}  & H\times H \arrow{r}{f\times f}  & G\times G \arrow[equal]{u} \\
 ...
 It follows immediately that the functor $L_G  \ceq  L_{W}\vert_{\CS_f(G)} : \CS_b(G) \to \CS_0(G)$, 
 given on objects by $(f,\psi) \mapsto (L(f,\psi), \mu(f,\psi))$,
 is a full and faithfull monoidal functor, completing the proof.
\end{proof}

%\Clifton{I'll address this point in the next section: "Because $\Gal(\bFq/\Fq)$ acts trivially on $\Aut({\bar f})$, the local system ${\bar L}_G(f,\psi)\ceq b_G^* L(f,\psi)$ comes equipped with a canonical isomorphism $F : \Frob_G^* {\bar L}_G(f,\psi) \to {\bar L}_G(f,\psi)$ of constructible sheaves on ${\bar G}$."}
...
\begin{proposition}\label{prop:connected-1}
  ...
  the full subcategories $\CS_f(G) \hookrightarrow \CS_0(G) \hookrightarrow \CS(G)$ 
  appearing in Section~\ref{sec:bounded} are equivalences 
  and (with reference to Section~\ref{sec:Frob})
  \[
  t_{G} : \CSiso{G} \to \Hom_\text{grp}(G(\Fq),\EEx)
  \]
...
\begin{proof}
 %Let $G$ be a geometrically connected algebraic group over $\Fq$.
  Observe that the forgetful functor $(\gcs{L},\mu,F) \mapsto (\gcs{L},F)$
  ...
  Now let $(\gcs{L},\mu,F)$ be a character sheaf on $G$ 
  and let $\rho : \W(G, \bg) \to \EEx$ be the character determined by $(\gcs{L},F)$. 
  ...
  it follows that $t^\Frob_\cs{L}(g) = \rho_g(\Frob)$ is a root of unity,
  for every $g\in G(\Fq)$.  Since $\Weil{\Fq}$ is generated by
  $\Frob$ and $\rho_g : \Weil{\Fq} \to \EEx$ is
  ...
  Then, as in Section~\ref{sec:bounded}, $(\cs{L}_0,\mu_0)$ is an object in $\CS_0(G)$ 
  and $\cs{L} = (\gcs{L},\mu,F)$ is isomorphic to $(b_G^*\cs{L}_0,b_{G\times G}^*\mu_0)$ in $\CS(G)$.
  Thus, the full and faithful functor $B_G^* : \CS_0(G) \to \CS(G)$ from Section~\ref{sec:bounded}
  is also essentially surjective, hence an equivalence.
  %(when $G$ is a connected, commutative algebraic group over $\Fq$).
  
  In Section~\ref{sec:finite} we saw that $\CS_f(G)$ is a full subcategory of $\CS_0(G)$;
  more precisely, we exhibited a full and faithful functor $L_G : \CS_f(G) \to \CS_0(G)$.
  We now show that this functor is essentially surjective 
  when $G$ is a connected, commutative algebraic group over $\Fq$.
  
  Let $(\cs{L}_0,\mu_0)$ be a bounded character sheaf on $G$. 
  Then $\cs{L}\ceq B_G^*(\cs{L}_0,\mu_0)$ is a character sheaf on $G$.
  Recall the definition of the character $t_\cs{L}^\Frob : G(\Fq) \to \EEx$ from Section~\ref{sec:Frob}. 
  ...
  $\psi(g)(v) = t_\cs{L}^\Frob(g) v$ for $v\in V$. 
  Then $(f,\psi)$ is a finite character sheaf. 
  
  We now show that $L_G(f,\psi) \iso (\cs{L}_0,\mu_0)$. 
  First, recall that $L_G(f,\psi) = (L(f,\psi), \mu(f,\psi))$. 
  By  \cite{Laumon}*{1.1.3.3}, $t_{L(f,\psi)}^{\Frob^n} = t_{\cs{L}}^{\Frob}\circ N_{\FF_{q^n}/\FF_q}$, for each positive integer $n$
  On the other hand, $t_{\cs{L}}^{\Frob}\circ N_{\FF_{q^n}/\FF_q} = 
  Using notation from \cite{Laumon}*{1.1},  this means $t_{\bullet L(f,\psi)} = t_{\bullet \cs{L}_0}$.
  It now \cite{Laumon}*{Th\'eor\`eme~1.1.2}, it follows that  $L(f,\psi) \iso \cs{L}_0$ in $\Loc(G)$ (admittedly, this is a big hammer).
  
  
   we simply apply the functions--sheaves dictionary
  (see \cite{Laumon}*{1.1.3} or \cite{SGA4.5}*{Sommes trigonométriques}) as follows.
  
  
 
  As we saw in Section~\ref{sec:finite}, the $\ell$-adic local system $L(f,\psi)$ on $G$ comes equipped with an isomorphism
  $m^* L(f,\psi) \iso L(f,\psi) \boxtimes L(f,\psi)$ in $\CS_b(G)$; 
  in the case at hand, this isomorphism is \cite[1.1.3.2]{Laumon}.
  By \cite[1.1.3.2]{Laumon}, 
  
  Since we have already seen (in Section~\ref{sec:finite}) that $L_G$ is full and faithful, 
  it will then follow that $L_G$ is an equivalence.
  
  
  
  
  
  \todo{Clifton is here. I will use \cite{Kamgarpour} to finish the proof.}
  
  We have now shown that $b_G^* : \CS_0(G) \to \CS(G)$ is an equivalence
  of rigid monoidal categories. It follows immediately that $b_G^*$
  determines an isomorphism
  \[
  b_G^* : \CSbiso{G} \mathop{\longrightarrow}^{\iso} \CSiso{G}
  \]
  ...
  \[
  t_{G} : \CSbiso{G} \to \Hom_\text{grp}(G(\Fq),\EEx)
  \]
...
\begin{proposition}\label{prop:etale}
  ...
  [\cs{L}] \mapsto [t_\cs{L}],
  \]
  where $t_\cs{L}: \Weil{\Fq}\to \Hom_\text{grp}(G(\bFq),\EEx)$ is a cocycle such that
  \[
...
\begin{proof}
  ...
  \labitem{(cs.2)}{cs.2} an indexed set of isomorphisms $F_{x} \ceq x^*F: \gcs{L}_{\Frob(x)} \to \gcs{L}_x$
    ...
    \[
    \begin{tikzcd}[row sep=40]
      \gcs{L}_{\Frob(x)+\Frob(y)} \arrow[swap]{d}{F_{x+y}} \arrow{rr}{\mu_{\Frob(x),\Frob(y)}}
      && \gcs{L}_{\Frob(x)}\otimes \gcs{L}_{\Frob(y)} \dar{F_x \tight{0}{\otimes}{0} F_y} \\
  ...
  \labitem{(cs.3)}{cs.3} an indexed set $\varphi_x \ceq x^*\varphi: \gcs{L}_x \to \gcs{L'}_x$
    ...
    \[
    \begin{tikzcd}[column sep=40]
      \arrow[swap]{d}{F_x} \gcs{L}_{\Frob(x)} \rar{\varphi_{\Frob(x)}} & \gcs{L'}_{\Frob(x)} \dar{F_x'}
...
%
  ...
  Now $t_\cs{L} : \Weil{\Fq} \to G(\bFq)^*$ is defined, recursively, by
  ...
  $a\in G(\bFq)^*$. Now consider the coboundary $t_0 : \Weil{\Fq} \to G(\bFq)^*$
...
\begin{proof}
  ...
  \[
  \begin{tikzcd}
  0 \rar & X^\Frob \rar & X \rar & X/X^\Frob \rar & 0
  \end{tikzcd}
  \]
...
\begin{proof}
  ...
  %\[
  %\xymatrix{
  %\ar[d]_{\Frob_x} \bG^x \ar[r] & \ar[d]_{\Frob_{G}} \bG\\
  %\bG^{\Frob(x)} \ar[r] & \bG\\
  %}
  %\]
  ...
  \node (12) [anchor=east] at (4,2.4) {$\Aut(\bf^{\Frob(x)})$};
  \node (22) [anchor=west] at (4.4,2.4) {$\bH^{\Frob(x)}$};
  \node (32) [anchor=west] at (8,2.4) {$\bG^{\Frob(x)}$};
  \node (01) at (0,1.2) {$\EEx$};
  \node (11) [anchor=east] at (4,1.2) {$\Aut(\bf^{\Frob(x)})$};
  \node (21) [anchor=west] at (4.4,1.2) {$\bH^{\Frob(x)} \times_{\bG^{\Frob(x)}} \bG^x$};
  ...
  \draw [->, densely dotted] (4.3,1.14) arc (330:40:0.12) -- (4.3,1.27);
  \draw [->] (4.3,-0.06) arc (330:40:0.12) -- (4.3,0.07);
  \draw [->] (12) to node[swap,label] {$\psi_{\Frob(x)}$} (01);
  \draw [->, dashed] (11) -- (01);
  \draw [->] (10) to node[label] {$\psi_x$} (01);
  \draw [->, dashed] ($(11.south) + (0,-0.7)$) -- (11);
  \draw [double equal sign distance] (11) -- (12);
  \draw [->] (22) to node[label] {$\bf^{\Frob(x)}$} (32);
  \draw [->] ($(22.225) + (0,-0.7)$) -- (22.225);
  \draw [->] (21) -- (31);
  \draw [->] ($(32.222) + (0,-0.7)$) to node[swap] {\footnotesize{$\Frob_x$}} (32.222);
  \draw [->] ($(22.225) + (0,-1.9)$) -- ($(22.225) + (0,-1.2)$);
  \draw [->] (20) -- (30);
  \draw [double equal sign distance] ($(32.222) + (0,-1.9)$) -- ($(32.222) + (0,-1.2)$);
  \end{tikzpicture}
  \]
  The covering group of $\bH^{\Frob(x)} \times_{\bG^{\Frob(x)}} \bG^x$
  is isomorphic to $\Aut(\bf^{\Frob(x)})$, canonically, so
  ...
  $\bH^x \to \bH^{\Frob(x)} \times_{\bG^{\Frob(x)}} \bG^x \to $;
  ...
  $F_x : (\Frob_{G}^* \gcs{L})\vert_{\bG^x} \iso \Frob_x^* \gcs{L}^{\Frob{x}} \to \gcs{L}^x$
...
\end{remark}
\Clifton{I can flesh out this remark once I've promoted the definition of $\CS_0(G)$ (category of bounded character sheaves on $G$).}
...
\begin{proof}
  ...
  \[
  \begin{tikzcd}[row sep=30]
    {}& \ker t_{\pi_0(G)} \dar & \arrow[dashed]{d} \ker t_{G} & \ker t_{G^0} \dar & \\
    0 \rar & \CSiso{\pi_0(G)} \rar \dar{t_{\pi_0(G)}}
    & \CSiso{G} \rar \dar{t_{G}} & \CSiso{G^0} \rar \dar{t_{G^0}} & 0\\
