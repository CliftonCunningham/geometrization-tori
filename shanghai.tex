...
\begin{proof}
 %Let $G$ be a geometrically connected algebraic group over $\Fq$.
  ...
  \[
  \trFrob{(G_{k'}\hookrightarrow G)^*L(f,\psi)} = \trFrob{\qcs{L}} \circ N_{k'/k},
  \]
   for each finite extension $k'/k$.
  On the other hand, 
  \[
  \trFrob{\qcs{L}} \circ N_{k'/k} = \trFrob{(G_{k'}\hookrightarrow G)^*\qcs{L}},
  \]
...
\Clifton{I'm working here, now.}
If $d\leq d'$, then $\pK^{d'} \hookrightarrow \pK^d$ so there is a canonical surjective ring homomorphism $A_{d'} \to A_{d}$. Set $S = \Spec{\OK}$ and $S_d = \Spec{A_d}$. 
\[
\begin{tikzcd}
S_1 \arrow[hook]{r} 
& S_2 \arrow[hook]{r} \cdots 
& S_d \arrow[hook]{r}  
& S_{d'} \arrow[hook]{r} \cdots 
& S
\end{tikzcd}
\]


 then $\GN{T}_d$ is a closed subgroup of $\GN{T}_{d'}$,
so these smooth commutative group schemes form a directed system.
\[
\begin{tikzcd}
\GN{T}_1 
& \arrow[two heads]{l} \GN{T}_2 
& \arrow[two heads]{l} \cdots 
& \arrow[two heads]{l} \GN{T}_d 
& \arrow[two heads]{l} \GN{T}_{d'} 
& \arrow[two heads]{l} \cdots
\end{tikzcd}
\]
