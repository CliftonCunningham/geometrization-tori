...
\Clifton{I took the reference to BLR for $P_n$ out, because it didn't actually give any information. 
This must be in DG, but I don't have a copy.}
...
Some fundamental properties of $f_n : \GN{T}_{n+1} \to \GN{T}_n$ are:
 \begin{enumerate}
  ...
  \labitem{(GN.1)}{smooth} 
  $\GN{T}_n$ is a smooth commutative group scheme locally of finite type over $\Fq$ 
  and the component group of $\GN{T}_n$ is geometrically finitely generated. 
  ...
  The morphism $f_n : \GN{T}_{n+1} \to \GN{T}_n$ is a morphism of group schemes over $\Fq$. 
  This follows from the fact that $P_{n}$ is a natural transformation between additive functors.
...
If $K = \mathbb{Q}_q$ and $T = \Gm{\mathbb{Q}_q}$ then $\GN{T}^0 = \mathbb{W}^*_{\FF_q}$, the group $\FF_q$-scheme of units in the ring scheme $\mathbb{W}_{\FF_q}$. 
...
\section{Weil restriction for local fields}
\label{sec:wrK}
...
\[
T \hookrightarrow \Res_{K'/K} T' =: T''
\]
induces a map of $k$-schemes
$\GN{T} \hookrightarrow \GN{T}''$
...
\begin{proof}
Adapting the argument in \cite{bosch-lutkebohmert-reynaud:NeronModels}*{\S 7.6, Prop 6} for lft-Neron models
gives a canonical isomorphism 
\begin{equation}\label{weilneron}
\underline{\Res_{K'/K}(T')} \iso \Res_{\mathcal{O}_{K'}/\OK}(\underline{T'}).
\end{equation}
By Lemma~\ref{lemma:wrK}, 
\begin{equation}\label{weilgreenberg}
\Gr_{n}\left( \Res_{\mathcal{O}_{K'}/\OK}(\underline{T'})\right)
\iso
\Res_{k'/k} \Gr_{en}(\underline{T'}).
...
This gives
\[
\GN{T}''_n \iso \Res_{k'/k} \GN{T}'_{en}.
\]
\Clifton{Now I'm stuck here.}
But
\[
\GN{T}'_{en} = \Gr_{en}(\underline{T'}) = \Gr_{en}(\underline{T}\times_{\OK} {\mathcal{O}_{K'}}) \iso . . . 
\]
Now use Proposition~\ref{prop:csbe}.
\end{proof}

If we specialize Proposition~\ref{prop:wrK} to the case when 
$T = \Gm{K}$ then $T'' = \Res_{K'/K} \Gm{K'}$ and we see that the functor $\QC(\GN{T}'')\to \QC(\GN{T})$ is the geometrization and categorification of restriction of quasicharacters from $(K')^\times$ to $K^\times$:
\[
\begin{tikzcd}[column sep=60]
\Hom_{\text{qc}}((K')^\times, \EEx) \arrow{r}{\chi \mapsto \chi\vert_{K^\times}} &\Hom_{\text{qc}}(K^\times, \EEx).
\end{tikzcd}
\] 
...
This comparison of has been made precise in \cite{chai-yu:01a}, \David{more specific citation?}\Clifton{In this case, maybe not necessary.}
