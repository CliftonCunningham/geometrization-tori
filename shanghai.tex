...
\begin{proof}
It suffices to find a morphism of schemes $i_d : \GN{T}_{d}\hookrightarrow \GN{T}_{d+1}$ such that $f_d\circ i_d = \id_{\GN{T}_d}$. Base change along $\OK/\pK^{d+1} \to \OK/\pK^d$ determines a closed immersion $\TT_{d+1} \hookrightarrow \TT_d$, though not a morphism of group schemes. Applying the Greenberg functor produces a closed immersion $i_d : \GN{T}_{d} \hookrightarrow \GN{T}_{d+1}$. Although $i_d$ is not a morphism of group schemes, it is a section of $f_d : \GN{T}_{d+1}\to \GN{T}_d$.
\end{proof}

Let $\QC(\GN{T})$ be the category of ind-objects built from the categories $\QC(\GN{T}_d)$; we refer to this as the \emph{category of quasicharacters for $T$}. This means a quasicharacter sheaf for $T$ is an equivalence class of quasicharacters on $\GN{T}_d$, for various $d$, under the relation that identifies $\qcs{L}\in \QC(\GN{T}_{d})$ with $f_d^*\qcs{L}\in \QC(\GN{T}_{d})$.
For each $\qcs{F}\in \QC(\GN{T})$ there is a unique least positive integer $d$ and a unique $\qcs{L}\in \GN{T}_d$ such that $\qcs{F} = [\qcs{L}]$; we refer to $d$ as the \emph{level} of $\qcs{F}$.
A map $\alpha : \qcs{F} \to \qcs{F}'$ in $\QC(\GN{T})$
