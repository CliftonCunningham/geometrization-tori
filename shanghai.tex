...
$\TT(\OK) \cong T(K)$.  For each positive integer $d$, let $\GN{T}_n$ be the
...
of the term ``character sheaf'' is due to Lusztig \cite{lusztig:85a}*{Def.~2.10}.  Most of his energy is devoted to
handling non-commutative groups; though he does define character sheaves for some
non-connected groups he restricts his attention to reductive groups with cyclic component group, which are extensions of $\ZZ/n\ZZ$ by a torus. 
In this case is not difficult to relate his Frobenius-stable character sheaves to our quasicharacter sheaves.
...
\begin{proof}
  ...
  Proposition~\ref{prop:pullback} to \eqref{eq:pi0}.  Since $k$ is perfect
  \cite{stacks-project}*{\href{http://stacks.math.columbia.edu/tag/035X}{Tag 035X}} implies that
  \David{Need another reference that an algebraic group over an algebraically closed field is reduced}
  $G$ is reduced.  Since $G^0$ is a finite type
  \cite{vdGeer-Moonen:AbelianVarieties}*{3.17}, it is thus a connected algebraic group and 
...
\[
%\begin{tikzcd}
\Gr_n : \left( \text{Schemes} \atop \text{lft over $\OK$}\right) \to \left( \text{Schemes} \atop \text{lft over $\Fq$}\right)
%\end{tikzcd}
\]
be the functor produced by precomposing pull-back along $\OK \to \OK/\pK^n$ with the Greenberg functor.
The surjective ring homomorphism $\OK/\pK^{n+1} \to \OK/\pK^n$ determines a natural transformation $P_{n} : \Gr_{n+1} \to \Gr_n$ 
%\Clifton{Do you think I should say more about $P_{n}$?}
...
\]
%
\Clifton{I guess this next bit needs to be re-structured.}
Some fundamental properties of $f_n : \GN{T}_{n+1} \to \GN{T}_n$ are:
 \begin{enumerate}
   \labitem{(GN.0)}{points} $\GN{T}_n(\Fq) = \TT(\OK/\pK^n)$, canonically.
  \labitem{(GN.1)}{smooth} $\GN{T}_n$ is a smooth commutative group scheme
  locally of finite type over $\Fq$ and the component group of $\GN{T}_n$ is
  geometrically finitely generated: $\underline{T}$ is a smooth group scheme over $\OK$, by construction; since smoothness is preserved under base change, $\TT_n \ceq \underline{T} \times_\Spec{\OK} \Spec{\OK/\pK^n}$ is a smooth group scheme over an Artinian ring with perfect residue field; the Greenberg functor preserves smoothness, so $\GN{T}_n$ is a smooth group scheme over $\Fq$; the identity component of any smooth group scheme is geometrically connected, hence connected, and the Greenberg functor takes the geometric identity component of $\TT_n$ to the geometric identity component of $\GN{T}_n$;
  in fact, $\pi_0(\GN{T}_n) = \pi_0(\TT) \times_{\Spec{\OK}} \Spec{\Fq}$;
  this group is finitely generated by \cite{xarles:93a}*{2.18}.
  \labitem{(GN.2)}{truncation}
  The morphism $f_n : \GN{T}_{n+1} \to \GN{T}_n$ is a surjective morphism of group schemes. To see this, it suffices to find a morphism of schemes $i_n : \GN{T}_{n}\hookrightarrow \GN{T}_{n+1}$ such that $f_n\circ i_n = \id_{\GN{T}_n}$. Base change along $\OK/\pK^{n+1} \to \OK/\pK^n$ determines a closed immersion of schemes $\TT_{n+1} \hookrightarrow \TT_n$ (not a morphism of group schemes). Applying the Greenberg functor produces a closed immersion of schemes $i_n : \GN{T}_{n} \hookrightarrow \GN{T}_{n+1}$. 
\Clifton{I still need to confirm this claim.}
Although $i_n$ is not a morphism of group schemes, it is a section of $f_n : \GN{T}_{n+1}\to \GN{T}_n$.
  \end{enumerate}
%
We remark that $f_n : \GN{T}_{n+1} \to \GN{T}_n$ is not induced from a morphism of Néron models.

\begin{definition}
The \emph{Greenberg transform of the N\'eron model of $T$} is the pro-group scheme over $\Fq$ given by $\cdef{\GN{T}} \ceq \varprojlim_{n\in \ZZ_{>0}} \GN{T}_{n}$ (with reference to the morphisms $f_n : \GN{T}_{n+1} \to \GN{T}_n$).
\end{definition}
...
\Clifton{Now I wonder if $f_{n\leq m}$ is a discrete isogeny!!! 
What's the covering group of $f_{n\leq m}$? 
Must be the finite group $\GN{T}_{m}(\Fq)/\GN{T}_n(\Fq)$, which could be $\TT^0(\pK^n/\pK^{m})$. 
We might be able to build some very special finite quasicharacter sheaves on $\GN{T}_n$ this way.}
Let $f_{n\leq m}^* : \QC(\GN{T}_n) \to \QC(\GN{T}_{m})$ be the functor promised by Proposition~\ref{prop:pullback}.
%
%\begin{proposition}\label{prop:nm}
The functor $f_{n\leq m}^* : \QC(\GN{T}_n) \to \QC(\GN{T}_{m})$ is full and faithful, by \ref{truncation}.
%\end{proposition}
%
%\begin{proof}
%
...
\end{tikzcd}
\]
defined by $F_n(\qcs{L}) = [\qcs{L}_n]$ and $F_n(\alpha_n) = [\alpha_n]$, for all positive integers $n\leq m$.
Passing to isomorphism classes of objects, we get 
\[
\begin{tikzcd}
\ & & & & \QCiso{\GN{T}}\\
\ & & & \varinjlim_{d\in \ZZ_{>0}} \QCiso{\GN{T}_d} \arrow{ur}{\exists ! \theta} & \\
\arrow{urrr}{F_n} \QCiso{\GN{T}_n} \arrow[swap]{rr}{f_{n\leq m}^*} && \QCiso{\GN{T}_m} \arrow[swap]{ur}{F_m} & & \\
\end{tikzcd}
\]
The group homomorphism 
$\theta: \varinjlim_{d\in \ZZ_{>0}} \QCiso{\GN{T}_d} \to \QCiso{\GN{T}}$ is a bijection 
with inverse $[\qcs{L}]_{/\text{iso}} \mapsto [\qcs{L}_{/\text{iso}}]$.
Now use Theorem~\ref{thm:application}.
\end{proof}
...
\begin{proof}
Weil restriction behaves properly with respect to formation of Néron models:
there is a canonical isomorphism
\[
\underline{\Res_{K'/K}(T')} \iso \Res_{\mathcal{O}_{K'}/\OK}(\underline{T'}).
\]
And Weil restriction behaves properly with respect to the Greenberg transform:
there is a canonical isomorphism 
\Clifton{References needed!}
\[
\Gr_{n} \Res_{\mathcal{O}_{K'}/\OK}(\underline{T'})
\iso
\Res_{k'/k} \Gr_n(\underline{T'}),
\]
where $k'$ is the residue field for $K'$.
And the Greenberg tranforms respects products:
\[
\Gr_n(\underline{T'}) = \Gr_n(\underline{T})\times_{k} {k'}.
\]
Now use Proposition~\ref{prop:csbe}.
\end{proof}
