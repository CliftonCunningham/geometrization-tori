...
and the set of isomorphism classes $\QCiso{G}$ forms a group.
...
that the component group of $G$ is finitely generated.
...
Let $T$ be a maximal torus defined over $K$.  The N\'eron model $\TT$ of $T$
...
group scheme $\GN{T}_d$ plays the r\^{o}le of $G$, above.  Theorem \ref{thm:application}
...
\Hom_{< d}(T(K),\EEx)
\] 
between quasicharacters on $\GN{T}_d$ and characters of $T(K)$ with depth less than $d$.
...
With Theorem~\ref{thm:snake} on hand, we can see why neither bounded quasicharacter sheaves nor finite quasicharacter sheaves are up to the task of geometrizing characters of $G(\Fq)$ in the generality we need.  
%
Consider the case when $G$ is the discrete etale group scheme $\ZZ$. Let $\chi : G(\Fq) \to \EEx$ be the character determined by $\chi(1) = \ell$ and let $\cs{L}$ be a quasicharacter sheaf in the isomorphism class corresponding to the character $\chi : G(\Fq) \to \EEx$ under Theorem~\ref{thm:snake}. Then $\trFrob{\cs{L}}(1) = \ell$. Suppose $\cs{L}$ was a bounded quasicharacter sheaf. Then the restriction of $\cs{L}$ to the connected component $1\in G$ would be a rank-1 local system on $1$, thus an $\ell$-adic character $\rho_1 : \pi_1(1,{\bar 1}) \to \EEx$ with $\rho_1(\Frob{\Fq}) = \ell$. But $\pi_1(1,{\bar 1}) = \Gal(\bFq/\Fq) \iso {\hat \ZZ}$ is compact, while $\rho_1(\pi_1(1,{\bar 1}))$ is not compact (it contains $\ell^\ZZ$). Since the Weil sheaf $\gcs{L}$ does not descend to a local system on $G$, the  quasicharacter sheaf$\cs{L}$ is not a bounded quasicharacter sheaf.
%
We can play the same game with the character $\chi : G(\Fq) \to \EEx$ defined by $\chi(1) = 1+\ell$. In this case, the corresponding geometric character $\cs{L}$ is bounded but not finite, since $1+\ell$ is not a root of unity. 
...
We write $\Hom_{< d}(T(K),\EEx)$ for the group of characters of $T(K)$ depth less than $d$ and $\Hom_\text{qc}(T(K),\EEx)$ for the group of quasicharacters of $T(K)$.
...
\section{Application to quasicharacters of bounded depth}
...
\]
\item
For every finite Galois extension $L/K$ and for every algebraic torus $T$ over $K$, the following diagram commutes:
\[
\begin{tikzcd}
\QCiso{\Gr(\underline{\Res_{L/K}(T_L)}_d)} \arrow[two heads]{r} \dar & \QCiso{\Gr({T}_d)} \dar\\
\Hom_{<d}(T(L), \EEx) \arrow[two heads]{r} & \Hom_{<d}(T(K), \EEx)
\end{tikzcd}
\]
\end{enumerate}
\end{theorem}
...
\begin{proof}
  ...
  Finally, we turn to restriction of scalars.
\end{proof}

\section{The geometrization of quasicharacters}
...
For every algebraic torus $T$ over $K$, the trace of Frobenius 
  \[
\trFrob{T}:  \QCiso{\GN{T}} \to \Hom_{\text{ad}}(T(K),\EEx)
  \]
is an isomomorphism. 
...
to the additive functor
\[
T \mapsto \Hom_{\text{ad}}(T(K),\EEx).
\]
\item
For every finite Galois extension $L/K$ and for every algebraic torus $T$ over $K$, the following diagram commutes:
\[
\begin{tikzcd}
\QCiso{\underline{\Res_{L/K}(T_L)}} \arrow[two heads]{r} \dar & \QCiso{\GN{T}} \dar\\
\Hom_{\text{ad}}(T(L), \EEx) \arrow[two heads]{r} & \Hom_{\text{ad}}(T(K), \EEx)
\end{tikzcd}
\]
\end{enumerate}
\end{corollary}
