 ...
 \labitem{(cs.1)}{cs.1} an indexed set of isomorphisms
  ...
  \[
   \begin{tikzcd}[row sep=40]
    ...
    \gcs{L}_{x} \otimes_{\EE}\gcs{L}_{y+z} \arrow{rr}{\id \otimes_{\EE}\mu_{y,z}}
 ...
 \labitem{(cs.3)}{cs.3} an indexed set $\alpha_x : \gcs{L}_x \to \gcs{L'}_x$
  ...
  \[
   \begin{tikzcd}[column sep=40]
    ...
    & {} & \gcs{L}_x\otimes\gcs{L}_y \rar{\alpha_x\otimes_{\EE}\alpha_y} & \gcs{L'}_x \otimes_{\EE}\gcs{L'}_y
   \end{tikzcd}
  \]
  both commute.
\end{enumerate}
...
\begin{proof}
  ...
  The condition $\mu_{0,0}(s(0)) = s(0) \otimes_{\EE}s(0)$ together with the non-triviality of $s$
  then forces $s(0) = 1$.
