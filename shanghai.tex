...
A geometrized character of $G$ is a triple $\cs{L}\ceq
...
\section{Geometrized characters}\label{sec:category}

\begin{definition}\label{def:GC}
A \emph{geometrized character on $G$} is a triple
...
%
  ...
  \]
\end{enumerate}
In short, a geometrized character on $G$  is a rank-$1$ Weil local system $(\gcs{L},\phi)$ on $G$ equipped with an isomorphism $m^* (\gcs{L},\phi) \to (\gcs{L},\phi)\boxtimes (\gcs{L},\phi)$. 


Morphisms in the category of geometrized characters of $G$, denoted by $\GC(G)$, are defined in the natural way:
\begin{enumerate}
...
\Clifton{Now that I'm working on the functoriality of trace of Frobenius, I  think the source of the functoriality is mainly this: $\GC$ is a stack. I'm thinking about this now.}

\section{Trace of Frobenius}\label{sec:Frob}

%\begin{proposition}\label{tensor}
The category $\GC(G)$ of geometrized characters of $G$ is a rigid symmetric monoidal category \Clifton{I still haven't found a good reference for this!} under
...
$\GC(G)$. Our main result regarding geometrized character sheaves on $G$ is the determination of $\GCiso{G}$ under the hypothesis that the component group scheme for $G$ is geometrically finitely generated; see Theorem~\ref{thm:snake}. To do this, we use the trace of Frobenius.
...
In this paper we $\GCiso{G} \iso G(\Fq)^*$ when the geometric component group of $G$ is finitely generated; see Theorem~\ref{thm:snake}. We also shows that this isomorphism is canonical; see 
...
\[
\GC(G_1)\times(G_2) \to \GC(G_1\times G_2)
\]
and 
\[
\begin{tikzcd}[column sep=60]
\arrow{d}{t_{G_1} \times t_{G_2}} \GCiso{G_1}\times \GCiso{G_2} \arrow{r}{\boxtimes} & \arrow{d}{t_{G_1\times G_2}} \GCiso{G_1\times G_2}\\
...
The full subcategory $\GCf(G) \subset \GCb(G)$ of finite geometrized characters is a proper subcategory, in general. Indeed, it follows from our main result regarding geometrized characters, Theorem~\ref{thm:snake}, that the restriction of $\trFrob{G} : \GCiso{G} \to G(\Fq)^*$ to the subgroup $\GCfiso{G}$ is an isomorphism onto the subgroup of $\chi \in G(\Fq)^*$ such that the image of $\chi$ is finite in $\EEx$; see Section~\ref{sec:snake}.
...
\Clifton{Re-phrase.}
we define a category of geometrized characters $\GC(G(\bFq))$.
...
\end{enumerate}
A morphism $\alpha : \cs{L} \to \cs{L'}$ in $\GC(G(\bFq))$ is given by 
\begin{enumerate}
...
\begin{proof}
When combined with the definition of $\GC(\GN{T})$ and Property~\ref{T(K)}, the first part of Proposition~\ref{prop:pullback} shows that $F_1$ is a functor, while the second part shows that Trace of Frobenius is a natural transformation $t: F_1 \to F_2$. When further combined with Lemma~\ref{lemma:product}, we see that $F_1$ is an additive functor and $t: F_1 \to F_2$ is a natural transformation between additive functors.
\end{proof}
