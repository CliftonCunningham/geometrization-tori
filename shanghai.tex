...
%
  ...
  \Frob{\GxxG}^* \bm^* \gqcs{L} \arrow{rr}{\Frob{\GxxG}^*\mu}
    ...
    && \Frob{G}^*\gqcs{L}\boxtimes \Frob{G}^*\gqcs{L} \dar{F\boxtimes \phi} \arrow[equal]{u} \\
...
Thus the pair $(\gqcs{L},\varphi_1)$ almost satisfies the criteria for an action of $\Weil{\Fq}$ on $\gqcs{L}$ compatible with the action of $\Gal(\bFq/\Fq)$ in the sense of \cite{deligne:SGA4.5}*{Expos\'e XIII,~1.1}, failing only because $\Weil{\Fq}$ is profinite \David{Do you mean to say that $\Weil{\Fq}$ is not profinite?}. This fact \David{Which fact?  That we can think about quasicharacter sheaves as an action on $\gqcs{L}$?  Or that they fail to fulfill the requirements?} goes to the heart of the notion of a Weil sheaf and is one of the essential aspects of our definition of $\QC(G)$. 
%\end{remark}
...
$\qcs{L} \otimes_{\EE}\qcs{L'} \ceq (\gqcs{L}\otimes\gqcs{L'}, \mu\otimes\mu', \phi\otimes_{\EE}\phi')$
\David{Why only some $\otimes_{\EE}$?}
...
\begin{proposition}\label{prop:basechange}
 The rule $\nu_{k'/k}: (\gqcs{L}, \mu, \varphi) \mapsto (\gqcs{L}, \mu, \varphi\vert_{\Weil{\Fq}})$ 
