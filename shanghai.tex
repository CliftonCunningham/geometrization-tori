...
\end{tikzcd}
\end{equation}
It is easy to see that $\CSiso{0}$ is trivial. In this paper we will find $\CSiso{G^0}$ and $\CSiso{\pi_0(G)}$ under the hypothesis that $\pi_0(G)$ is geometrically finitely generated; the snake lemma then allows us to find $\CSiso{G}$.
 
\section{Bounded character sheaves}

In this section we describe full subcategory of $\CS(G)$, 
called the category of bounded character sheaves on $G$ and denoted by $\CS_b(G)$, 
which will illuminate the nature of $\CS(G)$ and will also play a role in the arguments later in this paper.

Consider the category $\GC(G)$ of finite etale covers of $G$ that are also morphisms of abelian group schemes.
Recall that we fixed an algebraic closure $\bFq$ of $\Fq$ long ago; 
let ${\bar e} : \Spec{\bFq} \to G$ be the geometric point on $G$ obtained by composing $\Spec{\bFq} \to \Spec{\Fq}$ with the identity $e : \Spec{\Fq} \to G$ for $G$.
Let $\ker_{\bar e}$ be the set-valued functor on $\GC(G)$
defined by pull-back along the geometric point ${\bar e} : \Spec{\bFq} \to G$ 
followed by forgetting the structure sheaf and topology; 
thus, if $f : H\to G$ is a finite etale morphism of group schemes, then $\ker_{\bar e}(f)$ is the set underlying 
$\ker{\bar f} = H_{\bar e}$.
Let $\pi_1^\text{gp}(G)$ be the automorphism group of the functor $\ker_{\bar e}$. 
We will see that $\pi_1^\text{gp}(G)$ is a profinite abelian group.
A \cdef{bounded character sheaf on $G$} is a continuous irreducible representation of $\pi_1^\text{gp}(G)$. 
Let $\CS_b(G)$ be the monoidal category of irreducible continuous $\ell$-adic representations of $\pi_1^\text{gp}(G)$. 

\begin{proposition}
 $G\mapsto \pi_1^\text{gp}(G)$ is a functor from group schemes over $\Fq$ to profinite abelian groups, and .....
\end{proposition}

\begin{proof}
The category $\GC(G)$, together with the set-valued functor $\ker_{\bar e}$ satisfy conditions (G1) through (G6) appearing in \cite[Exposé V, \S 4, p. 118-119]{SGA1a}.
\end{proof}
...
\begin{proof}
  ...
  and $\mu_0: m^*\cs{L}_0 \to \cs{L}_0 \tight{0}{\boxtimes}{0} \cs{L}_0$ satisfies
  ...
  \David{This section should be modified once we're done with the section on bounded character sheaves}
  Let $b : \bG \to G$ be base change
...
We define a functor $\CS(G_{k'}) \to \CS(G)$ by.... \David{We can't define this by pullback
since $G_{k'} \to G$ goes in the wrong direction}
\begin{proposition} \label{prop:csbe}
The diagram
\[
\begin{tikzcd}
\CSiso{G_{k'}} \arrow[two heads]{r} \dar & \CSiso{G} \dar \\
\Hom(G(k'), \EEx) \arrow[two heads]{r} & \Hom(G(k), \EEx)
\end{tikzcd}
\]
commutes, where the vertical arrows are as in Theorem \ref{thm:snake}.
\end{proposition}
...
\[
\begin{tikzcd}
\CSiso{G} \rar{t_G} \dar & \Hom(G(k'), \EEx) \dar \\
\CSiso{\Res_{k'/k} G} \rar{t_{\Res_{k'/k} G}} & \Hom((\Res_{k'/k}G)(k), \EEx)
\end{tikzcd}
\]
\end{proposition}
...
\begin{proposition}
Let $K'/K$ be a finite Galois extension and let $T'$ be an algebraic torus over $K'$... 
\Clifton{This next part is very much under construction.}
The functor $\CS(\GN{T}) \to \CS(\GN{T}')$ defined above induces a group homomorphism 
that commutes, through trace of Frobenius, with ...
\[
\begin{tikzcd}
\CS(\GN{T}) \dar & \CSiso{\GN{T}} \rar{t_T} \dar & \Hom_\text{ad}(T(K),\EEx) \dar \\
\CS(\GN{T}') & \CSiso{\GN{T}'} \rar{t_{T'}} & \Hom_\text{ad}(T'(K),\EEx)
\end{tikzcd}
\]
\end{proposition}
