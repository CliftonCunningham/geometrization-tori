...
\Clifton{Combine these two paragraphs? It seems they make the same point. Waldspurger also uses D-L to study depth-zero distributions. Mention this, too? Mention neither?}
...
Note that our geometrization of quasicharacters is not limited
to the depth-zero case.
...
Let $\pi: B \to G^0$ be a discrete isogeny and set $A \ceq \ker \pi$.
  ...
  Write $W$ for the Weil group $\Weil{\Fq}$.  Extensions of
  $G^0(\bFq)$ by $A(\bFq)$, such as $A(\bFq) \to B(\bFq) \to G^0(\bFq)$,
  correspond to classes in $\Ext^1_{\ZZ[W]}(G^0(\bFq), A(\bFq))$.
  Similarly, extensions of $G(\bFq)$ by $A(\bFq)$ correspond to
  classes in $\Ext^1_{\ZZ[W]}(G(\bFq), A(\bFq))$.  The map
  $G^0(\bFq) \to G(\bFq)$ induces the map
  \[
  \Ext^1_{\ZZ[W]}(G(\bFq), A(\bFq)) \to \Ext^1_{\ZZ[W]}(G^0(\bFq), A(\bFq))
  \]
  fitting into the long exact sequence derived from applying
  the functor $\Hom(\mbox{---}, A(\bFq))$ to $G^0(\bFq) \to G(\bFq) \to \pi_0(G)(\bFq)$:
  \[
  \Ext^1_{\ZZ[W]}(G(\bFq), A(\bFq)) \to \Ext^1_{\ZZ[W]}(G^0(\bFq), A(\bFq)) \to \Ext^2_{\ZZ[W]}(\pi_0(G)(\bFq), A(\bFq)).
  \]
  Since $W$ has cohomological dimension $1$ \cite{brown:CohomologyGrps}*{Ex. 4.3},
  $\Ext^2_{\ZZ[W]}(\pi_0(G)(\bFq), A(\bFq))$ vanishes \cite{cartan-eilenberg:HomologicalAlgebra}*{Thm. 2.6}.
  ...
  the objects come equipped with continuous $W$-actions, and the
...
\begin{proof}
  ...
  \Clifton{Refer to the new lemma, here.}
  ...
  (by Proposition~\ref{prop:etale-iso}) and   \Clifton{Awkward sentence.}
  \[
  ...
  there is a quasicharacter sheaf $\qcs{L}_\chi$ on $\pi_0(G)$, unique up
...
\begin{remark}
\Clifton{I'm not sure if we should include this remark in the final version.}
