...
$\Weil{\Fq}\subset \Gal(\bFq/\Fq)$ is the subgroup generated by $\Frob{}$.
...
\Clifton{Should this be another lemma in this section?}
The set $W$ of weak isomorphisms in $C(G)$
is a saturated, left multiplicative system in $C(G)$ in the sense of
  \cite{kashiwara-schapira:CatgoriesSheaves}*{Defs 7.1.5, 7.1.19}.
Let \cdef{$C(G)[W^{-1}]$} be the localization of $C(G)$ at $W$.
A morphism in $C(G)[W^{-1}]$ is an equivalence class of a diagram
  \begin{equation}\label{eq:morGCf}
   \begin{tikzcd}[column sep=50]
   (f,\psi) \arrow{r}{(g_1,T_1)} & (f_1,\psi_1) & \arrow[swap]{l}{(h_1,U_1)} (f',\psi')
   \end{tikzcd}
  \end{equation}
 where $(g',T') : (f,\psi) \to (f',\psi')$ is any map in $C(G)$ 
 and $(h_1,U_1) :  (f',\psi') \to (f_1,\psi_1)$ is a weak isomorphism in $C(G)$; see \cite{kashiwara-schapira:CatgoriesSheaves}*{Thm 7.1.16} for more detail, including the equivalence relation mentioned above.
...
let $L(f,\psi)$ be the local system on $G$ given by the $\psi$-isotypic component of the local system $f_* V_H$ on $G$.
 ...
 %
 \Clifton{There's detail missing here; I might want to include some of it.}
 %
...
\begin{enumerate}
 ...
 \labitem{(cs.3)}{cs.3} an indexed set of isomorphisms $\phi_{x} : \gqcs{L}_{\Frob{}(x)} \to \gqcs{L}_x$
  such that
  \[
   \begin{tikzcd}[row sep=40]
...
\begin{proof}
  ...
  In general, \ref{cs.1} and \ref{cs.3}, together with \eqref{section}, guarantee that
 ...
 clearly, $\mu$ satisfies condition~\ref{cs.2}.
 Define $\phi_x : \gqcs{L}_{\Frob{}(x)} \to \gqcs{L}_x$ by $\phi_x(1) \ceq \tau(\Frob{})(x)$,
 for each $x\in G(\bFq)$. 
 Then $\phi$ satisfies condition~\ref{cs.3} because $\tau \in Z^1(\Weil{},G(\bFq)^*)$.
...
On the other hand, $G^0(\Fq)$ is finite by Lemma~\ref{lem:G0alg-grp} and thus $\trFrob{\qcs{L}}$ has
both finite and bounded image.  \David{Explain how we can conclude for $G$ from $G^0$ and $\pi_0(G)$.}

So suppose that $\qcs{L}$ is a bounded quasicharacter sheaf on an \'etale $\QC$-scheme $G$.
If $\bg$ is a geometric point lying above $g \in G(\Fq)$ then the restriction of $\qcs{L}$ to $\bg$
determines a character of $\pi_1(g, \bg) \cong \Gal(\bFq/\Fq)$.  Since $\Gal(\bFq/\Fq)$ is compact
the image of $\Frob{}$ under this character lies within $\Zlx$; equivalently $\trFrob{\qcs{L}}(g) \in \Zlx$.
Since $g$ was arbitrary, the image of $\trFrob{\qcs{L}}$ is bounded.

Conversely, suppose that $\trFrob{\qcs{L}}$ is bounded.   We may choose a geometric point $\bg$ in each
component $C$ of $G$ and construct a local system on the component by constructing a character of
$\pi_1(C, \bg)$.  \David{How do we construct this character when $C$ has no rational points?}  These
local systems piece together to form a sheaf $\qcs{L}_0$ on $G$ since the components form a disjoint
open cover of $G$.  Since the base change of $\qcs{L}_0$ to $\bG$ is just $\qcs{L}$, we may pull
back the isomorphism $\mu$ to obtain an isomorphism
$\mu_0 : m^* \qcs{L}_0 \to \qcs{L}_0 \boxtimes \qcs{L}_0$.

\David{Still need the analogous constructions for finite quasicharacter sheaves.}

\David{I've left the example here for now so that we can raid material from it.}
...
\]
where $k = \max(m, n)$.
Similarly, the dual of an object $\qcs{L}$ is defined by $\qcs{L}^\vee \ceq (n, \{\qcs{L}_i^\vee\}, \{ \alpha_{i \le j}^\vee \})$.
