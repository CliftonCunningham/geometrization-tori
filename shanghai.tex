...
=======
Applying \ref{CS.2} to the stalk of $\gqcs{L}^{\boxtimes n}$ at the point $(x, F(x), \ldots, F^{n-1}(x))$ yields a diagram
>>>>>>> d85e21ad760c484a7c7f96c98cd336fef65a9225
\[
\begin{tikzcd}
...
=======
for $v_i \in \gqcs{L}_{F^i(x)}$.  By \ref{CS.1}, $v$ maps to
>>>>>>> d85e21ad760c484a7c7f96c98cd336fef65a9225
...
% similar nomenclature is employed in \cite{kamgarpour:09a}*{\S 2.2}.
...
\begin{proof}
The key point is that pullback along $g$ takes isogenies to isogenies,
...
=======
Set $H_m \ceq H\times_G(G\times G)$, write $f_m : H_m \to G \times G$ for the pullback of $f$ to $H_m$, and let $A$ and $A_m$ be the kernels of $f$ and $f_m$ respectively.  Consider the diagram of Lemma~\ref{lemma:finite-pullback}:
>>>>>>> d85e21ad760c484a7c7f96c98cd336fef65a9225
\[
\begin{tikzcd}[column sep=40]
...
\begin{proof}
 ...
 Now we introduce a faithful functor $L$ from $C(G)$ to the category of local systems on $G$, which we will denote by $\Loc(G)$.
 ...
 commutes with pullback along any morphism of group schemes.
...
For general $G$, there are bounded quasicharacter sheaves which are not finite.  Identifying
$\QCf(G)$ with its image under $B_G^* \circ L_G$, we will see in Corollary~\ref{cor:bounded-and-finite} that
...
\begin{proof}
  ...
  In this way the Weil local system $(\gqcs{L},\phi)$ determines an $\ell$-adic character $\rho_g : \Weil{} \to \EEx$.
  %\[
  %\begin{tikzcd}
  %1 \rar & \pi_1(\bG, \bg) \rar &  \arrow{d}{\rho} \W(G,\bg) \rar
  %& \ar[swap,bend right, dotted]{l}{g} \arrow[dotted]{dl}{\rho_g} \Weil{} \rar & 1\\
  %&&\EEx&&
  %\end{tikzcd}
  %\]
  ...
  \[
  \begin{tikzcd}
  1 \rar & \ar[equal]{d} \pi_1(\bG, \bg) \rar & \W(G,\bg) \rar \dar & \Weil{} \arrow[bend left]{rr}{\rho_g} \rar \dar & 1 & \EEx\\
...
=======
 \labitem{(cs.2)}{cs.2} an indexed set of isomorphisms $\phi_{x} : \gqcs{L}_{\Frob{}(x)} \to \gqcs{L}_x$
>>>>>>> d85e21ad760c484a7c7f96c98cd336fef65a9225
  such that
  \[
   \begin{tikzcd}[row sep=40]
...
=======
  ...
  the scalar $\tau_\qcs{L}(\Frob{})(x)$ is non-zero.  Moreover, Condition~\ref{cs.1} forces
  $\tau_\qcs{L}(\Frob{})(x+y) = \tau_\qcs{L}(\Frob{})(x) \ \tau_\qcs{L}(\Frob{})(y)$
  for all $x,y \in G(\bFq)$.  Now define $\tau_\qcs{L} : \Weil{} \to G(\bFq)^*$ recursively by
>>>>>>> d85e21ad760c484a7c7f96c98cd336fef65a9225
  \[
...
=======
  condition~\ref{cs.3}, $\tau'(\Frob{}) = \tau(\Frob{}) \cdot (\,^{\Frob{}^{-1}}a \cdot a^{-1})$,
>>>>>>> d85e21ad760c484a7c7f96c98cd336fef65a9225
  %\Clifton{Fix this.}
...
=======
 ...
 for each $x\in G(\bFq)$. 
 Then $\phi$ satisfies condition~\ref{cs.2} because $\tau \in Z^1(\Weil{},G(\bFq)^*)$.
>>>>>>> d85e21ad760c484a7c7f96c98cd336fef65a9225
...
Let $X$ be a finitely generated abelian group equipped with an action of $\Weil{}$, and write $\Frob{} = \Frob{}$.  Then
\begin{align*}
 (X^*)_{\Frob{}} &\to (X^{\Frob{}})^* \\
 [f] &\mapsto f|_{X^{\Frob{}}}
\end{align*}
...
By Proposition~\ref{prop:pullback}, pullback along
...
\begin{proof}
  ...
  The surjectivity of $\QCiso{G} \to \QCiso{G^0}$ follows from Proposition~\ref{prop:restriction}.  Now suppose that
  $\qcs{L}$ is a quasicharacter sheaf on $G$ whose restriction to $G^0$ is trivial.... \David{Missing argument, discussed this afternoon}
...
\begin{enumerate}
\item $\qcs{L}$ is bounded if and only if $\trFrob{L}$ has bounded image,
\item $\qcs{L}$ is finite if and only if $\trFrob{L}$ has finite image.
\end{enumerate}
\end{corollary}
\begin{proof}
\David{These examples need to be modified to provide a proof of the corollary}
...
$\rho_1(\Frob{}) = \ell$, necessarily continuous. 
...
$\cup_{n\in\ZZ_{>0}} \obj \QC(\GN{T}_n)$.
We say that $\qcs{L}_n\in \QC(\GN{T}_n)$ is equivalent to $\qcs{L}_{m}\in \QC(\GN{T}_m)$ 
if there is some $k \ge m,n$ with $f_{n\leq k}^* \qcs{L}_{n} = f_{m\leq k}^* \qcs{L}_m$. \David{is it okay for this to be equality rather than isomorphism?} 
Objects in $\QC(\GN{T})$ all take the form
\[
[\qcs{L}_n] \ceq \{ f_{n\leq k}^* \qcs{L}_n \tq n\leq k\},
\]
for some unique $\qcs{L}_n \in \QC(\GN{T}_n)$.
A morphism $[\qcs{L}_n] \to [\qcs{L}_m]$ in $\QC(\GN{T})$ is an equivalence class
in $\cup_{n,m\leq k} \Hom(f_{n\leq k}^*\qcs{L}_n,f_{m\leq k}^*\qcs{L}_m)$.
We say that $\alpha_k\in \mor \QC(\GN{T}_k)$ is equivalent to 
$\beta_l \in \mor \QC(\GN{T}_l)$ if there is some $i \ge k, l$ with $f_{k\leq i}^*\alpha_k = f_{l\leq i}^* \beta_l$. 
Morphisms in $\QC(\GN{T})$ all take the form
\[
...
We may define a tensor structure for $\QC(\GN{T})$ by setting
\[
...
\begin{proof}
We may define full and faithful functors $F_n : \QC(\GN{T}_n) \to \QC(\GN{T})$ for each $n$ by $F_n(\qcs{L}) \ceq [\qcs{L}_n]$ and $F_n(\alpha_n) \ceq [\alpha_n]$.  For positive integers $n \le m$ these functors fit together into a diagram
\[
\begin{tikzcd}
\ & & & \QC(\GN{T})\\
...
The next proposition \David{Do we mean Lem 14.1 or Prop 14.2?} shows that the category of quasicharacter sheaves for 
...
or on the ramification of $T$. We may therefore ask if for a comparison of
