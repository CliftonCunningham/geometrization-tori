...
\labitem{(CS.3)}{CS.3} if $\cs{L} = (\gcs{L},\mu,\phi)$ and
  ...
  a morphism $\cs{L} \to \cs{L}'$ is a map $\alpha : \gcs{L} \to \gcs{L'}$
...
Thus the pair $(\gcs{L},\varphi_1)$ almost satisfies the criteria for an action of $\Weil{\Fq}$ on $\gcs{L}$ compatible with the action of $\Gal(\bFq/\Fq)$ in the sense of \cite{deligne:SGA4.5}*{Expos\'e XIII,~1.1}, failing only because $\Weil{\Fq}$ is profinite \David{Do you mean to say that $\Weil{\Fq}$ is not profinite?}. This fact \David{Which fact?  That we can think about quasicharacter sheaves as an action on $\gcs{L}$?  Or that they fail to fulfill the requirements?} goes to the heart of the notion of a Weil sheaf and is one of the essential aspects of our definition of $\QC(G)$. 
%\end{remark}
...
$\cs{L} \otimes_{\EE}\cs{L'} \ceq (\gcs{L}\otimes\gcs{L'}, \mu\otimes\mu', \phi\otimes_{\EE}\phi')$
...
\begin{proof}
The only non-trivial part is essential surjectivity. Set $G \ceq G_1\times G_2$. Let $\cs{L}$ be a quasicharacter sheaf on $G$. Define $f : G\to G\times G$ by $f(g_1,g_2) = (g_1,e_2,e_1,g_2)$. Observe that $m\circ f = \id_G$. 
...
so $f^*p_1^*\gcs{L} = r_1^* q_1^* \gcs{L}$ and $f^*p_2^*\gcs{L} = r_2^* q_2^* \gcs{L}$. 
Now $\cs{L}_1 \ceq r_1^* \cs{L}$ is a quasicharacter sheaf on $G_1$ 
and $\cs{L}_2 \ceq r_2^* \cs{L}$ is a quasicharacter sheaf on $G_2$, 
by Proposition~\ref{prop:pullback}. 
Then 
\[
f^*p_1^*\gcs{L}\otimes f^* p_2^*\gcs{L} = r_1^*\gcs{L}_1\otimes r_2^* \gcs{L}_2 = \gcs{L}_1\boxtimes \gcs{L}_2.
\]
...
\begin{proof}
 %Let $G$ be a geometrically connected algebraic group over $\Fq$.
  ...
  \[
  \trFrob{(G_{k'}\hookrightarrow G)^*L(f,\psi)} = \trFrob{\cs{L}} \circ N_{k'/k},
  \]
   for each finite extension $k'/k$.
  On the other hand, 
  \[
  \trFrob{\cs{L}} \circ N_{k'/k} = \trFrob{(G_{k'}\hookrightarrow G)^*\cs{L}},
  \]
   by Proposition~\ref{prop:basechange}.
  %Using notation from \cite{laumon:87a}*{1.1}, this means $t_{\bullet L(f,\psi)} = t_{\bullet \cs{L}_0}$.
...
\begin{enumerate}
 ...
 \labitem{(cs.1)}{cs.1} an indexed set of isomorphisms
  $\mu_{x,y} : \gcs{L}_{x+y} \mathop{\longrightarrow}\limits^{\iso} \gcs{L}_{x} \otimes_{\EE}\gcs{L}_{y}$,
  for all $x,y \in G(\bFq)$, such that
  \[
   \begin{tikzcd}[row sep=40]
    \gcs{L}_{x+y+z} \arrow{rr}{\mu_{x+y,z}} \arrow[swap]{d}{\mu_{x,y+z}}
    && \gcs{L}_{x+y}\otimes_{\EE}\gcs{L}_{z} \dar{\mu_{x,y} \tight{0.5}{\otimes}{1} \id} \\
    \gcs{L}_{x} \otimes_{\EE}\gcs{L}_{y+z} \arrow{rr}{\id \otimes\mu_{y,z}}
    && \gcs{L}_{x} \otimes\gcs{L}_{y} \otimes_{\EE}\gcs{L}_{z}
   \end{tikzcd}
  \]
 ...
 \labitem{(cs.2)}{cs.2} an indexed set of isomorphisms $\phi_{x} : \gcs{L}_{\Frob{\Fq}(x)} \to \gcs{L}_x$
  ...
  \[
   \begin{tikzcd}[row sep=40]
    ...
    && \gcs{L}_{\Frob{\Fq}(x)}\otimes_{\EE}\gcs{L}_{\Frob{\Fq}(y)} \dar{\phi_x \tight{0}{\otimes}{0} \phi_y} \\
 ...
 \labitem{(cs.3)}{cs.3} an indexed set $\alpha_x : \gcs{L}_x \to \gcs{L'}_x$
  ...
  \[
   \begin{tikzcd}[column sep=40]
    ...
    & {} & \gcs{L}_x\otimes\gcs{L}_y \rar{\alpha_x\otimes\alpha_y} & \gcs{L'}_x \otimes_{\EE}\gcs{L'}_y
   \end{tikzcd}
  \]
  both commute.
\end{enumerate}
...
as we use this hypothesis to show the existence of global sections of $\gcs{L}$ satisfying \eqref{section} for $\mu$.
...
\begin{proposition}\label{prop:etale}
  ...
  \cs{L}_{/\text{iso}} \mapsto [\tau_\cs{L}],
  \]
...
\begin{proof}
  Suppose $\cs{L}$ is a quasicharacter sheaf on $G$.  
  Using Lemma~\ref{lemma:section}, 
  we may choose a section $s = \{ s(x) \in \gcs{L}_x \tq x \in G(\bFq) \}$ satisfying \eqref{section}.  We will construct a group isomorphism between $\QCiso{G}$ and
  ...
  Let $\alpha : \cs{L} \to \cs{L'}$ be an isomorphism and
  ...
  \begin{equation}\label{pich1}
    \QCiso{G} \longrightarrow \Hh^1(\Weil{\Fq},G(\bFq)^*), \qquad \cs{L}_{/\text{iso}} \mapsto [\tau_\cs{L}]
  \end{equation}
  is well-defined.
  
 The paragraph above also shows that \eqref{pich1} is injective. 
 Suppose $[\tau_\cs{L}] = [\tau_\cs{L'}]$.
 Let $s$ (resp. $s'$) be a section appearing in the definition of $\tau_\cs{L}$ (resp. $\tau_\cs{L'}$);
 recall that $s$ (resp. $s'$) is compatible with $\mu$ (resp $\mu'$) 
 in the sense of Lemma~\ref{lemma:section}.
 Since $[\tau_\cs{L}] = [\tau_\cs{L'}]$, 
 it follows that $\tau_\cs{L}$ and $\tau_{\cs{L'}}$ are cohomologous, 
  so there is some $a\in G(\Fq)^*$ such that 
  $\tau'(\Frob{\Fq}) = \tau(\Frob{\Fq}) \cdot (\,^{\Frob{\Fq}^{-1}}a \cdot a^{-1})$. 
  Define $\alpha : \cs{L}\to \cs{L'}$ by $\alpha_x(s(x)) = a(x) s'(x)$, for each $x\in G(\bFq)$.
  it is straightforward to check that $\alpha$ satisfies condition~\ref{cs.3}
  and that $\alpha^{-1} : \cs{L'}\to \cs{L}$, defined by $\alpha^{-1}_x(s'(x)) = a(x)^{-1} s(x)$, 
  is its inverse. Thus, $\cs{L} \iso \cs{L'}$ in $\QC(G)$. 
  This shows that \eqref{pich1} is injective.
  
  Suppose that $\cs{L}$ and $\cs{L}'$ are two quasicharacter sheaves on $G$.
  If $s$ is a global section for $\cs{L}$ and $s'$ for $\cs{L}'$ then $s \otimes_{\EE}s'$ is a global section for
  $\cs{L} \otimes_{\EE}\cs{L}'$.  The three versions of \eqref{t} for $\cs{L}, \cs{L'}$ and $\cs{L} \otimes_{\EE}\cs{L}'$ imply that
  $\tau_{\cs{L} \otimes_{\EE}\cs{L}'}(\Frob{k}) = \tau_\cs{L}(\Frob{k}) \tau_{\cs{L}'}(\Frob{k})$, so \eqref{pich1} is a group homomorphism.
 
 Finally, to show that \eqref{pich1} is surjective, pick $[\tau]\in \Hh^1(\Weil{\Fq}, G(\bFq)^*)$.
 For each $x\in G(\bFq)$, set $\gcs{L}_x = \EE$.
 Define $\mu_{x,y} : \gcs{L}_{x+y} \to \gcs{L}_x\otimes_{\EE} \gcs{L}_y$ by
 $\mu_{x,y}(1) = 1\otimes_{\EE}1$, for $x,y\in G(\bFq)$; 
 clearly, $\mu$ satisfies condition~\ref{cs.1}.
 Define $\phi_x : \gcs{L}_{\Frob{\Fq}} \to \gcs{L}_x$ by $\phi_x(1) = \tau(\Frob{\Fq})(x)$,
 for each $x\in G(\bFq)$. 
 Then $\phi$ satisfies condition~\ref{cs.2} because $\tau \in Z^1(\Weil{\Fq},G(\bFq)^*)$.
 In this way we have built a quasicharacter sheaf $\cs{L} = (\gcs{L},\mu,\phi)$ 
 from $\tau \in \Hh^1(\Weil{\Fq}, G(\bFq)^*)$ such that the image of $\cs{L}_{/\text{iso}}$
