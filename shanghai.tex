...
Specifying an $F$ as in \ref{CS.2} is the same as giving an action of the Weil group $W_k$
...
\begin{proof}
  ...
  \[
  \begin{tikzcd}
  1 \rar & \pi_1(\bG, \bg) \rar &  \Weil(G,\bg) \rar
  & \ar[swap,bend right, dotted]{l}{g} \Weil(\bFq/\Fq) \rar & 1
  \end{tikzcd}
  \]
  ...
  \[
  \begin{tikzcd}
  1 \rar & \ar[equal]{d} \pi_1(\bG, \bg) \rar & \Weil(G,\bg) \rar \dar & \Weil(\bFq/\Fq) \rar \dar & 1\\
...
\begin{proof}
  ...
  $\CSiso{G(\bFq)}$ and $\Hh^1(W_{\Fq}, G(\bFq)^*)$.

  To produce the isomorphism between $\CSiso{G(\bFq)}$ and
  $\Hh^1(W_{\Fq}, G(\bFq)^*)$ we must pick a global section of the line
...
\Clifton{I'm going to remove all mention of line bundles from this paper.}
  ...
  \begin{equation}\label{cohomologous}
    \obj \CS(G(\bFq)) \longrightarrow \Hh^1(W_\Fq,G(\bFq)^*),
    \qquad \text{defined by} \qquad \cs{L} \mapsto [t_\cs{L}],
  \end{equation}
  is independent of the choice of global sections made above.

  We now show that the function $\obj \CS(G(\bFq)) \longrightarrow \Hh^1(W_\Fq,G(\bFq)^*)$
  ...
  \begin{equation}\label{pich1}
    \CSiso{G(\bFq} \longrightarrow \Hh^1(W_\Fq,G(\bFq)^*),
    \qquad \text{defined by} \qquad [\cs{L}] \mapsto [t_\cs{L}].
  \end{equation}
...
%\input{etale-2}
%Define $\rho \colon \Hh^1(W_{\Fq}, \Hom(G(\bFq), \EEx))
...
a character sheaf $\cs{L}_{k'} = (\gcs{L}, \mu, F_{k'})$ on $G_{k'}$ by setting
\[
...
\[
\begin{tikzcd}
\CSiso{G} \rar \dar & \CSiso{G_{k'}} \dar \\
\Hom(G(k), \EEx) \rar & \Hom(G(k'), \EEx)
\end{tikzcd}
\]
commutes, where vertical arrows are as in Theorem \ref{thm:snake}, the top arrow is as above,
and the bottom is induced by the norm map $G(k') \to G(k)$.
\end{proposition}
