...
\begin{proposition}\label{prop:pullback}
  ...
  \[
  \begin{tikzcd}[row sep=30, column sep=30]
...
\begin{proof}
 ...
 %
 Finally, we are ready to exhibit the full functor $L_G: \CS_f(G) \to \CS_0(G)$ promised above. 
 ...
 It follows immediately that 
  \[
  L_G  \ceq  L_{W}\vert_{\CS_f(G)} : \CS_b(G) \to \CS_0(G) 
  \qquad\text{defined by}\qquad 
  (f,\psi) \mapsto (L(f,\psi), \mu(f,\psi))
  \]
...
\begin{proof}
 %Let $G$ be a geometrically connected algebraic group over $\Fq$.
  ...
  The local system $L(f,\psi)$ (defined in Section~\ref{sec:finite}) naturally 
  
  
  To see that $L_G(f,\psi) \iso (\cs{L}_0,\mu_0)$ we simply apply the functions--sheaves dictionary
  
  We will see that $L_G(f,\psi) \iso (\cs{L}_0,\mu_0)$, 
  where $L_G$ is the functor constructed in Section~\ref{sec:finite}.
