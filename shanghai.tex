...
For such a $\Gal(\bFq/\Fq)$-group $G(\bFq)$, we define a category of character sheaves $\CS(G(\bFq))$.
An object in $\CS(G(\bFq))$ is given by the data of
...
The functor $\CS(G) \to \CS(G(\bFq))$ defined by
$(\gcs{L}, \mu, \phi) \mapsto (\gcs{L}_x = x^*\gcs{L}, \mu_{x,y} = (x \times y)^* \mu, x^*\phi)$
is easily seen to be an equivalence.  We may now define a \cdef{global section}
of a character sheaf $\cs{L}$ on an \'etale group scheme $G$ as a choice $s(x) \in \gcs{L}_x$ for every
...
\begin{lemma}\label{lemma:section}
 If $\cs{L}$ is a character sheaf on a commutative, \'etale group scheme $G$
 with $G(\bFq)$ finitely generated then nontrivial global sections exist.
\end{lemma}
...
We will use this equivalence implicitly in proving the following proposition.
...
\begin{proposition}\label{prop:etale}
  ...
  isomorphism
  \[
  \CSiso{G} \longrightarrow \Hh^1(\Weil{\Fq}, G(\bFq)^*),
...
\begin{proof}
  ...
  $\Hh^1(\Weil{\Fq}, G(\bFq)^*)$ we must pick a global section of $\cs{L}$, for each character sheaf $\cs{L}$ on $G$; 
...
%
  ...
  the scalar $z_\cs{L}(\Frob{k})(x)$ is non-zero, while condition~\ref{cs.1} forces
