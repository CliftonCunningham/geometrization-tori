...
Quasicharacter sheaves of $G$ form a category, denoted by $\QC(G)$. In
fact, $\QC(G)$ is a rigid symmetric monoidal category, so isomorphism
classes of quasicharacter sheaves on $G$ form a group, denoted by
$\QCiso{G}$; this is the Picard group of the category of
quasicharacter sheaves on $G$. Our application of quasicharacter sheaves to
characters of algebraic tori over non-Archimedean local fields rests
on finding the Picard group of quasicharacter sheaves on $G$, 
...
$\TT \times_{\Spec{\OK}} \Spec{\OK/\pK^d}$; the commutative group scheme $\GN{T}_d$
...
\Hom_{< d}(T(K),\EEx)
\] 
between the Picard group of the category of quasicharacter sheaves on
$\GN{T}_d$ and the group of characters $T(K) \to \EEx$ of
depth less than $d$.  Since these isomorphisms are compatible with
...
%be), we arrive at a model for the group of quasicharacters of
%$K^\times$: the monoidal category of quasicharacter sheaves on the
%Greenberg transform of the Neron model of
%$\Gm{K}$.

In this paper we show that the monoidal category of quasicharacter sheaves
...
quasicharacters of $T(K)$, where $K$ is any non-Archimedean local
field and $T$ is any algebraic torus over $K$.

Acknowledgements: Masoud Kamgarpour, Pramod Achar, Hadi Salmasian.

\tableofcontents

\Clifton{Declare definitions - about 10 of them.}

\part{Smooth commutative group schemes lft over finite fields}
...
\Clifton{Now that I'm working on the functoriality of trace of Frobenius, I  think the source of the functoriality is mainly this: $\QC$ is a stack. I'm thinking about this now.}
...
$\QC(G)$. Our main result regarding quasicharacter sheaf sheaves on $G$ is the determination of $\QCiso{G}$ under the hypothesis that the component group scheme for $G$ is geometrically finitely generated; see Theorem~\ref{thm:snake}. To do this, we use the trace of Frobenius.
...
\]
If $\cs{L} = (\gcs{L},\mu,\phi)$ is a geochar, then we will abbreviate write $\trFrob{\cs{L}}$ for $\trFrob{(\gcs{L},\phi)}$
\end{definition}

The isomorphism $\mu : \bm^* \gcs{L} \to \LxL$
...
\[
\cdef{\trFrob{G}} : \QCiso{G} \longrightarrow G(\Fq)^*,%\Hom_\text{grp}(G(\Fq),\EEx),
\qquad \text{defined by}\qquad \cs{L} \mapsto \trFrob{\cs{L}},
\]
from the Picard group of quasicharacter sheaves on $G$ to the character group of $G(\Fq)$.

Our main result on the geometrization of quasicharacters of
...
isomorphism from the Picard group of quasicharacter sheaves $G$ to the
...
\Clifton{Address functoriality here? Yes, but put proof at the end of the next section.}


In this paper we $\QCiso{G} \iso G(\Fq)^*$ when the geometric component group of $G$ is finitely generated; see Theorem~\ref{thm:snake}. We also shows that this isomorphism is canonical; see 



\section{Pull-back and products}\label{subsection:pullback}
...
\begin{proposition}\label{prop:pullback}
  ...
  \begin{tikzcd}[row sep=20, column sep=30]
   \QCiso{G} \rar{f^*} \arrow[swap]{d}{\trFrob{G}} & \QCiso{H} \dar{t_H} \\
   G(\Fq)^* \rar & H(\Fq)^*
...
\begin{proof}
  Let $\cs{L}$ be a quasicharacter sheaf of $G$. We start by showing that
...
\[
\begin{tikzcd}
\QCiso{G'} \dar{t_{G'}} \rar & \QCiso{\Res_{k'/k} G'} \dar{t_{\Res_{k'/k} G'}} \\
...
Let \cdef{$\QCb(G)$} be the category of pairs $(\cs{L}_0,\mu_0)$ 
...
morphisms in $\QCf(G)$ are defined as in (the second part of) Condition~\ref{CS.2}. 
This is the category of \cdef{bounded quasicharacter sheaves} on $G$. 
...
Let \cdef{$C_1(G)$} be the rigid monoidal full subcategory of $C(G)$ consisting of objects $(f,\psi)$ such that $\psi$ is a one-dimensional representation and let \cdef{$\QCf(G)$} be the localisation of $C_1(G)$ at weak isomorphisms; this is the rigid symmetric monoidal category of \cdef{finite quasicharacter sheaves} of $G$.
...
The functor $\QC(G) \to \QC(G(\bFq))$ defined by
...
of a quasicharacter sheaf $\cs{L}$ on an \'etale group scheme $G$ as a choice $s(x) \in \gcs{L}_x$ for every
...
\begin{lemma}\label{lemma:section}
 If $\cs{L}$ is a quasicharacter sheaf on a commutative, \'etale group scheme $G$
...
\begin{theorem}\label{thm:snake}
  ...
  \trFrob{G} : \QCiso{G} \to G(\Fq)^*
  \]
  from the Picard group of the category of quasicharacter sheaves on $G$ to
  the group of characters of $G(\Fq)$, is an isomorphism.
  \Clifton{Address functoriality, here?}
\end{theorem}
...
%\section{Application to quasicharacters}
...
This is the \cdef{Greenberg transform of the N\'eron model of $T$}.
 ...
 \begin{enumerate}
  ...
  \end{enumerate}
This gives us a natural way to geometrize all quasicharacters of $T(K)$
without reference to depth.
Let \cdef{$\QC(\GN{T})$} be the ind-category $\varinjlim_{d\in \ZZ_{>0}} \QC(\GN{T}_d)$;
we refer to objects of $\QC(\GN{T})$ as \cdef{quasicharacter sheaves on $T$}.

\begin{corollary}
Let $T$ be an algebraic torus over a non-Archimedean local field $K$.
The Picard group of the category $\QC(\GN{T})$ of quasicharacter sheaves on $T$ is canonically isomorphic to the group of quasicharacters of $T(K)$:
  \[
 \QCiso{\GN{T}} \iso \Hom_\text{ad}(T(K),\EEx).
  \]
\end{corollary}

\section{Functoriality}

\Clifton{Motivate with comments about LLC for tori. Draw triangle. Reduction to GL(1) via induced tori. Comment about Japanese work. Comment about pro-quasi-algebraic. Combine with next section.}

We have already seen that the trace of Frobenius defines a canonical homomorphism $\QCiso{\GN{T}} \to \Hom_\text{ad}(T(K),\EEx)$. 
In this section we show that this homomorphism is functorial, in the following sense.
\Clifton{Fold this section into the main result on characters of local tori?}

\begin{proposition}
Trace of Frobenius $\QCiso{\GN{T}} \to \Hom_\text{ad}(T(K),\EEx)$ 
...
\begin{proof}
When combined with the definition of $\QC(\GN{T})$ and Property~\ref{T(K)}, the first part of Proposition~\ref{prop:pullback} shows that $F_1$ is a functor, while the second part shows that Trace of Frobenius is a natural transformation $t: F_1 \to F_2$. When further combined with Lemma~\ref{lemma:product}, we see that $F_1$ is an additive functor and $t: F_1 \to F_2$ is a natural transformation between additive functors.
\end{proof}
...
\[
\begin{tikzcd}
\QCiso{\Res_{k'/k} \Gr(\TL)} \arrow[two heads]{r} \dar & \QCiso{\GN{T}} \dar\\
...
\Clifton{Work on motivation.}

The techniques in this paper apply to all non-Archimdean local fields $K$ with residue field $k$, and to all algebraic tori $T$ over $K$, without placing any restrictions on the characteristic of $K$ or on the ramification of $T$. Because of this, it is natural to ask if we can compare quasicharacter sheaves on $T$ 
