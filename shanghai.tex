...
In Part 2 we use Theorem~\ref{thm:snake} to make a sheaf--function dictionary for algebraic tori over local fields.  Let $K$ be a non-archimedian local field
...
of the term ``character sheaf'' traces back to Lusztig \cite{lusztig:85a}.  Most of his energy is devoted to
...
Let $\Frob{\Fq}$ denote the geometric Frobenius element in $\Gal(\bFq/\Fq)$ as well as the corresponding automorphism of $\Spec{\bFq}$. The Weil group $\Weil{\Fq}\subset \Gal(\bFq/\Fq)$ is the subgroup generated by $\Frob{\Fq}$. Let $\Frob{G} := \id_{G} \times \Frob{\Fq}$ be the Frobenius automorphism of $\bG$. 
...
\begin{enumerate}
\labitem{(CS.0)}{CS.0} $\gqcs{L}$ is an $\ell$-adic local system on $\bG$ (a constructible $\ell$-adic sheaf on $\bG$, locally constant on each connected component) of rank-1; 
...
%
  ...
  \]
\end{enumerate}
%In short, a quasicharacter sheaf on $G$  is a rank-1 Weil local system $(\gqcs{L},\phi)$ on $G$ equipped with an isomorphism $m^* (\gqcs{L},\phi) \to (\gqcs{L},\phi)\boxtimes (\gqcs{L},\phi)$. 
\end{definition}
...
The rule $(\gqcs{L},\mu,\phi) \mapsto (\gqcs{L},\phi)$ defines a forgetful functor from quasicharacter sheaves on $G$ to ($\ell$-adic) Weil sheaves on $G$ \cite{deligne:80a}*{Def.~1.1.10 (i)}.

As mentioned after \cite{deligne:80a}*{Def.~1.1.10}, a Weil sheaf $(\gqcs{L},\phi)$ on $G$ may be interpreted as a constructible $\ell$-adic sheaf $\gqcs{L}$ on $\bG$ together with an action of the Weil group $\Weil{\Fq}$ on $\gqcs{L}$ compatible with the action of $\Gal(\bFq/\Fq)$ on $\bG$. We now review this point of view for later use in Section \ref{sec:basechange}.  If $w = \Frob{\Fq}^n$ we write $w_G$ for $\Frob{G}^n$.  For each such $w$ we define an isomorphism $\varphi(w) : w_G^* \gqcs{L}\to \gqcs{L}$ by
\[
...
Thus the pair $(\gqcs{L},\varphi_1)$ almost satisfies the criteria for an action of $\Weil{\Fq}$ on $\gqcs{L}$ compatible with the action of $\Gal(\bFq/\Fq)$ in the sense of \cite{deligne:SGA4.5}*{Expos\'e XIII,~1.1}, failing only because $\Weil{\Fq}$ is not profinite. Our use of $\Weil{\Fq}$ rather than $\Gal(\bFq/\Fq)$ allows quasicharacter sheaves to correspond to unbounded characters.
%\end{remark}
...
\cite{etingof:09a} under the tensor product
...
Let $(\gqcs{L},\phi)$ be a Weil sheaf on $G$. on constructible $\ell$-adic sheaf on $\bG$. Every $g\in G(\Fq)$
...
The isomorphism $\mu : \bm^* \gqcs{L} \to \LxL$ guarantees 
...
\begin{proof}
  ...
  $\bf^*\gqcs{L}$ is locally constant on connected components of
  ...
  applying the functor $\pi_0$ to $\bf$; see \cite{vdGeer-Moonen:AbelianVarieties}*{III, 3.28}, for example.
  Write $c_i : \bG^i \hookrightarrow \bG$ for
  the inclusion of $\bG^i$ and $\bf^j : \bH^j \to \bG^i$
  for the restriction of $f$ to $\bH^j$.  Since $\qcs{L}$
  is a quasicharacter sheaf, $\gqcs{L}$ is locally constant on the connect
  ...
  Since $a_i^* (\gqcs{L}\vert_{\bG^i})$ is a constant sheaf and since the pull-back
  of a constant sheaf (along $g_j$, in this case) is a constant sheaf,
  it follows that $b_j^* \left( (\bf^*\gqcs{L})\vert_{\bH^j}\right)$
  is a constant sheaf. Thus, $\bf^*\gqcs{L}$ is locally constant on
  ...
  To see that $(\bfxf)^* \gqcs{L}$ satisfies
  ...
  (a few times). Likewise, to see that $\bf^*\gqcs{L}$ satisfies condition
  \ref{CS.2}, apply the same functor to \ref{CS.2} for $\gqcs{L}$.
  ...
  To see that $(\gqcs{L},\mu,\phi) \mapsto (\bf^*\gqcs{L},(\bfxf)^*\mu,\bf^*F)$
...
\begin{proof}
The only non-trivial part is essential surjectivity. Set $G \ceq G_1\times G_2$. Let $\qcs{L}$ be a quasicharacter sheaf on $G$. Define $f : G\to G\times G$ by $f(g_1,g_2) = (g_1,e_2,e_1,g_2)$. Observe that $m\circ f = \id_G$. 
...
\[
\begin{tikzcd}
G_1  \arrow[swap, bend right]{r}{q_1} & \arrow[swap, bend right]{l}{r_1} G_1\times G_2 \arrow[bend left]{r}{r_2} &  \arrow[bend left]{l}{q_2} G_2.
\end{tikzcd}
\]
...
\end{proposition}
\begin{proof}
Suppose $\qcs{L} = (\gqcs{L}, \mu, \phi) \in \QC(\Res_{k'/k}(G_{k'}))$ and $x \in G(\Fq)$.
The value $t_G(\iota^*(\qcs{L}))(x)$ is given by the trace of $\iota^*\phi$ on $(\iota^*\gqcs{L})_x$.
Similarly, $\iota_{k'/k}^*(t_{\Res_{k'/k}(G_{k'})}(\qcs{L}))(x)$ is given by the trace of $\phi$ on
$\gqcs{L}_{\iota(x)}$.  These are the same since $\iota$ is an inclusion and $\phi$
preserves the stalk at $\iota(x)$. \David{Is this the right justification?}
\end{proof}
...
a quasicharacter sheaf $\qcs{L}' = (\gqcs{L}, \mu, \phi_{k'})$ on 
...
\end{proposition}
\begin{proof}
Let $\qcs{L} = (\gqcs{L}, \mu, \phi) \in \QC(G)$ and $x \in G(k')$.  The value $t_{G_{k'}}(\nu_{k'/k}(\qcs{L})(x)$ is given by the trace of $\phi_{k'}$ on $\gqcs{L}_x$, while the value $\Nm_{k'/k}^*(t_G(\qcs{L}))(x)$ is given by the trace of $\phi$ on $\gqcs{L}_{\Nm(x)}$.  After choosing bases of $\gqcs{L}_x, \ldots, \gqcs{L}_{\Frob{G}^n(x)}$, an iteration of \ref{CS.2} applied to the stalk of $\gqcs{L}^{\boxtimes n}$ at $(x, \Frob{G}(x), \ldots, \Frob{G}^{n-1}(x))$ expresses the trace of $\phi$ at $\Nm_{k'/k}(x)$ as a product of 
\end{proof}
...
\begin{proof}
 ...
 It follows that functor $(\qcs{L}_0,\mu_0) \mapsto (b_G^*\qcs{L}_0,b_{G\times G}^* \mu_0, \phi)$, denoted by  $B_G^*: \QCb(G) \hookrightarrow \QC(G)$ below, is full and faithful.
\end{proof}
...
is the representation $\psi \otimes\psi' :  \ker(f\times_G f') \to \Aut(V\otimes_{\EE} V')$. 
...
\ & V \arrow{dd}{T} && \Aut(V) && \arrow{dl} \arrow[swap]{ll}{\psi_g} A_m \arrow[hook]{rr} && \arrow[swap]{ld}{m_f} H_m \arrow{rr}{f_m} && \arrow[equal]{dd} \arrow{ld}{m} G\times G\\
 && && \arrow{ul}{\psi} A  \arrow[hook]{rr} && H  \arrow[swap, pos=0.8]{rr}{f} && G  & \\
& V\otimes_{\EE} V && \Aut(V) && \arrow[swap]{ll}{\psi \otimes \psi} \arrow[swap,pos=0.8]{uu}{\theta\vert_{A\times A}} A\times A \arrow[hook]{rr} && \arrow[swap,pos=0.8]{uu}{\theta} \arrow{ul}{m_H} H\times H \arrow{rr}{f\times f} && G\times G
\end{tikzcd}
\]
...
\begin{proof}
 %Let $G$ be a geometrically connected algebraic group over $\Fq$.
  ...
  Since $b_G^*$ is full and faithful (again, see \cite{deligne-katz:SGA7.2}*{Expos\'e XIII} or even \cite{beilinson-bernstein-deligne:81a}*{Prop. 5.2.1}),
  $
  ...
  \trFrob{G} : \QCiso{G} \to G(\Fq)^*
  $
  is surjective because each character $\chi \in G(\Fq)^*$ determines a finite quasicharacter sheaf $(\Lang,\chi^{-1})$ such that the trace of Frobenius of the quasicharacter sheaf $B_G^* (L_G(\Lang,\chi^{-1}))$ is $\chi$.
\end{proof}
...
Weil groups and difficult to identify the relevant quotient of this group. Viewed in this light, Definition~\ref{def:QC} is comparatively simple.
...
\begin{enumerate}
 ...
 \labitem{(cs.1)}{cs.1} an indexed set of isomorphisms
  $\mu_{x,y} : \gqcs{L}_{x+y} \mathop{\longrightarrow}\limits^{\iso} \gqcs{L}_{x} \otimes_{\EE}\gqcs{L}_{y}$,
  for all $x,y \in G(\bFq)$, such that
  \[
   \begin{tikzcd}[row sep=40]
    \gqcs{L}_{x+y+z} \arrow{rr}{\mu_{x+y,z}} \arrow[swap]{d}{\mu_{x,y+z}}
    && \gqcs{L}_{x+y}\otimes_{\EE}\gqcs{L}_{z} \dar{\mu_{x,y} \tight{0.5}{\otimes}{1} \id} \\
    \gqcs{L}_{x} \otimes_{\EE}\gqcs{L}_{y+z} \arrow{rr}{\id \otimes\mu_{y,z}}
    && \gqcs{L}_{x} \otimes\gqcs{L}_{y} \otimes_{\EE}\gqcs{L}_{z}
   \end{tikzcd}
  \]
 ...
 \labitem{(cs.2)}{cs.2} an indexed set of isomorphisms $\phi_{x} : \gqcs{L}_{\Frob{\Fq}(x)} \to \gqcs{L}_x$
  ...
  \[
   \begin{tikzcd}[row sep=40]
    ...
    && \gqcs{L}_{\Frob{\Fq}(x)}\otimes_{\EE}\gqcs{L}_{\Frob{\Fq}(y)} \dar{\phi_x \tight{0}{\otimes}{0} \phi_y} \\
 ...
 \labitem{(cs.3)}{cs.3} an indexed set $\alpha_x : \gqcs{L}_x \to \gqcs{L'}_x$
  ...
  \[
   \begin{tikzcd}[column sep=40]
    ...
    &\arrow[draw=none]{d}[pos=.4,description]{\text{\normalsize{and}}}& \arrow[swap]{d}{\mu} \gqcs{L}_{x+y} \rar{\alpha_{x+y}} & \gqcs{L'}_{x+y} \dar{\mu'_{x+y}} \\
    \gqcs{L}_x \rar{\alpha_x} & \gqcs{L'}_x
    & {} & \gqcs{L}_x\otimes\gqcs{L}_y \rar{\alpha_x\otimes\alpha_y} & \gqcs{L'}_x \otimes_{\EE}\gqcs{L'}_y
   \end{tikzcd}
  \]
  both commute.
\end{enumerate}
...
\begin{lemma}\label{lemma:section}
 If $\qcs{L}$ is a quasicharacter sheaf on a commutative, \'etale group scheme $G$ over $\Fq$ and if $G(\bFq)$ is finitely generated, then there is a global section $s = \{ s(x) \in \gqcs{L}_x \tq x \in G(\bFq) \}$ satisfying
 \begin{equation}\label{section}
  \forall x,y \in G(\bFq), \qquad \mu_{x,y}(s(x+y)) = s(x)\otimes_{\EE}s(y).
 \end{equation}
\end{lemma}
...
\begin{proof}
  ...
  to the multiplication map $\EE \otimes_{\EE}\EE \to \EE$.
...
\begin{proof}
  ...
  we may choose a section $s = \{ s(x) \in \gqcs{L}_x \tq x \in G(\bFq) \}$ satisfying \eqref{section}.  We will construct a group isomorphism between $\QCiso{G}$ and
 ...
 The paragraph above also shows that \eqref{pich1} is injective. 
 ...
 it follows that $\tau_\qcs{L}$ and $\tau_{\qcs{L'}}$ are cohomologous, 
  ...
  $\tau'(\Frob{\Fq}) = \tau(\Frob{\Fq}) \cdot (\,^{\Frob{\Fq}^{-1}}a \cdot a^{-1})$. 
  ...
  If $s$ is a global section for $\qcs{L}$ and $s'$ for $\qcs{L}'$ then $s \otimes_{\EE}s'$ is a global section for
  $\qcs{L} \otimes_{\EE}\qcs{L}'$.  The three versions of \eqref{t} for $\qcs{L}, \qcs{L'}$ and $\qcs{L} \otimes_{\EE}\qcs{L}'$ imply that
  $\tau_{\qcs{L} \otimes_{\EE}\qcs{L}'}(\Frob{k}) = \tau_\qcs{L}(\Frob{k}) \tau_{\qcs{L}'}(\Frob{k})$, so \eqref{pich1} is a group homomorphism.
 
 Finally, to show that \eqref{pich1} is surjective, pick $[\tau]\in \Hh^1(\Weil{\Fq}, G(\bFq)^*)$.
 For each $x\in G(\bFq)$, set $\gqcs{L}_x = \EE$.
 Define $\mu_{x,y} : \gqcs{L}_{x+y} \to \gqcs{L}_x\otimes_{\EE} \gqcs{L}_y$ by
 $\mu_{x,y}(1) = 1\otimes_{\EE}1$, for $x,y\in G(\bFq)$; 
...
\begin{proof}
  ...
  sum of cyclic $\ZZ[\Gamma_m]$-modules \cite{brandal:CommRingsFGModDecompose}, each of the \David{Add a section to this reference}
...
defined by $\chi(1) = 1+\ell$. In this case, the corresponding quasicharacter sheaf $\qcs{L}$ is bounded, defined at $1$ by a pro-etale cover of $1$, but not finite, since $1+\ell$ is not a root of unity. 
%
...
is a separated, smooth, commutative group scheme locally of finite type over $\OK$ with generic fibre $T$, that enjoys the N\'eron mapping property:
...
For each positive integer $d$, set $\TT_{d} \ceq \TT \times_\Spec{\OK} \Spec{\OK/\pK^d}$. Observe that $\OK/\pK^d$ is an Artin ring and $\TT_{d}$ is smooth and locally of finite type over $\OK/\pK^d$.
%
...
See \cite{greenberg:63a} and \cite{bosch-lutkebohmert-reynaud:NeronModels}*{Ch. 9, \S 6} and the first few sections of \cite{stasinski:12a} for background on the Greenberg functor. 

For each positive integer $d$, consider the canonical surjective ring homomorphisms $\OK/\pK^{d+1} \to \OK/\pK^d$ and the associated cartesian diagram of schemes 
\[
\begin{tikzcd}
\Spec{\OK/\pK^d} \arrow[hook]{rr} && \Spec{\OK/\pK^{d+1}} \arrow[hook]{r} & \Spec{\OK} \\
 \arrow{u} \TT_{d} \arrow[hook]{rr} & \ \arrow[Rightarrow]{d}{\text{Greenberg functor}} & \arrow{u} \TT_{d+1}  &  \\
\GN{T}_{d} \arrow[hook]{rr} & \ & \GN{T}_{d+1}  \arrow{r} & \Spec{k}
\end{tikzcd}
\]
The map $\GN{T}_{d} \hookrightarrow \GN{T}_{d+1}$ is not a morphism of group schemes. There is, however, an epimorphism of group schemes $\GN{T}_{d+1} \to \GN{T}_{d}$ for which $\GN{T}_{d} \hookrightarrow \GN{T}_{d+1}$ is a section.

The fundamental properties of $\GN{T}_d$ that we need are:
 \begin{enumerate}
   \labitem{(GN.0)}{dFq} $\GN{T}_d(\Fq) = \TT(\OK/\pK^d)$, canonically.
  \labitem{(GN.1)}{GNd} $\GN{T}_d$ is a smooth, commutative group scheme, locally of finite type over $\Fq$, and the component group of $\GN{T}_d$ is geometrically finitely generated. (In fact, $\pi_0(\GN{T}_d) = \pi_0(\TT) \times_{\Spec{\OK}} \Spec{\Fq}$, and the latter is finitely generated by \cite{xarles:93a}.)
  \labitem{(GN.2)}{d+1} 
  \end{enumerate}
...
\trFrob{T,d}:  \QCiso{\GN{T}_d} \to \Hom_{< d}(T(K),\EEx)
  \]
is an isomomorphism. 
Moreover, the isomorphism $\trFrob{T,d}$ is functorial in the following sense: $T \mapsto \trFrob{T,d}$ is a natural transformation between the following two additive functors
\begin{enumerate}
...
it will be possible to reduce the essential points of the argument to the case $T= \Gm{K}$, provided that the category of quasicharacter sheaves behaves properly with regard to Weil restriction; see \cite{yu:09a}*{Thm 7.5 (2)}. 
...
can then be be viewed as pairing characters and parameters coming
 ...
 by the class field theory of Serre-Hazewinkel \citelist{\cite{serre:60a},\cite{serre:61a},\cite{demazure-gabriel:GroupesAlgebriques}*{Appendice}}, especially as revisited in \cite{suzuki-yoshida:12a}. 
