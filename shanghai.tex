...
\]
\item
For every finite Galois extension $L/K$ and for every algebraic torus $T$ over $K$, the following diagram commutes, where  . . . 
\[
\begin{tikzcd}
...
\[
\begin{tikzcd}
\QCiso{\Gr(\underline{\Res_{L/K}(T_L)})} \arrow[two heads]{r} \dar & \QCiso{\Gr({T})} \dar\\
\Hom_{\text{qc}}(T(L), \EEx) \arrow[two heads]{r} & \Hom_{\text{qc}}(T(K), \EEx)
\end{tikzcd}
\]
\end{enumerate}
\end{corollary}
