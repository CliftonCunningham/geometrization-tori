...
Since $\QC(G)$ is a rigid symmetric monoidal category
...
some of the potential benefits of Theorem~\ref{thm:application} here. Characters on $T$ have limited
...
As suggested above above, we hope that our quasicharacter sheaves 
...
play a role in the proof of many other propositions in this paper.
...
by $f(g_1,g_2) \ceq (g_1,e_2,e_1,g_2)$. Observe that $m\circ f = \id_G$. 
...
\end{proposition}
\begin{proof}
Let $\qcs{L} \ceq (\gqcs{L}, \mu, \phi) \in \QC(G)$ and $x \in G(k')$ and write $\Frob{}$ for $\Frob{G}$.
...
where $\psi^\vee : \ker f \to \Aut(V^\vee)$ and $V^\vee \ceq \Hom_\text{vec}(V,\EE)$. 
...
& V' && \Aut(V) && \arrow[swap,pos=0.7]{ll}{\psi'} \arrow[swap]{uu}{} \ker f'  \arrow[hook]{rr}
&& \arrow[swap]{uu} H' \arrow[pos=0.8]{rr}{f'} && G\\
 && && \arrow{ul} \ker f'\times_G G' \arrow{uu} \arrow{ur} \arrow[hook]{rr}
 && \arrow{uu} H'\times_G G' \arrow{ur} \arrow{rr} && \arrow[equals]{uu} G' \arrow[swap]{ur}{g} & 
\end{tikzcd}
\]
In this form we could treat $g^*$ as a functor $C(G')\to C(G)$,
...
where $f_m$ and $\psi_m$ are pictured below, where $H_m \ceq H\times_G(G\times G)$.
\[
\begin{tikzcd}[column sep=40]
...
Define $T : V \to V\otimes V$ by $T(v_1) \ceq v_1\otimes v_1$. Then the lower part of this
...
\begin{proposition}\label{prop:etale}
  If $G$ is a commutative \'etale group scheme over $\Fq$ and $G(\bFq)$ is finitely generated
...
\begin{proof}
  ...
  $m$ extension of $\Fq$ in $\bFq$ and $\Gamma_m$ for $\Gal(\Fqm/\Fq)$.  Since
  ...
  sum of cyclic $\ZZ[\Gamma_m]$-modules \cite{brandal:CommRingsFGModDecompose}, \David{more specific citation} \Clifton{I'm tracking this down.}
  ...
  and set
  $$B = b_i \sum_{j=0}^{i-1} \zeta^j -  \sum_{i=0}^{\phi(d)-1} \zeta^i.$$
  We have
...
\begin{proof}
  ...
  geometrically reduced group scheme of finite type over $\Fq$
...
\begin{proof}
  ...
  Proposition~\ref{prop:pullback} to \eqref{eq:pi0}. 
   ...
   We claim that $\ker \trFrob{G^0} =0$ and $\coker \trFrob{G^0}=0$. 
   This follows from Proposition~\ref{prop:connected} 
   once we confirm that $G^0$ is an algebraic group (it is clearly connected).
   To see this, note that $G^0$ is a smooth group scheme of finite type over $\Fq$ by \cite{vdGeer-Moonen:AbelianVarieties}*{3.17}. 
   Since $k$ is perfect, $G^0$ is also reduced, by
  \cite{stacks-project}*{\href{http://stacks.math.columbia.edu/tag/035X}{Tag 035X}}.
  \David{Need another reference that an algebraic group over an algebraically closed field is reduced}
  \Clifton{I'm working on this too.}
  Thus, $G^0$ is indeed an algebraic group.
