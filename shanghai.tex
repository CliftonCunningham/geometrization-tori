...
\begin{enumerate}
\labitem{(CS.0)}{CS.0} $\gqcs{L}$ is a rank one $\ell$-adic local system on $\bG$
-- a constructible $\ell$-adic sheaf on $\bG$, locally constant on each connected component; 
\labitem{(CS.1)}{CS.1} $\mu: \bm^* \gqcs{L} \to \LxL$ \David{We should explain the $\boxtimes$ notation}
...
\begin{proof}
  ...
  \David{can't we just say rank 1?  Why ``on connected components of $G$?"}
...
\end{tikzcd}
\]
Then $p_1\circ f = q_1 \circ r_1$ and $p_2 \circ f = q_2\circ r_2$, 
so $f^*p_1^*\gqcs{L} = r_1^* q_1^* \gqcs{L}$ and $f^*p_2^*\gqcs{L} = r_2^* q_2^* \gqcs{L}$. 
Now $\qcs{L}_1 \ceq q_1^* \qcs{L}$ is a quasicharacter sheaf on $G_1$ 
and $\qcs{L}_2 \ceq q_2^* \qcs{L}$ is a quasicharacter sheaf on $G_2$, 
...
\]
So $\boxtimes$ is essentially surjective. \David{Don't we also need to note that $f^*\mu$ is compatible with the rational structures?}
\end{proof}
...
\begin{proof}
  ...
  $\Ext^1_{\ZZ}(X/X^{\Frob{\Fq}}, \EEx) = 0$.  The long exact 
  sequence obtained by applying the functor
  $\Hom(\mbox{---}, \EEx)$ to
  \[
  \begin{tikzcd}
  ...
  \end{tikzcd}
  \]
  gives the existence of an $\tilde{f} \in \Hom(X, \EEx)$ restricting to
  ...
  for $x \in X$.  In fact, we may replace the construction of $\psi$
    \Clifton{I'm not sure I understand what you mean here.}
