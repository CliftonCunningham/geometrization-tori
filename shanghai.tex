...
The functor $\CS(G) \to \CS(G(\bFq))$ defined by $(\gcs{L}, \mu, \phi) \mapsto (\gcs{L}_x = x^*\gcs{L}, \mu_{x,y} = (x \times y)^* \mu, x^*\phi)$ is easily seen to be an equivalence.  We will use this equivalence implicitly in proving the following proposition.
