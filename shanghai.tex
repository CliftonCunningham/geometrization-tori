...
This functor is full and faithful; see \cite{SGA7.2}*{Expos\'e XIII} or even \cite{BBD}*{Prop. 5.2.1}.
...
\begin{proof}
 ...
 It follows immediately that 
  \[
  L_G  \ceq  L_{W}\vert_{\CS_b(G)} : \CS_b(G) \to \CS_0(G)
  \]
...
\begin{proof}
 %Let $G$ be a geometrically connected algebraic group over $\Fq$.
  ...
  $
  b_G^* : \Hom(m^*\cs{L}_0,\cs{L}_0\boxtimes\cs{L}_0) \to \Hom({\bar m}^*\gcs{L},\gcs{L}\boxtimes\gcs{L})
  $ 
  is a bijection. Let $\mu_0 : m^*\cs{L}_0 \to \cs{L}_0\boxtimes\cs{L}_0$ be the isomorphism matching 
  $\mu : {\bar m}^*\gcs{L} \to \gcs{L}\boxtimes\gcs{L}$, from the definition of $\cs{L}$. 
  Then $(\cs{L}_0,\mu_0)$ is an object in $\CS_0(G)$ 
  and $\cs{L} = (\gcs{L},\mu,F)$ is isomorphic to $(b_G^*\cs{L}_0,b_G^*\mu_0)$ in $\CS(G)$.
  Thus, the full and faithful functor $b_G^* : \CS_0(G) \to \CS(G)$ is also essentially surjective,
  hence an equivalence (when $G$ is a connected, commutative algebraic group over $\Fq$).
  
  In Proposition~\ref{prop:bounded} we saw that $\CS_f(G)$ is a full subcategory of $\CS_0(G)$;
  ...
  Let $(\cs{L}_0,\mu_0)$ be a bounded character sheaf. 
