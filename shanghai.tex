...
$\TT \otimes \OK/\pK^{d+1}$; the commutative group scheme $\GN{T}_d$
...
For each $d\in \NN$, let $\TT_{d}$ be the smooth group scheme over
$\OK/\pK^{d+1}$ obtained from $\TT$ by base change along $\OK \to
\OK/\pK^{d+1}$. Observe that $\OK/\pK^{d+1}$ is an Artin ring and
$\TT_{d}$ is locally of finite type over $\OK/\pK^{d+1}$.
...
\begin{theorem}\label{thm:application}
  ...
  Let $\GN{T}_d$ and $\GN{T}$ be as above.  For each $d\in \NN$, the
...
\begin{proof}
  ...
  define depth has the property that $\TT(\OK/\pK^{d+1}) \cong T(K) / T(K)_d$
  \cite[Prop 5.2]{Yu}.  Thus characters of $\GN{T}_d(\Fq)$ are
