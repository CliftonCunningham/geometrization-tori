...
For every non-negative integer $n$,  set $R_n = R/\m^{n+1}$.

Suppose $X_K$ is a connected commutative algebraic group over $K$ that admits a Néron model $X$ over $R$.
%
A {\it quasicharacter of $X_K(K)$} is a group homomorphism $X_K(K) \to \EEx$ that factors through $X_K(X) \to X(R_n)$, for some $n\in\NN_0$;
