...
We will prove this proposition at the end of Section \ref{sec:pullback}
...
Using these results on pullbacks we may now prove that Trace of Frobenius is a natural transformation.
...
of admissible characters of smooth commutative group schemes over local fields that admit Néron models.

Let $R$ be a complete discrete valuation ring $R$ with residue field $\Fq$.
...
Suppose $X_K$ is a connected commutative algebraic group over $K$ that admits a Néron model $X$ over $R$.
%
A {\it quasicharacter of $X_K(K)$} is a group homomorphism $X_K(K) \to \EEx$ that factors through $X_K(X) = X(R)\to X(R_n)$, for some $n\in\NN_0$;
...
Let $X_K$ be a commutative algebraic group over $K$ with Néron model $X$ over $R$. 
\begin{enumerate}
...
canonically.
\end{enumerate}
So Theorem~\ref{thm:QCX} now follows from the fact that each $\QC(\Gr^R_n(X))$ is a full subcategory of $\QC(\Gr_R(X))$ and the observation that $\QC(\Gr_R(X))$ a `union' of the subcategories $\QC(\Gr^R_n(X))$ in $\mathcal{C}(\Gr_R(X))$.
\end{proof}

\subsection{Group schemes that admit a Neron model}
...
It seems likely that there connected commutative algebraic groups over local fields that admit Neron models but are neither algebraic tori nor Abelian varieties; indeed, it is conjectured that K-wound unipotent group schemes over local fields like $\Fq((t))$ are examples. We have use the ponderous phrase "let $X_K$ be a connected commutative algebraic group over $K$ that admits a Néron model $X$" in Section~\ref{sec:applications} to draw attention to the fact that our theory of quasicharacter sheaves is applicable to more than algebraic tori and Abelian varieties.
...
Let $X_K$ and $X'_{K'}$ be connected commutative algebraic groups over $K$ and $K'$, respectively, that admit Néron models $X$ and $X'$ over $R$ and $R'$, respectively.
