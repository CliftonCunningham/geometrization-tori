...
Set $R_n = R/\m^{n+1}$ and $S_n = \Spec{R_n}$ and $S = \Spec{R}$; 
...
\]
for the functor produced by composing pullback along $S_n \to S$ with the Greenberg functor. This functor respects \'etale morphisms, smooth morphisms and geometric components, \todo{open immersions? closed immersion?} and
\[
\Gr_n(X)(\Fq) \iso X(R_n),
\]
...
natural transformation $\Gr_{m} \to \Gr_n$ between additive functors.
Crucially, $(\Gr_{m}\to \Gr_n)(X)$ is an affine morphism of $\Fq$-schemes, for every $R$-scheme $X$ and every $n\leq m$ \cite{bertrapelle-gonzales:Greenberg}*{Prop 4.3}.
This observation is key to the proof that, for any $X$ lft over $S$, the projective limit 
\[
\Gr(X) \ceq \varprojlim_{n\in \NN_0} \Gr_n(X)
\]
taken with respect to the surjective morphisms $(\Gr_{m}\to \Gr_n)(X)$,
...
\]
The Greenberg transform $\Gr$ respects \'etale morphisms, smooth morphisms  \todo{open immersions? closed immersion?} and geometric components, and
\[
\Gr(X)(\Fq) \iso X(R),
\]
canonically, for any $X$ lft over $R$.
...
Now suppose $X_K$ is a commutative algebraic group over $K$ 
that admits a Neron model, $X$.
...
A {\it quasicharacter sheaf for $X_K$} is a triple
$\qcs{F}\ceq (\gqcs{F},\mu_\qcs{F},\phi_\qcs{F})$ where
\begin{enumerate}
\item[(0)] $\gqcs{F}$ is a constructible $\ell$-adic sheaf on $\Gr(X)_{\bFq} \ceq \Gr(X)\otimes \bFq$,
\item[(1)]  $\mu_\qcs{F}: \bm^* \gqcs{F} \to \gqcs{F}\boxtimes\gqcs{F}$ is an isomorphism $\ell$-adic sheaves, where $\bm : \Gr(X)_{\bFq} \times \Gr(X)_{\bFq} \to \Gr(X)_{\bFq}$ is multiplication for $\Gr(X)_{\bFq}$,
\item[(2)] $\phi_\qcs{F} : \Frob{\Gr(X)}^* \gqcs{F} \to \gqcs{F}$ is an
  isomorphism of constructible $\ell$-adic sheaves, where $\Frob{\Gr(X)}^* : \Gr(X)_{\bFq}  \to \Gr(X)_{\bFq}$ is Frobenius for $\Gr(X)_{\bFq}$,
\end{enumerate}
such that, for some $n\in \NN_0$,
\begin{enumerate}
\item[(3)]
$\qcs{F} = P_n(X)^*\qcs{L}$ for some quasicharacter sheaf $\qcs{L}$ on $\Gr_n(X)$, where pull-back by $P_n(X)$ is defined as in Proposition~\ref{prop:pullback}.
\end{enumerate}

\begin{theorem}
Let $X_K$ be a commutative algebraic group over $K$ with Neron model $X$ over $R$. 
\begin{enumerate}
\item[(0)] Quasicharacter sheaves for $X_K$ form a rigid monoidal category, $\QC(X_K)$, in a natural way. 
\item[(1)] There is a canonical short exact sequence of abelian groups 
\[
0 \to \Hh^2(\pi_0(X)_{\bFq},\EEx)^{\Frob{}} \to \QCiso{X_K} \to \Hom_\text{ad}(X_K(K),\EEx) \to 0
\] 
where $\Hom_\text{ad}(X_K(K),\EEx)$ is the group of characters of $X_K(K)$ that factor through $X(R)\to X(R_n)$, for some $n\in\NN_0$.
