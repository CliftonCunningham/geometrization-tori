...
When commutative, it is not difficult to relate Frobenius-stable character sheaves to our character sheaves (Remark~\ref{rem:Lusztig}). The new features that we have found pertaining to Weil sheaves and $\Hh^2(\pi_0(\bG),\EEx)^{\Weil{}}$ do not arise in that context because, for such groups, Weil sheaves are unnecessary (\S \ref{ssec:alg_groups} and $\Hh^2(\pi_0(\bG),\EEx)^{\Weil{}}=0$ (Remark \ref{rem:H2}).
...
On the other hand, if $\brho_{\bar e}$ is non-trivial then the second diagram in \ref{CS.4}  implies that $\brho_{\bg}$ is non-trivial for all $\bg$ and thus an isomorphism, since the stalks of character sheaves are one-dimensional; in this case $\rho$ is an isomorphism.
\end{proof}
\fi
...
any homomorphism $b : \bG \to \EEx$ will satisfy \eqref{eq:nohom}, and the corresponding $\alpha \oplus \beta$ are non-cohomologous in $\Hh^2(E_G^\bullet)$.
