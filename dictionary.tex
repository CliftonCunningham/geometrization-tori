...
giving functorial versions of the restriction and norm homomorphisms $G(k')^* \to G(k)^*$ and $G(k)^* \to G(k')^*$
...
%of étale commutative group schemes,
...
%Note that Proposition~\ref{prop:autornaught_etale} tells us that if $G$ is an étale commutative group scheme over $\Fq$ then every hom-set is either trivial or $\{ 0 \} \cup \left(G(\bFq)_{\Weil{}}\right)^*$, non-canonically.
...
In this section we consider connected commutative algebraic groups over $K$ that admit a Néron model, by which we mean an lft Néron model.
...
\subsection{Quasicharacter sheaves on the Greenberg transform of a Néron model} \label{ssec:qc_on_GN}
...
The following statements follow directly from Lemma~\ref{lem:autornaught}, Proposition~\ref{prop:functorialG}, Theorems~\ref{thm:snake} and \ref{thm:autornaught}, properties of Néron models and our observations about the component group of $\Gr^R_n(X)$.
\begin{enumerate}
...
Let $X'$ be a Néron model for $X_{K'}$.
