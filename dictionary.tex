...
and also keep track of the isomorphisms \eqref{introbox} (existence of such an isomorphism is not enough), leading to the notation of {\it quasicharacter sheaves on $G$}. 
...
is a rigid monoidal category and that the trace of Frobenius morphism provides an isomorphism
\begin{equation}
\QCiso{G} \to \Hom(G(\Fq), \EEx).
\end{equation}
In this way we find the geometrization and categorification of characters of $G(\Fq)$.

A simple example will illustrate some of the issues that arise when extending the sheaf--function dictionary to disconnected algebraic groups over finite fields. Consider the etale group scheme $G = \ZZ_{\Fq}$ over $\Fq$; this is a $\QC$-scheme.
Characters of $G(\Fq) = \ZZ$ are neither finite nor bounded, in general; for example, the $\ell$-adic character $\ZZ \to \EEx$ defined by $1 \mapsto 1+ \ell$ is not finite but bounded, which the character $\ZZ \to \EEx$ defined by $1 \mapsto \ell$ is unbounded.  As we show in the paper, local systems on $G$ cannot be used to geometrize either of these characters; however, quasicharacter sheaves do the job, handily. We remark that such characters arise when considering unramified quasicharacters of the $p$-adic group $\Qp^\times$. This example informs our choice of the term ``quasicharacter sheaf''. Indeed, in \cite{cunningham-roe:p-adic_tori} we use this paper to geometrize and categorify all quasicharacters of all algebraic tori over all local fields. 

We also want to situate the choice of term ``quasicharacter sheaf'' in a historical context.
Some people refer to local systems satisfying \eqref{introbox} as as character sheaves; 
