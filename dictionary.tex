...
In general, the defect in the dictionary for characters of commutative algebraic groups over finite fields may be addressed with the following observation: if $(\cs{E},\mu_\cs{E})$ and $(\cs{E}',\mu_\cs{E}')$ determine the same character of $G(\Fq)$ then $\cs{E}\iso \cs{E}'$ as local systems on $G$.
...
We begin a study of the category $\CS(G)$ of character sheaves on $G$ by returning to the case when $G$ is a connected commutative algebraic group over $\Fq$ and thus by revisiting Deligne's function-sheaf dictionary (\S\ref{sec:category}).
...
a relationship with the Hochschild-Serre spectral sequence (\S\ref{ssec:E}) for $\Weil{} \ltimes \bG$, where $\Weil{} \subset \Gal(\Fq/\Fq)$ is the Weil group for $\Fq$.
...
One can show, using the K\"unneth formula, that $\Hh^2(\bG,\EEx)$ is non-trivial unless $\bG$ is finite cyclic.
...
\subsection{Descent, revisited}

\begin{corollary}\label{cor:bounded-etale}
Let $G$ be a commutative etale group scheme over $\Fq$.
Then the image of $\bCS(G)$ under $\TrFrob{G} : \CS(G) \to G(\Fq)^*$ is $\Hom(\bG,\mathbb{\bar Z}_{\ell}^\times)$.
\end{corollary}

\begin{proof}
Let $\cs{L}$ be a character sheaf on $G$.
Using Proposition~\ref{prop:SGiso}, let $[\alpha \oplus \beta \oplus 0]\in \Hh^2(E^\bullet_G)$ be the corresponding class and let  $a : \bG \times \bG \to \EEx$ and $b : \bG \to \EEx$ be multiplicative versions of $\alpha$ and $\beta$, following the notation at the beginning of this section.
Then $\trFrob{\cs{L}}(x) = b(x)$ for $x\in G(\Fq)$.
To prove the corollary we will show that $\cs{L}$ descends to $G$ if and only if $b(x)\in \mathbb{\bar Z}_{\ell}^\times$ for every $x\in G(\Fq)$.

The identity component $G^0$ of $G$ may be identified with $\Spec{\Fq}$, 
By \cite{beilinson-bernstein-deligne:81a}*{Rem. 5.1.2},
$\cs{L}$ descends to $G$ if and only if $b(x) \in \EEx$ is bounded, for all $x$.  
...
for \'etale group schemes $G$, which is done in Corollary~\ref{cor:bounded-etale}.
\end{proof}
