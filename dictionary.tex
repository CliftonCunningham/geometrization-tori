...
%% Categories
\newcommand{\lQC}{{\mathcal{Q\hskip-0.8pt C}}}
\newcommand{\lQCiso}[1]{\lQC(#1)_{/\text{iso}}}
\newcommand{\QC}{{\lQC_{\text{rig}}}}
\newcommand{\QCb}{{\lQC_0}}
\newcommand{\QCf}{{\lQC_f}}
...
A \emph{quasicharacter sheaf} on $G$ is a Weil sheaf $(\gqcs{L},\phi)$ on $G$
...
is a functorial isomorphism from isomorphism classes of quasicharacter sheaves on $G$
...
\todo{The bottom arrow is an isomorphism to $G(k')^*_{\Gal(k'/k)}$. We will see that the horizontal arrows are isomorphisms on quasicharacter sheaves. Can we make sense of $\QC(G_{k'})_{\Gal(k'/k)}$? If so, is the functor $\nu_{k'/k}$ into $\QC(G_{k'})_{\Gal(k'/k)}$ an equivalence of categories?}
...
In this section use that perspective to find a perfect dictionary between quasicharacter sheaves on $G$ and characters of $G(\Fq)$.

\subsection{Quasicharacter sheaves}\label{sec:loose}
...
We form a category from quasicharacter sheaves by declaring that a morphism $(\gqcs{L},\phi) \to (\gqcs{L'},\phi')$ is a sheaf homomorphism $\alpha : \gqcs{L} \to \gqcs{L'}$ for which the two diagrams in Condition~\ref{QC.4} commute, for some choice of $\mu$, $\mu'$ such that $(\gqcs{L},\mu,\phi)$ and $(\gqcs{L'},\mu',\phi')$ are quasicharacter sheaves on $G$.
The category of quasicharacter sheaves on $G$ is denoted by $\lQC(G)$.
\end{definition}

The category of quasicharacter sheaves on $G$ is rigid monoidal and the functor $\QC(G) \to \lQC(G)$ is monoidal and essentially surjective but neither full nor faithful, generally.
%
...
and let $(\gqcs{L},\phi)$ be a quasicharacter sheaf on $G$ with $\trFrob{(\gqcs{L},\phi)}=1$.
...
the quasicharacter sheaf on $G$ corresponding, under Theorem~\ref{thm:snake},
to the character $1 \mapsto \ell$ is not a bounded quasicharacter sheaf,
while the quasicharacter sheaf on $G$ corresponding
