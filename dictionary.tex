...
Returning to the case of connected commutative algebraic groups $G$ over $\FF_q$, we confirm that, in this case, the category of quasicharacter sheaves on $G$ is equivalent to the category of characters $\psi$ of $G(\FF_q)$, the equivalence being given by pullback by the Lang isogeny for $G$. 
%Indeed, one may show that isomorphism classes of quasicharacter sheaves on $G$ are determined by classes of extensions of $\bG$ by $\EEx$ which are fixed by Frobenius, given by $\Hh^2(\bG,\EEx)^{\Gal({\bar\FF}_q/\FF_q)}$.
%
...
%This observation makes plain the fact that w
...
As indicated above, our interest in the function--sheaf dictionary for smooth commutative group schemes over finite fields comes from an application to $p$-adic representation theory. We found, however, that this application applies to all local fields $K$ with finite residue field $\Fq$ and all connected commutative algebraic groups $X_K$ over $K$ that do not contain a subgroup of type $\mathbb{G}_\text{a}$. Such algebraic groups $X_K$ always admit a Neron model $X$ over $R$, the ring of integers of $K$.
In this paper we show that quasicharacters of $X_K$ are geometrized and categorified by quasicharacter sheaves on the
Greenberg transform $\Gr_R(X)$ of the N\'eron model $X$; although not smooth, $\Gr_R(X)$ is a commutative group scheme over $\Fq$ and it is a projective limit of smooth commutative group schemes $\Gr^R_n(X)$.
