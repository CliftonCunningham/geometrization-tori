...
Characters of $G(\Fq) = \ZZ$ are neither finite nor bounded, in general. For example, the $\ell$-adic character $\ZZ \to \EEx$ defined by $1 \mapsto 1+ \ell$ is bounded but not finite, while the character $\ZZ \to \EEx$ defined by $1 \mapsto \ell$ is unbounded.  As we show in the paper, local systems on $G$ cannot be used to geometrize either of these characters; however, quasicharacter sheaves do the job, handily. We remark that such characters arise when considering unramified quasicharacters of the $p$-adic group $\Qp^\times$. This example informs our choice of the term ``quasicharacter sheaf''. Indeed, in \cite{cunningham-roe:p-adic_tori} we use this paper to geometrize and categorify all quasicharacters of all algebraic tori over all local fields. 
