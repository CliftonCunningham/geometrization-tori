...
every $\alpha\in \Hom(\qcs{L},\qcs{L}')$ is either trivial or an isomorphism. Moreover, the trace map induces an isomorphism of groups
\[
...
Having found $\QCiso{G}$ and $\Hom(\qcs{L},\qcs{L}')$ when $G$ is an etale commutative group scheme over $\Fq$, we now wish to find these for all commutative group scheme over $\Fq$. The next section introduces a concept that will be helpful in that regard.
...
\begin{definition}
Let $\bQC(G)$ be the category of pairs $(\qcs{L}_0,\mu_0)$
...
\begin{proof}
 ...
 This construction defines the functor $B_G : (\qcs{L}_0,\mu_0) \mapsto (b_G^*\qcs{L}_0,b_{G\times G}^*\mu_0, \phi)$
 and also shows that it is full and faithful.
\end{proof}

We will identify $\bQC(G)$ with its image under $B_G$
and say that a quasicharacter sheaf $\qcs{L} \in \QC(G)$ \emph{descends to a bounded quasicharacter sheaf} on $G$
...
However, if $G$ is connected, the categories are equivalent, as we now show.

\begin{proposition}\label{prop:bounded_connected}
If $G$ is a connected commutative algebraic group over $\Fq$ then
$B_G : \bQC(G) \to \QC(G)$ is an equivalence of categories.
\end{proposition}
...
\subsection{Objects, when $G$ is connected and of finite type}\label{sec:connected}

\begin{proposition}\label{prop:obj_connected}
 If $G$ is a connected, commutative algebraic group over $\Fq$ then
 \[
...
\begin{proof}
At its heart, this is Deligne's function--sheaf dictionary for connected commutative algebraic groups over finite fields, as we now briefly recall from \citelist{\cite{deligne:SGA4.5}*{Sommes trig.}, \cite{laumon:87a}*{1.1.3}}.

We begin by showing that $\TrFrob{G} : \QCiso{G} \to G(\Fq)^*$ is surjective.
Let $\chi : G(\Fq) \to \EEx$ be a character.
Let $\Lang : G \to G$ be the Lang isogeny for $G$.
Set $\qcs{L}_0 = ( \Lang_* \EE)_{\chi^{-1}}$, the $\chi^{-1}$-isotypic component of the push-forward of the constant sheaf on $G$ along the Lang isogeny.
Let $\gqcs{L}$ be the local system on $\bG$ obtained by pull-back along $\bG \to G$.
Then $\gqcs{L}$ comes equipped with an isomorphism $\phi : \Frob{G}^* \gqcs{L} \to \gqcs{L}$.
As explained in \cite{laumon:87a}*{1.1.3}, the trace of the Weil sheaf $(\gqcs{L},\phi)$ is $\chi$.
The sheaf $\qcs{L}_0$ also comes equipped with an isomorphism 
$\mu_0 : m^*\qcs{L}_0 \to \qcs{L}_0\boxtimes \qcs{L}_0$ 
\cite{laumon:87a}*{1.1.3.2},
which satisfies the analogue of \ref{QC.2} over $G$.
Writing $\mu : \bm^* \gqcs{L}  \to \gqcs{L} \boxtimes \gqcs{L}$ for the isomorphism obtained by base change, it follows immediately that $\mu$ satisfies conditions \ref{QC.2} and \ref{QC.3}.
Thus, $\qcs{L}\ceq (\gqcs{L},\mu,\phi)$ is a quasicharacter sheaf on $G$ with $\trFrob{\qcs{L}} = \chi$.
It follows that $\TrFrob{G} : \QCiso{G} \to G(\Fq)^*$ is surjective.


Now, let $\qcs{L}$ and $\qcs{L}'$ be quasicharacter sheaves on $G$ with $\trFrob{\qcs{L}} = \trFrob{\qcs{L}'}$. We will show $\qcs{L} \iso \qcs{L}$.
We have just seen that $\qcs{L}$ and $\qcs{L}'$ descend to bounded quasicharacter sheaves on $G$: $B_G(\qcs{L}_0,\mu_0) \iso \qcs{L}$ and $B_G(\qcs{L}'_0,\mu'_0) \iso \qcs{L}'$.
Since bounded quasicharacter sheaves enjoy property \cite{laumon:87a}*{1.1.3.3}, \todo{Justify} it follows (arguing as in \cite{laumon:87a}*{1.1.2.1}) that there is an isomorphism $\qcs{L}_0 \to \qcs{L}'_0$. This morphism is compatible with $\mu_0$ and $\mu'_0$, \todo{Justify} hence a morphism of bounded quasicharacter sheaves.
Its image under $B_G$ is the isomorphism $\qcs{L} \to \qcs{L}'$ of quasicharacter sheaves we seek.
\end{proof}
...
\todo{rephrase objective of section}
...
%Let $(\gqcs{E},\phi')$ be a quasicharacter sheaf on $G$ with $\trFrob{(\gqcs{E},\phi')}=1$.
...
\subsection{Morphisms, in the general case}
...
In this section we show that $\fQC(G)$ is a full subcategory of $\bQC(G)$.
...
Each finite quasicharacter sheaf $(f,\psi)$ in $\fQC(G)$ determines a
canonical isomorphism $\mu(f,\psi) : m^*(f,\psi) \to (f,\psi)\boxtimes(f,\psi)$ in $\fQC(G\times G)$.
 \end{lemma}
...
$\fQC(G\times G)$, the resulting isomorphism,
...
However, after passing from $C_1(G\times G)$ to $\fQC(G\times G)$, the resulting isomorphism 
$\mu(f,\psi): m^*(f,\psi) \to (f,\psi)\boxtimes (f,\psi)$ in $\fQC(G\times G)$ is independent of the choice of basis for $V$.
 \end{proof}

\begin{proposition}\label{prop:bounded}
 $\fQC(G)$ is a full subcategory of $\bQC(G)$.
\end{proposition}

\begin{proof}
 To prove the proposition we exhibit a full and faithful monoidal functor $L_G : \fQC(G) \to \bQC(G)$.
...
let $L(f,\psi)$ be the local system on $G$ given by the $\psi$-isotypic component of the local system $f_* V_H$ on $G$.
 ...
 By Lemma~\ref{lemma:finite-iso}, each $(f,\psi) \in \fQC(G)$ canonically determines
 an isomorphism
 \[
 ...
 \[
 L_G  \ceq  L_{W}\vert_{\fQC(G)} : \fQC(G) \to \bQC(G),
 \]
