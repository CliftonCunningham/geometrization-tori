...
As we show in this paper, if the component group scheme $\pi_0(G)$ (which is an étale commutative group scheme over $\FF_q$) is not cyclic, then while the trace of Frobenius provides a surjection from isomorphism classes in the category of pairs $(\qcs{E},\mu_\qcs{E})$ to characters of $G(\FF_q)$, that group homomorphism need not be injective. 
...
While this might appear to be a defect in the dictionary for characters of commutative algebraic groups over finite fields, it is not a serious defect since if $(\qcs{E},\mu_\qcs{E})$ and $(\qcs{E}',\mu_\qcs{E}')$ determine the same character of $G(\FF_q)$, then $\qcs{E}\iso \qcs{E}'$ as local systems on $G$.
...
\begin{equation}
0 \to \Hh^2(\pi_0(\bG),\EEx)^{\Gal({\bar\FF}_q/\FF_q)} \to \QCiso{G} \to \Hom(G(\FF_q),\EEx) \to 0.
\end{equation}
We remark that $\Hh^2(\pi_0(\bG),\EEx)^{\Gal({\bar\FF}_q/\FF_q)}$ is non-trivial even for some commutative algebraic groups.
...
\begin{equation}
\Aut(\qcs{L}) \iso \Hom(G(\bFq)_{\Gal(\bFq/\Fq)},\EEx),
\end{equation}
...
In this paper we show that quasicharacters of $p$-adic tori $T$ over local fields $K$ are geometrized and categorified by quasicharacter sheaves on the
Greenberg transform of the N\'eron model of $T$, which is a commutative group scheme over the residue field of $K$, denoted by $\Gr(\underline{T})$.
While $\Gr(\underline{T})$ is not smooth, it is a projective limit of smooth commutative group schemes $\Gr_n(\underline{T})$, and this allows us to adapt the work above to construct a category $\QC(\Gr_R(\underline{T}))$ of quasicharacter sheaves on $\Gr(\underline{T})$.
The ability to generalize the function--sheaf dictionary to non-connected group schemes, of which the component group of the special fibre of the Néron model of a torus is an important example, plays a crucial role in this application
since these group schemes are not connected in general.
This paper gives a canonical short exact sequence 
\begin{equation}
0 \to \Hh^2(X_*(T)_{\mathcal{I}_K},\EEx)^{\Frob{}} \to \QCiso{\Gr_R(\underline{T})} \to \Hom_\text{qc}(T(K),\EEx) \to 0
\end{equation}
and shows that automorphism groups in the $\QC(\Gr_R(\underline{T}))$ are given, for every quasicharacter sheaf $\qcs{F}$ on $\Gr_R(X)$, by
\begin{equation}
\Aut(\qcs{F}) \iso (\check{T}_\ell)^{\Weil{K}}
\end{equation}
where $\Weil{K}$ is the Weil group for $K$ and $\check{T}_\ell$ is the $\ell$-adic dual torus to $T$.

Joseph Bernstein has suggested to us that we should not interpret the fact that the quasicharacter sheaves on $\Gr(\underline{T})$ do not correspond exactly to quasicharacters of $T(K)$ as an imperfection. Rather, he has suggested that the category $\QC(\Gr(\underline{T})$ may be more closely tied to characters of $T(K)$ and its pure rational forms in the sense of \cite{vogan:93a}. A better understanding of 
$\Hh^2(X_*(T)_{\mathcal{I}_K},\EEx)^{\Frob{}}$ should provide insight into this suggestion.
...
The interesting new features that we have found pertaining to Weil sheaves and the non-triviality of $\Hh^2(\pi_0({\bar G}),\EEx)^{\Gal({\bar\FF}_q/\FF_q)}$ do not arise in this context.
...
Section~\ref{sec:etale} is devoted to the left-most vertical morphism in \eqref{eq:intro-snake}
In Section~\ref{sec:basechange} we study how quasicharacter sheaves behave under base change
and Weil restriction.
Sections~\ref{sec:descentG} and \ref{sec:finite} describe bounded quasicharacter sheaves and finite quasicharacter sheaves, respectively,
...
\subsection{Objects, when $G$ is etale}\label{sec:SandT}
...
Before turning our attention to quasicharacter sheaves on connected commutative algebraic groups in Section~\ref{sec:connected}, we treat two special classes of quasicharacter sheaves on arbitrary smooth commutative group schemes $G$:  in Section~\ref{sec:descentG}, those that descend from local systems on $\bG$ to local systems on $G$; and in Section~\ref{sec:discrete_isogenies}, those that are defined by discrete isogenies onto $G$. The latter will play a role in Sections~\ref{sec:connected} and \ref{sec:restriction}.

\subsection{Descent to G}\label{sec:descentG}
...
%
 ...
 % 
  ...
  Then, as in Section~\ref{sec:descentG}, $(\qcs{E},\mu_\qcs{E})$ is an object in $\bQC(G)$
  and $\qcs{L} \ceq (\gqcs{L},\mu,\phi)$ is isomorphic to $(b_G^*\qcs{E},b_{G\times G}^*\mu_\qcs{E})$ in $\QC(G)$.
  Thus, the full and faithful functor $B_G : \bQC(G) \to \QC(G)$ from Section~\ref{sec:descentG}
...
in Sections~\ref{sec:SandT} and \ref{sec:connected}, respectively.
...
We now wish to use the apply the results of Section~\ref{sec:connected} to the identity component of $G$; to do so, we must know that the identity component of $G$ is actually an algebraic group over $\Fq$.
