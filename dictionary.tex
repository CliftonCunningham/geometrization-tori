...
Now suppose $X_K$ is a connected commutative algebraic group over $K$ that admits a Néron model $X$ over $R$.
...
the group of quasicharacters of $X_K(K)$ will be denoted by $\Hom_\text{qc}(X_K(K),\EEx)$.


Then $\Gr_R(X)$ is commutative group scheme over $\Fq$
...
$
as smooth group schemes over $\Fq$ and, by Proposition~\ref{prop:pullback} the isomorphism above determines an equivalence of categories
...
\Hom(X(R_m),\EEx) \iso \Hom(X'(R'_{m'}),\EEx).
\end{equation}
Recall that $\Hom(X(R_m),\EEx)$ is a subgroup of $\Hom_\text{qc}(X_K(K),\EEx)$ and $\Hom(X'(R'_{m'}),\EEx)$ is a subgroup of $\Hom_\text{qc}(X_K(K),\EEx)$.
...
\subsection{Algebraic tori and Abelian varieties over local fields}
