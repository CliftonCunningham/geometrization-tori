...
Unless noted otherwise, we place absolutely no conditions on the \'etale group scheme $\pi_0(G)$, though it is finitely generated as a group scheme.
...
Think about this: It follows from Proposition~\ref{prop:etale-iso} that, when $G$ is an \'etale commutative group scheme, a character sheaf $\cs{L} \in \CS(G)$ descends to $G$ if and only if it lies in the kernel of the trace of Frobenius. This is in stark contrast to the case of connected commutative algebraic groups over $\Fq$, when all character sheaves $\cs{L} \in \CS(G)$ descend to $G$ and the kernel of the trace of Frobenius is trivial.
