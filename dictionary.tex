...
\section{Recollections on character sheaves for connected algebraic groups}\label{ssec:connected}
...
\begin{proposition} \label{prop:etale-iso}
 If $G$ is \'etale then the map $S_G : \CSiso{G}\to \Hh^2(E^\bullet_G)$ induces an isomorphism of short exact sequences
\[
\begin{tikzcd}
...
\begin{theorem}\label{thm:snake}
  ...
  \TrFrob{G} : \CSiso{G} \to G(\Fq)^*
  \]
is surjective with kernel canonically isomorphic to $\Hh^2(\pi_0(\bG),\EEx)^{\Weil{}}$.
\end{theorem}
...
bounded.
If $\chi' : \ZZ \to \EEx$ is the character of $G(\Fq)$ determined by $\chi'(1) = 1+\ell$ and if $\cs{L}'$ corresponds to $\chi'$ under Theorem~\ref{thm:snake},
...
If $\chi'' : \ZZ \to \EEx$ is the character of $G(\Fq)$ determined by $\chi''(1) = \zeta$, a root of unity in $\EEx$, and if $\cs{L}''$ corresponds to $\chi''$ under Theorem~\ref{thm:snake},
