...
\end{align*}
is called \emph{Trace of Frobenius}.\todo{Does this really warrant a declared definition?
...
\end{equation}
which is independent of the choice of $\{ v_x \in \gqcs{L}_x \tq x\in G(\bFq)\}$.
...
This terminology will be justified later when\todo{Point somewhere.} we show that a non-trivial quasicharacter sheaf $\qcs{L}$ on $G$ is invisible if and only if $\trFrob{\qcs{L}}$ is trivial.
...
which we assemble now.\todo{This section is undergoing massive reconstruction. Don't read.}
%
...
%Now we see that the Trace of Frobenius is surjective.
...
\todo{Actually, the lemma is false.}
\end{proof}
...
In this section we show that \eqref{eq:pi2} is exact.\todo{I wonder if $(F_1,F_2)$, see \ref{prop:functorialG}, form an adjoint pair.}
...
\begin{theorem}\label{thm:snake}
  ...
  \TrFrob{G} : \QCiso{G} \to G(\Fq)^*
  \]
  is a functorial surjection with kernel $\Hh^0(\Weil{}, \Hh^2(\pi_0({\bar G}), \EE))$ and a canonical section.\todo{I wonder if $(F_1,F_2)$, see \ref{prop:functorialG}, form an adjoint pair. That seems to be what this theorem is saying.}
\end{theorem}
