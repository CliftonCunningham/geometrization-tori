...
A finite, \'etale, surjective morphism $H\to G$ of smooth group schemes over $\Fq$ for which the action of $\Gal(\bFq/\Fq)$ on the kernel is trivial is called a {\it discrete isogeny}, following \cite{kamgarpour:09a}*{\S 2.2}.
...
Since $\Hh^1(\bG,\EEx) = \Hom(\bG,\EEx)$, evaluation $\chi \mapsto \chi({\bar x}^i)$ defines $\Hh^1(\Fq,\Hh^1(\bG,\EEx)) \to \Hh^1(\Fq,\EEx)$. 
...
\begin{theorem}\label{thm:snake}
  If $G$ is a smooth commutative group scheme over $\Fq$ then
  $\TrFrob{G} : \CSiso{G} \to G(\Fq)^*$ is surjective, admits a section, and has kernel canonically isomorphic to $\Hh^2(\pi_0(\bG),\EEx)^{\Weil{}}$:
    \[
  \begin{tikzcd}
0 \arrow{r} & \Hh^2(\pi_0(\bG),\EEx)^{\Weil{}} \arrow{r} & \CSiso{G} \arrow{r}{\TrFrob{G}} & G(\Fq)^* \arrow{r} & 0. 
  \end{tikzcd}
