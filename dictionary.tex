...
\end{equation}
provided by the trace of Frobenius is surjective with kernel $\Hh^2(\bG,\EEx)^{\Gal({\bar\FF}_q/\FF_q)}$, canonically.
One can show, using the Kunneth formula, that $\Hh^2(\bG,\EEx)$ is non-trivial unless $\bG$ is cyclic.
The fact that $T_G$ is not an isomorohism should not be too troubling because if quasicharacter sheaves $(\gqcs{L}, \mu,\phi)$ and $(\gqcs{L}', \mu',\phi')$ determine the same character of $G(\FF_q)$ then $(\gqcs{L},\phi)$ and $(\gqcs{L}',\phi')$ are isomorphic as Weil sheaves.
...
In this section we find a complete characterization of quasicharacter sheaves on étale commutative group schemes over finite fields.
...
\subsection{Objects, when $G$ is étale}\label{sec:SandT}
