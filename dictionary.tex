...
Chasing through the diagram in Condition~\ref{QC.3} we see that
\begin{equation}\label{nohom}
f(x+y) = f(x) f(y) \frac{g(\Frob{G}(x),\Frob{G}(y))}{g(x,y)}
\end{equation}
...
In the meantime, we end this section with a simple property of $S_{G}: \QCiso{G} \to H^0(\Weil{},H^2(G(\bFq),\EEx))$.

\begin{lemma}
The function \[S_{G}: \QCiso{G} \to H^0(\Weil{},H^2(G(\bFq),\EEx))\] is a group homomorphism.
\end{lemma}
...
g_{\qcs{L}\otimes \qcs{L'}} = g_{\qcs{L}} \, g_\qcs{L'}
\]
for every $\qcs{L}, \qcs{L'} \in \QC(G)$.\todo{Under construction.}
\end{proof}
