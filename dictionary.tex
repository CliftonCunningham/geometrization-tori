...
In order to show that $\TrFrob{G} : \QCiso{G} \to G(\Fq)^*$ is injective we need to recall some facts about Weil sheaves.
%
  
Now, let $\qcs{L}$ and $\qcs{L}'$ be quasicharacter sheaves on $G$ with $\trFrob{\qcs{L}} = \trFrob{\qcs{L}'}$.\todo{The rest of the proof is under construction.}
\end{proof}
