...
\section{Quasicharacter sheaves on connected commutative algebraic groups}

Before turning our attention to quasicharacter sheaves on connected commutative algebraic groups in Section~\ref{sec:connected}, we being with two sections that treat two special classes of quasicharacter sheaves on arbitrary smooth commutative group schemes $G$: those that descend from local systems on $\bG$ to local systems on $G$ in Section~\ref{sec:bounded}; and those local systems on $G$ that are defined by discrete isogenies onto $G$ in Section~\ref{sec:discrete_isogenies}. The latter will play a role in Sections~\ref{sec:connected} and \ref{sec:restriction}.
