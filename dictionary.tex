...
\newcommand{\CSs}[1]{{\mathcal{#1}}}
\newcommand{\gCSs}[1]{{\mathcal{\bar #1}}}
\newcommand{\dualgcs}[1]{\gCSs{#1}^\dagger}
\newcommand{\dualcs}[1]{\CSs{#1}^\dagger}
%% Categories
...
\newcommand{\LxL}{\text{$\gCSs{L} \tight{0}{\boxtimes}{0} \gCSs{L}$}}
\newcommand{\ExE}{\text{$\CSs{E}\tight{0}{\boxtimes}{0}\CSs{E}$}}
\newcommand{\bExE}{\text{$\gCSs{E}\tight{0}{\boxtimes}{0}\gCSs{E}$}}
...
this paper extends Deligne's function-sheaf dictionary from connected commutative algebraic groups to smooth commutative group schemes over finite fields. 
...
m^* \CSs{E} \iso \CSs{E} \boxtimes \CSs{E},
\end{equation}
where $m : G\times G\to G$ is the multiplication map.
%
If one wishes to make a category from this class of local systems, one is led to consider morphisms $\CSs{E} \to \CSs{E}'$ of sheaves which are compatible with particular choices of \eqref{introbox} for $\CSs{E}$ and $\CSs{E'}$. 
{\it A priori}, the composition $\CSs{E} \to \CSs{E}' \to \CSs{E}''$ of two such morphisms need not be compatible with the choices of \eqref{introbox} for $\CSs{E}$ and $\CSs{E}''$.
...
If $G$ is a commutative algebraic group over $\Fq$ which is not connected, then the isomorphism \eqref{introbox} need not be unique. In order to track the choice of isomorphism, consider the category of pairs $(\CSs{E},\mu_\CSs{E})$ where $\CSs{E}$ is a rank-one local system on $G$ and $\mu_\CSs{E} : m^*\CSs{E} \to \CSs{E}\boxtimes\CSs{E}$ is a chosen isomorphism of local systems on $G\times G$. 
In this case, the trace of Frobenius provides an epimorphism from isomorphism classes of pairs $(\CSs{E},\mu_\CSs{E})$ to characters of $G(\Fq)$, but the epimomorphism need not be injective; consequently,
every character of $G(\Fq)$ may be geometrized as a pair $(\CSs{E},\mu_\CSs{E})$ but perhaps not uniquely in the category of pairs. 
While this might appear to be a defect in the dictionary for characters of commutative algebraic groups over finite fields, it is not serious: if $(\CSs{E},\mu_\CSs{E})$ and $(\CSs{E}',\mu_\CSs{E}')$ determine the same character of $G(\Fq)$ then $\CSs{E}\iso \CSs{E}'$ as local systems on $G$.
...
we simply replace the local system $\CSs{E}$ on $G$ with a Weil local system while retaining the extra structure $\mu_\CSs{E}$. 
In this way we are led to the category $\CS(G)$ of {\it quasicharacter sheaves} on $G$ (\S\ref{ssec:category}).
Objects in $\CS(G)$ are triples $(\gCSs{L}, \mu,\phi)$, where $\gCSs{L}$ is a rank-one local system on ${\bar G} := G\times_{\Spec{\Fq}} \Spec{\bFq}$
and $\phi : \Frob{G}^* \gCSs{L}\to \gCSs{L}$ and $\mu : {\bar m}^* \gCSs{L} \iso \gCSs{L} \boxtimes \gCSs{L}$ are isomorphisms of sheaves satisfying certain compatibility conditions.
...
Our key tools for understanding the trace of Frobenius in the \'etale case are a reinterpretation of $\CS(G)$ in terms of stalks (\S\ref{ssec:stalks}) and a
...
%The fact that $\TrFrob{G}$ may not be an isomorphism should not be too troubling: if quasicharacter sheaves $(\gCSs{L}, \mu,\phi)$ and $(\gCSs{L}', \mu',\phi')$ determine the same character of $G(\Fq)$ then $(\gCSs{L},\phi)$ and $(\gCSs{L}',\phi')$ are isomorphic as Weil sheaves.
...
case we show that every character sheaf can be described in both of these ways (\S\ref{ssec:connected}).  We use this fact to prove that $\TrFrob{G}$ is an isomorphism for connected commutative algebraic groups $G$
...
If $G$ is a smooth commutative group scheme over $\Fq$ then
\[
\Aut_{\CS{}}(\CSs{L}) \iso \Hom(\pi_0(\bG)_{\Weil{}},\EEx),
\]
for all quasicharacter sheaves $\CSs{L}$ on $G$.
\end{theorem*}
...
We further show that automorphism groups in $\CS(T_K)$ are given, for every quasicharacter sheaf $\CSs{F}$ for $T_K$, by
\[
\Aut_{\QCS{}}(\CSs{F}) \iso (\check{T}_\ell)^{\Weil{K}},
\]
...
$\CSs{L}\ceq (\gCSs{L},\mu,\phi)$ where:
\begin{enumerate}
\labitem{(CS.1)}{CS.1} $\gCSs{L}$ is a rank-one $\ell$-adic local system on $\bG$, by which we mean a constructible $\ell$-adic sheaf on $\bG$, {\it lisse} on each connected component of $\bG$, whose stalks are one-dimensional $\EE$-vector spaces;
\labitem{(CS.2)}{CS.2} $\mu: \bm^* \gCSs{L} \to \LxL$ is an isomorphism of
sheaves on $\bGxG$ such that the following diagram commutes,
  ...
  \bm_3^*\gCSs{L} \arrow{rr}{(\bm \tight{1}{\times}{2} \id)^*\mu} \arrow[swap]{d}{(\id \tight{2}{\times}{1} \bm)^*\mu}
    &&  \bm^*\gCSs{L} \boxtimes \gCSs{L} \dar{\mu \tight{0}{\boxtimes}{1} \id} \\
    \gCSs{L} \boxtimes \bm^* \gCSs{L} \arrow{rr}{\id \boxtimes \mu}
    &&  \gCSs{L} \tight{0}{\boxtimes}{0} \LxL
  \end{tikzcd}
  \]
\labitem{(CS.3)}{CS.3} $\phi : \Frob{G}^* \gCSs{L} \to \gCSs{L}$ is an
  ...
  \Frob{\GxxG}^* \bm^* \gCSs{L} \arrow{rr}{\Frob{\GxxG}^*\mu}
    && \Frob{\GxxG}^*(\LxL)\\
    \arrow[equal]{u} \bm^*  \Frob{G}^* \gCSs{L} \arrow[swap]{d}{\bm^* \phi}
    && \Frob{G}^*\gCSs{L}\boxtimes \Frob{G}^*\gCSs{L} \dar{\phi\boxtimes \phi} \arrow[equal]{u} \\
    \bm^*\gCSs{L} \arrow{rr}{\mu}
    && \LxL
  \end{tikzcd}
  \]
\end{enumerate}
...
\begin{enumerate}
\labitem{(CS.4)}{CS.4} if $\CSs{L} = (\gCSs{L},\mu,\phi)$ and
  $\CSs{L'} = (\gCSs{L'},\mu',\phi')$ are character sheaves on $G$ then
  a morphism $\rho : \CSs{L} \to \CSs{L}'$ is a map $\brho : \gCSs{L} \to \gCSs{L'}$
  of constructible $\ell$-adic sheaves on $\bG$ such that the following diagrams both commute.
  \[
  \begin{tikzcd}[column sep=40]
  \Frob{G}^* \gCSs{L} \rar{\Frob{G}^* \brho} \arrow[swap]{d}{\phi} & \Frob{G}^* \gCSs{L'} \dar{\phi'}
  & & \arrow[swap]{d}{\mu} \bm^* \gCSs{L} \rar{\bm^* \brho} & \bm^* \gCSs{L'} \dar{\mu'} \\
  \gCSs{L} \rar{\brho} & \gCSs{L'}
  & {} & \LxL \rar{\tight{1}{\rho\boxtimes \rho}{1}} & \gCSs{L'} \tight{0}{\boxtimes}{0} \gCSs{L'}
  \end{tikzcd}
  \]
\end{enumerate}
...
$\CSs{L} \otimes \CSs{L'}$ defined by $(\gCSs{L}\otimes\gCSs{L'}, \mu\otimes\mu', \phi\otimes \phi')$,
...
and the sets $\Hom(\CSs{L},\CSs{L}')$ in Theorem~\ref{thm:autornaught}; in this way we provide a complete description of the category $\CS(G)$.
%
In the meantime, we close this section with an elementary observation about $\Hom(\CSs{L},\CSs{L}')$.
...
If $\CSs{L}$ and $\CSs{L}'$ are character sheaves on $G$, then
every $\rho\in \Hom(\CSs{L},\CSs{L}')$ is either trivial or an isomorphism. 
\end{lemma}

\begin{proof}
Suppose $\rho \in \Hom(\CSs{L},\CSs{L}')$.
We prove the lemma by considering the linear transformations $\brho_{\bar g} : \gCSs{L}_{\bar g} \to \gCSs{L}_{\bar g}$ at the stalks above geometric points ${\bar g}$ on $G$ and showing that, either each $\brho_{\bar g}$ trivial or each $\brho_{\bar g}$ is an isomorphism.
...
Let $(\gCSs{L},\phi)$ be a Weil sheaf on $G$. Every $g\in G(\Fq)$
determines a geometric point $\bg$ fixed by $\Frob{G}$. 
Together with the canonical isomorphism $(\Frob{G}^*\gCSs{L})_{\bg} \iso  \gCSs{L}_{\Frob{G}(\bg)}$, the automorphism $\phi$ determines an automorphism $\phi_{\bg}$ of the $\EE$-vector space $\gCSs{L}_{\bg}$.
Let $\trace(\phi_{\bg};\gCSs{L}_{\bg})$ be the trace of $\phi_{\bg} \in \Aut_{\EE}(\gCSs{L}_{\bg})$ and let $\trFrob{(\gCSs{L},\phi)} : G(\Fq)\to \EE$ be the function defined by 
\begin{equation}\label{trWeil}
\trFrob{(\gCSs{L},\phi)}(g) \ceq \trace(\phi_{\bg};\gCSs{L}_{\bg}),
\end{equation}
commonly called the {\em trace of Frobenius of $(\gCSs{L},\phi)$}.
Note that if $(\gCSs{L},\phi) \iso (\gCSs{L'},\phi')$ as Weil sheaves, 
then $\trFrob{(\gCSs{L},\phi)} = \trFrob{(\gCSs{L'},\phi')}$ as functions on $G(\Fq)$.

Now suppose $\CSs{L} = (\gCSs{L},\mu,\phi)$ is a quasicharacter sheaf on $G$.
Then the isomorphism $\bm^* \gCSs{L} \iso \gCSs{L} \boxtimes\gCSs{L}$ and the diagram of
\ref{CS.3} guarantee
that the function $\trFrob{(\gCSs{L},\phi)} : G(\Fq)\to \EEx$ is a group homomorphism, which we will also denote by $\trFrob{\CSs{L}}$.  Moreover,
this homomorphism depends only on the isomorphism class of $\CSs{L}$, so we obtain a map
\begin{align*}
\TrFrob{G} : \CSiso{G} &\to G(\Fq)^*, \\
\CSs{L} &\mapsto \trFrob{\CSs{L}}.
\end{align*}
Since tensor products on the stalks of $\CSs{L}$ induce pointwise multiplication on the trace of Frobenius, $\TrFrob{G}$ is a group homomorphism.  We devote the remainder of this section to
...
\begin{lemma}\label{lem:pullback}
  ...
  \begin{align*}
  f^* : \CS(G) &\to \CS(H) \\
  (\gCSs{L},\mu,\phi) &\mapsto (\bf^*\gCSs{L},(\bfxf)^*\mu,\bf^*F)
  \end{align*}
...
\end{lemma}
\begin{proof}
  Let $\CSs{L}$ be a character sheaf on $G$. 
  ...
  to \ref{CS.2} for $\CSs{L}$ and use the canonical isomorphism
  $(\bfxf)^*(\LxL) \iso \bf^*\gCSs{L} \tight{-3}{\boxtimes}{-3} \bf^*\gCSs{L}$.
  To show that $f^*\CSs{L}$ satisfies
  \ref{CS.3}, apply the same functor to \ref{CS.3} for $\CSs{L}$.
...
$\boxtimes : (\CSs{L}_1,\CSs{L}_2)\to \CSs{L}_1\boxtimes\CSs{L}_2$ defines an equivalence of categories
\[
\boxtimes : \CS(G_1)\times \CS(G_2) \to \CS(G_1\times G_2)
\]
such that
\[
\begin{tikzcd}[column sep=60]
\arrow{d}{\TrFrob{G_1} \times \TrFrob{G_2}} \CSiso{G_1}\times \CSiso{G_2} \arrow{r}{\boxtimes}
...
\[\boxtimes
 : \CS(G_1)\times \CS(G_2) \to \CS(G_1\times G_2)
 \]
...
%
Now, let $\CSs{L} \ceq (\gCSs{L},\mu,\phi)$ be a character sheaf on $G$
and set $\CSs{L}_1 \ceq q_1^* \CSs{L}$ and $\CSs{L}_2 \ceq q_2^* \CSs{L}$.
By Lemma~\ref{lem:pullback}, $\CSs{L}_1$ is a character sheaf on $G_1$
and $\CSs{L}_2$ is a character sheaf on $G_2$.
We will obtain an isomorphism $\CSs{L} \iso  \CSs{L}_1\boxtimes \CSs{L}_2$.

Applying the functor $f^*$ to the isomorphism $\mu$ yields
\begin{equation}\label{eq:fm}
f^*\mu : f^* m^* \gCSs{L} \to f^*(\gCSs{L}\boxtimes \gCSs{L}) .
\end{equation}
We have already seen that $m\circ f = \id_G$, so $f^* m^* \gCSs{L} = \gCSs{L}$.  
Since $f^*p_1^*\gCSs{L} = r_1^* q_1^* \gCSs{L} = r_1^* \gCSs{L}_1$ and $f^*p_2^*\gCSs{L} = r_2^* q_2^* \gCSs{L} = r_2^*\gCSs{L}_2$,
we have 
\[
f^*(\gCSs{L}\boxtimes \gCSs{L})  = f^*p_1^*\gCSs{L}\otimes f^* p_2^*\gCSs{L} = \gCSs{L}_1\boxtimes \gCSs{L}_2.
\]
It follows that \eqref{eq:fm} gives an isomorphism $\gCSs{L} \to  \gCSs{L}_1\boxtimes \gCSs{L}_2$.
...
$\CSs{L} \to \CSs{L}_1\boxtimes \CSs{L}_2$ of quasicharacters sheaves on $G\times G$.
...
replacing the Weil sheaf $(\gCSs{L}, \phi)$ on $\bG$ in the definition of a character sheaf with an $\ell$-adic local system on $G$ itself.

\begin{definition}
Let $\bCS(G)$ be the category of pairs $(\CSs{E},\mu_\CSs{E})$
where $\CSs{E}$ is an $\ell$-adic local system on $G$ of rank-one,
equipped with an isomorphism $\mu_\CSs{E} : m^* \CSs{E} \to \CSs{E} \boxtimes \CSs{E}$
...
\begin{proof}
 Suppose $(\CSs{E},\mu_\CSs{E})$ in an object of $\bCS(G)$.
 Let $b_G : {\bar G} \to G$ be the pullback of $\Spec{\bFq} \to \Spec{\Fq}$ along $G\to \Spec{\Fq}$.
 Set $\gCSs{L} = b_G^* \CSs{E}$. 
 The functor $b_G^*$ takes local systems on $G$ to local systems on $\bG$.
 The local system $\gCSs{L}$ comes equipped with an isomorphism
 $\phi: \Frob{G}^* \gCSs{L} \to \gCSs{L}$.
 The resulting functor from local systems on $G$ to Weil local systems on $\bG$, given on objects by $\CSs{E} \mapsto (\gCSs{L},\phi)$, 
 is full and faithful; see \cite{deligne-katz:SGA7.2}*{Expos\'e XIII} and \cite{beilinson-bernstein-deligne:81a}*{Prop. 5.2.1}.
 The isomorphism $\mu \ceq b_{G\times G}^*\mu_\CSs{E}$ satisfies \ref{CS.2}
 for $\gCSs{L}$ and $\phi$ is compatible with $\mu$ in the sense of \ref{CS.3}.
 This construction defines the functor $B_G : \bCS(G) \to \CS(G)$ given on objects by $(\CSs{E},\mu_\CSs{E}) \mapsto (\gCSs{L},\mu, \phi)$, as defined here. 
...
We will say that a character sheaf $\CSs{L} \in \CS(G)$ \emph{descends to $G$} if it is isomorphic to some $B_G(\CSs{E}, \mu_\CSs{E})$.
...
\begin{proof}
Let $f$, $A$, $V$ and $\psi$ be as above and set $\CSs{E} = (f_! V_H)_\psi$.
Since $A$ is abelian, $\CSs{E}$ is an $\ell$-adic local system on $G$ of rank one.
We must show that $\CSs{E}$ comes equipped with an isomorphism $\mu_\CSs{E} : m^* \CSs{E} \to \CSs{E}\boxtimes\CSs{E}$.
...
In particular, $f^*\CSs{E}$ is the $A$-equivariant constant sheaf $V$ on $H$ with character $\psi$.
Since $f$ is a morphism of group schemes, the functor $f^*$ defines $\mu_\CSs{E} : m^*\CSs{E} \to \CSs{E}\boxtimes\CSs{E}$
...
%
  ...
  Now let $(\gCSs{L},\mu,\phi)$ be a character sheaf on $G$
  and let $\lambda : \Weil{}(G, \bg) \to \EEx$ be the character determined by $(\gCSs{L},\phi)$.
  ...
  $
  \lambda_g(\Frob{}) =  \trFrob{\CSs{L}}(g).
  $
%
  On the other hand, we have already seen that $\trFrob{\CSs{L}} : G(\Fq) \to \EEx$
  is a group homomorphism. 
  Since $G$ is an algebraic group over $\Fq$, $G(\Fq)$ is finite.
  Therefore $\trFrob{\CSs{L}}(g) = \lambda_g(\Frob{})$ is a root of unity
...
%
 ...
 % 
  ...
  $\CSs{E}$ be a local system on $G$ in the isomorphism class
  determined by this $\ell$-adic character of $\pi_1(G,\bg)$.
  Then $b_G^*\CSs{E} \iso \gCSs{L}$.
...
\cite{deligne-katz:SGA7.2}*{Expos\'e XIII} or \cite{beilinson-bernstein-deligne:81a}*{Prop. 5.2.1}),
 \[
  b_{G\times G}^* : \Hom(m^*\CSs{E},\CSs{E}\boxtimes\CSs{E}) \to \Hom({\bar m}^*\gCSs{L},\gCSs{L}\boxtimes\gCSs{L})
 \]
  is a bijection
  ...
  Let $\mu_\CSs{E} : m^*\CSs{E} \to \CSs{E}\boxtimes\CSs{E}$ be the isomorphism matching
  $\mu : {\bar m}^*\gCSs{L} \to \gCSs{L}\boxtimes\gCSs{L}$,
  the latter appearing in the definition of $\CSs{L}$.
  Then, as in Section~\ref{ssec:descentG}, $(\CSs{E},\mu_\CSs{E})$ is an object of $\bCS(G)$
  and $\CSs{L} \ceq (\gCSs{L},\mu,\phi)$ is isomorphic to $(b_G^*\CSs{E},b_{G\times G}^*\mu_\CSs{E})$ in $\CS(G)$.
...
\begin{proposition}\label{prop:connected}
 If $G$ is a connected, commutative algebraic group over $\Fq$ then:
 \begin{enumerate}
 \item $\TrFrob{G} : \CSiso{G} \to G(\Fq)^*$ is an isomorphism of groups;
 \item every character sheaf on $G$ is isomorphic to one defined by a discrete isogeny;
 \item $\Aut(\CSs{L}) = 1$, for all character sheaves $\CSs{L}$ on $G$.
 \end{enumerate}
 \end{proposition}
\begin{proof}
In Lemma~\ref{lem:bounded_connected}, we saw that every character sheaf $\CSs{L}$ on $\bG$ descends to $G$; let $\CSs{E}$ be an object of $\bCS(G)$ for which $B_G(\CSs{E}) \iso \CSs{L}$.
 Since the functor $B_G : \bCS(G) \to \CS(G)$ is full and faithful, $\Aut(\CSs{L}) = \Aut(\CSs{E})$.
...
Under this equivalence, local systems $\CSs{E}$ on $G$ arising from objects in $\bCS(G)$ are matched with $G(\Fq)$-equivariant constant local systems of rank-one on $G$, and therefore with one-dimensional representations of $G(\Fq)$. 
In the same way, pullback along the isogeny $\Lang\times\Lang : G\times G\to G\times G$ matches the extra structure $\mu_\CSs{E} : m^*\CSs{E} \to \CSs{E}\boxtimes\CSs{E}$ with an isomorphism $m^*V \to V\boxtimes V$ of one-dimensional representations of $G(\Fq)\times G(\Fq)$, which is exactly an isomorphism $V \to V\otimes V$ of one-dimensional representations, which is exactly the choice of an isomorphism $V\iso \EE$.
We see that $\bCS(G)$ is equivalent to the category of characters of $G(\Fq)$.
Let $\bg$ be a geometric point above $g \in G(\Fq)$.  If $\CSs{E}$ matches $\psi : G(\Fq)\to \EEx$ under this equivalence, a simple calculation on stalks reveals that the action of Frobenius on $\CSs{E}_{\bar g}$ is multiplication by $\psi(g)^{-1}$.
In other words, for every $\CSs{E}$ in $\bCS(G)$, the trace of $\Lang^*\CSs{E}$ is $\trFrob{\CSs{E}}^{-1}$ as a representation of $G(\Fq)$, proving parts (i) and (ii).

To finish the proof of part (iii), suppose $\Lang^*\CSs{E} = V$ with isomorphism $V \to V\otimes V$.  Observe that the equivalence above establishes a bijection between $\Aut(\CSs{E})$ and the group of automorphisms of $\rho : V\to V$ for which 
\[
\begin{tikzcd}
...
\]
commutes. 
Since the only such isomorphism $\rho$ is $\id_V$, it follows that $\Aut(\CSs{E}) = 1$, completing the proof.
\end{proof}
...
We have also just seen that if the character of $\Lang^*\CSs{E}$ is $\psi$ then the canonical isomorphism $m^*\psi \iso \psi \boxtimes \psi$ determines the isomorphism $\mu_\CSs{E} : \CSs{E} \to \CSs{E}\boxtimes\CSs{E}$.
This fact leads to a perspective on the function-sheaf dictionary common in the literature in which one considers one-dimensional local systems $\CSs{E}$ on $G$ for which \emph{there exists} an isomorphism $m^*\CSs{E} \iso \CSs{E} \boxtimes\CSs{E}$ \cite{kamgarpour:09a}*{Intro}.
As a slight variation, one may also consider one-dimensional local systems $\gCSs{L}$ on $\bG$ for which \emph{there exists} an isomorphism $\Frob{G}^*\gCSs{L} \iso \gCSs{L}$ and an isomorphism $\bm^*\gCSs{L} \iso \gCSs{L} \boxtimes\gCSs{L}$.
...
%
A character sheaf $\CSs{L}$ on an \'etale commutative group scheme $G$ over $\Fq$ is:
\begin{enumerate}
 \labitem{(cs.1)}{cs.1} an indexed set of one-dimensional
  $\EE$-vector spaces $\gCSs{L}_x$, as $x$ runs over
  $G(\bFq)$;

 \labitem{(cs.2)}{cs.2} an indexed set of isomorphisms
  $\mu_{x,y} : \gCSs{L}_{x+y} \xrightarrow{\iso} \gCSs{L}_{x} \otimes\gCSs{L}_{y}$,
  for all $x,y \in G(\bFq)$, such that
  \[
   \begin{tikzcd}[row sep=40]
    \gCSs{L}_{x+y+z} \arrow{rr}{\mu_{x+y,z}} \arrow[swap]{d}{\mu_{x,y+z}}
    && \gCSs{L}_{x+y}\otimes\gCSs{L}_{z} \dar{\mu_{x,y} \tight{0.5}{\otimes}{1} \id} \\
    \gCSs{L}_{x} \otimes\gCSs{L}_{y+z} \arrow{rr}{\id \otimes\mu_{y,z}}
    && \gCSs{L}_{x} \otimes\gCSs{L}_{y} \otimes\gCSs{L}_{z}
   \end{tikzcd}
  \]
  commutes, for all $x,y,z\in G(\bFq)$; and
 \labitem{(cs.3)}{cs.3} an indexed set of isomorphisms $\phi_{x} : \gCSs{L}_{\Frob{}(x)} \to \gCSs{L}_x$
  such that
  \[
   \begin{tikzcd}[row sep=40]
    \gCSs{L}_{\Frob{}(x)+\Frob{}(y)} \arrow[swap]{d}{\phi_{x+y}} \arrow{rr}{\mu_{\Frob{}(x),\Frob{}(y)}}
    && \gCSs{L}_{\Frob{}(x)}\otimes\gCSs{L}_{\Frob{}(y)} \dar{\phi_x \tight{0}{\otimes}{0} \phi_y} \\
    \gCSs{L}_{x+y} \arrow{rr}{\mu_{x,y}}
    && \gCSs{L}_x \otimes\gCSs{L}_y
   \end{tikzcd}
  \]
  commutes, for all $x,y\in G(\bFq)$.
\end{enumerate}
Under this equivalence, a morphism $\rho : \CSs{L} \to \CSs{L'}$ of character sheaves on $G$ is given by
\begin{enumerate}
 \labitem{(cs.4)}{cs.4} an indexed set $\brho_x : \gCSs{L}_x \to \gCSs{L'}_x$
  of linear transformations such that
  \[
   \begin{tikzcd}[column sep=40]
    \arrow[swap]{d}{\phi_x} \gCSs{L}_{\Frob{}(x)} \rar{\brho_{\Frob{}(x)}} & \gCSs{L'}_{\Frob{}(x)} \dar{\phi_x'}
    &\arrow[draw=none]{d}[pos=.4,description]{\text{\normalsize{and}}}
    & \arrow[swap]{d}{\mu_{x,y}} \gCSs{L}_{x+y} \rar{\brho_{x+y}} & \gCSs{L'}_{x+y} \dar{\mu'_{x,y}} \\
    \gCSs{L}_x \rar{\brho_x} & \gCSs{L'}_x
    & {} & \gCSs{L}_x\otimes\gCSs{L}_y \rar{\brho_x\otimes\brho_y} & \gCSs{L'}_x \otimes\gCSs{L'}_y
   \end{tikzcd}
  \]
...
\]
Let $\CSs{L} = (\gCSs{L},\mu,\phi)$ be a character sheaf on $G$.
For each geometric point $x\in {\bar G}$, choose a basis $\{ v_x \}$ for $\gCSs{L}_x$.
Through this choice, $\CSs{L}$ determines functions
\begin{align*}
...
Although the cocycle $\alpha\oplus \beta$ is not well defined by $\CSs{L}$, its class in $\Hh^2(E^\bullet_G)$ is.
To see this, let $\{ v'_x \in \gCSs{L}_x^\times \tq x \in {\bar G}\}$ be another choice and let $\alpha'\oplus \beta' \in Z^2(E^\bullet_G)$ be defined by $\CSs{L}$ and this choice, as above.
...
\]
so the class $[\alpha\oplus\beta]$ of $\alpha\oplus\beta$ in $\Hh^2(E^\bullet_G)$ is independent of the choice made above. It is also easy to see that $[\alpha\oplus\beta] = [\alpha_0\oplus\beta_0]$ when $\CSs{L} \iso \CSs{L}_0$,
which concludes the definition of the function
\begin{align*}
S_G : \CSiso{G} &\to \Hh^2(E^\bullet_G)\\
[\CSs{L}] &\mapsto [\alpha\oplus \beta].
\end{align*}
It is also easy to see that $[\alpha_1\oplus\beta_1] + [\alpha_2\oplus\beta_2] = [\alpha_3\oplus\beta_3]$ when $\CSs{L}_3 = \CSs{L}_1\otimes \CSs{L}_2$, so $S_G$ is a group homomorphism.
...
\end{proposition}
\begin{proof}
Suppose $[\CSs{L}] \in \CSiso{G}$ with $S_G([\CSs{L}]) = [\alpha \oplus \beta] = 0$,
so that $\alpha \oplus \beta = d\sigma$ for some $\sigma \in C^0(\Weil{},C^1({\bar G},\EEx) = C^1({\bar G},\EEx)$.
For each $x\in {\bar G}$, define $\sigma_x : \gCSs{L}_x \to \EE$ by $\sigma_x : v_x \mapsto \sigma(x)$.
Then the indexed set of isomorphisms $\{ \sigma_x : \gCSs{L}_x \to \EE \tq x\in {\bar G}\}$
defines an isomorphism $\CSs{L} \to (\EE)_G$.
Since $\CSs{L} = 0 \in \CSiso{G}$, $S_G$ is injective.
...
Set $\gCSs{L}_x = \EE$, define $\mu_{x,y} : \gCSs{L}_{x+y} \to \gCSs{L}_x\otimes\gCSs{L}_y$
by $\mu_{x,y}(1) = a(x,y) (1\otimes 1)$ and $\phi_x : \gCSs{L}_{\Frob{G}(x)} \to \gCSs{L}_x$ by $\phi_x(1)= b(x)$.
Then \ref{CS.1} holds since $d_G \alpha =0$ and \ref{CS.2} holds since $d_{\Weil{}}\alpha =d_G \beta$.
Tracing the construction backward, we have defined a character sheaf $\CSs{L}$ on $G$ with
$S_G(\CSs{L}) = [\alpha\oplus\beta\oplus 0]$, showing that $S_G$ is surjective.
\end{proof}
...
Set $\gCSs{L}_x = \EE$ for every $x\in G(\bFq)$.
Define $\mu_{x,y} : \gCSs{L}_{x+y} \to \gCSs{L}_x\otimes \gCSs{L}_y$ by $\mu_{x,y}(1) = 1 \otimes 1$ and
$\phi_{x} : \gCSs{L}_{\Frob{}(x)} \to \gCSs{L}_x$ by $\phi_{x}(1) = \beta(\Frob{})(x)$.
Since $\beta(\Frob{}) : G(\bFq) \to \EEx$ is a group homomorphism,
condition \eqref{nohom} is satisfied with $a =1$.
So $\CSs{L} = (\gCSs{L}, \mu, \phi)$
is a character sheaf with $\trFrob{\CSs{L}} = \chi$.
...
Let $\CSs{L}'$ be the character sheaf on $G$ defined by $\beta'$, as above.
For each $x\in G(\bFq)$, define $\brho_x :\CSs{L}_x\to \CSs{L}'_x$ by $\brho_x(1) = d(x)$.
The collection of isomorphisms $\{ \brho_x \tq x\in G(\bFq)\}$ satisfies condition~\ref{CS.4}, so it defines a morphism $\rho : \CSs{L}\to \CSs{L}'$, which is clearly an isomorphism. 
%
We have now defined a section of $\TrFrob{G}$. 

Now suppose $\chi_1, \chi_2 \in G(\Fq)^*$. Pick cocycles $\beta_1,\beta_2\in Z^1(\Weil{},\bG^*)$ and construct character sheaves $\CSs{L}_1$ and $\CSs{L}_2$ on $G$ as above. Since $\CSs{L}_1\otimes \CSs{L}_2$ is exactly the character sheaf built from the cocycle $\beta_1\cdot \beta_2$, and since $\trFrob{\CSs{L}_1\otimes \CSs{L}_2} = \trFrob{\CSs{L}_1}\cdot \trFrob{\CSs{L}_2}$, the section of $\TrFrob{G}$ defined here is a homomorphism.
\end{proof}
...
While most of this paper focuses on the question of determining the isomorphism classes of objects, a complete understanding of the morphisms in $\CS(G)$ also requires a description of the automorphisms of an arbitrary character sheaf $\CSs{L}$.

\begin{proposition}\label{prop:autornaught_etale}
Let $G$ be an \'etale commutative group scheme over $\Fq$.
If $\CSs{L}$ and $\CSs{L}'$ are character sheaves on $G$ then
every $\rho\in \Hom(\CSs{L},\CSs{L}')$ is either trivial or an isomorphism. Moreover, the trace map induces an isomorphism of groups
\[
\Aut(\CSs{L}) \to \Hom(G(\bFq)_{\Weil{}}, \EEx).
\]
\end{proposition}

\begin{proof}
We have already seen, in Lemma~\ref{lem:autornaught}, that every $\rho\in \Hom(\CSs{L},\CSs{L}')$ is either trivial or an isomorphism.
Now suppose $\rho \in \Aut(\CSs{L})$.
...
Conversely, if $\rho : G(\bFq)_{\Weil{}} \to \EEx$ is any homomorphism, then defining $\brho_x$ as multiplication by $\rho(x)$ will define a morphism $\gCSs{L} \to \gCSs{L}'$ satisfy the two diagrams in \ref{CS.4}.  
...
\end{lemma}
\begin{proof}
Let $\CSs{L}$ be a character sheaf on $\pi_0(G)$ and let $\rho : \pi_0^*\CSs{L} \to (\EE)_{G}$ be an isomorphism in $\CS(G)$. 
For each $x\in \pi_0(\bG)$, set $\bG^x \ceq \pi_0^{-1}(x)$.
The restriction $\pi_0^*\gCSs{L}\vert_{\bG^x}$ is the constant sheaf $(\gCSs{L}_x)_{\bG^x}$ so the isomorphism $\brho\vert_{\bG^x} : (\gCSs{L}_x)_{\bG^x} \to (\EE)_{\bG^x}$ determines an isomorphism $\brho_x : \gCSs{L}_x \to (\EE)_x$. The collection $\{ \brho_x \tq x\in \pi_0(\bG) \}$ determines an isomorphism $\CSs{L} \to (\EE)_{\pi_0(G)}$  in $\CS(\pi_0(G))$.
\end{proof}
...
So it suffices to show that if $\CSs{L} = (\gCSs{L},\mu,\phi)$ is a character sheaf on $G$
with $\CSs{L}\vert_{G^0} = (\EE)_{G^0}$ then $\CSs{L}$ is in the essential image of $\pi_0^*$.
...
Let $\mu_{g,a} : \gCSs{L}_{ga} \to \gCSs{L}_g \otimes \gCSs{L}_a$
be the isomorphism of vector spaces obtained by restriction of
$\mu : m^*\gCSs{L} \to \gCSs{L} \boxtimes \gCSs{L}$ to the
geometric point $(g,a)$ on $\bG^x \times \bG^0$.
Since $\CSs{L}\vert_{G^0} = (\EE)_{G^0}$,
the stalk of $\gCSs{L}$ at $a$ is $\EE$.
In this way the pair of geometric points $g, g' \in \bG^x$
determines an isomorphism $\varphi_{g,g'} \ceq \mu_{g,a}^{-1}$
from $\gCSs{L}_{g}$ to $\gCSs{L}_{g'}$.
%
The isomorphisms $\varphi_{g,g'}: \gCSs{L}_{g} \to \gCSs{L}_{g'}$ are canonical
in the following sense: if $g,g'\in \bG^x$ and $h,h'\in \bG^y$
then it follows from \ref{CS.2} and \ref{CS.3}
that
 \begin{equation}\label{eq:CS}
  \begin{tikzcd}[column sep=40]
   \gCSs{L}_{gh} \arrow{r}{\varphi_{gh,g'h'}} \arrow[swap]{d}{\mu_{g,h}}
  & \gCSs{L}_{g'h'} \arrow{d}{\mu_{g',h'}}
  &\arrow[draw=none]{d}[pos=.4,description]{\text{\normalsize{and}}}
  &  \gCSs{L}_{\Frob{}(g)} \arrow{r}{\varphi_{\Frob{}(g),\Frob{}(g')}} \arrow[swap]{d}{\phi_{g}} & \gCSs{L}_{\Frob{}(g')} \arrow{d}{\phi_{g'}} \\
  \gCSs{L}_{g} \otimes \gCSs{L}_{h} \arrow{r}{\varphi_{g,g'}\otimes \varphi_{h,h'}}
  & \gCSs{L}_{g'} \otimes \gCSs{L}_{h'}
  & {}
  & \gCSs{L}_{g} \arrow{r}{\varphi_{g,g'}} & \gCSs{L}_{g'}
  \end{tikzcd}
 \end{equation}
both commute.

For each $x\in \pi_0(\bG)$, pick $g(x)\in \bG^x$
and set $\gCSs{E}_x \ceq \gCSs{L}_{g(x)}$.
Let $\phi_x : \gCSs{E}_{\Frob{}(x)} \to \gCSs{E}_x$
be the isomorphism of $\EE$-vector spaces obtained by composing
$\varphi_{g(\Frob{}(x)),\Frob{}(g(x))} : \gCSs{L}_{g(\Frob{}(x))} \to \gCSs{L}_{\Frob{}(g(x))}$
with $\phi_{g(x)} : \gCSs{L}_{\Frob{}(g(x))} \to \gCSs{L}_{g(x)}$.
For each pair $x,y\in \pi_0(\bG)$
let $\mu_{x,y} : \gCSs{E}_{x+y}\to \gCSs{E}_x\otimes \gCSs{E}_y$
be the isomorphism of $\EE$-vector spaces obtained by composing
$\varphi_{g(x+y),g(x)g(y)} : \gCSs{L}_{g(x+y)} \to \gCSs{L}_{g(x)g(y)}$
with $\mu_{g(x),g(y)} : \gCSs{L}_{g(x)g(y)} \to \gCSs{L}_{g(x)}\otimes \gCSs{L}_{g(y)}$.
Using \eqref{eq:CS}, it follows that \ref{CS.1}, \ref{CS.2} and \ref{CS.3} are satisfied for
$\CSs{E} \ceq (\gCSs{E}_x, \mu_{x,y}, \phi_x)$, thus defining a character sheaf on $\pi_0(G)$.

The pullback $\pi_0^*(\CSs{E})$ of $\CSs{E}$ along $\pi_0 : G \to \pi_0(G)$ is constant
on geometric connected components, with stalks given by
$(\pi_0^* \CSs{E})_g = \CSs{E}_{x}$ for all $g\in \bG^x$.  Thus both $\pi_0^*\CSs{E}$ and $\CSs{L}$
are constant on geometric connected components of $G$.
The choices above define isomorphisms
$\gCSs{L}\vert_{\bG^x} \to  (\gCSs{E}_{x})_{\bG^x}$ for each $x\in \pi_0(\bG)$.
The resulting isomorphism $\gCSs{L} \to \pi_0^* \gCSs{E}$ satisfies \ref{CS.4},
thus defining an isomorphism $\CSs{L} \to \pi_0^* \CSs{E}$ in $\CS(G)$.
\end{proof}
...
\[
\Aut(\CSs{L}) \iso \Hom(\pi_0(\bG)_{\Weil{}},\EEx).
\]
\end{theorem}
\begin{proof} 
Fix $\CSs{L} = (\gCSs{L},\mu,\phi)$ and consider the group homomorphism from $\Aut(\CSs{L})$ to $\Hom(\bG_{\Weil{}},\EEx)$ defined in Proposition~\ref{prop:autornaught_etale}.
...
Homomorphisms in the image of $\Aut(\CSs{L}) \to \Hom(\bG_{\Weil{}},\EEx)$ are continuous when $\bG$ is viewed as the base of the espace \'etal\'e attached to $\gCSs{L}$.
Since $\ell$ is invertible in $\Fq$, it follows that the image of $\Aut(\CSs{L}) \to \Hom(\bG_{\Weil{}},\EEx)$ is contained in $\Hom(\pi_0(\bG_{\Weil{}}),\EEx)$. 
We also have $\pi_0(\bG_{\Weil{}})=\pi_0(\bG)_{\Weil{}}$. 
To see that $\Aut(\CSs{L}) \to \Hom(\pi_0(\bG)_{\Weil{}},\EEx)$ is surjective, begin with $\theta\in\Hom(\pi_0(\bG)_{\Weil{}},\EEx)$ and, for each $[x] \in \pi_0(\bG)_{\Weil{}}$ define $\bar\rho^y: \gCSs{L}^y \to \gCSs{L}^y$ by scalar multiplication by $\theta([x])\in \EEx$ for each $y\in [x]$.
This defines an isomorphism $\bar\rho : \gCSs{L}\to \gCSs{L}$ of local systems on $\bG$ compatible with $\mu$ and $\phi$, and thus an isomorphism $\rho :\CSs{L}\to \CSs{L}$ which maps to $\theta$ under $\Aut(\CSs{L}) \to \Hom(\pi_0(\bG)_{\Weil{}},\EEx)$.
\end{proof}
...
If $\CSs{L} \ceq (\gCSs{L}, \mu, \phi)$ is a character sheaf on $G$, we define
$\CSs{L}' \ceq (\gCSs{L}, \mu, \phi_{k'})$ on the base change
...
as restricting the action $\varphi$ of $\Weil{k}$ on $\gCSs{L}$,
defined in Section~\ref{ssec:category}, to $\Weil{k'}$.

\begin{proposition}\label{prop:basechange}
With notation above,
the rule $\nu_{k'/k}: (\gCSs{L}, \mu, \phi) \mapsto (\gCSs{L}, \mu, \phi_{k'})$
...
\begin{proof}
Let $\CSs{L} \ceq (\gCSs{L}, \mu, \phi) \in \CS(G)$ and write $F$ for $\Frob{G}$.  For any $x \in G(k')$,
we may compute the value of $\TrFrob{G_{k'}}(\nu_{k'/k}\CSs{L})(x)= t_{\nu_{k'/k}\CSs{L}}(x)$ as the trace of $\phi_{k'}$ on $\gCSs{L}_x$,
and the value of $\Nm^*(\TrFrob{G}(\CSs{L}))(x)$ as the trace of $\phi$ on $\gCSs{L}_{\Nm(x)}$.
Applying \ref{CS.3} to the stalk of $\gCSs{L}^{\boxtimes n}$ at the point $(x, \Frob{}(x), \ldots, \Frob{}^{n-1}(x))$ yields a diagram
\[
\begin{tikzcd}
\gCSs{L}_{\Nm(x)} \rar \dar{\phi_{\Nm(x)}} & \gCSs{L}_{F(x)} \otimes \gCSs{L}_{F^2(x)} \otimes \cdots \otimes \gCSs{L}_x
\dar{\phi_x \otimes (F^*\phi)_x \otimes \cdots \otimes ((F^{n-1})^*\phi)_x} \\
\gCSs{L}_{\Nm(x)} \rar & \gCSs{L}_x \otimes \gCSs{L}_{F(x)} \otimes \cdots \otimes \gCSs{L}_{F^{n-1}(x)}.
\end{tikzcd}
\]
Choose a basis vector $v$ for $\gCSs{L}_{\Nm(x)}$ and write $v_0 \otimes v_1 \otimes \cdots \otimes v_{n-1}$ for the image of $v$ under the
bottom map,
for $v_i \in \gCSs{L}_{\Frob{}^i(x)}$.  By \ref{CS.2}, $v$ maps to
...
the trace of $\phi_{\Nm(x)}$.  We may now equate the trace $\alpha$ of $\phi$ on $\gCSs{L}_{\Nm(x)}$
with the product $\alpha_0 \cdots \alpha_{n-1}$, which is the trace of $\phi_{k'}$ on $\gCSs{L}_x$.
\end{proof}
...
\begin{enumerate}
\item
objects in $\QCS(X_K)$ are the $\ell$-adic sheaves $\varrho^R_n(X)^*\CSs{L}$, for $n\in \NN$ and $\CSs{L} \in \CS(\Gr^R_n(X))$; 
\item
morphisms $\varrho^R_n(X)^*\CSs{L} \to \varrho^R_m(X)^*\CSs{L}'$ in $\QCS(X_K)$ are those morphisms in $\CS(\Gr_R(X))$ which take the form $\varrho^R_m(X)^*\rho$ for $\rho \in \Hom(\varrho^R_{n\leq m}(X)^*\CSs{L},\CSs{L}')$ when $n\leq m$, and $\varrho^R_n(X)^*\rho$ for $\rho \in \Hom(\CSs{L},\varrho^R_{m\leq n}(X)^*\CSs{L}')$ when $m\leq n$.
\end{enumerate}
...
\[
\CSs{F} \ceq (n, \{\CSs{F}_i\}_{n\leq i}, \{\alpha_{i \le j}\}_{n\le i \le j}),
\] where $n$
is a non-negative integer, each $\CSs{F}_i$ is a character sheaf on $\Gr^R_i(X)$ and each 
\[
\alpha_{i \le j} : \CSs{L}_j \to \varrho^R_{i \le j}(X)^* \CSs{L}_i
\]
 is an isomorphism; here $\alpha_{i \le i}$ is the identity and the $\alpha_{i \le j}$ are compatible with each other.  
If $\CSs{F} \ceq (n, \{\CSs{F}_i\}, \{\alpha_{i \le j}\})$
and $\CSs{F}' \ceq (m, \{\CSs{F}'_i\}, \{\alpha'_{i \le j}\})$ are objects then $\Hom(\CSs{F}, \CSs{F}')$ is the set
of equivalence classes of pairs $(k, \{\beta_i\}_{k \le i})$, where $n,m \le k$ and the $\beta_i : \CSs{F}_i \to \CSs{F}'_i$ are
morphisms of character sheaves so that
\[
\begin{tikzcd}
\CSs{F}_j \rar{\alpha_j} \dar{\beta_i} & \varrho^R_{i \le j}(X)^* \CSs{F}_i \dar{f_{i \le j}^*\beta_j} \\
\CSs{F}'_j \rar{\alpha_j} & \varrho^R_{i \le j}(X)^* \CSs{L}'_i
\end{tikzcd}
\]
...
\] 
\labitem{(3)}{x3} for all quasicharacter sheaves $\CSs{F}$, $\CSs{F}'$ on $\Gr_R(X)$, and for every $\rho \in \Hom(\CSs{F},\CSs{F}')$, either $\rho$ is trivial or $\rho$ is an isomorphism;
\labitem{(4)}{x4} for all quasicharacter sheaves $\CSs{F}$ for $X_K$, there is a canonical isomorphism
\[
\Aut(\CSs{F}) \iso \Hom((\pi_0(X)_{\bFq})_{\Weil{}},\EEx).
\]
\end{enumerate}
\end{theorem}
...
\]
Also by Theorem~\ref{thm:CSXK} and \cite{bitan:discriminant}*{Eq 3.1}, the automorphism group of every $\CSs{F} \in \CS(T_K)$ is given by 
\[
\Aut(\CSs{F}) \iso \Hom(X_*(T_K)_{\Weil{K}},\EEx),
\]
...
\subsection{Examples of quasicharacter sheaves for $p$-adic tori} 

Here we consider some examples that illustrate some of the essential features of the category of quasicharacter sheaves for $p$-adic tori. In this section, $K$ is a finite extension of $\QQ_p$.

First, suppose $T = \GL(1)_{/K}$,
