...
[Add reference to Takaski Suzuki's paper]
...
\begin{proposition}\label{prop:connected}
 ...
 \labitem{(3)}{c3} $\Aut_\CS{}(\cs{L}) = 1$, for all character sheaves $\cs{L}$ on $G$.
 \end{enumerate}
 \end{proposition}
\begin{proof}
...
\[
\Aut_\CS{}(\cs{L}) \iso \Hom(\pi_0(\bG)_{\Weil{}},\EEx).
\]
\end{theorem}
\begin{proof} 
...
\qquad\text{and}\qquad
F_{\text{}}: X_K \mapsto \Hom_{\text{}}(X(K),\EEx)
\]
...
\labitem{(3)}{x3} for all quasicharacter sheaves $\cs{F}$ for $X_K$, there is a canonical isomorphism
\[
...
To prove \ref{x3}, use Theorem~\ref{thm:autornaught} with $G = \Gr^R_n(X)$, the fact that the component group of $\Gr^R_n(X)$ is independent of $n$ and then argue as in part \ref{x0}.
%\end{enumerate}
\end{proof}
...
This is now provided by specializing Theorem~\ref{thm:CSXK} to the case when $X_K\in \mathcal{N}_K$ is an algebraic torus over $K$. 

\begin{corollary}\label{cor:CS_tori}
Let $T_K$ be an algebraic toris over $K$ and let $T$ be a Neron model for $T_K$.
...
\[
\Aut_{\QCS(\pi_0(T))}(\cs{E})\iso (\check{T}_\ell)^{\Weil{K}},
\quad
\Aut_{\QCS(T)}(\cs{F})\iso (\check{T}_\ell)^{\Weil{K}},
\quad
\Aut_{\QCS(T^0)}(\cs{F}^0)\iso 1,
\]
 where $\check{T}_\ell \ceq \Hom(X_*(T_K),\EEx)$, the $\ell$-adic dual torus to $T_K$.
\end{corollary}

\begin{proof}
We already know $\Aut_{\QCS(T^0)}(\cs{E}) =1$ from Proposition~\ref{prop:connected}, part \ref{x3}.
By Theorem~\ref{thm:CSXK}, $\Aut(\cs{F})  \iso \Hom((\pi_0(X)_{\bFq})_{\Weil{}},\EEx)$, canonically, for $X = T$ and $X=\pi_0(T)$ (and for $X= T^0$, though that case has already been treated by a different argument, above).
...
\subsection{Geometric perspective on the Langlands correspondence for $p$-adic tori}\label{ssec:LLCT}
...
{}  & 0 \arrow{d} & 0 \arrow{d} &  & \\ 
<<<<<<< Updated upstream
   & \Hh^0(\Weil{}, \Hh^2(X_*(T_K)_{\mathcal{I}_K},\EEx))  \arrow{d} & \arrow{d} \Hh^0(\Weil{}, \Hh^2(X_*(T_K)_{\mathcal{I}_K},\EEx)) & 0 \arrow{d} & \\
    0 \rar & \QCSiso{\pi_0(T)} \arrow{r}{\pi_0^*} \arrow{d}
    & \QCSiso{T} \arrow{r}{\iota_0^*}  \arrow{d} & \QCSiso{T^0} \arrow{r} \arrow{d} & 0\\
   0 \arrow{r}  
 & \Hh^1(\Weil{}, \Hh^0(\mathcal{I}_K, \check{T}_\ell)) \arrow{r}{\text{infl'n}} \arrow{d}
 & \Hh^1(\Weil{K}, \check{T}_\ell) \arrow{r}{\text{rest'n}} \arrow{d}
 & \Hh^0(\Weil{}, \Hh^1(\mathcal{I}_K, \check{T}_\ell)) \arrow{r} \arrow{d}  
=======
   & \Hh^2(X_*(T_K)_{\mathcal{I}_K},\EEx)^{\Weil{\Fq}}  \arrow{d} & \arrow{d} \Hh^2(X_*(T_K)_{\mathcal{I}_K},\EEx)^{\Weil{\Fq}} & 0 \arrow{d} & \\
    0 \rar & \QCSiso{\pi_0(T)} \arrow{r}{\pi_0^*} \arrow{d}
    & \QCSiso{T} \arrow{r}{\iota_0^*}  \arrow{d} & \QCSiso{T^0} \arrow{r} \arrow{d} & 0\\
...
Even the case $T_K = \Gm{K}$ is remarkable, in which case \eqref{eq:GLCT} becomes

In fact, the answer is yes; as such, these maps provide a geometric perspective on the Langlands correspondence for $p$-adic tori.

As in \cite{yu:09a}, the general case can be built up from the case $T_K = \Gm{K}$.


or by a small part of Proposition~\ref{prop:etale-iso}

[insert discussion of Takashi Suzuki's work]
=======
...
The isomorphism $\QCSiso{\Gm{R}}\iso \Hom(\mathcal{I}_K, \EEx)$ may be deduced directly from the class field theory of Serre and Hezewinkel. The monoidal category of character sheaves on $\Gr^R_n(\Gm{R})$ is canonically equivalent to the category of $\ell$-adic characters of the equivariant etale fundamental group $\pi_1^\text{eq}(\Gr^R_n(\Gm{R}),\bFq)$.
%and, using the Lang isogeny, that there is a canonical isomorphism 
%\[\pi_1^\text{eq}(\Gr^R_n(\Gm{R}),\bFq) \iso \Gr^R_n(\Gm{R})(\Fq) = R_n^*.\]
Accordingly, quasicharacter sheaves on $\Gm{R}$ may be apprehended as $\ell$-adic characters of
the inverse limit of the groups $\pi_1^\text{eq}(\Gr^R_n(\Gm{R}),\bFq)$, as $n$ ranges over $\NN$.
By \cite{serre:isogenies}, there is a canonical isomorphism 
\[
\varprojlim_{n\in \NN} \pi_1^\text{eq}(\Gr^R_n(\Gm{R}),\bFq) \iso \mathcal{I}_K^\text{ab};
\]
this isomorphism is {\it not} restricted to the case when $K$ and $\Fq$ have equal characteristic.
This last isomorphism now provides the isomorphism $\QCSiso{\Gm{R}}\iso \Hom(\mathcal{I}_K, \EEx)$ in \eqref{eq:Gm}. 
Since the sequences are split, this gives \eqref{eq:Gm}, geometrically. 
We have been glib here because all this is treated carefully and beautifully in \cite{suzuki-yoshida:12a}. 
Indeed, in that paper, an isomorphism is constructed directly between the equivariant fundamental group of $\Gr_R(T)$ (with respect to a site defined in \cite{suzuki-yoshida:12a}) when $T$ is a Neron model of $\Gm{K}$, and $\Weil{K}^\text{ab}$, improving greatly on the arguments of this paragraph.

We expect that the general case of \eqref{eq:GLCT}, where $K$ is any local field and $T_K$ is any torus over $K$, may be deduced from the case \eqref{eq:Gm}, by an adaptation of arguments from \cite{yu:09a}. 
In anticipation of work in that direction, Section~\ref{ssec:wrK} provides a key tool for that argument.

We close this section with some further remarks on sites.
