...
write $\m$ for the maximal ideal of $R$ and $K$ for the quotient 
In this section we use quasicharacter sheaves to geometrize and categorify the group 
of admissible characters of smooth commutative group schemes over $R$.



We will need the Greenberg functor \citelist{\cite{greenberg:61} \cite{greenberg:63a}}:
...
\]
for the functor produced by precomposing pullback along $R \to R_n$ with the Greenberg functor.

The surjective ring homomorphism $R_{n+1} \to R_n$ determines a
...
We list some fundamental properties of $f_n$ and $\GN{X}_n$ for further use:
 \begin{enumerate}
  \labitem{(GN.0)}{truncation}
  $f_n : \GN{X}_{n+1} \to \GN{X}_n$ is a surjective morphism of group schemes over $\Fq$
  \citelist{\cite{bosch-lutkebohmert-reynaud:NeronModels}*{p. 276} \cite{bertrapelle-gonzales:Greenberg}*{Prop. 7.2}}.
  We remark that $f_n : \GN{X}_{n+1} \to \GN{X}_n$ is not induced from a morphism of Néron models.
  \labitem{(GN.1)}{smooth}
...
Let $X$ be a smooth commutative group scheme over a 
For every $n\in \NN_0$ set $R_n = R/\m^{n+1}$.
We say that a character $\chi: X(R)\to \EEx$ is {\it admissible} if there is some $n \in \NN_0$ such that $\chi$ factors through $X(R) \to X(R_n)$.
