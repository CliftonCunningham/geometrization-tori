...
That leads to a perspective on the function--sheaf dictionary common in the literature in which one considers $1$-dimensional local systems $\qcs{E}$ on $G$ for which \emph{there exists} an isomorphism $m^*\qcs{E} \iso \qcs{E} \boxtimes\qcs{E}$.
% as in \cite{kamgarpour:09a}*{Introduction} (or sometimes, $1$-dimensional local systems $\gqcs{L}$ on $\bG$ for which \emph{there exists} an isomorphism $\Frob{G}^*\gqcs{L} \iso \gqcs{L}$ and an isomorphism $\bm^*\gqcs{L} \iso \gqcs{L} \boxtimes\gqcs{L}$).
%
...
Indeed, Proposition~\ref{prop:etale-iso} shows that $\Hh^0(\Weil{},\Hh^2(\bG,\EEx))$ measures the possibilities for $\mu'$ when $G$ is etale.
We will see in Section~\ref{sec:main} that, for any smooth commutative group scheme $G$, the possibilities for $\mu'$ are measured by $\Hh^0(\Weil{},\Hh^2(\pi_0(\bG),\EEx))$.
...
Motivated to an application to the geometrization and categorification of quasicharacters of p-adic tori,
...
As Deligne explained in \cite{deligne:SGA4.5}*{sommes trig.}, if $G$ is a connected commutative algebraic group over $\FF_q$, then the trace of Frobenius provides a perfect dictionary between characters of $G(\FF_q)$ and isomorphism classes of rank-one $\ell$-adic local systems $\mathcal{E}$ on $G$ for which $m^*\qcs{E} \iso \qcs{E} \boxtimes \qcs{E}$, where $m : G\times G\to G$ is the multiplication map.
If one wishes to make a category from this class of local systems, one is lead to consider sheaf morphisms $\qcs{E} \to \qcs{E}'$ which are compatible with particular choices of isomorphisms $m^*\qcs{E} \iso \qcs{E} \boxtimes \qcs{E}$ and $m^*\qcs{E}' \iso \qcs{E}'\boxtimes \qcs{E}'$. {\it A priori}, these sheaf morphisms do not form a category, since 
