...
\[
0 \to \Hh^2(\pi_0(X)_{\bFq},\EEx)^{\Weil{}} \to \QCSiso{X} \to \Hom_\text{qc}(X(K),\EEx) \to 0;
\] 
...
after removing isomorphism groups.
\[
...
{}  & 0 \arrow{d} & 0 \arrow{d} &  & \\ 
   & \Hh^2(X_*(T_K)_{\mathcal{I}_K},\EEx)^{\Weil{}}  \arrow{d} & \arrow{d} \Hh^2(X_*(T_K)_{\mathcal{I}_K},\EEx)^{\Weil{}} & 0 \arrow{d} & \\
    ...
    & \arrow{d}{\iso} \Hom_\text{qc}(T(K),\EEx) \arrow{r}{\text{rest'n}} & \Hom_\text{qc}(T^0(R),\EEx) \arrow{r} \arrow{d}{\iso} & 0\\
 &  0  & 0 & 0 & 
  \end{tikzcd}
 \]
 \end{corollary}
...
Accordingly, by Theorem~\ref{thm:CSXK}, the kernel of the trace of Frobenius is
\[
...
By Theorem~\ref{thm:CSXK}, $\Aut(\cs{F})  \iso \Hom((\pi_0(X)_{\bFq})_{\Weil{}},\EEx)$, canonically, for $X = T$ and $X=\pi_0(T)$ (and for $X= T^0$, though that case is already treated).
In case $X = T^0$, it is clear that $\Hom((\pi_0(X)_{\bFq})_{\Weil{}},\EEx)$ is trivial, so $\Aut_{\QCS(T^0)}(\cs{E}) =1$; in fact, we alre
...
\subsection{Geometric perspective on the Langlands correspondence for $p$-adic tori}

\[
\begin{tikzcd}
   0 \rar & \arrow[bend right=70]{dd} \QCSiso{\pi_0(T)} \arrow{r}{\pi_0^*} \dar{\TrFrob{\pi_0(T)}}
    & \QCSiso{T} \arrow{r}{\iota_0^*} \arrow[dotted, bend right=50]{dd}  \arrow{d}{\TrFrob{\Gr_R(T)}} & \QCSiso{T^0} \arrow[dotted, bend right=50]{dd}  \arrow{r} \arrow{d}{\TrFrob{\Gr_R(T)^0}} & 0\\
   0 \arrow{r} & \Hom(\pi_0(T)(\Fq), \EEx) \arrow{r}{\text{infl'n}} \arrow{d}{\iso}
    & \arrow{d}{\iso} \Hom_\text{qc}(T(K),\EEx) \arrow{r}{\text{rest'n}} & \Hom_\text{qc}(T^0(R),\EEx) \arrow{r} \arrow{d}{\iso} & 0\\    
    0 \arrow{r}  
