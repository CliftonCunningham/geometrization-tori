...
=======
$S_{G}: \QCiso{G} \to H^0(\Weil{},H^2(G(\bFq),\EEx))$.
It is easy to see that the function $S_{G}: \QCiso{G} \to H^0(\Weil{},H^2(G(\bFq),\EEx))$ is a group homomorphism.\todo{I still need to write down the details, for ourselves.}
>>>>>>> FETCH_HEAD

\begin{definition}\label{def:invisible}
Let $\qcs{L}$ be a quasicharacter sheaf on $G$. We say $\qcs{L}$ is \emph{visible} if $S_G(\qcs{L}) \in H^0(\Weil{},H^2(G(\bFq),\EEx))$ is trivial; otherwise, we say $\qcs{L}$ is \emph{invisible}.
\end{definition}

<<<<<<< HEAD
\subsection{Group cohomology}
=======
%If $G$ is a connected and of finite type, then $\Hh^2(G(\bFq),\EEx)$ is trivial\todo{Reference?} so all quasicharacter sheaves on connected commutative algebraic groups $G$ are visible; by contrast, if $G$ is etale then $\Hh^1(\Weil{},\Hh^2(G(\bFq),\EEx))$ may be non-trivial so $G$ may admit invisible quasicharacters sheaves. We will return to that case in Section~\ref{sec:etale}.
>>>>>>> FETCH_HEAD
