...
\todo{The bottom arrow is an isomorphism to $G(k')^*_{\Gal(k'/k)}$. We will see that the horizontal arrows are isomorphisms on loose quasicharacter sheaves. Can we make sense of $\QC(G_{k'})_{\Gal(k'/k)}$? If so, is the functor $\nu_{k'/k}$ into $\QC(G_{k'})_{\Gal(k'/k)}$ an equivalence of categories?}
