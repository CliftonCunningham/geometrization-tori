...
Motivated by ...., in this paper we ... The component group of the Neron model of algebraic torus over local field ...   The sheaf--function dictionary for etale group schemes is significantly different . . .\todo{Needs a new front end to really set the tone, and highten the contrast with the connected case, and point to applications.}
...
\todo{Build an example of an invisible quasicharacter sheaf.}
...
\begin{lemma}\label{prop:etale-2cocycle}
...
Then set of isomorphisms $\{ \phi_{x} \tq x \in G(\bFq) \}$ satisfies \ref{qc.3} by \eqref{gf}. In this way, the cocycle $g\in Z^0(\Weil{}, Z^2(G(\bFq),\EEx))$ defines a quasicharacter sheaf $\qcs{L}$. It is now clear that $\qcs{L}$ maps to $[g] \in \Hh^0(\Weil{}, \Hh^2(G(\bFq),\EEx))$ under \eqref{2cocycle}.
\end{proof}
...
\begin{lemma}\label{prop:etale-trace}
If $G$ is an \'etale commutative group scheme over $\Fq$
...
\begin{proposition}\label{prop:etale-fg}
  If $G$ is an \'etale commutative group scheme over $\Fq$ and
  $G(\bFq)$ is finitely generated,
  then
  \[
...
\begin{proof}
  ...
  For each $x \in G(\bFq)$ define $a(x) \in \EEx$ by $s_2(x) = a(x) s_1(x)$.
  Then \eqref{section} and
