...
\begin{abstract}
  ...
  In $G$ is etale, however, the kernel is generally non-trivial,
  leading to the interesting notion of invisible quasicharacter sheaves.
  In this paper we find a cohomological classification of invisible quasicharacter sheaves on all smooth commutative group schemes over $\mathbb{F}_q$,
  and also explain how to make invisible quasicharacter sheaves disappear,
...
By contract, the etale group scheme $\ZZ \times \ZZ$ with trivial Galois action
 does admit invisible quasicharacter sheaves -- there are non-trivial quasicharacter sheaves on this group
with trivial trace of Frobenius.\todo{Under construction}
(Move this example: Define $g: \ZZ\times \ZZ \to \EEx$ by $g((n_1,m_1), (n_2, m_2)) = \ell^{m_1-m_2 + n_1-n_2}$.
...
is a group homomorphism from $\QCiso{G}$ to $\Hh^2(E^\bullet_G)$, defined in this section. In Section~\ref{sec:restriction} we will see that this homomorphism is an isomorphism for all smooth commutative group schemes over $\Fq$.

[Things I need from this section:
definition of $E^\bullet_G$ (Section~\ref{sec:E});
the short exact sequence attached to $\Hh^2(E^\bullet_G)$ (Section~\ref{sec:SES});
the exactness of all sequences in the diagram below (Section~\ref{sec:front});
the map $S_G$ from $\QCiso{G}$ to $\Hh^2(E^\bullet_G)$ (Section~\ref{sec:S} done) commutes with everything in sight;
that $\Hh^0(\Weil,\Hh^2({\bar G},\EEx))=0$ if $G$ is a connected algebraic group over $\Fq$ (Section~\ref{sec:connected-iso}).]
...
Define $\Hh^2(E^\bullet_G)$.

Do we have
 \[
  \begin{tikzcd}[column sep=30]
    ? \cdots \arrow{r}
%    & \Hh^1(E^\bullet_{A}) \arrow{r}
    & \Hh^2(E^\bullet_{C}) \arrow{r}
    & \Hh^2(E^\bullet_{B}) \arrow{r}
    & \Hh^2(E^\bullet_{A}) \arrow{r}
%    & \Hh^3(E^\bullet_{C}) \arrow{r}
    & \cdots ? \\
  \end{tikzcd}
 \]

[Question: If $C$ is etale, then $\Hh^3(E^\bullet_{C}) =0?$]

\[\Hh^*(E^\bullet_{G}) = \Hh^*({\bar G}\rtimes \Weil{},\EEx)\]
...
\subsection{Front}\label{sec:front}

 \[
  \begin{tikzcd}[row sep=20, column sep=15]
    &  ?  & 0 \arrow{d} & 0\arrow{d} & \\
0 \arrow{r} \arrow{d}   & \Hh^0(\Weil{},\Hh^2(C,\EEx)) \arrow{r} \arrow{d} & \Hh^0(\Weil{},\Hh^2(B,\EEx)) \arrow{r} \arrow{d}
    & \Hh^0(\Weil{},\Hh^2(A,\EEx)) \arrow{d} \arrow{r} & ? \\
    %\arrow{rr}  &   & 0 \\
    ? \arrow{r}
    & \Hh^2(E^\bullet_{C}) \arrow{r} \arrow{d}
    & \Hh^2(E^\bullet_{B}) \arrow{r} \arrow{d}
    & \Hh^2(E^\bullet_{A}) \arrow{r} \arrow{d}
    & ? \\
    %\arrow{rr}&& 0\\
? \arrow{r}  & \Hh^1(\Weil{},\Hh^1(C,\EEx)) \arrow{d} \arrow{r}
    & \Hh^1(\Weil{},\Hh^1(B,\EEx)) \arrow{d} \arrow{r}
    & \Hh^1(\Weil{},\Hh^1(A,\EEx)) \arrow{d} \arrow{r}  &  ? \\
    &  0 & 0 & 0 & \\
  \end{tikzcd}
 \]
...
\begin{tikzcd}
 0 \arrow{r}
 & \ker \TrFrob{G} \arrow{r} \arrow{d}
 & \QCiso{G}\arrow{d}{S_G} \arrow{r}{\TrFrob{G}}
 & G(\Fq)^* \arrow{d}{\iso} \arrow{r}
 & \coker \TrFrob{G} \\
 0 \arrow{r}
 & \Hh^0(\Weil{},\Hh^2({\bar G},\EEx)) \arrow{r}
 & \Hh^2(E^\bullet_G) \arrow{r}
...
\begin{proof}
We've discussed the fact that the square on the right commutes, using Lemma~\ref{lem:dual-inv}. I'll add that, later.
So, if $\qcs{L} \in \QC(G)$ and $S_G(\qcs{L}) = [\alpha\oplus\beta]$  then $\trFrob{\qcs{L}} =1$ implies $[\beta]=0$. In Section~\ref{sec:SES} we identified\todo{ ... will identify ...} $\{ [\alpha\oplus\beta] \in \Hh^2(E^\bullet_G) \tq [\beta]=0 \}$ with $\Hh^0(\Weil{},\Hh^2({\bar G},\EEx))$.
\end{proof}
...
\subsection{Invisible quasicharacter sheaves}

\begin{definition}\label{def:invisible}
We say $\qcs{L}\in \QC(G)$ is \emph{visible} if the projection of $S_G(\qcs{L})$ to $\Hh^0(\Weil{}, \Hh^2({\bar G},\EEx))$ is trivial; otherwise, we say $\qcs{L}$ is \emph{invisible}.
\end{definition}
...
\subsection{Looking back to connected commutative algebraic groups}\label{sec:connected-iso}

We end this Section~\ref{sec:spectral} by appling the tools developed here to the case when $G$ is a connected commutative algebraic group over $\Fq$.

\begin{proposition} \label{prop:connected-iso}
 If $G$ is a connected commutative algebraic group over $\Fq$
  then the map $\QCiso{G}\to \Hh^2(E^\bullet_G)$ from Section~\ref{sec:spectral} induces an morphism of short exact sequences:
\[
\begin{tikzcd}
 0 \arrow{r} & \ker \TrFrob{G} \arrow{d} \arrow{r} & \QCiso{G}\arrow{d}{S_G} \arrow{r}{\TrFrob{G}}  \arrow{r} & G(\Fq)^* \arrow{d}{\iso} \arrow{r} & 0\\
...
\begin{proof}
The top sequence is exact by Corollary~\ref{cor:connected}.
The bottom sequence is exact by Section~\ref{sec:SES}.
The diagram commutes and the right-hand vertical arrow is an isomorphism, by Proposition~\ref{prop:ST}.\todo{ If $\Hh^0(\Weil{},\Hh^2({\bar G},\EEx))=0$ then we can improve this lemma.}
\end{proof}


\section{The Hochschild-Serre Spectral Sequence}

Consider the double complex $E^{\bullet, \bullet}$ defined by
\[
...
\item the cohomology $\Hh^\bullet(\Weil{} \ltimes G(\bFq), \EEx)$ is isomorphic to the cohomology of the total complex $\Tot(E^{\bullet, \bullet})$,
\item there is a filtration $\Hh^n(\Weil{}, \ltimes G(\bFq), \EEx) = F_n \supset \cdots \supset F_0 \supset F_{-1} = 0$ where $F_i / F_{i-1} \cong E_{\infty}^{n-i, i}$.
\end{enumerate}
\end{proposition}
...
\[
1 \to \Hh^1(\Weil{}, \Hh^1(G(\bFq), \EEx)) \to \Hh^2(\Weil{} \ltimes G(\bFq), \EEx) \to \Hh^0(\Weil{}, \Hh^2(G(\bFq), \EEx)) \to 1.
\]
\end{corollary}

% Replace H^i with \Hh^i
The map $S_{G}: \QCiso{G} \to \Hh^0(\Weil{},H^2(G(\bFq),\EEx))$ defined in Section \ref{sec:invisible} factors through a map $R_G : \QCiso{G} \to \Hh^2(G(\bFq) \rtimes \Weil{}, \EEx)$.  
...
We may now apply the tools developed in Section~\ref{sec:spectral} to the case at hand, where $G$ is an etale connected group scheme over $\Fq$.

\begin{proposition} \label{prop:etale-iso}
 If $G$ is an \'etale commutative group scheme over $\Fq$
  then the map $\QCiso{G}\to \Hh^2(E^\bullet_G)$ from Section~\ref{sec:spectral} induces an isomorphism of short exact sequences:
\[
\begin{tikzcd}
...
In particular,
$\TrFrob{G} : \QCiso{G} \to G(\Fq)^*$
is surjective with a canonical section.
\end{proposition}

\begin{proof}
We start by showing that the top sequence is exact by showing that $\TrFrob{G}$ is surjective.
Pick $\chi \in G(\Fq)^*$.
...
Since we saw that the bottom sequence in exact in Section~\ref{sec:SES} and that the diagram commutes in Proposition~\ref{prop:ST}, the diagram defines a map of short exact sequences.
...
\begin{corollary}\label{cor:kerT-surjective}
$\ker T_{\pi_0(G)} \to \ker T_G$ is surjective.
\end{corollary}
...
%we will also give a cohomological description of $\ker \TrFrob{G}$.

\begin{theorem}\label{thm:snake}
  If $G$ is a smooth, commutative group scheme over
  $\Fq$ then $\TrFrob{G} : \QCiso{G} \to G(\Fq)^*$ is surjective.



  $\QCiso{G}\to \Hh^2(E^\bullet_G)$ from Section~\ref{sec:spectral} induces an isomorphism of short exact sequences:
\[
\begin{tikzcd}
 0 \arrow{r} & \ker \TrFrob{G} \arrow{d} \arrow{r} & \QCiso{G}\arrow{d} \arrow{r} \arrow{r} & G(\Fq)^* \arrow{d} \arrow{r} & 0\\
  0 \arrow{r} & \Hh^0(\Weil{},\Hh^2({\bar G},\EEx)) \arrow{r} & \Hh^2(E^\bullet_G) \arrow{r} & \Hh^1(\Weil{},\Hh^1({\bar G},\EEx)) \arrow{r} & 0.
 \end{tikzcd}
 \]
In particular,
...
%
We may thus apply the snake lemma and conclude that $\coker \TrFrob{G} =0$.
\end{proof}

Note that the snake lemma also gives  $\ker \TrFrob{\pi_0(G)} \to \ker \TrFrob{G}$ is surjective.

\begin{theorem}
We need to find $\ker T_G$.
\end{theorem}
...
Suppose $G$ is a $\QC$-scheme and $\qcs{L}$ is a quasicharacter sheaf on $G$.\todo{I haven't had a chance to go over this again, yet.}  Then
\begin{enumerate}
...
In this section we use that perspective to repair the defect in the sheaf--function dictionary, effectively making invisible quasicharacter sheaves disappear.

Let $G$ be a smooth commutative group scheme over $\Fq$.
Rather that considering triples $(\gqcs{L},\mu,\phi)$ as in Definition~\ref{def:QC}, consider Weil sheaves $(\gqcs{L},\phi)$ on $G$ for which there \emph{exists} an isomorphism $\mu: \bm^* \gqcs{L} \to \gqcs{L} \boxtimes \gqcs{L}$ such that the triple $(\gqcs{L},\mu,\phi)$ satisfies the conditions appearing in Definition~\ref{def:QC}.
...
We form a category from these objects by declaring that a morphism $(\gqcs{L},\phi) \to (\gqcs{L'},\phi')$ is a sheaf homomorphism $\alpha : \gqcs{L} \to \gqcs{L'}$ for which the two diagrams in Condition~\ref{QC.4} commute, for \emph{some} choice of $\mu$, $\mu'$ such that $(\gqcs{L},\mu,\phi)$ and $(\gqcs{L'},\mu',\phi')$ are quasicharacter sheaves on $G$.
This is the category of \emph{loose} quasicharacter sheaves on $G$, denoted by $\lQC(G)$.
...
there is an evident meaning for the Trace of Frobenius $\lTrFrob{G} : \lQCiso{G} \to G(\Fq)^*$.
It is clear, too, that $\lTrFrob{G} : \lQCiso{G} \to G(\Fq)^*$. is a surjective group homomorphism, since $\QCiso{G}  \to G(\Fq)^*$ is surjective, by Theorem~\ref{thm:snake}. What is the kernel of this group homomorphism?

\begin{theorem}\label{thm:loose}
The Trace of Frobenius on $\lTrFrob{G}: \lQCiso{G}  \to G(\Fq)^*$ is a functorial isomorphism.
\end{theorem}

\begin{proof}
We have already seen that $\lTrFrob{G}$ is a surjective group homomorphism, so here we will show that the kernel of $\lTrFrob{G}$ is trivial.
...
Then $(\EE,\mu', 1)$ is a quasicharacter sheaf the isomorphisms $\gqcs{E}_x \to \EE$ defined by $v_x \mapsto 1$ gives an isomorphism $(\gqcs{E},\mu,\phi')\to ((\EE)_{\bar C},\mu', 1)$ in $\QC(G)$. It follows that $(\gqcs{E},\phi')\iso ((\EE)_{\bar G},1)$ in $\lQC(G)$, so $\lTrFrob{G}$ is injective. This completes the proof of Proposition~\ref{prop:loose} in the case that $G$ is etale.
...
In Corollary~\ref{cor:kerT-surjective} (and Theorem~\ref{thm:snake}) we saw that $\ker \TrFrob{\pi_0(G)}\to \TrFrob{G}$ is surjective,
...
This completes the proof of Theorem~\ref{thm:loose}.
\end{proof}
