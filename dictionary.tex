...
\begin{proof}
In Lemma~\ref{lem:bounded_connected} we saw that every quasicharacter sheaf on $G$ descends to bounded quasicharacter sheaf on $G$. 
...
The key observation is that, by descent theory, pull-back by the Lang isogeny $\Lang : G\to G$ defines an equivalence of categories from $\bQC(G)$ to the category of $1$-dimensional $\ell$-adic representations $V$ of $G(\Fq)$ equipped with an isomorphism $V \to V\otimes V$ such that the trace of $\Lang^*\qcs{L}$ is $\trFrob{\qcs{L}}^{-1}$ for every bounded quasicharacter sheaf $\qcs{L}$ on $G$. 
Note that since $V$ is $1$-dimensional, the choice of $V \to V\otimes V$ is exactly the choice of an isomorphism $V\iso \EE$.
Using the equivalence given by $\Lang^*$, it follows that if $\Lang^*\qcs{L} = V$ with character $\psi$ and isomorphism $\mu : V\to V\otimes V$ then $\Aut(\qcs{L})$ is the group of automorphisms of $\rho : V\to V$ for which 
\[
\begin{tikzcd}
\arrow{d}{\mu} V \arrow{r}{\rho} & V\arrow{d}{\mu}\\
V\otimes V \arrow{r}{\rho\otimes \rho} & V\otimes V
\end{tikzcd}
\]
commutes. 
Since the only such isomorphism $\rho$ is $\id_V$, it follows that $\Aut(\qcs{L}) = 1$.
\end{proof}


We have just seen that, on a connected commutative algebraic group $G$ over $\Fq$, the category of quasicharacters $\qcs{L}$ on $G$ is equivalent to the category of characters $\psi$ of $G(\Fq)$. This can be used to understand all quasicharacter sheaves on such $G$ using central extensions of the form
\[
\]
Kamgarpour
...
We have also just seen moreover, that the equality $m^*\psi = \psi \boxtimes \psi$ determines the isomorphism $\mu : \qcs{L} \to \qcs{L}\boxtimes\qcs{L}$ which is part of the data of the quasicharacter sheaf $\qcs{L}$. Means leads us back to a more common perspective on the function--sheaf dictionary in which one considers $1$-dimensional local systems $\qcs{L}$ on $G$ for which \emph{there exists} an isomorphism $m^*\qcs{L} \iso \qcs{L} \boxtimes\qcs{L}$, or sometimes $1$-dimensional local systems $\gqcs{L}$ on $\bG$ for which \emph{there exists} an isomorphism $\Frob{G}^*\gqcs{L} \iso \gqcs{L}$ and an isomorphism $\bm^*\gqcs{L} \iso \gqcs{L} \boxtimes\gqcs{L}$. 
...
We have seen that the category of quasicharacter sheaves on commutative smooth group schemes over finite fields specializes to  
that notion when $G$ is connected and of finite type.
However, as we have already seen in Section~\ref{sec:etale}, that notion is not up to the not appropriate if one wishes to extend the dictionary to commutative smooth group schemes over finite fields.
