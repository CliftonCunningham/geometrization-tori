...
% AMS packages
\usepackage{amsmath, amsthm, amssymb, todonotes}
...
Motivated by ...., in this paper we ... The component group of the Neron model of algebraic torus over local field ...   The sheaf--function dictionary for etale group schemes is significantly different . . .\todo{Needs a new front end to really set the tone, heighten the contrast to the connected case and point to applications.}

Let us briefly review how the function--sheaf dictionary for connected commutative algebraic groups over finite fields \cite{deligne:SGA4.5}*{sommes trig.} translates characters into local systems.
...
and the kernel of the Trace of Frobenius is classified by the classes in $\Hh^2(\pi_0({\bar G}),\EEx)$ that are fixed by Frobenius.
In contrast to the case of connected commutative algebraic groups, the Trace of Frobenius is not injective.
...
Three examples will illustrate some of the issues that arise when extending the function--sheaf dictionary to smooth commutative group schemes.
...
\QCiso{G} \to H^0(\Weil{},H^2(G(\bFq),\EEx))
\end{equation}
defined below.
...
It is also easy to see that the class that $g_\mathcal{L} = g_\mathcal{L'}$ when $\qcs{L} \iso \qcs{L'}$. This defines
\[
...
f(x+y) = f(x) f(y) \frac{(\,^{\Frob{}}g)(x,y)}{g(x,y)}
\end{equation}
for all $x,y \in G(\bFq)$. Thus, the class $\,^{\Frob{}}g_\qcs{L}$ of $\,^{\Frob{}}g$ coincides
with the class $g_\qcs{L}$ of $g$. This concludes the definition of
\[
\QCiso{G} \to H^0(\Weil{},H^2(G(\bFq),\EEx)).
\]
...
If $G$ is a connected and of finite type, 
then $H^2(G(\bFq),\EEx)$ is trivial \todo{Reference?} so all quasicharacter sheaves on connected commutative algebraic groups $G$ are visible; 
 by contrast, if $G$ is etale then $H^2(G(\bFq),\EEx)$ may be non-trivial and $G$ may admit invisible quasicharacters sheaves.
We will return to the latter case in Section~\ref{sec:etale}.
In the meantime, we end this section with a simple property of \eqref{2cocycle}.

\begin{lemma}
The function defined in \eqref{2cocycle} is a homomorphism:
\[
...
\begin{lemma}\label{lem:etale-2cocycle}
    ...
    \[
    \QCiso{G} \to \Hh^0(\Weil{}, \Hh^2(G(\bFq),\EEx)),
    \]
    defined in Section~\ref{sec:invisible}, 
...
Then set of isomorphisms $\{ \phi_{x} \tq x \in G(\bFq) \}$ satisfies \ref{qc.3} by \eqref{gf}. In this way, the cocycle $g\in Z^0(\Weil{}, Z^2(G(\bFq),\EEx))$ defines a quasicharacter sheaf $\qcs{L}$. It is clear that $\qcs{L}$ maps to $[g] \in \Hh^0(\Weil{}, \Hh^2(G(\bFq),\EEx))$ under \eqref{2cocycle}.\todo{Canonical section?} 
\end{proof}
...
It is now clear that we have defined a \emph{visible} quasicharacter sheaf $\qcs{L}$ with $\trFrob{\qcs{L}} = \chi$.\todo{Can I improve this proof to show that the kernel of Trace of Frobenius is exactly $\Hh^0(\Weil{},\Hh^2(G(\bFq),\EEx))$? Perhaps . . . }
\end{proof}
...
\begin{proof}
The Hochschild-Serre spectral sequence (for finitely generated abelian groups!) provides decomposition 
\[
\Hh^2(G(\bFq),\EEx)  \iso \Hh^1(\Weil{}, \Hh^1(G(\bFq)\rtimes \Weil{},\EEx)) \oplus \Hh^0(\Weil{}, \Hh^2(G(\bFq),\EEx))
\]
and Lemmas~\ref{lem:etale-2cocycle} and \ref{lem:etale-trace} provide the maps from $\QCiso{G}$ pictured below:
\[
\begin{tikzcd}
\ &&  \QCiso{G} \arrow[swap]{dl}{\TrFrob{G}} \arrow{dr}{ S_{G} } && \\
0 \arrow{r}{} &  \Hh^1(\Weil{}, \Hh^1(G(\bFq),\EEx)) \arrow{r}{} & \Hh^2(G(\bFq)\rtimes\Weil{},\EEx) \arrow{r}{} & \Hh^0(\Weil{}, \Hh^2(G(\bFq),\EEx)) \arrow{r}{} & 0.
\end{tikzcd}
\]

\todo{Clifton, you are here.}
\end{proof}
