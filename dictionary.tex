...
The trace of Frobenius determines a canonical short exact sequence of commutative groups 
\[
0 \to \Hh^2(X_*(T_K)_{\mathcal{I}_K},\EEx)^{\Weil{}} \to \QCSiso{T} \to \Hom_\text{QC}(T(K),\EEx) \to 0.
\] 
%For all quasicharacter sheaves $\cs{F}$, $\cs{F}'$ on $\Gr_R(X)$, and for every $\rho \in \Hom(\cs{F},\cs{F}')$, either $\rho$ is trivial or $\rho$ is an isomorphism.
For all quasicharacter sheaves $\cs{F}$ for $X_K$, there is a canonical isomorphism
\[
...
\begin{corollary}
Let $T_K$ be an algebraic torus over $K$ and let $T$ be a Neron model for $T_K$.
Then
\[
\QCSiso{T} \iso \Hh^1(\Weil{K}, \check{T}_\ell) 
\qquad \text{and}\qquad
\QCSiso{T^0} \iso \Hh^1(\mathcal{I}_K, \check{T}_\ell)^{\Weil{}}.
\]
\end{corollary}
\begin{proof}
\[
  \begin{tikzcd}
  0 \rar & \QCSiso{X_*(T_K)_{\mathcal{I}_K}}\arrow{r}{\pi_0^*} & \QCSiso{T} \arrow{r}{\iota_0^*} & \QCSiso{T^0} \rar & 0.
  \end{tikzcd}
\]
\end{proof}
