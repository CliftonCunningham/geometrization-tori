...
\newcommand{\rQC}{{\QC_{\text{rig}}}}
\newcommand{\rbQC}{{\QC_0}}
\newcommand{\rfQC}{{\QC_\text{fin}}}
\newcommand{\rQCiso}[1]{\rQC(#1)_{/\text{iso}}}
\newcommand{\rbQCiso}[1]{\rbQC(#1)_{/\text{iso}}}
\newcommand{\rfQCiso}[1]{\rfQC(#1)_{/\text{iso}}}
...
A \emph{rigid quasicharacter sheaf} on $G$ is a triple $\qcs{L}\ceq
...
A \emph{flabby quasicharacter sheaf} on $G$ is a Weil sheaf $(\gqcs{L},\phi)$ on $G$
for which $(\gqcs{L},\mu,\phi)$ is a rigid quasicharacter sheaf on $G$, for some $\mu$.
...
%namely, when $\Hh^2(\pi_0({\bar G}),\EEx)$ is non-trivial, if $(\gqcs{L},\phi)$ is a quasicharacter sheaf on $G$ then there are many $\mu$ for which $(\gqcs{L},\mu,\phi)$ is a rigid quasicharacter sheaf on $G$;
%
 In the main result of this paper, Theorem~\ref{thm:snake},
we form a rigid monoidal category $\lQC(G)$ of flabby quasicharacter sheaves on $G$
...
in Section~\ref{sec:category} we define rigid quasicharacter sheaves and give an interpretation of
...
We define pullbacks and products of rigid quasicharacter sheaves in Section~\ref{sec:pullback}, and use
them to prove a functoriality result on Trace of Frobenius.
In Section~\ref{sec:basechange} we study how rigid quasicharacter sheaves behave under base change
...
forming two important subcategories of $\rQC(G)$.
%These subcategories play a key role in the proof of Proposition~\ref{prop:restriction}.
In Section \ref{sec:connected} we show that all rigid quasicharacter sheaves on $G$ are finite quasicharacter sheaves when $G$ is connected, and that Trace of Frobenius is an isomorphism in this case.
We shift to \'etale group schemes $G$ in Section~\ref{sec:etale} and find a cohomological interpretation
of rigid quasicharacter sheaves in this case.
In Section~\ref{sec:restriction} we show that for non-connected $G$, restriction to the identity component
induces an essentially surjective functor of rigid quasicharacter sheaves.
...
Comments about Joseph Bernstein's comments?
...
%For the sake of brevity, we define a \emph{$\rQC$-scheme} to be such a $G$ with the additional property that the geometric component group of $G$ is finitely generated.  This last condition is only needed in the proof of Lemma \ref{lemma:section}.  A morphism of $\rQC$-schemes is a morphism of group schemes over $\Fq$.
...
\subsection{Rigid quasicharacter sheaves}\label{sec:category}

\begin{definition}\label{def:QC}
A \emph{rigid quasicharacter sheaf on $G$} is a triple
...
Morphisms in the category of rigid quasicharacter sheaves on $G$, denoted by $\rQC(G)$, are defined in the natural way:
\begin{enumerate}
\labitem{(QC.4)}{QC.4} if $\qcs{L} = (\gqcs{L},\mu,\phi)$ and
  $\qcs{L'} = (\gqcs{L'},\mu',\phi')$ are rigid quasicharacter sheaves on $G$ then
...
The category $\rQC(G)$ of rigid quasicharacter sheaves on $G$ is a rigid monoidal category
...
$\sheafHom(\ - \ ,\EE)$.
The category of rigid quasicharacter sheaves on $G$ is not abelian, so $\rQC(G)$
is not a tensor category in the sense of \cite{deligne:02a}*{0.1}.
This rigid monoidal category structure for $\rQC(G)$ gives the set $\rQCiso{G}$
of isomorphism classes in $\rQC(G)$ the structure of a group.

\subsection{Flabby quasicharacter sheaves}\label{sec:loose}

Observe that the rule $(\gqcs{L},\mu,\phi) \mapsto (\gqcs{L},\phi)$ defines a forgetful functor from rigid quasicharacter
...
A \emph{flabby quasicharacter sheaf} on $G$ is a Weil sheaf $(\gqcs{L},\phi)$ on $G$
for which $(\gqcs{L},\mu,\phi)$ is a rigid quasicharacter sheaf on $G$
for some $\mu: \bm^* \gqcs{L} \to \gqcs{L} \boxtimes \gqcs{L}$.
%The resulting objects lie in the essential image of the forgetful functor $(\gqcs{L},\mu,\phi) \mapsto (\gqcs{L},\phi)$ from $\rQC(G)$ to Weil sheaves on $G$.
We form the category of flabby quasicharacter sheaves by declaring that a morphism $(\gqcs{L},\phi) \to (\gqcs{L'},\phi')$ is a sheaf homomorphism $\alpha : \gqcs{L} \to \gqcs{L'}$ for which the two diagrams in Condition~\ref{QC.4} commute, for some choice of $\mu$, $\mu'$ such that $(\gqcs{L},\mu,\phi)$ and $(\gqcs{L'},\mu',\phi')$ are rigid quasicharacter sheaves on $G$.
The category of flabby quasicharacter sheaves on $G$ is denoted by $\lQC(G)$.
\end{definition}
...
%
Like $\rQC(G)$, the category $\lQC(G)$ of quasicharacter sheaves on $G$ is rigid monoidal. The functor $\rQC(G) \to \lQC(G)$ is monoidal and essentially surjective but neither full nor faithful, generally.
%

\subsection{Trace of Frobenius}\label{sec:Frob}

We now introduce a tool which will help us understand isomorphism classes of objects in $\rQCiso{G}$ and $\lQCiso{G}$.
...
Now suppose $\qcs{L} = (\gqcs{L},\mu,\phi)$ is a rigid quasicharacter sheaf on $G$.
...
that if $(\gqcs{L},\mu,\phi)$ and $(\gqcs{L},\mu,\phi')$ are rigid quasicharacter sheaves, then $\trFrob{(\qcs{L},\phi)} = \trFrob{(\qcs{L},\phi')}$. \todo{Sketch proof?}
...
\[
\begin{tikzcd}
\rQCiso{G} \arrow{rr} \arrow{dr}[swap]{\qcs{L} \mapsto t_{\qcs{L}}}{\TrFrob{G}} && \arrow{dl}[swap]{\TrFrob{G}}{(\gqcs{L},\phi) \mapsto t_{\gqcs{L},\phi}} \lQCiso{G} \\
& G(\Fq)^* & 
\end{tikzcd}
\]
As remarked above, the group homomorphism $\rQCiso{G} \to \lQCiso{G}$ is surjective;
we will see that $\rQCiso{G} \to \lQCiso{G}$ need not be injective.
However, in Theorem~\ref{thm:snake}, we will see that the group homomorphism $\rQCiso{G} \to G(\Fq)^*$ is surjective
...
Trace of Frobenius $\TrFrob{G} : \rQCiso{G} \to G(\Fq)^*$ is a natural transformation
between the two contravariant additive functors
\begin{align*}
F_1 : G &\mapsto \rQCiso{G} \\
F_2 : G &\mapsto G(\Fq)^*
\end{align*}
...
\begin{proposition}\label{prop:pullback}
  ...
  \begin{tikzcd}[row sep=20, column sep=30]
   \rQCiso{G} \rar{f^*} \arrow[swap]{d}{\TrFrob{G}} & \rQCiso{H} \dar{\TrFrob{H}} \\
   G(\Fq)^* \rar & H(\Fq)^*
  \end{tikzcd}
  \]
...
\begin{proof}
  ...
  a functor from $\rQC(G)$ to $\rQC(H)$.  Since tensor products commute with pullback in schemes,
  $f^* : \rQC(G) \to \rQC(H)$ is a monoidal functor.
  The diagram relating $f^* : \rQC(G) \to \rQC(H)$, $f^* : G(k)^* \to H(k)^*$ and Trace of Frobenius
...
We end this section with a simple result on products of rigid quasicharacter sheaves,
...
\[
\rQC(G_1)\times \rQC(G_2) \to \rQC(G_1\times G_2)
\]
such that
\[
\begin{tikzcd}[column sep=60]
\arrow{d}{\TrFrob{G_1} \times \TrFrob{G_2}} \rQCiso{G_1}\times \rQCiso{G_2} \arrow{r}{\boxtimes}
& \arrow{d}{\TrFrob{G_1\times G_2}} \rQCiso{G_1\times G_2}\\
...
\begin{proof}
The only non-trivial part of the proof is to show that $\boxtimes : \rQC(G_1)\times \rQC(G_2) \to \rQC(G_1\times G_2)$
...
Now, let $\qcs{L} \ceq (\gqcs{L},\mu,\phi)$ be a rigid quasicharacter sheaf on $G$.
Set $\qcs{L}_1 \ceq q_1^* \qcs{L}$ and $\qcs{L}_2 \ceq q_2^* \qcs{L}$;
by Proposition~\ref{prop:pullback}, $\qcs{L}_1$ a rigid quasicharacter sheaf on $G_1$
and $\qcs{L}_2$ is a rigid quasicharacter sheaf on $G_2$.
...
\end{align*}
We can interpret this operation on characters in terms of rigid quasicharacter sheaves:

\begin{proposition} \label{prop:csbe}
Set $G_{k'} \ceq G\times_\Spec{k} \Spec{k'}$ and let
\[
\rQC(\Res_{k'/k}(G_{k'})) \xrightarrow{\iota^*} \rQC(G)
\]
...
\[
\begin{tikzcd}
\rQCiso{\Res_{k'/k}(G_{k'})} \arrow[two heads]{r}{\iota^*} \dar[swap]{\TrFrob{\Res_{k'/k}(G_{k'})}} & \rQCiso{G} \dar{\TrFrob{G}} \\
...
\end{align*}
We can also interpret this operation on characters in terms of rigid quasicharacter sheaves.

If $\qcs{L} \ceq (\gqcs{L}, \mu, \phi)$ is a rigid quasicharacter sheaf on $G$, we define
a rigid quasicharacter sheaf $\qcs{L}' \ceq (\gqcs{L}, \mu, \phi_{k'})$ on the base change
...
the rule $\nu_{k'/k}: (\gqcs{L}, \mu, \phi) \mapsto (\gqcs{L}, \mu, \phi_{k'})$
 defines a monoidal functor $\rQC(G) \to \rQC(G_{k'})$.
 Moreover, the following diagram commutes:
\[
\begin{tikzcd}[column sep=60]
\rQCiso{G} \rar{\nu_{k'/k}} \dar{\TrFrob{G}} & \rQCiso{G_{k'}} \dar{\TrFrob{G_{k'}}} \\
G(k)^*  \rar{\Nm^*} & G(k')^*.
\end{tikzcd}
\]
\end{proposition}
...
\begin{proof}
Let $\qcs{L} \ceq (\gqcs{L}, \mu, \phi) \in \rQC(G)$ and write $F$ for $\Frob{G}$.  For any $x \in G(k')$,
...
\[
\begin{tikzcd}
\rQCiso{\Res_{k'/k} G'} \dar{\TrFrob{\Res_{k'/k} G'}} \rar{\rho} & \rQCiso{G'} \dar{\TrFrob{G'}}\\
G'(k')^* \rar & G'(k')^*
\end{tikzcd}
\]
\end{proposition}
...
\section{Two subcategories of rigid quasicharacter sheaves}%\label{sec:bounded}

\subsection{Bounded quasicharacter sheaves}\label{sec:bounded}

In this section we consider a class of rigid quasicharacter sheaves on $G$ obtained by
replacing the Weil sheaf $(\gqcs{L}, \phi)$ on $\bG$ in the definition of a rigid quasicharacter
sheaf with an $\ell$-adic local system on $G$ itself.

\begin{definition}
Let $\rbQC(G)$ be the category of pairs $(\qcs{L}_0,\mu_0)$
...
morphisms in $\rbQC(G)$ are defined as in the second part of
\ref{QC.3}.
...
$B_G : \rbQC(G) \hookrightarrow \rQC(G)$.
\end{proposition}
...
In general, $\rbQC(G)$ is an essentially
proper subcategory of $\rQC(G)$. We will identify $\rbQC(G)$ with its image under $B_G$
and say that a rigid quasicharacter sheaf $\qcs{L} \in \rQC(G)$ is \emph{bounded}
if it is isomorphic to some $B_G(\qcs{L}_0, \mu_0)$.
We will see in Corollary~\ref{cor:bounded-and-finite} that bounded quasicharacter sheaves are exactly rigid quasicharacter sheaves with bounded Trace of Frobenius.
...
of bounded quasicharacter sheaves; finite quasicharacter sheaves will play a role in Section \ref{sec:restriction}.
...
if it is finite, surjective, \'etale and the action of $\Gal(\bFq/\Fq)$ on the \'etale group
scheme $\ker f = f^{-1}({\bar e)} = \Aut(f)$ is trivial.
Consider the category $C(G)$ whose objects consist of triples $(f,\psi,V)$, where
\begin{itemize}
...
\end{itemize}
A morphism $(f,\psi,V) \to (f',\psi',V')$ in $C(G)$ is a pair $(\gamma,T)$, where
\begin{itemize}
\item $\gamma : H' \to H$ is a morphism of group schemes such that $f' = f\circ \gamma$,
...
V' & \Aut(V') & \arrow[swap]{l}{\psi'} \arrow[swap]{u}{\gamma\vert_{\ker f'}} \ker f' \arrow[hook]{r} & \arrow[swap]{u}{\gamma} H' \arrow{r}{f'} & G
\end{tikzcd}
\]
\end{itemize}

We put a tensor category structure on $C(G)$ as follows.
Let $(f,\psi,V)$ and $(f',\psi',V')$ be objects in $C(G)$.
...
We may thus define $(f,\psi,V)\otimes(f',\psi',V') \ceq (f\times_G f' ,\psi\otimes\psi', V\otimes V')$ and
$(f,\psi,V)\oplus(f',\psi',V')\ceq (f\times_G f' ,\psi\oplus\psi', V\oplus V')$. To define duals in $C(G)$,
let $V^\vee \ceq \Hom_\text{vec}(V,\EE)$ and $\psi^\vee : \ker f \to \Aut(V^\vee)$ be
the dual representation.  We may then set $(f,\psi,V)^\vee \ceq (f,\psi^\vee, V^\vee)$.
...
\begin{tikzcd}[column sep=30]
   ...
   \end{tikzcd}
\]
 where $(\gamma,T) : (f,\psi,V) \to (f',\psi',V')$ is any map in $C(G)$
 and $(\gamma_1, T_1) :  (f',\psi',V') \to (f_1,\psi_1,V_1)$ is a weak isomorphism in $C(G)$;
for the equivalence relation on these diagrams, see \cite{kashiwara-schapira:CatgoriesSheaves}*{(7.1.3)}.

It is straightforward to verify that the subcategory $C_1(G)$ of $C(G)$ consisting of $(f,\psi,V)$ for which $\psi$ is one-dimensional, is a full, rigid monoidal subcategory of $C(G)$ and further, that the family $W_1$ of weak isomorphisms in $C_1(G)$ is a right multiplicative system.

\begin{definition}
The category of \emph{finite quasicharacter sheaves} of $G$, denoted by $\rfQC(G)$,
is the rigid monoidal category $C_1(G)[W_1^{-1}]$ obtained by localizing $C_1(G)$ at weak isomorphisms $W_1$.
\end{definition}

Trivial morphisms in $\rfQC(G)$ are defined as follows. Let $(f,\psi,V)$ and $(f_1,\psi_1,V_1)$ be finite quasicharacter sheaves on $G$.  
Form $(f\times_G f_1,1\otimes \psi_1, V\otimes V_1)$ in $C_1(G)$. 
...
%
The morphism in $\rfQC(G)$ represented by
\[
\begin{tikzcd}[column sep=30]
   (f,\psi,V) \arrow{r}{(\pi,0)} & (f\times_G f_1,1\otimes \psi_1, V\otimes V_1) & \arrow[swap]{l}{(\pi_1,T_v)}[swap]{\text{w-iso}}  (f_1,\psi_1,V_1),
   \end{tikzcd}
\]
does not depend on the choice of $0\ne v\in V$; this defines the {\em trivial morphism} $0: (f,\psi,V) \to (f_1,\psi_1,V_1)$ in $\rfQC(G)$.

\begin{proposition}\label{prop:autornaught}
Every morphism in $\rfQC(G)$ is either an isomorphism or trivial.
\todo{I may add something here about $\Aut_{\rfQC(G)}(f,\psi,V)$.}
\end{proposition}
...
\begin{tikzcd}[column sep=30]
   ...
   \end{tikzcd}
\]
represent a morphism $(f,\psi,V)  \to (f_1,\psi_1,V_1)$ in $\rfQC(G)$;
...
it follows that $(f,\psi,V)  \to (f_1,\psi_1,V_1)$ is the trivial morphism in $\rfQC(G)$.

Now suppose $T$ is an isomorphism, then $(\gamma,T)$ is a weak isomorphism, in which case the morphism $(f,\psi,V) \to (f_1,\psi_1,V_1)$ is an isomorphism in $\rfQC(G)$.
\todo{This is where some information about $\Aut_{\rfQC(G)}(f,\psi,V)$ will go, when I've sorted it out.}
\end{proof}

\subsection{Finite implies Bounded}

In this section we show that $\rfQC(G)$ is a full subcategory of $\rbQC(G)$.
To prepare for that result, we need the next two lemmas.

\begin{lemma}\label{lemma:finite-pullback}
Let $g : J \to G$ be a morphism of smooth groups schemes over $\Fq$.
...
Each finite quasicharacter sheaf $(f,\psi)$ in $\rfQC(G)$ determines a
canonical isomorphism $\mu(f,\psi) : m^*(f,\psi) \to (f,\psi)\boxtimes(f,\psi)$ in $\rfQC(G\times G)$.
 \end{lemma}
...
$\rfQC(G\times G)$, the resulting isomorphism,
...
However, after passing from $C_1(G\times G)$ to $\rfQC(G\times G)$, the resulting isomorphism 
$\mu(f,\psi): m^*(f,\psi) \to (f,\psi)\boxtimes (f,\psi)$ in $\rfQC(G\times G)$ is independent of the choice of basis for $V$.
 \end{proof}

\begin{proposition}\label{prop:bounded}
 $\rfQC(G)$ is a full subcategory of $\rbQC(G)$.
\end{proposition}

\begin{proof}
 To prove the proposition we exhibit a full and faithful monoidal functor $L_G : \rfQC(G) \to \rbQC(G)$.
...
let $L(f,\psi)$ be the local system on $G$ given by the $\psi$-isotypic component of the local system $f_* V_H$ on $G$.
 ...
 commutes with pullback along any morphism of $\rQC$-schemes,
 ...
 By Lemma~\ref{lemma:finite-iso}, each $(f,\psi) \in \rfQC(G)$ canonically determines
 an isomorphism
 \[
 ...
 \[
 L_G  \ceq  L_{W}\vert_{\rfQC(G)} : \rfQC(G) \to \rbQC(G),
 \]
 given on objects by $(f,\psi) \mapsto (L(f,\psi), L_{W}(\mu(f,\psi)))$,
 is a full and faithful monoidal functor, completing the proof.
\end{proof}

On general smooth group $\Fq$-schemes $G$, 
there are bounded quasicharacter sheaves which are not finite.  
Identifying $\rfQC(G)$ with its image under $B_G \circ L_G$, as defined in
...
a rigid quasicharacter sheaf is finite if and only if its Trace of Frobenius has finite image.


\section{The case of connected commutative algebraic groups} \label{sec:connected}

For connected groups, the subcategories introduced in the previous two sections agree with $\rQC(G)$.

\subsection{When rigid quasicharacter sheaves are finite}

\begin{proposition}\label{prop:connected}
  If $G$ is a connected commutative algebraic group over $\Fq$ then
  the full subcategories $\rfQC(G) \hookrightarrow \rbQC(G) \hookrightarrow \rQC(G)$
...
\begin{proof}
  ...
  sends rigid quasicharacter sheaves on $G$ to $\ell$-adic Weil sheaves on $G$ \cite{deligne:80a}*{1.1.10}.
  While is it not true that all Weil sheaves on $G$ descend to local systems on $G$,
  we will see that those that appear in the image of this forgetful functor from rigid quasicharacter sheaves do.
...
$\Weil{}\to \W(G,\bg)$ of $\W(G,\bg)\to \Weil{}$.
  ...
  Now let $(\gqcs{L},\mu,\phi)$ be a rigid quasicharacter sheaf on $G$
  ...
  Then, as in Section~\ref{sec:bounded}, $(\qcs{L}_0,\mu_0)$ is an object in $\rbQC(G)$
  and $\qcs{L} \ceq (\gqcs{L},\mu,\phi)$ is isomorphic to $(b_G^*\qcs{L}_0,b_{G\times G}^*\mu_0)$ in $\rQC(G)$.
  Thus, the full and faithful functor $B_G : \rbQC(G) \to \rQC(G)$ from Section~\ref{sec:bounded}
  is also essentially surjective, hence an equivalence.

  In Section~\ref{sec:finite} we saw that $\rfQC(G)$ is a full subcategory of $\rbQC(G)$;
  more precisely, we exhibited a full and faithful functor $L_G : \rfQC(G) \to \rbQC(G)$.
  We now show that this functor is essentially surjective
  when $G$ is a connected, commutative algebraic group over $\Fq$.

  Let $(\qcs{L}_0,\mu_0)$ be a bounded quasicharacter sheaf on $G$.
  Then $\qcs{L}\ceq B_G(\qcs{L}_0,\mu_0)$ is a rigid quasicharacter sheaf on $G$.
  ...
  It is now clear that $(L(f,\psi),\mu(f,\psi)) \iso (\qcs{L}_0,\mu_0)$ in $\rbQC(G)$.
...
it is appropriate to replace rigid quasicharacter sheaves on $G$
with the conceptually simpler category $\rfQC(G)$ of finite quasicharacter sheaves on $G$.
...
In this generality, however, neither of the full subcategories $\rfQC(G) \subset \rbQC(G) \subset \rQC(G)$ are equivalences;
see the end of Section~\ref{sec:snake}.
Nevertheless, every rigid quasicharacter sheaf $(\gqcs{L},\mu,\phi)$ on $G$
...
We are in a good position to understand $\rQC(G)$ when $G$ is a connected algebraic group over $\Fq$, since we can now describe objects and maps in this category in very concrete terms. We begin by recovering a classical result. 

\begin{corollary}\label{cor:connected}
 If $G$ is a connected, commutative algebraic group over $\Fq$ then
 \[
  \TrFrob{G} : \rQCiso{G} \to G(\Fq)^*
 \]
 is an isomorphism of groups.
\end{corollary}

\begin{proof}
 It suffices\todo{Explain why $T_G$ is injective.} to observe that the group homomorphism
 $\TrFrob{G} : \rQCiso{G} \to G(\Fq)^*$
 is surjective: each character $\chi \in G(\Fq)^*$ determines a finite quasicharacter
 sheaf $(\Lang,\chi^{-1})$ such that Trace of Frobenius of the rigid quasicharacter sheaf
 $B_G (L_G(\Lang,\chi^{-1}))$ is $\chi$.
\end{proof}

Morphisms in $\rQC(G)$ are also very simple.

\begin{proposition}
If $G$ is a connected algebraic group then $\Aut_{\rQC(G)}(\qcs{L})$ is . . . .
\end{proposition}
...
To see why, we begin by specializing the definition of rigid quasicharacter sheaves to the case
...
Under this equivalence, a rigid quasicharacter sheaf on an etale commutative group scheme $G$ over $\Fq$ is given by the data of:
\begin{enumerate}
...
\end{enumerate}
Under this equivalence, a morphism $\alpha : \qcs{L} \to \qcs{L'}$ of rigid quasicharacter sheaves on $G$ is given by
\begin{enumerate}
 \labitem{(qc.4)}{qc.4} an indexed set $\rho_x : \gqcs{L}_x \to \gqcs{L'}_x$
  ...
  \[
   \begin{tikzcd}[column sep=40]
    ...
    & \arrow[swap]{d}{\mu} \gqcs{L}_{x+y} \rar{\rho_{x+y}} & \gqcs{L'}_{x+y} \dar{\mu'_{x+y}} \\
...
\todo{Find an appropriate place for this discussion}

If $\rho_0 = 0$ then the second diagram for $y=0$ implies that $\rho_x = 0$ for all $x$.  Conversely, if $\rho_0$ is nonzero then the second diagram for $y = -x$ implies that $\rho_x$ is nonzero for all $x$ and thus an isomorphism since $\gqcs{L}_x$ is one dimensional.  Most of this paper focuses on the question of determining the isomorphism classes of objects; a complete understanding of the morphisms in $\rQC(G)$ also requires a description of the automorphisms of an arbitrary quasicharacter sheaf $\qcs{L}$.

\begin{proposition}
The trace map induces an isomorphism of groups
\[
\Aut(\qcs{L}) \cong \Hom(G(\bFq)_{\Weil{}}, \EEx).
\]
\end{proposition}

\begin{proof}
The second diagram in \ref{qc.4} shows that the association $x \mapsto \rho_x$ is a homomorphism from $G(\bFq)$ to $\EEx$ and the first shows that it factors through $G(\bFq)_{\Weil{}}$.  Conversely, if $\rho : G(\bFq)_{\Weil{}} \to \EEx$ is any homomorphism then defining $\rho_x$ as multiplication by $\rho(x)$ will satisfy the two diagrams.  Composition of morphisms corresponds to pointwise multiplication in this correspondence, showing that the resulting bijection is actually a group ismorphism.
\end{proof}

We will see that the Trace of Frobenius may not provide complete
information about isomorphism classes of rigid quasicharacter sheaves on $G$ when $G$ is not a connected algebraic group.
Our main tool for understanding this phenomenon
is a group homomorphism from $\rQCiso{G}$ to $\Hh^2(E^\bullet_G)$, defined in Section~\ref{sec:S}, for which the next two sections are preparation.
...
\subsection{From rigid quasicharacter sheaves to the total complex}\label{sec:S}

In this section we define an isomorphism
\[
S_G : \rQCiso{G} \to \Hh^2(E^\bullet_G).
\]
Let $\qcs{L} = (\gqcs{L},\mu,\phi)$ be a rigid quasicharacter sheaf on $G$.
...
\begin{remark}
Although we will only use the group homomorphism $S_G : \rQCiso{G}\to \Hh^2(E^\bullet_G)$ when $G$ is an etale commutative group scheme, the function is defined for all smooth commutative group schemes $G$.
...
\end{proposition}
\begin{proof}
We first show that $S_G$ is injective.  Suppose $\qcs{L} \in \rQCiso{G}$ with $S_G(\qcs{L}) = [\alpha \oplus \beta] = 0$,
...
Since $\qcs{L} = 0 \in \rQCiso{G}$, $S_G$ is injective.
...
Tracing the construction backward, we have defined a rigid quasicharacter sheaf $\qcs{L}$ on $G$ with
$S_G(\qcs{L}) = [\alpha\oplus\beta\oplus\gamma]$, showing that $S_G$ is surjective.
\end{proof}

\subsection{$S$ and $T$}

In this section we describe the relationship between the group homomorphisms $\TrFrob{G}$ and $S_G$.
%
...
\begin{corollary}
If $G$ is \'etale then $\TrFrob{G} : \rQCiso{G} \to G(\Fq)^*$ is surjective.
\end{corollary}
\begin{proof}
Pick $\chi \in G(\Fq)^*$.
...
is a rigid quasicharacter sheaf with $\trFrob{\qcs{L}} = \chi$.
\end{proof}



\begin{proposition} \label{prop:etale-iso}
 If $G$ is \'etale then the map $S_G : \rQCiso{G}\to \Hh^2(E^\bullet_G)$ induces an isomorphism of short exact sequences
\[
\begin{tikzcd}
 0 \arrow{r} & \ker \TrFrob{G} \arrow{d} \arrow{r} & \rQCiso{G}\arrow{d}{S_G} \arrow{r}{\TrFrob{G}} \arrow{r} & G(\Fq)^* \arrow{d} \arrow{r} & 0\\
  0 \arrow{r} & \Hh^0(\Weil{},\Hh^2({\bar G},\EEx)) \arrow{r} & \Hh^2(E^\bullet_G) \arrow{r} & \Hh^1(\Weil{},\Hh^1({\bar G},\EEx)) \arrow{r} & 0.
 \end{tikzcd}
 \]
\end{proposition}
\begin{proof}
This result follows easily from Proposition \ref{prop:ses_desc}.
\end{proof}
...
In this section use that perspective to find a perfect dictionary between flabby quasicharacter sheaves on $G$ and characters of $G(\Fq)$.
...
\begin{tikzcd}
\rQC(0) \rar & \rQC(\pi_0(G)) \arrow{r}{\pi_0^*} & \rQC(G) \arrow{r}{\iota_0^*} & \rQC(G^0) \rar & \rQC(0)
\end{tikzcd}
\end{equation}
...
\begin{tikzcd}
0 \rar &
\rQCiso{\pi_0(G)} \arrow{r}{\pi_0^*} & \rQCiso{G} \arrow{r}{\iota_0^*} & \rQCiso{G^0} \rar & 0.
\end{tikzcd}
\end{equation}
 Note that we found the groups $\rQCiso{G^0}$
and $\rQCiso{\pi_0(G)}$
...
For every rigid quasicharacter sheaf $\qcs{L}^0$ on $G^0$ there is some finite quasicharacter sheaf
$\qcs{L}$ on $G$ such that $\qcs{L}\vert_{G^0} \iso \qcs{L}^0$.
As a consequence, the restriction functor $\iota_0^* : \rQC(G)\to \rQC(G^0)$ is essentially surjective.
\end{proposition}

\begin{proof}
  By Lemma~\ref{lem:G0alg-grp} and Proposition~\ref{prop:connected}, every
  rigid quasicharacter sheaf on $G^0$ is isomorphic to a
...
\begin{proof}
The sequence is exact at $\rQCiso{G^0}$ by Proposition~\ref{prop:restriction},
so here we show that it is also exact at $\rQCiso{G}$.
First note that $\iota_0^* \circ \pi_0^*$ is trivial by Proposition~\ref{prop:pullback}.
So it suffices to show that if $\qcs{L} = (\gqcs{L},\mu,\phi)$ is a rigid quasicharacter sheaf on $G$
...
$\qcs{E} \ceq (\gqcs{E}_x, \mu_{x,y}, \phi_x)$, thus defining a rigid quasicharacter sheaf on $\pi_0(G)$.
...
thus defining an isomorphism $\qcs{L} \to \pi_0^* \qcs{E}$ in $\rQC(G)$.
\end{proof}
...
$\TrFrob{G} : \rQCiso{G} \to G(\Fq)^*$ is a functorial group homomorphism.
...
\begin{proof}
  ...
  \begin{tikzcd}[row sep=20]
    ...
    %0 \rar
    & \rQCiso{\pi_0(G)} \rar \dar{\TrFrob{\pi_0(G)}}
    & \rQCiso{G} \rar \dar{\TrFrob{G}} & \rQCiso{G^0} \rar \dar{\TrFrob{G^0}} & 0\\
...
The top row is also exact:
  the surjectivity of $\rQCiso{G} \to \rQCiso{G^0}$ follows from Proposition~\ref{prop:restriction}
  while exactness at $\rQCiso{G}$ is given by Proposition~\ref{prop:middleexact}.
...
Let $(\gqcs{E},\phi')$ be a rigid quasicharacter sheaf on $G$ with $\trFrob{(\gqcs{E},\phi')}=1$.
...
Let $a\in Z^2({\bar G},\EEx)$ be the multiplicative version of $\alpha$
and let $\mu'$ be the indexed set of isomorphisms $\mu'_{x,y} : \EE \to \EE\otimes \EE$ defined by $1 \mapsto a(x,y) 1 \otimes 1$.
Then $(\EE,\mu', 1)$ is a rigid quasicharacter sheaf the isomorphisms $\gqcs{E}_x \to \EE$ defined by $v_x \mapsto 1$ gives an isomorphism $(\gqcs{E},\mu,\phi')\to ((\EE)_{\bar C},\mu', 1)$ in $\rQC(G)$.
...
and let $(\gqcs{L},\phi)$ be a flabby quasicharacter sheaf on $G$ with $\trFrob{(\gqcs{L},\phi)}=1$.
Let $\qcs{L} = (\gqcs{L},\mu,\phi)$ be a rigid quasicharacter sheaf.
We saw above that $\ker \TrFrob{\pi_0(G)}\to \TrFrob{G}$ is surjective in this case,
so $\qcs{L} = \pi_0^*\qcs{E}$ for some $\qcs{E}\in \rQC(\pi_0(G))$.
...
So $\qcs{L} = \pi_0^*\qcs{E} \iso \pi_0^*((\EE)_{\pi_0({\bar G})},\mu', 1) = ((\EE)_{\bar G},\pi_0^*\mu', \pi_0^*1)$ in $\rQC(G)$.
...
\[
0 \to \lQCiso{\pi_0(G)} \to \lQCiso{G} \to \lQCiso{G^0} \to 0
\]
of abelian groups.
\end{corollary}
...
using the category $\lQC(G)$ in place of $\rQC(G)$.
\end{proof}

\begin{corollary} \label{cor:bounded-and-finite}
Let $G$ be a smooth commutative group scheme over $\Fq$.
\begin{enumerate}
\item $\qcs{L} \in \rQC(G)$ is bounded if and only if $\trFrob{\qcs{L}}$ has bounded image,
\item $\qcs{L}\in \rQC(G)$ is finite if and only if $\trFrob{\qcs{L}}$ has finite image.
\end{enumerate}
\end{corollary}
...
$G^0$, all rigid quasicharacter sheaves are bounded and finite by Proposition~\ref{prop:connected}.
...
one can easily detect when a rigid quasicharacter sheaf is bounded or finite.
...
we see that all rigid quasicharacter sheaves on $G$ are finite quasicharacter sheaves
...
$\rfQC(G) \subset \rbQC(G) \subset \rQC(G)$ are equivalences.
...
{\smaller\smaller\smaller
  ...
  \begin{tikzcd}[row sep=20, column sep=3]
    ...
    & \rQCiso{C} \arrow[>->>]{dr}\arrow{rr} \arrow{dd}%{\TrFrob{C}}
    && \rQCiso{B} \arrow[>->]{dr} \arrow{rr} \arrow{dd}%{\TrFrob{B}}
    && \rQCiso{A} \arrow[>->>]{dr} \arrow{dd}%{\TrFrob{A}}
    \arrow{rr}  &   & 0 & \\
...
\begin{corollary}\label{cor:exact}
  ...
  $\Fq$ then pullback yields an exact sequence
\[
\Hh^1(E^\bullet_{G^0}) \to \rQCiso{\pi_0(G)} \to \rQCiso{G} \to \rQCiso{G^0} \to 0
\]
