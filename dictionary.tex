...
and also the set $\Hom(\qcs{L},\qcs{L}')$, for all quasicharacter sheaves $\qcs{L}$, $\qcs{L}'$ on $G$ [point]; in this was we provide a complete description of the category $\QC(G)$.
...
\subsection{Objects}%\texorpdfstring{$S$ and $T$}{S and T}}
...
If $\rho_0 = 0$ then the second diagram for $y=0$ implies that $\rho_x = 0$ for all $x\in G$.  Conversely, if $\rho_0$ is nonzero then the second diagram for $y = -x$ implies that $\rho_x$ is nonzero for all $x$ and thus an isomorphism since the stalk of quasicharacter sheaves are one dimensional.  
%
...
Having found $\QCiso{G}$ and $\Hom(\qcs{L},\qcs{L}')$ when $G$ is an etale commutative group scheme over $\Fq$, we now wish to find these groups for all commutative group scheme over $\Fq$.

\subsection{The case of connected commutative algebraic groups over finite fields}\label{sec:connected}.
...
proper subcategory of $\QC(G)$. We will identify $\bQC(G)$ with its image under $B_G$
