...
\]
We also illuminate the nature of the category $\QC(G)$ by showing that every morphism in this category is either an isomorphism or trivial, and by showing that $\Aut(\qcs{L}) \iso \Hom(G(\bFq)_{\Gal(\bFq/\Fq)},\EEx)$, canonically, for all quasicharacter sheaves $\qcs{L}$ on $G$.

Joseph Bernstein has suggested to us that we should not view 


Our interest in the function--sheaf dictionary for smooth commutative group schemes over $\Fq$ comes from an application to $p$-adic representation theory.
In this paper we show quasicharacters of $p$-adic tori are geometrized and categorified by quasicharacter sheaves on the
Greenberg transform of the N\'eron model of the $p$-adic torus. 
...
As our application reveals, quasicharacter sheaves evolved from this notion,
...
The interesting new features pertaining to Weil sheaves and the non-triviality of $\Hh^2(\pi_0({\bar G}),\EEx)^{\Gal({\bar\FF}_q/\FF_q)}$ do not arise in this context.
