...
% If your latex is outdated, you may get an error that \frenchsetup
% is not defined, and this may result in weird pagebreaks. You can ignore these
% errors, as it will compile properly on an up-to-date latex.
\pdfoutput=1
...
% Many packages are already loaded by the class file, including notably
% xcolor, amsmath, amsfonts, mathtools, hyperref
% only add packages you really need
...
\daterevised{2020-10-20}
\dateaccepted{2021-01-06}

\editor{Sebastien Gouezel}
...
% All standard abbreviations for theorems or lemmas are normally already
% present. If yours is missing, add it here (do not change ``cdrthm'')
\AtBeginDocument{
\newtheorem{myname}[cdrthm]{My beautiful theorem}
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Add your personal commands here
...
\author[\initial{C.} \lastname{Cunningham}]{\firstname{Clifton} \lastname{Cunningham}}
\address{
...
\author[\initial{D.} \lastname{Roe}]{\firstname{David} \lastname{Roe}}
...
%\alttitle{Version longue du titre, mais en fran\c{c}ais}
...
%\begin{altabstract}
%Dans le r\'esum\'e en fran\c{c}ais, les d\'etails ne rentrent pas plus, mais ce n'est pas
%pour cela qu'il faut s'en dispenser. (If you don't speak French, don't worry, the
%editors will translate the abstract for you)
%\end{altabstract}
...
\thanks{Roe was supported by Simons Foundation grant 550033.}
...
Hypothesis~\ref{H1a} may be used to geometrize each quasicharacter $\varphi_i$ inside the Grothendieck group $\mathsf{K}_{\bar\QQ}\mathsf{D}^b_{c,G_i}(G_i;\EE)$.
...
Taking the trivial representation for $\orho^0$ gives a datum not satisfying \ref{H1b}.
Note that this datum does not satisfy \ref{H1a} either since the depth 1 Greenberg transform
of the hyperspecial $\oK^0$ is not reductive.
